...
WifiErrorCode GetScanInfoList(WifiScanInfo* result, unsigned int* size)
{
    ...
    for (int i = 0; i < scanSize; i++) {
        memcpy_s(result[i].ssid, WIFI_MAX_SSID_LEN, scanResult[i].ssid, WIFI_MAX_SSID_LEN);
        memcpy_s(result[i].bssid, WIFI_MAC_LEN, scanResult[i].bssid, WIFI_MAC_LEN);
...
int SetLocalWifiGw(void)
{
    ...
    for (i = 0; i < index_max; i++) {
        ...
        if (netif != NULL) {
            LZ_HARDWARE_LOGE(WIFI_DEVICE_TAG, "%c %c num:%d %s",
                             netif->name[0], netif->name[1], netif->num,
                             inet_ntoa(ip_addr_get_ip4_u32(&netif->ip_addr)));
        }
    }
