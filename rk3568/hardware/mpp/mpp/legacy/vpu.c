...
static RK_S32 vpu_api_set_client_type(int dev, RK_S32 client_type)
{
    ...
    if (ioctl_version > 0) {
        ...
        vcodec_type = (*(mRKMppApi.Hdimpp_get_vcodec_type))();
        ...
        if (vpu_api_ioctl_version < 0) {
            ...
            if (ret)
                HDF_LOGE("can not find valid client type ioctl\n");
        } else {
    ...
    if (ret)
        HDF_LOGE("set client type failed ret %d errno %d\n", ret, errno);
...
int VPUClientInit(VPU_CLIENT_TYPE type)
{
    ...
    path = (*(mRKMppApi.Hdimpp_get_vcodec_dev_name))(ctx_type, coding);
    fd = open(path, O_RDWR | O_CLOEXEC);

    (*(mRKMppApi.HdiMppEnvGetU32))("vpu_debug", &vpu_debug, 0);

    ioctl_version = (*(mRKMppApi.Hdimpp_get_ioctl_version))();

    if (fd == -1) {
        HDF_LOGE("failed to open %s, errno = %d, error msg: %s\n", \
...
RK_S32 VPUClientSendReg(int socket, RK_U32 *regs, RK_U32 nregs)
{
    ...
    if (ioctl_version > 0) {
        ...
        if (extra_info && extra_info->magic == VPU_EXTRA_INFO_MAGIC) {
            ...
            reqs[2].cmd = MPP_CMD_SET_REG_ADDR_OFFSET; //  reqs[2]
            reqs[2].flag = 0; // reqs[2]
            reqs[2].offset = 0; // reqs[2]
            reqs[2].size = extra_info->count * sizeof(extra_info->patchs[0]); // reqs[2]
            reqs[2].data_ptr = REQ_DATA_PTR((void *)&extra_info->patchs[0]); // reqs[2]

            reqs[0].size =  reg_size * sizeof(RK_U32);
            reqs[1].size =  reg_size * sizeof(RK_U32);
            reqs[1].flag |= VPU_MPP_FLAGS_MULTI_MSG;
            reqs[2].flag |= VPU_MPP_FLAGS_LAST_MSG; // reqs[2]
            ret = (RK_S32)ioctl(fd, MPP_IOC_CFG_V1, &reqs);
        } else {
            MppReqV1 reqs_tmp[2];
            reqs[1].flag |= VPU_MPP_FLAGS_LAST_MSG;
            if (memcpy_s(reqs_tmp,  sizeof(MppReqV1) * 2, reqs, sizeof(MppReqV1) * 2) != EOK) { // reqs[2]
    ...
    if (ret)
        HDF_LOGE("ioctl VPU_IOC_SET_REG failed ret %d errno %d %s\n", ret, errno, strerror(errno));
...
RK_S32 VPUClientSendReg2(RK_S32 socket, RK_S32 offset, RK_S32 size, void *param)
{
    ...
    if (ret)
        HDF_LOGE("ioctl VPU_IOC_WRITE failed ret %d", ret);
...
RK_S32 VPUClientWaitResult(int socket, RK_U32 *regs, RK_U32 nregs, VPU_CMD_TYPE *cmd, RK_S32 *len)
{
    ...
    if (ret) {
        HDF_LOGE("ioctl VPU_IOC_GET_REG failed ret %d errno %d %s\n", ret, errno, strerror(errno));
...
RK_S32 VPUClientGetHwCfg(int socket, RK_U32 *cfg, RK_U32 cfg_size)
{
    ...
    if (ret)
        HDF_LOGE("ioctl VPU_IOC_GET_HW_FUSE_STATUS failed ret %d\n", ret);
...
RK_U32 VPUCheckSupportWidth()
{
    ...
    memset_s(&hwCfg, sizeof(VPUHwDecConfig_t), 0, sizeof(VPUHwDecConfig_t));
    if (fd >= 0) {
        if (VPUClientGetHwCfg(fd, (RK_U32*)&hwCfg, sizeof(hwCfg))) {
            HDF_LOGE("Get HwCfg failed\n");
            close(fd);
            return -1;
        }
        close(fd);
