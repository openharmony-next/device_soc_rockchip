...
OMX_ERRORTYPE Rkvpu_OMX_InputBufferReturn(OMX_COMPONENTTYPE *pOMXComponent, OMX_BUFFERHEADERTYPE* bufferHeader)
{
    ...
    VIDEO_DBG(VIDEO_DBG_LOG_PORT, "[%s]: empty buffer done(%p) timeus: %lld us, flags: 0x%lx",
...
OMX_ERRORTYPE Rockchip_OMX_OutputBufferReturn(OMX_COMPONENTTYPE *pOMXComponent, OMX_BUFFERHEADERTYPE* bufferHeader)
{
    ...
    if (bufferHeader->nFlags & OMX_BUFFERFLAG_EOS) {
        VIDEO_DBG(VIDEO_DBG_LOG_PORT, "[%s]: fill EOS buffer done(%p) timeus: %lld us, flags: 0x%lx",
                  pRockchipComponent->componentName,
                  bufferHeader->pBuffer,
                  bufferHeader->nTimeStamp,
                  bufferHeader->nFlags);
    } else {
        VIDEO_DBG(VIDEO_DBG_LOG_PORT, "[%s]: fill buffer done(%p) timeus: %lld us, flags: 0x%lx",
                  pRockchipComponent->componentName,
                  bufferHeader->pBuffer,
                  bufferHeader->nTimeStamp,
                  bufferHeader->nFlags);
    ...
    goto EXIT;
EXIT:
    omx_trace("bufferHeader:0x%p", bufferHeader);
    return ret;
}
    ...
    OMX_IN OMX_BUFFERHEADERTYPE *pBuffer)
{
    ...
    if ((!CHECK_PORT_ENABLED(pRockchipPort)) ||
        (CHECK_PORT_BEING_FLUSHED(pRockchipPort) && (!CHECK_PORT_TUNNELED(pRockchipPort) ||
...
OMX_ERRORTYPE Rockchip_OMX_Port_Constructor(OMX_HANDLETYPE hComponent)
{
    ...
    pRockchipInputPort->extendBufferHeader =
        Rockchip_OSAL_Malloc(sizeof(ROCKCHIP_OMX_BUFFERHEADERTYPE) * MAX_BUFFER_NUM);
    ...
    Rockchip_OSAL_Memset(pRockchipInputPort->extendBufferHeader, 0,
        sizeof(ROCKCHIP_OMX_BUFFERHEADERTYPE) * MAX_BUFFER_NUM);
    ...
    Rockchip_OSAL_QueueCreate(&pRockchipOutputPort->bufferQ, MAX_QUEUE_ELEMENTS);

    pRockchipOutputPort->extendBufferHeader =
        Rockchip_OSAL_Malloc(sizeof(ROCKCHIP_OMX_BUFFERHEADERTYPE) * MAX_BUFFER_NUM);
    ...
    Rockchip_OSAL_Memset(pRockchipOutputPort->extendBufferHeader, 0,
        sizeof(ROCKCHIP_OMX_BUFFERHEADERTYPE) * MAX_BUFFER_NUM);
...
OMX_ERRORTYPE Rockchip_Shared_BufferToData(
    ROCKCHIP_OMX_DATABUFFER *pUseBuffer,
    ROCKCHIP_OMX_DATA *pData,
    ROCKCHIP_OMX_PLANE nPlane)
{
    OMX_ERRORTYPE ret = OMX_ErrorNone;
