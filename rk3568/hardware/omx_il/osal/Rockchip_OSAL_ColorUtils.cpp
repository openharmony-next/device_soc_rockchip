                                          ...
                                          OMX_COLORASPECTS * aspects)
{
    ...
    if (!findCodecAspects(coeffs, (OMX_U32 *)&aspects->mMatrixCoeffs,
        sIsoMatrixCoeffs, ARRAY_SIZE(sIsoMatrixCoeffs))) {
...
{
    if (!findColorAspects((OMX_U32)codecAspect->mPrimaries, &colorAspect->mPrimaries,
        sIsoPrimaries, ARRAY_SIZE(sIsoPrimaries))) {
        colorAspect->mPrimaries = 2; // 2:value of mPrimaries
    }
    if (!findColorAspects((OMX_U32)codecAspect->mTransfer, &colorAspect->mTransfer,
        sIsoTransfers, ARRAY_SIZE(sIsoTransfers))) {
        colorAspect->mTransfer = 2; // 2:value of mTransfer
    }
    if (!findColorAspects((OMX_U32)codecAspect->mMatrixCoeffs, &colorAspect->mMatrixCoeffs,
        sIsoMatrixCoeffs, ARRAY_SIZE(sIsoMatrixCoeffs))) {
        colorAspect->mMatrixCoeffs = 2; // 2:value of mMatrixCoeffs
    }
    (colorAspect->mRange) = codecAspect->mRange == RangeFull ? 2 : 0; // 2:value
}
...
OMX_U32 handleColorAspectsChange(
    const OMX_COLORASPECTS *a /* mDefaultColorAspects */,
...
void updateFinalColorAspects(
    const OMX_COLORASPECTS *otherAspects,
