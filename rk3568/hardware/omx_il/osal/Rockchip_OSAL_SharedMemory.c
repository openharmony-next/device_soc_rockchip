...
int ion_get_phys(int fd, ion_user_handle_t handle, unsigned long *phys)
{
    ...
    omx_err("ion_get_phys:phys_data.phys = ox%lx", phys_data.phys);
...
func_mmap64 mpp_rt_get_mmap64(void)
{
...
static void* drm_mmap(int fd, size_t len, int prot, int flags, loff_t offset)
{
    ...
    if (offset & 4095) { // 4095:byte alignment
        return NULL;
    }
...
static int drm_handle_to_fd(int fd, RK_U32 handle, int *map_fd, RK_U32 flags)
{
    ...
    memset_s(&dph, sizeof(struct drm_prime_handle), 0, sizeof(struct drm_prime_handle));
    dph.handle = handle;
    dph.fd = -1;
    dph.flags = flags;
...
static int drm_alloc(int fd, size_t len, size_t align, RK_U32 *handle, int flag)
{
    ...
    memset_s(&dmcb, sizeof(struct drm_mode_create_dumb), 0, sizeof(struct drm_mode_create_dumb));
    dmcb.bpp = 8; // 8:value of dmcb.bpp
    ...
    *handle = dmcb.handle;
    ...
    omx_trace("drm_alloc success:  handle %d size %llu", *handle, dmcb.size);
...
OMX_HANDLETYPE Rockchip_OSAL_SharedMemory_Open()
{
    ...
    if (!access("/dev/dri/card0", F_OK)) {
        SharedMemoryClient = open("/dev/dri/card0", O_RDWR);
        mem_type = MEMORY_TYPE_DRM;
    } else {
        SharedMemoryClient = open("/dev/ion", O_RDWR);
        mem_type = MEMORY_TYPE_ION;
    }

    if (SharedMemoryClient <= 0) {
        omx_err("SharedMemoryClient create Error: %d", SharedMemoryClient);
        Rockchip_OSAL_Free((void *)pHandle);
        pHandle = NULL;
        goto EXIT;
    }

    pHandle->fd = SharedMemoryClient;
...
void Rockchip_OSAL_SharedMemory_Close(OMX_HANDLETYPE handle, OMX_BOOL b_secure)
{
    ...
    while (pCurrentElement != NULL) {
        ...
        if (b_secure) {
            ...
            map_fd = pnative_handle_t->data[0];
            pTrueAddree = (void *)Rockchip_OSAL_SharedMemory_HandleToSecureAddress(handle,
                pDeleteElement->mapAddr, pDeleteElement->allocSize);
...
OMX_PTR Rockchip_OSAL_SharedMemory_Alloc(OMX_HANDLETYPE handle, OMX_U32 size, MEMORY_TYPE memoryType)
{
    ...
    switch (memoryType) {
        ...
        case SYSTEM_MEMORY:
            ...
            err = ion_alloc((int)pHandle->fd, size,
                4096, mask, flag, (ion_user_handle_t *)&ion_hdl); // 4096:byte alignment
            if (err < 0) {
                omx_err("ion_alloc Error: %lu", ion_hdl);
