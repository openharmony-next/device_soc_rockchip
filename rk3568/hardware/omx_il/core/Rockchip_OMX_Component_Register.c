...
OMX_ERRORTYPE Rockchip_OMX_Component_Register(ROCKCHIP_OMX_COMPONENT_REGLIST **compList, OMX_U32 *compNum)
{
    ...
    componentList = (ROCKCHIP_OMX_COMPONENT_REGLIST *)Rockchip_OSAL_Malloc(sizeof(ROCKCHIP_OMX_COMPONENT_REGLIST) *
    ...
    for (i = 0; i < ARRAY_SIZE(kCompInfo); i++) {
        ...
        if ((soHandle = Rockchip_OSAL_dlopen(com_inf.lib_name, RTLD_NOW)) != NULL) {
            ...
            if ((Rockchip_OMX_COMPONENT_Library_Register = Rockchip_OSAL_dlsym(soHandle,
                "Rockchip_OMX_COMPONENT_Library_Register")) != NULL) {
                int i = 0;
                unsigned int j = 0;
                componentNum = (*Rockchip_OMX_COMPONENT_Library_Register)(NULL);
                omx_err_f("in num: %d", componentNum);
                rockchipComponentsTemp =
                    (RockchipRegisterComponentType **)Rockchip_OSAL_Malloc(sizeof(RockchipRegisterComponentType*) *
                        componentNum);
                ...
                for (i = 0; i < componentNum; i++) {
                    Rockchip_OSAL_Strcpy(componentList[totalCompNum].component.componentName,
                        rockchipComponentsTemp[i]->componentName);
                    for (j = 0; j < rockchipComponentsTemp[i]->totalRoleNum; j++)
                        Rockchip_OSAL_Strcpy(componentList[totalCompNum].component.roles[j],
                            rockchipComponentsTemp[i]->roles[j]);
...
OMX_ERRORTYPE Rockchip_OMX_ComponentAPICheck(OMX_COMPONENTTYPE *component)
{
    ...
    if ((component->GetComponentVersion == NULL)    ||
        (component->SendCommand == NULL)            ||
        (component->GetParameter == NULL)           ||
        (component->SetParameter == NULL)           ||
        (component->GetConfig == NULL)              ||
        (component->SetConfig == NULL)              ||
        (component->GetExtensionIndex == NULL)      ||
        (component->GetState == NULL)               ||
        (component->ComponentTunnelRequest == NULL) ||
        (component->UseBuffer == NULL)              ||
        (component->AllocateBuffer == NULL)         ||
        (component->FreeBuffer == NULL)             ||
        (component->EmptyThisBuffer == NULL)        ||
        (component->FillThisBuffer == NULL)         ||
        (component->SetCallbacks == NULL)           ||
        (component->ComponentDeInit == NULL)        ||
        (component->UseEGLImage == NULL)            ||
        (component->ComponentRoleEnum == NULL))
...
OMX_ERRORTYPE Rockchip_OMX_ComponentLoad(ROCKCHIP_OMX_COMPONENT *rockchip_component)
{
    ...
    ret = (*Rockchip_OMX_ComponentConstructor)((OMX_HANDLETYPE)pOMXComponent,
