...
#ifndef ASM_PLAT_IOVMM_H
#define ASM_PLAT_IOVMM_H
...
enum rk_iommu_inttype { IOMMU_PAGEFAULT, IOMMU_BUSERROR, IOMMU_FAULT_UNKNOWN, IOMMU_FAULTS_NUM };
 ...
 */
typedef int (*rockchip_iommu_fault_handler_t)(struct device *dev, enum rk_iommu_inttype itype,
                                              unsigned long pgtable_base, unsigned long fault_addr, unsigned int statu);
...
static inline dma_addr_t rockchip_iovmm_map(struct device *dev, struct scatterlist *sg, off_t offset, size_t size)
{
    return -ENOSYS;
}
...
static inline int rockchip_iovmm_map_oto(struct device *dev, phys_addr_t phys, size_t size)
{
    return -ENOSYS;
}
...
static inline void rockchip_iovmm_set_fault_handler(struct device *dev, rockchip_iommu_fault_handler_t handler)
{
}
