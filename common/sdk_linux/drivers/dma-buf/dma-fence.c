...
struct dma_fence *dma_fence_get_stub(void)
{
    ...
    if (!dma_fence_stub.ops) {
        dma_fence_init(&dma_fence_stub, &dma_fence_stub_ops, &dma_fence_stub_lock, 0, 0);
...
static struct lockdep_map dma_fence_lockdep_map = {.name = "dma_fence_map"};
...
void _dma_fence_might_wait(void)
{
    bool tmp;
...
void dma_fence_release(struct kref *kref)
{
    ...
    if (WARN(!list_empty(&fence->cb_list) && !test_bit(DMA_FENCE_FLAG_SIGNALED_BIT, &fence->flags),
...
static void dma_fence_default_wait_cb(struct dma_fence *fence, struct dma_fence_cb *cb)
{
    struct default_wait_cb *wait = container_of(cb, struct default_wait_cb, base);
...
static bool dma_fence_test_signaled_any(struct dma_fence **fences, uint32_t count, uint32_t *idx)
{
    int i;
