...
static void rk817_shutdown_prepare(void)
{
    ...
    regmap_update_bits(rk808->regmap,
               ...
               (RK_REG_VAL_THREE << RK_REG_SHIFT_MASK_FIVE), (RK_REG_VAL_THREE << RK_REG_SHIFT_MASK_FIVE));
    regmap_update_bits(rk808->regmap,
               RK817_RTC_INT_REG,
               (RK_REG_VAL_THREE << RK_REG_SHIFT_MASK_TWO), (RK_REG_VAL_ZERO << RK_REG_SHIFT_MASK_TWO));

    if (rk808->pins && rk808->pins->p && rk808->pins->power_off) {
        ret = regmap_update_bits(rk808->regmap,
                     RK817_SYS_CFG(RK_REG_VAL_THREE),
