                     ...
                     bool idle)
{
    ...
    ret = readx_poll_timeout_atomic(rockchip_pmu_domain_is_idle, pd,
                    is_idle, is_idle == idle, 0, READX_POLL_TIMEOUT_US);
    if (ret) {
        dev_err(pmu->dev,
...
static int rockchip_pmu_save_qos(struct rockchip_pm_domain *pd)
{
    ...
    for (i = 0; i < pd->num_qos; i++) {
        regmap_read(pd->qos_regmap[i],
                QOS_PRIORITY,
                &pd->qos_save_regs[QOS_SAVE_REGS_INDEX_ZERO][i]);
        regmap_read(pd->qos_regmap[i],
                QOS_MODE,
                &pd->qos_save_regs[QOS_SAVE_REGS_INDEX_ONE][i]);
        regmap_read(pd->qos_regmap[i],
                QOS_BANDWIDTH,
                &pd->qos_save_regs[QOS_SAVE_REGS_INDEX_TWO][i]);
        regmap_read(pd->qos_regmap[i],
                QOS_SATURATION,
                &pd->qos_save_regs[QOS_SAVE_REGS_INDEX_THREE][i]);
        regmap_read(pd->qos_regmap[i],
                QOS_EXTCONTROL,
                &pd->qos_save_regs[QOS_SAVE_REGS_INDEX_FOUR][i]);
    }
    return 0;
}
                        ...
                        bool on)
{
    ...
    ret = readx_poll_timeout_atomic(rockchip_pmu_domain_is_on, pd, is_on,
                    is_on == on, 0, READX_POLL_TIMEOUT_US);
    if (ret) {
        dev_err(pmu->dev,
                      ...
                      struct device_node *node)
{
    ...
    if (pd->num_qos > 0) {
        ...
        pd->qos_save_regs[QOS_SAVE_REGS_INDEX_ZERO] = (u32 *)devm_kmalloc(pmu->dev,
                               sizeof(u32) *
                               MAX_QOS_REGS_NUM *
                               pd->num_qos,
                               GFP_KERNEL);
        ...
        qos_is_need_init[QOS_IS_NEED_INIT_INDEX_ZERO] = kzalloc(sizeof(bool) *
                          MAX_QOS_REGS_NUM *
                          pd->num_qos,
                          GFP_KERNEL);
