...
#define VPG_DEFS(name, dsi) ((void __force *)&((*dsi).vpg_defs.name))
...
static void dw_mipi_dsi_wait_for_two_frames(const struct drm_display_mode *mode)
{
    ...
    two_frames = DIV_ROUND_UP(MSEC_PER_SEC, refresh) * 0x2;
    msleep(two_frames);
}
...
static int dw_mipi_dsi_host_attach(struct mipi_dsi_host *host, struct mipi_dsi_device *device)
{
    ...
    dsi->lanes = (device->lanes > max_data_lanes) ? device->lanes / 0x2 : device->lanes;
...
static int dw_mipi_dsi_host_detach(struct mipi_dsi_host *host, struct mipi_dsi_device *device)
{
    struct dw_mipi_dsi *dsi = host_to_dsi(host);
    const struct dw_mipi_dsi_plat_data *pdata = dsi->plat_data;
    int ret;
...
static void dw_mipi_message_config(struct dw_mipi_dsi *dsi, const struct mipi_dsi_msg *msg)
{
     ...
     */
    dsi_write(dsi, DSI_DPI_LP_CMD_TIM, OUTVACT_LPCMD_TIME(16) | INVACT_LPCMD_TIME(4));
...
static int dw_mipi_dsi_gen_pkt_hdr_write(struct dw_mipi_dsi *dsi, u32 hdr_val)
{
    ...
    ret = readl_poll_timeout(dsi->base + DSI_CMD_PKT_STATUS, val, !(val & GEN_CMD_FULL), 0x3e8,
    ...
    ret =
        readl_poll_timeout(dsi->base + DSI_CMD_PKT_STATUS, val, (val & mask) == mask, 0x3e8, CMD_PKT_STATUS_TIMEOUT_US);
    if (ret) {
...
static int dw_mipi_dsi_write(struct dw_mipi_dsi *dsi, const struct mipi_dsi_packet *packet)
{
    ...
    while (len) {
        ...
        ret = readl_poll_timeout(dsi->base + DSI_CMD_PKT_STATUS, val, !(val & GEN_PLD_W_FULL), 0x3e8,
                                 CMD_PKT_STATUS_TIMEOUT_US);
        if (ret) {
...
static int dw_mipi_dsi_read(struct dw_mipi_dsi *dsi, const struct mipi_dsi_msg *msg)
{
    ...
    ret = readl_poll_timeout(dsi->base + DSI_CMD_PKT_STATUS, val, !(val & GEN_RD_CMD_BUSY), 0x3e8,
                             CMD_PKT_STATUS_TIMEOUT_US);
    if (ret) {
...
static ssize_t dw_mipi_dsi_host_transfer(struct mipi_dsi_host *host, const struct mipi_dsi_msg *msg)
{
    struct dw_mipi_dsi *dsi = host_to_dsi(host);
...
static void dw_mipi_dsi_video_mode_config(struct dw_mipi_dsi *dsi)
{
    ...
    if (dsi->vpg_defs.vpg) {
        ...
        val |= dsi->vpg_defs.vpg_horizontal ? VID_MODE_VPG_HORIZONTAL : 0;
...
static void dw_mipi_dsi_init(struct dw_mipi_dsi *dsi)
{
    ...
    if (phy_ops->get_esc_clk_rate) {
        ret = phy_ops->get_esc_clk_rate(dsi->plat_data->priv_data, &esc_rate);
        if (ret) {
            DRM_DEBUG_DRIVER("Phy get_esc_clk_rate() failed\n");
        }
    } else {
        esc_rate = 0x14; /* Default to 20MHz */
    }
     ...
     */
    dsi_write(dsi, DSI_CLKMGR_CFG, TO_CLK_DIVISION(0xa) | TX_ESC_CLK_DIVISION(esc_clk_division));
}
...
static void dw_mipi_dsi_video_packet_config(struct dw_mipi_dsi *dsi, const struct drm_display_mode *mode)
{
    ...
    dsi_write(dsi, DSI_VID_PKT_SIZE,
              dw_mipi_is_dual_mode(dsi) ? VID_PKT_SIZE(mode->hdisplay / 0x2) : VID_PKT_SIZE(mode->hdisplay));
}
...
static u32 dw_mipi_dsi_get_hcomponent_lbcc(struct dw_mipi_dsi *dsi, const struct drm_display_mode *mode, u32 hcomponent)
{
    u32 lbcc;

    lbcc = hcomponent * dsi->lane_mbps * MSEC_PER_SEC / 0x8;
...
static void dw_mipi_dsi_line_timer_config(struct dw_mipi_dsi *dsi, const struct drm_display_mode *mode)
{
...
static void dw_mipi_dsi_vertical_timing_config(struct dw_mipi_dsi *dsi, const struct drm_display_mode *mode)
{
...
static void dw_mipi_dsi_dphy_timing_config(struct dw_mipi_dsi *dsi)
{
    ...
    ret = phy_ops->get_timing(dsi->plat_data->priv_data, dsi->lane_mbps, &timing);
    if (ret) {
    ...
    if (hw_version >= HWVER_131) {
        ...
        dsi_write(dsi, DSI_PHY_TMR_RD_CFG, MAX_RD_TIME_V131(0x2710));
    } else {
        dsi_write(dsi, DSI_PHY_TMR_CFG,
                  PHY_HS2LP_TIME(timing.data_hs2lp) | PHY_LP2HS_TIME(timing.data_lp2hs) | MAX_RD_TIME(0x2710));
    }

    dsi_write(dsi, DSI_PHY_TMR_LPCLK_CFG, PHY_CLKHS2LP_TIME(timing.clk_hs2lp) | PHY_CLKLP2HS_TIME(timing.clk_lp2hs));
}
...
{
    /* Clear PHY state */
    dsi_write(dsi, DSI_PHY_RSTZ, PHY_DISFORCEPLL | PHY_DISABLECLK | PHY_RSTZ | PHY_SHUTDOWNZ);
...
static void dw_mipi_dsi_dphy_enable(struct dw_mipi_dsi *dsi)
{
    ...
    ret = readl_poll_timeout(dsi->base + DSI_PHY_STATUS, val, val & PHY_LOCK, 0x3e8, PHY_STATUS_TIMEOUT_US);
    if (ret) {
        DRM_ERROR("failed to wait phy lock state\n");
    }

    ret = readl_poll_timeout(dsi->base + DSI_PHY_STATUS, val, val & PHY_STOP_STATE_CLK_LANE, 0x3e8,
...
static void dw_mipi_dsi_mode_set(struct dw_mipi_dsi *dsi, const struct drm_display_mode *adjusted_mode)
{
    const struct dw_mipi_dsi_phy_ops *phy_ops = dsi->plat_data->phy_ops;
    ...
    ret = phy_ops->get_lane_mbps(priv_data, adjusted_mode, dsi->mode_flags, lanes, dsi->format, &dsi->lane_mbps);
    if (ret) {
...
static void dw_mipi_dsi_bridge_mode_set(struct drm_bridge *bridge, const struct drm_display_mode *mode,
                                        const struct drm_display_mode *adjusted_mode)
{
    ...
    DRM_DEV_INFO(dsi->dev, "final DSI-Link bandwidth: %u x %d Mbps\n", dsi->lane_mbps,
                 dsi->slave ? dsi->lanes * 0x2 : dsi->lanes);
}
...
static int dw_mipi_dsi_bridge_attach(struct drm_bridge *bridge, enum drm_bridge_attach_flags flags)
{
    struct dw_mipi_dsi *dsi = bridge_to_dsi(bridge);
...
DEFINE_DEBUGFS_ATTRIBUTE(fops_x32, dw_mipi_dsi_debugfs_show, dw_mipi_dsi_debugfs_write, "%llu\n");
...
static void debugfs_create_files(void *data)
{
    ...
    for (i = 0; i < ARRAY_SIZE(debugfs); i++) {
        debugfs_create_file(dsi->debugfs_vpg[i].name, 0x1a4, dsi->debugfs, &dsi->debugfs_vpg[i], &fops_x32);
    }
}
                                              ...
                                              const struct dw_mipi_dsi_plat_data *plat_data)
{
    ...
    if (!plat_data->phy_ops->init || !plat_data->phy_ops->get_lane_mbps || !plat_data->phy_ops->get_timing) {
