...
void analogix_dp_lane_swap(struct analogix_dp_device *dp, bool enable)
{
    ...
    if (enable) {
        reg = LANE3_MAP_LOGIC_LANE_0 | LANE2_MAP_LOGIC_LANE_1 | LANE1_MAP_LOGIC_LANE_2 | LANE0_MAP_LOGIC_LANE_3;
    } else {
        reg = LANE3_MAP_LOGIC_LANE_3 | LANE2_MAP_LOGIC_LANE_2 | LANE1_MAP_LOGIC_LANE_1 | LANE0_MAP_LOGIC_LANE_0;
    }
...
void analogix_dp_init_analog_param(struct analogix_dp_device *dp)
{
    ...
    reg = PD_RING_OSC | AUX_TERMINAL_CTRL_50_OHM | TX_CUR1_2X | TX_CUR_16_MA;
    analogix_dp_write(dp, ANALOGIX_DP_PLL_FILTER_CTL_1, reg);

    reg = CH3_AMP_400_MV | CH2_AMP_400_MV | CH1_AMP_400_MV | CH0_AMP_400_MV;
...
void analogix_dp_reset(struct analogix_dp_device *dp)
{
    ...
    if (dp->plat_data && is_rockchip(dp->plat_data->dev_type)) {
        reg = RK_VID_CAP_FUNC_EN_N | RK_VID_FIFO_FUNC_EN_N | SW_FUNC_EN_N;
    } else {
        reg = MASTER_VID_FUNC_EN_N | SLAVE_VID_FUNC_EN_N | AUD_FIFO_FUNC_EN_N | AUD_FUNC_EN_N | HDCP_FUNC_EN_N |
              SW_FUNC_EN_N;
    }

    analogix_dp_write(dp, ANALOGIX_DP_FUNC_EN_1, reg);

    reg = SSC_FUNC_EN_N | AUX_FUNC_EN_N | SERDES_FIFO_FUNC_EN_N | LS_CLK_DOMAIN_FUNC_EN_N;
    analogix_dp_write(dp, ANALOGIX_DP_FUNC_EN_2, reg);

    usleep_range(0x14, 0x1e);
...
void analogix_dp_set_analog_power_down(struct analogix_dp_device *dp, enum analog_power_block block, bool enable)
{
    u32 reg;
...
int analogix_dp_init_analog_func(struct analogix_dp_device *dp)
{
    ...
    reg &= ~(SERDES_FIFO_FUNC_EN_N | LS_CLK_DOMAIN_FUNC_EN_N | AUX_FUNC_EN_N);
    analogix_dp_write(dp, ANALOGIX_DP_FUNC_EN_2, reg);
    return 0;
}
...
void analogix_dp_init_aux(struct analogix_dp_device *dp)
{
    ...
    reg |= AUX_HW_RETRY_COUNT_SEL(0) | AUX_HW_RETRY_INTERVAL_600_MICROSECONDS;
...
void analogix_dp_set_link_bandwidth(struct analogix_dp_device *dp, u32 bwtype)
{
    ...
    ret = readx_poll_timeout(analogix_dp_get_pll_lock_status, dp, status, status != PLL_UNLOCKED, 120,
...
void analogix_dp_set_lane_link_training(struct analogix_dp_device *dp)
{
    ...
    for (lane = 0; lane < dp->link_train.lane_count; lane++) {
        analogix_dp_write(dp, ANALOGIX_DP_LN0_LINK_TRAINING_CTL + 4 * lane, dp->link_train.training_lane[lane]);
    }
    ...
    if (dp->phy) {
        ...
        for (lane = 0; lane < dp->link_train.lane_count; lane++) {
            ...
            vs = (training_lane & DP_TRAIN_VOLTAGE_SWING_MASK) >> DP_TRAIN_VOLTAGE_SWING_SHIFT;
            pe = (training_lane & DP_TRAIN_PRE_EMPHASIS_MASK) >> DP_TRAIN_PRE_EMPHASIS_SHIFT;
...
{
    return analogix_dp_read(dp, ANALOGIX_DP_LN0_LINK_TRAINING_CTL + 4 * lane);
}
...
void analogix_dp_set_training_pattern(struct analogix_dp_device *dp, enum pattern_set pattern)
{
    u32 reg;
    ...
    switch (pattern) {
        ...
        case TRAINING_PTN3:
            ...
            break;
        case DP_NONE:
            reg = SCRAMBLING_ENABLE | LINK_QUAL_PATTERN_SET_DISABLE | SW_TRAINING_PATTERN_SET_NORMAL;
            analogix_dp_write(dp, ANALOGIX_DP_TRAINING_PTN_SET, reg);
            break;
        default:
            break;
...
void analogix_dp_set_video_color_format(struct analogix_dp_device *dp)
{
    ...
    reg = (dp->video_info.dynamic_range << IN_D_RANGE_SHIFT) | (dp->video_info.color_depth << IN_BPC_SHIFT) |
...
void analogix_dp_set_video_cr_mn(struct analogix_dp_device *dp, enum clock_recovery_m_value_type type, u32 m_value,
                                 u32 n_value)
{
    u32 reg;
...
int analogix_dp_send_psr_spd(struct analogix_dp_device *dp, struct dp_sdp *vsc, bool blocking)
{
    unsigned int val;
    int ret;
    ssize_t psr_status;
    ...
    analogix_dp_write(dp, ANALOGIX_DP_PSR_FRAME_UPDATE_CTRL, PSR_FRAME_UP_TYPE_BURST | PSR_CRC_SEL_HARDWARE);
    ...
    ret = readx_poll_timeout(analogix_dp_get_psr_status, dp, psr_status,
                             psr_status >= 0 && ((vsc->db[1] && psr_status == DP_PSR_SINK_ACTIVE_RFB) ||
...
ssize_t analogix_dp_transfer(struct analogix_dp_device *dp, struct drm_dp_aux_msg *msg)
{
    u32 reg;
    ...
    ret = readx_poll_timeout(readl, dp->reg_base + ANALOGIX_DP_AUX_CH_CTL_2, reg, !(reg & AUX_EN), 0x19, 0x7a120);
    if (ret) {
    ...
    if (msg->request & DP_AUX_I2C_READ) {
        ...
        for (i = 0; i < msg->size; i++) {
            reg = analogix_dp_read(dp, ANALOGIX_DP_BUF_DATA_0 + 0x4 * i);
...
void analogix_dp_set_video_format(struct analogix_dp_device *dp)
{
    ...
    analogix_dp_write(dp, ANALOGIX_DP_TOTAL_LINE_CFG_L, TOTAL_LINE_CFG_L(mode->vtotal));
    analogix_dp_write(dp, ANALOGIX_DP_TOTAL_LINE_CFG_H, TOTAL_LINE_CFG_H(mode->vtotal >> 0x8));
    analogix_dp_write(dp, ANALOGIX_DP_ACTIVE_LINE_CFG_L, ACTIVE_LINE_CFG_L(mode->vdisplay));
    analogix_dp_write(dp, ANALOGIX_DP_ACTIVE_LINE_CFG_H, ACTIVE_LINE_CFG_H(mode->vdisplay >> 0x8));
    analogix_dp_write(dp, ANALOGIX_DP_V_F_PORCH_CFG, V_F_PORCH_CFG(vfp));
    analogix_dp_write(dp, ANALOGIX_DP_V_SYNC_WIDTH_CFG, V_SYNC_WIDTH_CFG(vsw));
    analogix_dp_write(dp, ANALOGIX_DP_V_B_PORCH_CFG, V_B_PORCH_CFG(vbp));
    analogix_dp_write(dp, ANALOGIX_DP_TOTAL_PIXEL_CFG_L, TOTAL_PIXEL_CFG_L(mode->htotal));
    analogix_dp_write(dp, ANALOGIX_DP_TOTAL_PIXEL_CFG_H, TOTAL_PIXEL_CFG_H(mode->htotal >> 0x8));
    analogix_dp_write(dp, ANALOGIX_DP_ACTIVE_PIXEL_CFG_L, ACTIVE_PIXEL_CFG_L(mode->hdisplay));
    analogix_dp_write(dp, ANALOGIX_DP_ACTIVE_PIXEL_CFG_H, ACTIVE_PIXEL_CFG_H(mode->hdisplay >> 0x8));
    analogix_dp_write(dp, ANALOGIX_DP_H_F_PORCH_CFG_L, H_F_PORCH_CFG_L(hfp));
    analogix_dp_write(dp, ANALOGIX_DP_H_F_PORCH_CFG_H, H_F_PORCH_CFG_H(hfp >> 0x8));
    analogix_dp_write(dp, ANALOGIX_DP_H_SYNC_CFG_L, H_SYNC_CFG_L(hsw));
    analogix_dp_write(dp, ANALOGIX_DP_H_SYNC_CFG_H, H_SYNC_CFG_H(hsw >> 0x8));
    analogix_dp_write(dp, ANALOGIX_DP_H_B_PORCH_CFG_L, H_B_PORCH_CFG_L(hbp));
    analogix_dp_write(dp, ANALOGIX_DP_H_B_PORCH_CFG_H, H_B_PORCH_CFG_H(hbp >> 0x8));
}
