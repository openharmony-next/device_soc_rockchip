...
#define GUC_FW_BLOB(prefix_, major_, minor_, patch_)                                                                   \
    UC_FW_BLOB((major_), (minor_), MAKE_GUC_FW_PATH(prefix_, (major_), (minor_), patch_))

#define HUC_FW_BLOB(prefix_, major_, minor_, bld_num_)                                                                 \
    UC_FW_BLOB((major_), (minor_), MAKE_HUC_FW_PATH(prefix_, (major_), (minor_), bld_num_))
...
static void __uc_fw_auto_select(struct drm_i915_private *i915, struct intel_uc_fw *uc_fw)
{
    ...
    for (i = 0; i < ARRAY_SIZE(fw_blobs) && p <= fw_blobs[i].p; i++) {
        if (p == fw_blobs[i].p && rev >= fw_blobs[i].rev) {
            const struct uc_fw_blob *blob = &fw_blobs[i].blobs[uc_fw->type];
    ...
    if (IS_ENABLED(CONFIG_DRM_I915_SELFTEST)) {
        for (i = 1; i < ARRAY_SIZE(fw_blobs); i++) {
            ...
            if (fw_blobs[i].p == fw_blobs[i - 1].p && fw_blobs[i].rev < fw_blobs[i - 1].rev) {
                continue;
            }

            pr_err("invalid FW blob order: %s r%u comes before %s r%u\n", intel_platform_name(fw_blobs[i - 1].p),
                   fw_blobs[i - 1].rev, intel_platform_name(fw_blobs[i].p), fw_blobs[i].rev);
...
{
    if (i915->params.enable_guc & (ENABLE_GUC_SUBMISSION | ENABLE_GUC_LOAD_HUC)) {
...
void intel_uc_fw_init_early(struct intel_uc_fw *uc_fw, enum intel_uc_fw_type type)
{
    ...
    intel_uc_fw_change_status(uc_fw, uc_fw->path
                                         ? *uc_fw->path ? INTEL_UC_FIRMWARE_SELECTED : INTEL_UC_FIRMWARE_DISABLED
...
static void __force_fw_fetch_failures(struct intel_uc_fw *uc_fw, int e)
{
    ...
    if (i915_inject_probe_error(i915, e)) {
        ...
        uc_fw->user_overridden = user;
    } else if (uc_fw->major_ver_wanted && i915_inject_probe_error(i915, e)) {
        /* require prev major version */
        uc_fw->major_ver_wanted -= 1;
        uc_fw->minor_ver_wanted = 0;
        uc_fw->user_overridden = user;
    } else if (uc_fw->minor_ver_wanted && i915_inject_probe_error(i915, e)) {
...
int intel_uc_fw_fetch(struct intel_uc_fw *uc_fw)
{
    ...
    if (unlikely(fw->size < sizeof(struct uc_css_header))) {
        drm_warn(&i915->drm, "%s firmware %s: invalid size: %zu < %zu\n", intel_uc_fw_type_repr(uc_fw->type),
                 uc_fw->path, fw->size, sizeof(struct uc_css_header));
        err = -ENODATA;
        goto fail;
    }
    ...
    if (unlikely(size != sizeof(struct uc_css_header))) {
        drm_warn(&i915->drm, "%s firmware %s: unexpected header size: %zu != %zu\n", intel_uc_fw_type_repr(uc_fw->type),
                 uc_fw->path, fw->size, sizeof(struct uc_css_header));
        err = -EPROTO;
        goto fail;
    }
    ...
    if (unlikely(css->key_size_dw != UOS_RSA_SCRATCH_COUNT)) {
        drm_warn(&i915->drm, "%s firmware %s: unexpected key size: %u != %u\n", intel_uc_fw_type_repr(uc_fw->type),
                 uc_fw->path, css->key_size_dw, UOS_RSA_SCRATCH_COUNT);
        err = -EPROTO;
        goto fail;
    }
    ...
    if (uc_fw->major_ver_found != uc_fw->major_ver_wanted || uc_fw->minor_ver_found < uc_fw->minor_ver_wanted) {
        drm_notice(&i915->drm, "%s firmware %s: unexpected version: %u.%u != %u.%u\n",
                   intel_uc_fw_type_repr(uc_fw->type), uc_fw->path, uc_fw->major_ver_found, uc_fw->minor_ver_found,
...
fail:
    intel_uc_fw_change_status(uc_fw, err == -ENOENT ? INTEL_UC_FIRMWARE_MISSING : INTEL_UC_FIRMWARE_ERROR);
...
static int uc_fw_xfer(struct intel_uc_fw *uc_fw, u32 dst_offset, u32 dma_flags)
{
    ...
    /* Start the DMA */
    intel_uncore_write_fw(uncore, DMA_CTRL, _MASKED_BIT_ENABLE(dma_flags | START_DMA));

    /* Wait for DMA to finish */
    ret = intel_wait_for_register_fw(uncore, DMA_CTRL, START_DMA, 0, 0x64);
    if (ret) {
