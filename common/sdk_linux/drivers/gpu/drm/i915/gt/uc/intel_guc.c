...
static inline i915_reg_t guc_send_reg(struct intel_guc *guc, u32 i)
{
    ...
    return _MMIO(guc->send_regs.base + 0x4 * i);
}
...
void intel_guc_init_send_regs(struct intel_guc *guc)
{
    ...
    for (i = 0; i < guc->send_regs.count; i++) {
        fw_domains |= intel_uncore_forcewake_for_reg(gt->uncore, guc_send_reg(guc, i), FW_REG_READ | FW_REG_WRITE);
    }
...
static u32 guc_ctl_debug_flags(struct intel_guc *guc)
{
    ...
    if (!GUC_LOG_LEVEL_IS_VERBOSE(level)) {
        ...
        flags |= GUC_LOG_LEVEL_TO_VERBOSITY(level) << GUC_LOG_VERBOSITY_SHIFT;
    }
...
static u32 guc_ctl_ctxinfo_flags(struct intel_guc *guc)
{
    ...
    if (intel_guc_submission_is_used(guc)) {
        ...
        base >>= PAGE_SHIFT;
        flags |= (base << GUC_CTL_BASE_ADDR_SHIFT) | (ctxnum << GUC_CTL_CTXNUM_IN16_SHIFT);
    }
    return flags;
}
...
#define FLAG 0
    ...
    BUILD_BUG_ON((DPC_BUFFER_SIZE / UNIT - 1) > (GUC_LOG_DPC_MASK >> GUC_LOG_DPC_SHIFT));
    BUILD_BUG_ON((ISR_BUFFER_SIZE / UNIT - 1) > (GUC_LOG_ISR_MASK >> GUC_LOG_ISR_SHIFT));

    flags = GUC_LOG_VALID | GUC_LOG_NOTIFY_ON_HALF_FULL | FLAG |
            ((CRASH_BUFFER_SIZE / UNIT - 1) << GUC_LOG_CRASH_SHIFT) |
            ((DPC_BUFFER_SIZE / UNIT - 1) << GUC_LOG_DPC_SHIFT) | ((ISR_BUFFER_SIZE / UNIT - 1) << GUC_LOG_ISR_SHIFT) |
...
int intel_guc_send_mmio(struct intel_guc *guc, const u32 *action, u32 len, u32 *response_buf, u32 response_buf_size)
{
     ...
     */
    ret = __intel_wait_for_register_fw(uncore, guc_send_reg(guc, 0), INTEL_GUC_MSG_TYPE_MASK,
                                       INTEL_GUC_MSG_TYPE_RESPONSE << INTEL_GUC_MSG_TYPE_SHIFT, 10, 10, &status);
    ...
    if (response_buf) {
        ...
        for (i = 0; i < count; i++) {
            response_buf[i] = intel_uncore_read(uncore, guc_send_reg(guc, i + 1));
        }
    }
...
int intel_guc_to_host_process_recv_msg(struct intel_guc *guc, const u32 *payload, u32 len)
{
    ...
    if (msg & (INTEL_GUC_RECV_MSG_FLUSH_LOG_BUFFER | INTEL_GUC_RECV_MSG_CRASH_DUMP_POSTED)) {
...
{
    u32 action[] = {INTEL_GUC_ACTION_AUTHENTICATE_HUC, rsa_offset};
...
int intel_guc_suspend(struct intel_guc *guc)
{
    ...
    u32 status;
    u32 action[] = {
        INTEL_GUC_ACTION_ENTER_S_STATE, GUC_POWER_D1, /* any value greater than GUC_POWER_D0 */
    };
    ...
    intel_uncore_write(uncore, SOFT_SCRATCH(0xe), INTEL_GUC_SLEEP_STATE_INVALID_MASK);
    ...
    ret = __intel_wait_for_register(uncore, SOFT_SCRATCH(0xe), INTEL_GUC_SLEEP_STATE_INVALID_MASK, 0, 0, 0xa, &status);
    if (ret) {
        return ret;
    }
