...
static int get_afbc_size(uint32_t width, uint32_t height, uint32_t bpp)
{
    ...
    size = hdr_size + n_blocks * ALIGN(bpp * AFBC_SUPERBLK_PIXELS / 0x8, AFBC_SUPERBLK_ALIGNMENT);
...
int rockchip_drm_dump_plane_buffer(struct vop_dump_info *dump_info, int frame_count)
{
    ...
    if (dump_info->yuv_format) {
        ...
        uv_size = (width * dump_info->height * bpp >> 0x3) * 0x2 / hsub / vsub;
        snprintf(file_name, 0x64, "%s/video%d_%d_%s.%s", DUMP_BUF_PATH, width, dump_info->height, format, "bin");
    } else {
        width = dump_info->pitches * 0x8 / bpp;
        flags = O_RDWR | O_CREAT;
        snprintf(file_name, 0x64, "%s/win%d_area%d_%dx%d_%s%s%d.%s", DUMP_BUF_PATH, dump_info->win_id,
                 dump_info->area_id, width, dump_info->height, format, dump_info->AFBC_flag ? "_AFBC_" : "_",
                 frame_count, "bin");
    }
    kvaddr = vmap(dump_info->pages, dump_info->num_pages, VM_MAP, pgprot_writecombine(PAGE_KERNEL));
    if (!kvaddr) {
    ...
    if (dump_info->AFBC_flag) {
        ...
        size = (width * dump_info->height * bpp >> 0x3) + uv_size;
    }

    ptr = file_name;
    file = filp_open(ptr, flags, 0x1a4);
    if (!IS_ERR(file)) {
...
static ssize_t rockchip_drm_dump_buffer_write(struct file *file, const char __user *ubuf, size_t len, loff_t *offp)
{
    ...
    buf[len - 1] = '\0';
    if (strncmp(buf, "dumpon", 0x6) == 0) {
        rockchip_crtc->vop_dump_status = DUMP_KEEP;
        DRM_INFO("keep dumping\n");
    } else if (strncmp(buf, "dumpoff", 0x7) == 0) {
        rockchip_crtc->vop_dump_status = DUMP_DISABLE;
        DRM_INFO("close keep dumping\n");
    } else if (strncmp(buf, "dump", 0x4) == 0) {
        if (isdigit(buf[0x4])) {
            for (i = 0x4; i < strlen(buf); i++) {
                dump_times += temp_pow(0xa, (strlen(buf) - i - 1)) * (buf[i] - '0');
            }
            rockchip_crtc->vop_dump_times = dump_times;
        } else {
            drm_modeset_lock_all(crtc->dev);
            list_for_each_entry_safe(pos, n, &rockchip_crtc->vop_dump_list_head, entry)
            {
                rockchip_drm_dump_plane_buffer(&pos->dump_info, rockchip_crtc->frame_count);
            }
...
int rockchip_drm_add_dump_buffer(struct drm_crtc *crtc, struct dentry *root)
{
    ...
    ent = debugfs_create_file("dump", 0x1a4, vop_dump_root, crtc, &rockchip_drm_dump_buffer_fops);
    if (!ent) {
