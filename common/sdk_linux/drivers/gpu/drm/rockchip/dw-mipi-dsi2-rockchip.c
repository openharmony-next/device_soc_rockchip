...
{
    const u32 field = dsi2->id ? dsi2->pdata->dsi1_grf_reg_fields[index] : dsi2->pdata->dsi0_grf_reg_fields[index];
    u16 reg;
    u8 msb, lsb;
    ...
    reg = (field >> 0x10) & 0xffff;
    lsb = (field >> 0x8) & 0xff;
    msb = (field >> 0x0) & 0xff;

    regmap_write(dsi2->grf, reg, (val << lsb) | (GENMASK(msb, lsb) << 0x10));
}
...
static int cri_fifos_wait_avail(struct dw_mipi_dsi2 *dsi2)
{
    ...
    ret = regmap_read_poll_timeout(dsi2->regmap, DSI2_CORE_STATUS, sts, !(sts & mask), 0, CMD_PKT_STATUS_TIMEOUT_US);
    if (ret < 0) {
...
static void dw_mipi_dsi2_set_vid_mode(struct dw_mipi_dsi2 *dsi2)
{
    ...
    ret = regmap_read_poll_timeout(dsi2->regmap, DSI2_MODE_STATUS, mode, mode & VIDEO_MODE, 0x3e8,
...
static void dw_mipi_dsi2_set_data_stream_mode(struct dw_mipi_dsi2 *dsi2)
{
    ...
    ret = regmap_read_poll_timeout(dsi2->regmap, DSI2_MODE_STATUS, mode, mode & DATA_STREAM_MODE, 0x3e8,
                                   MODE_STATUS_TIMEOUT_US);
    if (ret < 0) {
...
static void dw_mipi_dsi2_set_cmd_mode(struct dw_mipi_dsi2 *dsi2)
{
    ...
    ret = regmap_read_poll_timeout(dsi2->regmap, DSI2_MODE_STATUS, mode, mode & COMMAND_MODE, 0x3e8,
                                   MODE_STATUS_TIMEOUT_US);
    if (ret < 0) {
        dev_err(dsi2->dev, "failed to enter data stream mode\n");
    }
...
static void dw_mipi_dsi2_set_lane_rate(struct dw_mipi_dsi2 *dsi2)
{
    ...
    if (!of_property_read_u32(dev->of_node, "rockchip,lane-rate", &value)) {
         ...
         */
        if (dsi2->c_option) {
            tmp = DIV_ROUND_CLOSEST_ULL(tmp * 0x64, 0xe4);
        }
    ...
    phy_mipi_dphy_get_default_config(target_pclk, bpp, lanes, &dsi2->phy_opts.mipi_dphy);
...
static void dw_mipi_dsi2_host_softrst(struct dw_mipi_dsi2 *dsi2)
{
    ...
    udelay(0x64);
    regmap_write(dsi2->regmap, DSI2_SOFT_RESET, SYS_RSTN | PHY_RSTN | IPI_RSTN);
}
...
static void dw_mipi_dsi2_phy_clk_mode_cfg(struct dw_mipi_dsi2 *dsi2)
{
    ...
    esc_clk_div = DIV_ROUND_UP(sys_clk, 0xa * 0x2);
...
static void dw_mipi_dsi2_phy_ratio_cfg(struct dw_mipi_dsi2 *dsi2)
{
     ...
     */
    if (dsi2->c_option) {
        phy_hsclk = DIV_ROUND_CLOSEST_ULL(dsi2->lane_hs_rate * MSEC_PER_SEC, 0x7);
    } else {
        phy_hsclk = DIV_ROUND_CLOSEST_ULL(dsi2->lane_hs_rate * MSEC_PER_SEC, 0x10);
    }

    /* IPI_RATIO_MAN_CFG = PHY_HSTX_CLK / IPI_CLK */
    pixel_clk = mode->clock;
    ipi_clk = pixel_clk / 0x4;

    tmp = DIV_ROUND_CLOSEST_ULL(phy_hsclk << 0x10, ipi_clk);
    regmap_write(dsi2->regmap, DSI2_PHY_IPI_RATIO_MAN_CFG, PHY_IPI_RATIO(tmp));

    /*
     * SYS_RATIO_MAN_CFG = MIPI_DCPHY_HSCLK_Freq / MIPI_DCPHY_HSCLK_Freq
     */
    tmp = DIV_ROUND_CLOSEST_ULL(phy_hsclk << 0x10, sys_clk);
    regmap_write(dsi2->regmap, DSI2_PHY_SYS_RATIO_MAN_CFG, PHY_SYS_RATIO(tmp));
}
...
static void dw_mipi_dsi2_lp2hs_or_hs2lp_cfg(struct dw_mipi_dsi2 *dsi2)
{
    ...
    hstx_clk = DIV_ROUND_CLOSEST_ULL(dsi2->lane_hs_rate * USEC_PER_SEC, 0x10);
    ...
    tmp = DIV_ROUND_CLOSEST_ULL(tmp << 0x10, ui);
...
static void dw_mipi_dsi2_tx_option_set(struct dw_mipi_dsi2 *dsi2)
{
    ...
    if (dsi2->scrambling_en) {
        regmap_write(dsi2->regmap, DSI2_DSI_SCRAMBLING_CFG, SCRAMBLING_EN);
    }
...
static void dw_mipi_dsi2_ipi_color_coding_cfg(struct dw_mipi_dsi2 *dsi2)
{
    ...
    val = IPI_DEPTH(color_depth) | IPI_FORMAT(dsi2->dsc_enable ? IPI_FORMAT_DSC : IPI_FORMAT_RGB);
...
static void dw_mipi_dsi2_ipi_set(struct dw_mipi_dsi2 *dsi2)
{
    ...
    pixel_clk = mode->clock / 0x3e8;

    if (dsi2->c_option) {
        phy_hs_clk = DIV_ROUND_CLOSEST_ULL(dsi2->lane_hs_rate, 0x7);
    } else {
        phy_hs_clk = DIV_ROUND_CLOSEST_ULL(dsi2->lane_hs_rate, 0x10);
    }

    tmp = hsa * phy_hs_clk;
    hsa_time = DIV_ROUND_CLOSEST_ULL(tmp << 0x10, pixel_clk);
    regmap_write(dsi2->regmap, DSI2_IPI_VID_HSA_MAN_CFG, VID_HSA_TIME(hsa_time));

    tmp = hbp * phy_hs_clk;
    hbp_time = DIV_ROUND_CLOSEST_ULL(tmp << 0x10, pixel_clk);
    regmap_write(dsi2->regmap, DSI2_IPI_VID_HBP_MAN_CFG, VID_HBP_TIME(hbp_time));

    tmp = hact * phy_hs_clk;
    hact_time = DIV_ROUND_CLOSEST_ULL(tmp << 0x10, pixel_clk);
    regmap_write(dsi2->regmap, DSI2_IPI_VID_HACT_MAN_CFG, VID_HACT_TIME(hact_time));

    tmp = hline * phy_hs_clk;
    hline_time = DIV_ROUND_CLOSEST_ULL(tmp << 0x10, pixel_clk);
    ...
    regmap_write(dsi2->regmap, DSI2_IPI_VID_VACT_MAN_CFG, VID_VACT_LINES(vact));
    regmap_write(dsi2->regmap, DSI2_IPI_VID_VFP_MAN_CFG, VID_VFP_LINES(vfp));
}

static void dw_mipi_dsi2_work_mode(struct dw_mipi_dsi2 *dsi2, u32 mode)
{
    /*
...
static void dw_mipi_dsi2_pre_enable(struct dw_mipi_dsi2 *dsi2)
{
    ...
    if (!(dsi2->mode_flags & MIPI_DSI_CLOCK_NON_CONTINUOUS)) {
        regmap_update_bits(dsi2->regmap, DSI2_PHY_CLK_CFG, CLK_TYPE_MASK, CONTIUOUS_CLK);
    }
...
static void dw_mipi_dsi2_encoder_enable(struct drm_encoder *encoder)
{
    ...
    DRM_DEV_INFO(dsi2->dev, "final DSI-Link bandwidth: %u x %d %s\n", dsi2->lane_hs_rate,
                 dsi2->slave ? dsi2->lanes * 0x2 : dsi2->lanes, dsi2->c_option ? "Msps" : "Mbps");
}
...
static void dw_mipi_dsi2_encoder_atomic_mode_set(struct drm_encoder *encoder, struct drm_crtc_state *crtc_state,
...
static void dw_mipi_dsi2_encoder_loader_protect(struct drm_encoder *encoder, bool on)
{
    struct dw_mipi_dsi2 *dsi2 = encoder_to_dsi2(encoder);
...
static const struct drm_encoder_helper_funcs dw_mipi_dsi2_encoder_helper_funcs = {
...
static int dw_mipi_dsi2_connector_mode_valid(struct drm_connector *connector, struct drm_display_mode *mode)
{
    struct videomode vm;
     ...
     */
    if (!(vm.hsync_len < 0x4 || vm.hback_porch < 0x4 || vm.hfront_porch < 0x4 || vm.hactive < 0x4)) {
        return MODE_OK;
    }
    ...
    if (vm.hback_porch < 0x4) {
        vm.hback_porch = 0x4;
    }

    if (vm.hfront_porch < 0x4) {
        vm.hfront_porch = 0x4;
    }
...
static int dw_mipi_dsi2_dual_channel_probe(struct dw_mipi_dsi2 *dsi2)
{
    ...
    if (np) {
        dsi2->data_swap = of_property_read_bool(dsi2->dev->of_node, "rockchip,data-swap");
...
static int dw_mipi_dsi2_get_dsc_params_from_sink(struct dw_mipi_dsi2 *dsi2, struct drm_panel *panel,
                                                 struct drm_bridge *bridge)
{
    ...
    while (len > sizeof(*header)) {
        ...
        if (header->cmd_type == MIPI_DSI_PICTURE_PARAMETER_SET) {
            dsc_packed_pps = devm_kmemdup(dsi2->dev, d, header->payload_length, GFP_KERNEL);
            if (!dsc_packed_pps) {
                return -ENOMEM;
            }
...
static int dw_mipi_dsi2_bind(struct device *dev, struct device *master, void *data)
{
    ...
    ret = drm_of_find_panel_or_bridge(dev->of_node, 1, -1, &dsi2->panel, &dsi2->bridge);
    if (ret) {
    ...
    encoder->possible_crtcs = rockchip_drm_of_find_possible_crtcs(drm_dev, of_node);
    ...
    if (dsi2->panel) {
        ret = drm_connector_init(drm_dev, connector, &dw_mipi_dsi2_atomic_connector_funcs, DRM_MODE_CONNECTOR_DSI);
        if (ret) {
            DRM_DEV_ERROR(dev, "Failed to initialize connector\n");
            goto encoder_cleanup;
        }

        drm_connector_helper_add(connector, &dw_mipi_dsi2_connector_helper_funcs);
...
static void dw_mipi_dsi2_unbind(struct device *dev, struct device *master, void *data)
{
    struct dw_mipi_dsi2 *dsi2 = dev_get_drvdata(dev);
...
static const struct dsi2_irq_data dw_mipi_dsi2_irq_data[] = {
    {DSI2_INT_ST_PHY, "int_st_phy"}, {DSI2_INT_ST_TO, "int_st_to"},     {DSI2_INT_ST_ACK, "int_st_ack"},
    {DSI2_INT_ST_IPI, "int_st_ipi"}, {DSI2_INT_ST_FIFO, "int_st_fifo"}, {DSI2_INT_ST_PRI, "int_st_pri"},
...
static int dw_mipi_dsi2_host_attach(struct mipi_dsi_host *host, struct mipi_dsi_device *device)
{
    ...
    if (device->lanes < 0x1 || device->lanes > 0x8) {
        return -EINVAL;
    }
...
static int dw_mipi_dsi2_host_detach(struct mipi_dsi_host *host, struct mipi_dsi_device *device)
{
    return 0x0;
}
...
static ssize_t dw_mipi_dsi2_transfer(struct dw_mipi_dsi2 *dsi2, const struct mipi_dsi_msg *msg)
{
    ...
    /* Send payload */
    while (DIV_ROUND_UP(packet.payload_length, 0x4)) {
        /* check cri interface is not busy */
        if (packet.payload_length < 0x4) {
            /* send residu payload */
            val = 0;
...
static int dw_mipi_dsi2_probe(struct platform_device *pdev)
{
    ...
    dsi2->regmap = devm_regmap_init_mmio(dev, regs, &dw_mipi_dsi2_regmap_config);
    ...
    dsi2->grf = syscon_regmap_lookup_by_phandle(dev->of_node, "rockchip,grf");
    if (IS_ERR(dsi2->grf)) {
    ...
    if (dsi2->te_gpio) {
        ret = devm_request_threaded_irq(dsi2->dev, gpiod_to_irq(dsi2->te_gpio), NULL, dw_mipi_dsi2_te_irq_handler,
                                        IRQF_TRIGGER_FALLING | IRQF_ONESHOT, "PANEL-TE", dsi2);
        if (ret) {
    ...
    ret = devm_request_irq(dev, dsi2->irq, dw_mipi_dsi2_irq_handler, IRQF_SHARED, dev_name(dev), dsi2);
    if (ret) {
...
static const struct dev_pm_ops dw_mipi_dsi2_rockchip_pm_ops = {
    SET_RUNTIME_PM_OPS(dw_mipi_dsi2_runtime_suspend, dw_mipi_dsi2_runtime_resume, NULL)};

static const u32 rk3588_dsi0_grf_reg_fields[MAX_FIELDS] = {
    [TXREQCLKHS_EN] = GRF_REG_FIELD(0x0000, 11, 11), [GATING_EN] = GRF_REG_FIELD(0x0000, 10, 10),
    [IPI_SHUTDN] = GRF_REG_FIELD(0x0000, 9, 9),      [IPI_COLORM] = GRF_REG_FIELD(0x0000, 8, 8),
    [IPI_COLOR_DEPTH] = GRF_REG_FIELD(0x0000, 4, 7), [IPI_FORMAT] = GRF_REG_FIELD(0x0000, 0, 3),
};

static const u32 rk3588_dsi1_grf_reg_fields[MAX_FIELDS] = {
    [TXREQCLKHS_EN] = GRF_REG_FIELD(0x0004, 11, 11), [GATING_EN] = GRF_REG_FIELD(0x0004, 10, 10),
    [IPI_SHUTDN] = GRF_REG_FIELD(0x0004, 9, 9),      [IPI_COLORM] = GRF_REG_FIELD(0x0004, 8, 8),
    [IPI_COLOR_DEPTH] = GRF_REG_FIELD(0x0004, 4, 7), [IPI_FORMAT] = GRF_REG_FIELD(0x0004, 0, 3),
};
