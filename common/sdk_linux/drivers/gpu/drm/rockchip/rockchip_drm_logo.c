...
static struct rockchip_drm_sub_dev *find_sub_dev_by_node(struct drm_device *drm_dev, struct device_node *node)
{
...
static struct rockchip_drm_sub_dev *find_sub_dev_by_bridge(struct drm_device *drm_dev, struct device_node *node)
{
    ...
    for_each_child_of_node(port, endpoint)
    {
        ...
        if (!np_connector) {
            dev_err(drm_dev->dev, "can't found connector node, please init!\n");
            goto err_put_port;
        }
...
static unsigned long rockchip_drm_free_reserved_area(void *start, void *end, int poison, const char *s)
{
    void *pos;
...
static int init_loader_memory(struct drm_device *drm_dev)
{
    ...
    if (private->domain) {
        ret = iommu_map(private->domain, start, start, ALIGN(size, pg_size), IOMMU_WRITE | IOMMU_READ);
        if (ret) {
...
static struct drm_framebuffer *get_framebuffer_by_node(struct drm_device *drm_dev, struct device_node *node)
{
    struct rockchip_drm_private *private = drm_dev->dev_private;
    ...
    mode_cmd.pitches[0] = ALIGN(mode_cmd.width * bpp, 0x20) / 0x8;
...
static struct rockchip_drm_mode_set *of_parse_display_resource(struct drm_device *drm_dev, struct device_node *route)
{
    struct rockchip_drm_private *private = drm_dev->dev_private;
    ...
    if (!crtc || !sub_dev) {
        dev_warn(drm_dev->dev, "No available crtc or connector for display");
    ...
    set->ratio = 1;
    if (!of_property_read_string(route, "logo,mode", &string) && !strcmp(string, "fullscreen")) {
        set->ratio = 0;
    }
...
static int rockchip_drm_fill_connector_modes(struct drm_connector *connector, uint32_t maxX, uint32_t maxY)
{
    ...
    list_for_each_entry(mode, &connector->modes, head) mode->status = MODE_STALE;
 ...
 */
static struct drm_encoder *rockchip_drm_connector_get_single_encoder(struct drm_connector *connector)
{
...
static int setup_initial_state(struct drm_device *drm_dev, struct drm_atomic_state *state,
                               struct rockchip_drm_mode_set *set)
{
    ...
    num_modes = rockchip_drm_fill_connector_modes(connector, 0x1000, 0x1000);
    if (!num_modes) {
    ...
    list_for_each_entry(mode, &connector->modes, head)
    {
        if (mode->clock == set->clock && mode->hdisplay == set->hdisplay && mode->vdisplay == set->vdisplay &&
            mode->crtc_hsync_end == set->crtc_hsync_end && mode->crtc_vsync_end == set->crtc_vsync_end &&
    ...
    if (!found) {
        ...
        DRM_INFO("uboot set mode: h/v display[%d,%d] h/v sync_end[%d,%d] vfresh[%d], flags[0x%x], aspect_ratio[%d]\n",
                 set->hdisplay, set->vdisplay, set->crtc_hsync_end, set->crtc_vsync_end, set->vrefresh, set->flags,
                 set->picture_aspect_ratio);
        goto error_conn;
    }
    ...
    primary_state->src_w = fb_width << 0x10;
    primary_state->src_h = fb_height << 0x10;
    if (set->ratio) {
        if (set->fb->width >= hdisplay) {
            primary_state->crtc_x = 0;
            primary_state->crtc_w = hdisplay;
        } else {
            primary_state->crtc_x = (hdisplay - fb_width) / 0x2;
        ...
        if (set->fb->height >= vdisplay) {
            ...
            primary_state->crtc_y = (vdisplay - fb_height) / 0x2;
...
static int update_state(struct drm_device *drm_dev, struct drm_atomic_state *state, struct rockchip_drm_mode_set *set,
                        unsigned int *plane_mask)
{
    ...
    if (set->mode_changed) {
        ...
        ret = encoder_helper_funcs->atomic_check(encoder, crtc->state, conn_state);
        if (ret) {
            return ret;
        }

        if (encoder_helper_funcs->atomic_mode_set) {
            encoder_helper_funcs->atomic_mode_set(encoder, crtc_state, conn_state);
...
void rockchip_drm_show_logo(struct drm_device *drm_dev)
{
    ...
    old_state = drm_atomic_helper_duplicate_state(drm_dev, mode_config->acquire_ctx);
    if (IS_ERR(old_state)) {
    ...
    state = drm_atomic_helper_duplicate_state(drm_dev, mode_config->acquire_ctx);
    if (IS_ERR(state)) {
        dev_err(drm_dev->dev, "failed to duplicate atomic state for logo display\n");
    ...
    for (i = 0; i < state->num_connector; i++) {
        if (state->connectors[i].new_state->connector->status != connector_status_connected) {
...
static const char *const loader_protect_clocks[] __initconst = {
    "hclk_vio",  "hclk_vop",  "hclk_vopb", "hclk_vopl", "aclk_vio",    "aclk_vio0",
    "aclk_vio1", "aclk_vop",  "aclk_vopb", "aclk_vopl", "aclk_vo_pre", "aclk_vio_pre",
    "dclk_vop",  "dclk_vop0", "dclk_vop1", "dclk_vop2", "dclk_vopb",   "dclk_vopl",
};
