...
#define VOP_INTR_GET_TYPE(vop2, intr, name, type) vop2_get_intr_type((vop2), (intr), &(intr)->name, (type))

#define VOP_MODULE_GET(x, module, name) vop2_read_reg((x), 0, &(module)->regs->name)

#define VOP_WIN_GET(vop2, win, name) vop2_read_reg((vop2), (win)->offset, &VOP_WIN_NAME(win, name))

#define VOP_WIN_NAME(win, name) (vop2_get_win_regs((win), &(win)->regs->name)->name)

#define VOP_WIN_TO_INDEX(vop2_win) ((vop2_win) - (vop2_win)->vop2->win)
...
static inline uint32_t vop2_read_reg(struct vop2 *vop2, uint32_t base, const struct vop_reg *reg)
{
...
static inline void vop2_mask_write(struct vop2 *vop2, uint32_t offset, uint32_t mask, uint32_t shift, uint32_t v,
                                   bool write_mask, bool relaxed)
{
    ...
    if (write_mask) {
        ...
        cached_val = vop2->regsbak[offset >> 0x2];
...
static inline const struct vop2_win_regs *vop2_get_win_regs(struct vop2_win *win, const struct vop_reg *reg)
{
...
static inline uint32_t vop2_get_intr_type(struct vop2 *vop2, const struct vop_intr *intr, const struct vop_reg *reg,
                                          int type)
{
    uint32_t val, i;
    uint32_t ret = 0;
...
static void vop2_load_hdr2sdr_table(struct vop2_video_port *vp)
{
    ...
    for (i = 0; i < 0x21; i++) {
        vop2_writel(vop2, regs->hdr2sdr_eetf_oetf_y0_offset + i * 0x4, hdr2sdr_eetf_oetf_yn[i]);
    }

    for (i = 0; i < 0x9; i++) {
        vop2_writel(vop2, regs->hdr2sdr_sat_y0_offset + i * 0x4, table->hdr2sdr_sat_yn[i]);
    }
}
...
static void vop2_load_sdr2hdr_table(struct vop2_video_port *vp, int sdr2hdr_tf)
{
    ...
    for (i = 0; i < 0x41; i++) {
        vop2_writel(vop2, regs->sdr2hdr_eotf_oetf_y0_offset + i * 0x4, sdr2hdr_eotf_oetf_yn[i]);
    }

    for (i = 0; i < 0x40; i++) {
        sdr2hdr_oetf_dx_dxpow[i] = table->sdr2hdr_st2084oetf_dxn[i] + (table->sdr2hdr_st2084oetf_dxn_pow2[i] << 0x10);
        vop2_writel(vop2, regs->sdr2hdr_oetf_dx_pow1_offset + i * 0x4, sdr2hdr_oetf_dx_dxpow[i]);
    }

    for (i = 0; i < 0x3f; i++) {
        vop2_writel(vop2, regs->sdr2hdr_oetf_xn1_offset + i * 0x4, table->sdr2hdr_st2084oetf_xn[i]);
    }
}
...
{
    uint32_t offset = RK3568_SYS_STATUS0 + (vp->id << 0x2);

    return vop2_readl(vp->vop2, offset) >> 0x10;
}
...
static void vop2_wait_for_irq_handler(struct drm_crtc *crtc)
{
     ...
     */
    ret = readx_poll_timeout_atomic(vop2_fs_irq_is_pending, vp, pending, !pending, 0, 0x2710);
    if (ret) {
...
static void vop2_wait_for_fs_by_done_bit_status(struct vop2_video_port *vp)
{
    ...
    ret = readx_poll_timeout_atomic(vop2_vp_done_bit_status, vp, done_bit, done_bit, 0, 0xc350);
    if (ret) {
...
static void vop2_wait_for_port_mux_done(struct vop2 *vop2)
{
     ...
     * is done.
     */
    ret = readx_poll_timeout_atomic(vop2_read_port_mux, vop2, port_mux_cfg, port_mux_cfg == vop2->port_mux_cfg, 0,
                                    0xc350);
    if (ret) {
...
static void vop2_wait_for_layer_cfg_done(struct vop2 *vop2, u32 cfg)
{
     ...
     */
    ret = readx_poll_timeout_atomic(vop2_read_layer_cfg, vop2, atv_layer_cfg, atv_layer_cfg == cfg, 0, 0xc350);
    if (ret) {
...
static int32_t vop2_pending_done_bits(struct vop2_video_port *vp)
{
    ...
    if (hweight32(done_bits) == 1 || (hweight32(done_bits) == 0x2 && (done_bits & BIT(vp->id)))) {
        ...
        if (vcnt > ((adjusted_mode->crtc_vtotal * 0x7) >> 0x3)) {
        ...
        first_vp_safe_time = (0xf4240 / drm_mode_vrefresh(first_mode)) >> 0x3;
        ...
        /* set last 1/8 frame time as safe section */
        second_vp_safe_time = (0xf4240 / drm_mode_vrefresh(second_mode)) >> 0x3;
        ...
        if (first_vp_left_time > first_vp_safe_time && second_vp_left_time > second_vp_safe_time) {
...
static inline void rk3568_vop2_cfg_done(struct drm_crtc *crtc)
{
    ...
    cfg_layer_sel_val = vop2->regsbak[layer_sel_offset >> 0x2];
    /**
    ...
    if (old_layer_sel_val != cfg_layer_sel_val) {
        ...
        DRM_DEV_DEBUG(vop2->dev, "vp%d need to wait fs as old layer_sel val[0x%x] != new val[0x%x]\n", vp->id,
                      old_layer_sel_val, cfg_layer_sel_val);
    }
}
...
static inline void rk3588_vop2_cfg_done(struct drm_crtc *crtc)
{
    ...
    val = RK3568_VOP2_GLB_CFG_DONE_EN | BIT(vp->id) | (BIT(vp->id) << 0x10);
    if (vcstate->splice_mode) {
        val |= BIT(vp_data->splice_vp_id) | (BIT(vp_data->splice_vp_id) << 0x10);
    }
...
{
    struct vop2 *vop2 = vp->vop2;
    uint32_t val = RK3568_VOP2_WB_CFG_DONE | (RK3568_VOP2_WB_CFG_DONE << 0x10);
    uint32_t done_bits;
    unsigned long flags;
...
static void vop2_wait_power_domain_off(struct vop2_power_domain *pd)
{
    ...
    ret = readx_poll_timeout_atomic(vop2_power_domain_status, pd, val, !val, 0, 0xc350);
    if (ret) {
...
static void vop2_power_domain_put(struct vop2_power_domain *pd)
{
    ...
    if (--pd->ref_count == 0 && pd->module_on) {
        if (pd->vop2->data->delayed_pd) {
            schedule_delayed_work(&pd->power_off_work, msecs_to_jiffies(0x9c4));
        } else {
    ...
    if (win->enabled) {
        ...
        if (win->feature & WIN_FEATURE_CLUSTER_MAIN) {
            ...
            for (i = 0; i < vop2->registered_num_wins; i++) {
                ...
                if ((sub_win->phys_id == win->phys_id) && (sub_win->feature & WIN_FEATURE_CLUSTER_SUB)) {
...
static uint32_t vop2_afbc_transform_offset(struct vop2_plane_state *vpstate, int xoffset)
{
    ...
    uint32_t vir_width  = 0;
    uint32_t width = drm_rect_width(src) >> 0x10;
    uint32_t height = drm_rect_height(src) >> 0x10;
    uint32_t act_xoffset = src->x1 >> 0x10;
    uint32_t act_yoffset = src->y1 >> 0x10;
    ...
    /* 64 pixel align */
    if (height_tmp & 0x3f) {
        align64_crop = 0x40 - (height_tmp & 0x3f);
    }

    top_crop_line_num = top_crop << 0x2;
    if (top_crop == 0x0) {
        bottom_crop_line_num = align16_crop + align64_crop;
    } else if (top_crop == 0x1) {
        bottom_crop_line_num = align16_crop + align64_crop + 0xc;
    } else if (top_crop == 0x2) {
        bottom_crop_line_num = align16_crop + align64_crop + 0x8;
    }
    ...
    if (vpstate->xmirror_en) {
        if (vpstate->ymirror_en) {
            if (vpstate->afbc_half_block_en) {
                ...
                transform_xoffset = 0x10 - (transform_tmp & 0xf);
    ...
    return (transform_xoffset & 0xf) | ((transform_yoffset & 0xf) << 0x10);
}
...
#define VOP2_BILI_SCL_FAC_CHECK(src, dst, fac) ((((fac) * ((dst)-1)) >> 12) < ((src)-1))
#define VOP2_COMMON_SCL_FAC_CHECK(src, dst, fac) ((((fac) * ((dst)-1)) >> 16) < ((src)-1))

static uint16_t vop2_scale_factor(enum scale_mode mode, int32_t filter_mode, uint32_t src, uint32_t dst)
{
    uint32_t fac = 0;
    int i = 0;
...
static void vop2_setup_scale(struct vop2 *vop2, const struct vop2_win *win, uint32_t src_w, uint32_t src_h,
                             uint32_t dst_w, uint32_t dst_h, uint32_t pixel_format)
{
    ...
    if (info->is_yuv) {
        ...
        if (cbcr_src_h >= (0x4 * dst_h)) {
            gt4 = 1;
        } else if (cbcr_src_h >= (0x2 * dst_h)) {
            gt2 = 1;
        }
        ...
        val = vop2_scale_factor(cbcr_hor_scl_mode, hscl_filter_mode, cbcr_src_w, dst_w);
        VOP_SCL_SET(vop2, win, scale_cbcr_x, val);
        val = vop2_scale_factor(cbcr_ver_scl_mode, vscl_filter_mode, cbcr_src_h, dst_h);
...
static void vop2_disable_all_planes_for_crtc(struct drm_crtc *crtc)
{
    ...
    if (need_wait_win_disabled) {
        vop2_cfg_done(crtc);
        ret = readx_poll_timeout_atomic(vop2_is_allwin_disabled, crtc, active, active, 0, 0x7a120);
        if (ret) {
...
static void vop2_setup_csc_mode(struct vop2_video_port *vp, struct vop2_plane_state *vpstate)
{
...
static enum drm_connector_status vop2_wb_connector_detect(struct drm_connector *connector, bool force)
{
...
static struct drm_connector_state *vop2_wb_connector_duplicate_state(struct drm_connector *connector)
{
    struct vop2_wb_connector_state *wb_state;
...
static int vop2_wb_connector_get_modes(struct drm_connector *connector)
{
    ...
    for (i = 0; i < 0x2; i++) {
        ...
        mode->clock = 0x24414 >> i;
        mode->hdisplay = 0x780 >> i;
        mode->hsync_start = 0x78a >> i;
        mode->hsync_end = 0x794 >> i;
        mode->htotal = 0x7c6 >> i;
        mode->vdisplay = 0x438 >> i;
        mode->vsync_start = 0x442 >> i;
        mode->vsync_end = 0x44c >> i;
        mode->vtotal = 0x44c >> i;
        mode->flags = 0;
...
static enum drm_mode_status vop2_wb_connector_mode_valid(struct drm_connector *connector, struct drm_display_mode *mode)
{
...
static int vop2_wb_encoder_atomic_check(struct drm_encoder *encoder, struct drm_crtc_state *cstate,
                                        struct drm_connector_state *conn_state)
{
    ...
    if ((fb->width > cstate->mode.hdisplay) ||
        ((fb->height != cstate->mode.vdisplay) && (fb->height != (cstate->mode.vdisplay >> 1)))) {
        DRM_DEBUG_KMS("Invalid framebuffer size %ux%u, Only support x scale down and 1/2 y scale down\n", fb->width,
                      fb->height);
        return -EINVAL;
    }

    wb_state->scale_x_factor = vop2_scale_factor(SCALE_DOWN, VOP2_SCALE_DOWN_BIL, cstate->mode.hdisplay, fb->width);
...
static int vop2_wb_connector_init(struct vop2 *vop2, int nr_crtcs)
{
    ...
    ret = drm_writeback_connector_init(vop2->drm_dev, &vop2->wb.conn, &vop2_wb_connector_funcs,
                                       &vop2_wb_encoder_helper_funcs, vop2_data->wb->formats, vop2_data->wb->nformats);
    if (ret) {
...
static void rk3568_crtc_load_lut(struct drm_crtc *crtc)
{
    ...
    if (vop2->data->nr_gammas && vp_enable_gamma_nr >= vop2->data->nr_gammas && vp->gamma_lut_active == false) {
...
#define CTRL_GET(name) VOP_MODULE_GET(vop2, vp, name)
    ...
    for (i = 0; i < vp->gamma_lut_len; i++) {
        vop2_write_lut(vop2, i << 0x2, vp->lut[i]);
    }
...
static void rk3588_crtc_load_lut(struct drm_crtc *crtc, u32 *lut)
{
    ...
    VOP_CTRL_SET(vop2, gamma_port_sel, vp->id);
    for (i = 0; i < vp->gamma_lut_len; i++) {
        vop2_write_lut(vop2, i << 0x2, lut[i]);
    }
...
static void rockchip_vop2_crtc_fb_gamma_set(struct drm_crtc *crtc, u16 red, u16 green, u16 blue, int regno)
{
    struct vop2_video_port *vp = to_vop2_video_port(crtc);
...
static void rockchip_vop2_crtc_fb_gamma_get(struct drm_crtc *crtc, u16 *red, u16 *green, u16 *blue, int regno)
{
    struct vop2_video_port *vp = to_vop2_video_port(crtc);
    u32 lut_len = vp->gamma_lut_len;
    u32 r, g, b;
...
static int vop2_crtc_legacy_gamma_set(struct drm_crtc *crtc, u16 *red, u16 *green, u16 *blue, uint32_t size,
                                      struct drm_modeset_acquire_ctx *ctx)
{
    ...
    for (i = 0; i < size; i++) {
        rockchip_vop2_crtc_fb_gamma_set(crtc, red[i], green[i], blue[i], i);
    }
...
static int vop2_crtc_atomic_gamma_set(struct drm_crtc *crtc, struct drm_crtc_state *old_state)
{
    struct vop2_video_port *vp = to_vop2_video_port(crtc);
    ...
    for (i = 0; i < vp->gamma_lut_len; i++) {
        rockchip_vop2_crtc_fb_gamma_set(crtc, lut[i].red, lut[i].green, lut[i].blue, i);
    }
    vop2_crtc_load_lut(crtc);
...
static int vop2_crtc_atomic_cubic_lut_set(struct drm_crtc *crtc, struct drm_crtc_state *old_state)
{
    struct rockchip_crtc_state *vcstate = to_rockchip_crtc_state(crtc->state);
    struct vop2_video_port *vp = to_vop2_video_port(crtc);
    ...
    if (!private->cubic_lut[vp->id].enable) {
        if (!vp->cubic_lut_gem_obj) {
            size_t size = (vp->cubic_lut_len + 1) / 0x2 * 0x10;
    ...
    if (vp->cubic_lut_len % 0x2) {
        *cubic_lut_kvaddr++ =
            (lut[0x2 * i].red & 0xfff) + ((lut[0x2 * i].green & 0xfff) << 0xc) + ((lut[0x2 * i].blue & 0xff) << 0x18);
        *cubic_lut_kvaddr++ = (lut[0x2 * i].blue & 0xf00) >> 0x8;
        *cubic_lut_kvaddr++ = 0;
        *cubic_lut_kvaddr = 0;
    }
...
static void drm_crtc_enable_cubic_lut(struct drm_crtc *crtc, unsigned int cubic_lut_size)
{
    ...
    if (cubic_lut_size) {
        drm_object_attach_property(&crtc->base, config->cubic_lut_property, 0);
        drm_object_attach_property(&crtc->base, config->cubic_lut_size_property, cubic_lut_size);
    }
}
...
static void vop2_layer_map_initial(struct vop2 *vop2, uint32_t current_vp_id)
{
    ...
    for (i = 0; i < vop2->data->nr_vps - 1; i++) {
        ...
        port_mux_cfg |= port_mux << (vp->id * 0x4);
    }

    /* the last VP is fixed */
    if (vop2->data->nr_vps >= 1) {
        port_mux_cfg |= 0x7 << (0x4 * (vop2->data->nr_vps - 1));
    }
...
static void vop2_crtc_atomic_disable(struct drm_crtc *crtc, struct drm_crtc_state *old_state)
{
    ...
    if (vop2->dscs[vcstate->dsc_id].enabled && vop2->dscs[vcstate->dsc_id].attach_vp_id == vp->id &&
        vop2->data->nr_dscs) {
        if (dual_channel) {
    ...
    ret = wait_for_completion_timeout(&vp->dsp_hold_completion, msecs_to_jiffies(0x32));
    if (!ret) {
...
static int vop2_cluster_two_win_mode_check(struct drm_plane_state *pstate)
{
    ...
    int actual_w = drm_rect_width(&pstate->src) >> 0x10;
    int xoffset;

    if (pstate->fb->modifier == DRM_FORMAT_MOD_LINEAR) {
        xoffset = 0;
    } else {
        xoffset = pstate->src.x1 >> 0x10;
    }

    if ((actual_w + xoffset % 0x10) > 0x800) {
    ...
    if (pstate->fb->modifier != main_pstate->fb->modifier) {
        DRM_ERROR("%s(fb->modifier: 0x%llx) must use same data layout as %s(fb->modifier: 0x%llx)\n", win->name,
                  pstate->fb->modifier, main_win->name, main_pstate->fb->modifier);
        return -EINVAL;
    }
    ...
    if (main_pstate->fb->modifier == DRM_FORMAT_MOD_LINEAR) {
        ...
        xoffset = main_pstate->src.x1 >> 0x10;
    }
    actual_w = drm_rect_width(&main_pstate->src) >> 0x10;
    if ((actual_w + xoffset % 0x10) > 0x800) {
...
static int vop2_cluter_splice_scale_check(struct vop2_win *win, struct drm_plane_state *pstate, u16 hdisplay)
{
    ...
    /* scale up is ok */
    if ((drm_rect_width(&src) >> 0x10) <= drm_rect_width(&dst)) {
        return 0;
    }
    ...
    if ((dst.x1 < half_hdisplay) && (dst.x2 > half_hdisplay)) {
        ...
        if (drm_rect_calc_hscale(&src, &dst, 1, FRAC_16_16(0x6, 0x5)) < 0) {
            DRM_ERROR("%s %d --> %d scale down factor should < 1.2 in splice mode\n", win->name,
                      drm_rect_width(&src) >> 0x10, drm_rect_width(&dst));
            return -EINVAL;
        }
    }
...
static int vop2_plane_atomic_check(struct drm_plane *plane, struct drm_plane_state *state)
{
    ...
    struct drm_gem_object *obj, *uv_obj;
    struct rockchip_gem_object *rk_obj, *rk_uv_obj;
    int min_scale = win->regs->scl ? FRAC_16_16(0x1, 0x8) : DRM_PLANE_HELPER_NO_SCALING;
    int max_scale = win->regs->scl ? FRAC_16_16(0x8, 0x1) : DRM_PLANE_HELPER_NO_SCALING;
    int max_input_w;
    int max_input_h;
    unsigned long offset;
    dma_addr_t dma_addr;
    ...
    ret = drm_atomic_helper_check_plane_state(state, cstate, min_scale, max_scale, true, true);
    if (ret) {
        return ret;
    }
    ...
    if (!state->visible) {
        DRM_ERROR("%s is invisible(src: pos[%d, %d] rect[%d x %d] dst: pos[%d, %d] rect[%d x %d]\n", plane->name,
                  state->src_x >> 0x10, state->src_y >> 0x10, state->src_w >> 0x10, state->src_h >> 0x10, state->crtc_x,
    ...
    vpstate->global_alpha = state->alpha >> 0x8;
    ...
    if (drm_rect_width(src) >> 0x10 < 0x4 || drm_rect_height(src) >> 0x10 < 0x4 || drm_rect_width(dest) < 0x4 ||
        drm_rect_width(dest) < 0x4) {
        DRM_ERROR("Invalid size: %dx%d->%dx%d, min size is 4x4\n", drm_rect_width(src) >> 0x10,
                  drm_rect_height(src) >> 0x10, drm_rect_width(dest), drm_rect_height(dest));
        state->visible = false;
        return 0;
    }

    if (((drm_rect_width(src) >> 0x10) > max_input_w) || ((drm_rect_height(src) >> 0x10) > max_input_h)) {
        DRM_ERROR("Invalid source: %dx%d. max input: %dx%d\n", drm_rect_width(src) >> 0x10,
                  drm_rect_height(src) >> 0x10, max_input_w, max_input_h);
        return -EINVAL;
    }
    ...
    if (vp->vop2->version > VOP_VERSION_RK3568) {
        ...
        if (vop2_cluster_window(win) && !vpstate->afbc_en && (win->supported_rotations & state->rotation)) {
    ...
    if (fb->format->is_yuv && fb->format->num_planes > 1) {
        ...
        if (vpstate->ymirror_en && !vpstate->afbc_en) {
            offset += fb->pitches[1] * ((state->src_h >> 0x10) - 0x2) / vsub;
        }
...
static void vop2_plane_setup_color_key(struct drm_plane *plane)
{
    ...
    switch (fb->format->format) {
        case DRM_FORMAT_RGB565:
        case DRM_FORMAT_BGR565:
            r = (vpstate->color_key & 0xf800) >> 0xb;
            g = (vpstate->color_key & 0x7e0) >> 0x5;
        ...
        case DRM_FORMAT_RGB888:
        case DRM_FORMAT_BGR888:
            r = (vpstate->color_key & 0xff0000) >> 0x10;
            g = (vpstate->color_key & 0xff00) >> 0x8;
    ...
    color_key = (r << 0x14) | (g << 0xa) | b;
    VOP_WIN_SET(vop2, win, color_key_en, color_key_en);
    VOP_WIN_SET(vop2, win, color_key, color_key);
}

static void vop2_calc_drm_rect_for_splice(struct vop2_plane_state *vpstate, struct drm_rect *left_src,
                                          struct drm_rect *left_dst, struct drm_rect *right_src,
                                          struct drm_rect *right_dst)
{
    ...
    int src_w = drm_rect_width(src) >> 0x10;
    ...
    if (!right_dst_w) {
        ...
        left_src_w = (left_dst_w * hscale) >> 0x10;
    }
    left_src->x1 = src->x1;
    left_src->x2 = src->x1 + (left_src_w << 0x10);
    left_dst->x1 = dst->x1;
                                   ...
                                   struct drm_plane_state *pstate)
{
    ...
    if (win->splice_mode_right) {
        splice_pixel_offset = (src->x1 - left_src->x1) >> 0x10;
    ...
    DRM_DEV_DEBUG(vop2->dev, "vp%d update %s[%dx%d->%dx%d@(%d, %d)] fmt[%.4s_%s] addr[%pad]\n", vp->id, win->name,
                  actual_w, actual_h, dsp_w, dsp_h, dsp_stx, dsp_sty,
                  drm_get_format_name(fb->format->format, &format_name), vpstate->afbc_en ? "AFBC" : "",
                  &vpstate->yrgb_mst);
    ...
    if (vpstate->afbc_en) {
        ...
        afbc_tile_num = ALIGN(actual_w, 0x10) >> 0x4;

        /* The right win should have a src  offset in splice mode */
        afbc_xoffset = (src->x1 >> 0x10);
        ...
        VOP_AFBC_SET(vop2, win, dsp_offset, (dst->x1 | (dst->y1 << 0x10)));
    ...
    if (vpstate->rotate_90_en || vpstate->rotate_270_en) {
        ...
        actual_w = drm_rect_height(src) >> 0x10;
        actual_h = drm_rect_width(src) >> 0x10;
    }
    ...
    if (fb->format->is_yuv) {
        VOP_WIN_SET(vop2, win, uv_vir, DIV_ROUND_UP(fb->pitches[1], 0x4));
...
static void vop2_plane_atomic_update(struct drm_plane *plane, struct drm_plane_state *old_state)
{
    ...
    if (vcstate->splice_mode) {
        DRM_DEV_DEBUG(vop2->dev, "vp%d update %s[%dx%d->%dx%d@(%d,%d)] fmt[%.4s_%s] addr[%pad]\n", vp->id, win->name,
                      drm_rect_width(&vpstate->src) >> 0x10, drm_rect_height(&vpstate->src) >> 0x10,
                      drm_rect_width(&vpstate->dest), drm_rect_height(&vpstate->dest), vpstate->dest.x1,
                      vpstate->dest.y1, drm_get_format_name(fb->format->format, &format_name),
    ...
    if (planlist) {
        ...
        planlist->dump_info.height = drm_rect_height(psrc) >> 0x10;
    ...
    if (vp->rockchip_crtc.vop_dump_status == DUMP_KEEP || vp->rockchip_crtc.vop_dump_times > 0) {
                                                               ...
                                                               struct drm_modeset_acquire_ctx *ctx)
{
    ...
    if ((pstate->crtc && pstate->crtc->cursor == plane) || vpstate->async_commit) {
...
static void vop2_atomic_plane_destroy_state(struct drm_plane *plane, struct drm_plane_state *state)
{
    struct vop2_plane_state *vpstate = to_vop2_plane_state(state);
...
static int vop2_atomic_plane_set_property(struct drm_plane *plane, struct drm_plane_state *state,
...
static int vop2_atomic_plane_get_property(struct drm_plane *plane, const struct drm_plane_state *state,
                                          struct drm_property *property, uint64_t *val)
{
    struct rockchip_drm_private *private = plane->dev->dev_private;
    struct vop2_plane_state *vpstate = to_vop2_plane_state(state);
...
static void vop2_crtc_cancel_pending_vblank(struct drm_crtc *crtc, struct drm_file *file_priv)
{
...
static void vop2_crtc_regs_dump(struct drm_crtc *crtc, struct seq_file *s)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        pr_info("%s:\n", regs[i].name);
        for (j = 0; j < len; j++) {
            buf[j] = vop2_readl(vop2, base + (0x4 * j));
        }
        print_hex_dump(KERN_INFO, "", DUMP_PREFIX_OFFSET, 0x10, 0x4, buf, len << 0x2, 0);
    }
}
...
static int vop2_crtc_debugfs_init(struct drm_minor *minor, struct drm_crtc *crtc)
{
    ...
    ret = snprintf(name, sizeof(name), "video_port%d", vp->id);
    vop2->debugfs = debugfs_create_dir(name, minor->debugfs_root);
    if (!vop2->debugfs) {
        return -ENOMEM;
    }

    vop2->debugfs_files = kmemdup(vop2_debugfs_files, sizeof(vop2_debugfs_files), GFP_KERNEL);
    ...
    drm_debugfs_create_files(vop2->debugfs_files, ARRAY_SIZE(vop2_debugfs_files), vop2->debugfs, minor);
    return 0;
remove:
...
static enum drm_mode_status vop2_crtc_mode_valid(struct drm_crtc *crtc, const struct drm_display_mode *mode)
{
    struct rockchip_crtc_state *vcstate = to_rockchip_crtc_state(crtc->state);
    struct vop2_video_port *vp = to_vop2_video_port(crtc);
    struct vop2 *vop2 = vp->vop2;
    ...
    if (request_clock <= VOP2_MAX_DCLK_RATE) {
        clock = clk_round_rate(vp->dclk, request_clock * 0x3e8) / 0x3e8;
    } else {
        clock = request_clock;
    }
     ...
     */
    if (vcstate->output_type == DRM_MODE_CONNECTOR_HDMIA || vcstate->output_type == DRM_MODE_CONNECTOR_DisplayPort) {
...
static u64 vop2_calc_max_bandwidth(struct vop2_bandwidth *bw, int start, int count, int y2)
{
    u64 max_bandwidth = 0;
    int i;
    ...
    for (i = start; i < count; i++) {
        ...
        bandwidth += vop2_calc_max_bandwidth(bw, i + 1, count, min(bw[i].y2, y2));
...
static size_t vop2_crtc_bandwidth(struct drm_crtc *crtc, struct drm_crtc_state *crtc_state,
...
static bool vop2_crtc_mode_fixup(struct drm_crtc *crtc, const struct drm_display_mode *mode,
                                 struct drm_display_mode *adj_mode)
{
    ...
    if (adj_mode->crtc_clock <= VOP2_MAX_DCLK_RATE) {
        adj_mode->crtc_clock = DIV_ROUND_UP(clk_round_rate(vp->dclk, adj_mode->crtc_clock * 0x3e8), 0x3e8);
    }
    return true;
}
...
static void vop2_post_config(struct drm_crtc *crtc)
{
    ...
    u16 hsize = hdisplay * (vcstate->left_margin + vcstate->right_margin) / 0xc8;
    u16 vsize = vdisplay * (vcstate->top_margin + vcstate->bottom_margin) / 0xc8;
    ...
    vact_st += vdisplay * (0x64 - vcstate->top_margin) / 0xc8;
...
static bool vop2_crtc_mode_update(struct drm_crtc *crtc)
{
    ...
    if (htotal_sync != VOP_MODULE_GET(vop2, vp, htotal_pw) || hactive_st_end != VOP_MODULE_GET(vop2, vp, hact_st_end) ||
        vtotal_sync != VOP_MODULE_GET(vop2, vp, vtotal_pw) || vactive_st_end != VOP_MODULE_GET(vop2, vp, vact_st_end) ||
...
static int vop2_set_dsc_clk(struct drm_crtc *crtc, u8 dsc_id)
{
    ...
    /* set clk parent */
    ret = snprintf(clk_name, sizeof(clk_name), "dclk%d", vp->id);
                            ...
                            struct vop2_clk *if_pixclk, struct vop2_clk *if_dclk)
{
    ...
    if (!if_dclk &&
        (vcstate->output_type == DRM_MODE_CONNECTOR_HDMIA || vcstate->output_type == DRM_MODE_CONNECTOR_eDP)) {
        return -EINVAL;
    }
    if ((vcstate->output_flags & ROCKCHIP_OUTPUT_DUAL_CHANNEL_LEFT_RIGHT_MODE) &&
     ...
     *
     */
    ret = snprintf(clk_name, sizeof(clk_name), "dclk_core%d", vp->id);
    dclk_core = vop2_clk_get(vop2, clk_name);

    ret = snprintf(clk_name, sizeof(clk_name), "dclk_out%d", vp->id);
    dclk_out = vop2_clk_get(vop2, clk_name);

    if (vcstate->dsc_enable) {
        if ((vcstate->dsc_txp_clk_rate >= dclk_core_rate) && (vcstate->dsc_txp_clk_rate >= if_pixclk->rate)) {
...
static int vop2_calc_dsc_clk(struct drm_crtc *crtc)
{
     ...
     */
    vcstate->dsc_cds_clk_rate = v_pixclk / 0x8;

    return 0;
}

static int vop2_calc_cru_cfg(struct drm_crtc *crtc, int conn_id, struct vop2_clk **if_pixclk, struct vop2_clk **if_dclk)
{
    struct vop2_video_port *vp = to_vop2_video_port(crtc);
    struct rockchip_crtc_state *vcstate = to_rockchip_crtc_state(crtc->state);
    struct vop2 *vop2 = vp->vop2;
    ...
    ret = snprintf(clk_name, sizeof(clk_name), "%s%d", if_data->clk_parent_name, vp->id);
...
static void vop2_crtc_load_pps(struct drm_crtc *crtc, u8 dsc_id)
{
    ...
    if ((config_pps.pps_3 & 0xf) > dsc->max_linebuf_depth) {
        ...
        DRM_WARN("DSC%d max_linebuf_depth is: %d, current set value is: %d\n", dsc_id, dsc->max_linebuf_depth,
                 config_pps.pps_3 & 0xf);
    }

    for (i = 0; i < DSC_NUM_BUF_RANGES; i++) {
        config_pps.rc_range_parameters[i] =
            ((pps->rc_range_parameters[i] >> 0x3) & 0x1f) | ((pps->rc_range_parameters[i] >> 0xe) & (0x3 << 0x5)) |
            ((pps->rc_range_parameters[i] >> 0x0) & (0x7 << 0x7)) | \
            ((pps->rc_range_parameters[i] >> 0x8) & (0x3f << 0xa));
    }

    for (i = 0; i < ROCKCHIP_DSC_PPS_SIZE_BYTE / 0x4; i++) {
        vop2_writel(vop2, offset + i * 0x4, *pps_val++);
    }
}
...
static void vop2_crtc_enable_dsc(struct drm_crtc *crtc, struct drm_crtc_state *old_state, u8 dsc_id)
{
    ...
    if (vcstate->dsc_slice_num > dsc_data->max_slice_num) {
        DRM_ERROR("DSC%d supported max slice is: %d, current is: %d\n", dsc_data->id, dsc_data->max_slice_num,
                  vcstate->dsc_slice_num);
    }
    ...
    if (!mipi_ds_mode) {
        ...
        u32 v_pixclk_mhz = adjusted_mode->crtc_clock / 0x3e8; /* video timing pixclk */
        u32 dly_num, dsc_cds_rate_mhz, val = 0;

        if (target_bpp >> 0x4 < dsc->min_bits_per_pixel) {
        ...
        dsc_hsync = hsync_len / 0x2;
        dsc_htotal = htotal / (1 << dsc_cds_clk->div_val);
        val = (dsc_htotal << 0x10) | dsc_hsync;
        VOP_MODULE_SET(vop2, dsc, dsc_htotal_pw, val);

        dsc_hact_st = hact_st / 0x2;
        dsc_hact_end = ((hdisplay * target_bpp) >> 0x4) / 0x18 + dsc_hact_st;
        val = (dsc_hact_end << 0x10) | dsc_hact_st;
        VOP_MODULE_SET(vop2, dsc, dsc_hact_st_end, val);

        VOP_MODULE_SET(vop2, dsc, dsc_vtotal_pw, (vtotal << 0x10) | vsync_len);
        VOP_MODULE_SET(vop2, dsc, dsc_vact_st_end, (vact_end << 0x10) | vact_st);
    }
    ...
    vop2->regsbak[offset >> 0x2] = reg_base[offset >> 0x2];
    ...
    VOP_MODULE_SET(vop2, dsc, dsc_ifep, dsc_sink_cap->version_minor == 0x2 ? 1 : 0);
    VOP_MODULE_SET(vop2, dsc, dsc_pps_upd, 1);

    DRM_DEV_INFO(vop2->dev, "DSC%d: txp:%lld div:%d, pxl:%lld div:%d, dsc:%lld div:%d\n", dsc->id,
                 vcstate->dsc_txp_clk_rate, dsc_txp_clk->div_val, vcstate->dsc_pxl_clk_rate, dsc_pxl_clk->div_val,
...
static void vop2_crtc_atomic_enable(struct drm_crtc *crtc, struct drm_crtc_state *old_state)
{
    ...
    if (vcstate->dsc_enable) {
        ...
        vcstate->dsc_pixel_num = vcstate->dsc_slice_num > 0x4 ? 0x4 : vcstate->dsc_slice_num;
    ...
    if (vcstate->output_if & VOP_OUTPUT_IF_BT1120) {
        ...
        if (vop2->version == VOP_VERSION_RK3588) {
            VOP_CTRL_SET(vop2, bt1120_en, 0x3);
        } else {
            VOP_CTRL_SET(vop2, rgb_en, 1);
    ...
    if (vcstate->output_flags &
        (ROCKCHIP_OUTPUT_DUAL_CHANNEL_ODD_EVEN_MODE | ROCKCHIP_OUTPUT_DUAL_CHANNEL_LEFT_RIGHT_MODE)) {
    ...
    if ((vcstate->output_mode == ROCKCHIP_OUT_MODE_AAAA && !(vp_data->feature & VOP_FEATURE_OUTPUT_10BIT)) ||
    ...
    VOP_MODULE_SET(vop2, vp, htotal_pw, (htotal << 0x10) | hsync_len);
    val = hact_st << 0x10;
    val |= hact_end;
    ...
    VOP_INTR_SET(vop2, intr, line_flag_num[1], act_end - us_to_vertical_line(adjusted_mode, for_ddr_freq));

    VOP_MODULE_SET(vop2, vp, vtotal_pw, (vtotal << 0x10) | vsync_len);
    ...
    if (dclk) {
        ...
        DRM_DEV_INFO(vop2->dev, "set %s to %ld, get %ld\n", __clk_get_name(vp->dclk), dclk->rate,
                     clk_get_rate(vp->dclk));
    } else {
        clk_set_rate(vp->dclk, adjusted_mode->crtc_clock * 0x3e8);
    }
...
static int vop2_crtc_atomic_check(struct drm_crtc *crtc, struct drm_crtc_state *crtc_state)
{
    return 0;
}
...
static void vop2_parse_alpha(struct vop2_alpha_config *alpha_config, struct vop2_alpha *alpha)
{
...
static void vop2_setup_alpha(struct vop2_video_port *vp, const struct vop2_zpos *vop2_zpos)
{
    ...
    for_each_set_bit(phys_id, &win_mask, ROCKCHIP_MAX_LAYER)
    {
        ...
        if (vpstate->zpos == 0 && vpstate->global_alpha != 0xff && !vop2_cluster_window(win)) {
            /*
    ...
    for (i = 1; i < vp->nr_layers; i++) {
        ...
        if (i == 1) {
            if (bottom_layer_alpha_en || vp->hdr_en) {
                ...
                VOP_MODULE_SET(vop2, vp, hdr_src_alpha_ctrl, alpha.src_alpha_ctrl.val);
                VOP_MODULE_SET(vop2, vp, hdr_dst_alpha_ctrl, alpha.dst_alpha_ctrl.val);
            } else {
...
static u16 vop2_calc_bg_ovl_and_port_mux(struct vop2_video_port *vp)
{
    ...
    for (i = 0; i < vop2_data->nr_vps - 1; i++) {
        ...
        port_mux_cfg |= port_mux << (prev_vp->id * 0x4);
...
static void vop2_setup_layer_mixer_for_vp(struct vop2_video_port *vp, const struct vop2_zpos *vop2_zpos)
{
    struct vop2_video_port *prev_vp;
    struct vop2 *vop2 = vp->vop2;
    struct vop2_layer *layer = &vop2->layers[0];
    ...
    old_layer_cfg = vop2->regsbak[layer_cfg_reg_offset >> 0x2];
    ...
    if ((new_layer_cfg != old_layer_cfg) && (atv_layer_cfg != old_layer_cfg) && !vp->splice_mode_right) {
...
static void vop2_setup_dly_for_vp(struct vop2_video_port *vp)
{
    ...
    if (vp_data->hdr_table) {
        if (vp->hdr_in) {
            if (vp->hdr_out) {
                bg_dly = vp_data->pre_scan_max_dly[0x2];
            }
        } else {
            if (vp->hdr_out) {
                bg_dly = vp_data->pre_scan_max_dly[0x1];
            } else {
                bg_dly = vp_data->pre_scan_max_dly[0x3];
            }
        }
    }
    ...
    if (vcstate->splice_mode) {
        pre_scan_dly = bg_dly + (hdisplay >> 0x2) - 1;
    } else {
        pre_scan_dly = bg_dly + (hdisplay >> 1) - 1;
    }

    if (vop2->version == VOP_VERSION_RK3588 && hsync_len < 0x8) {
        hsync_len = 0x8;
    }

    pre_scan_dly = (pre_scan_dly << 0x10) | hsync_len;
...
static void vop2_crtc_atomic_begin(struct drm_crtc *crtc, struct drm_crtc_state *old_crtc_state)
{
    ...
    if (vcstate->splice_mode) {
        vop2_zpos_splice = kmalloc_array(vop2->data->win_size, sizeof(*vop2_zpos), GFP_KERNEL);
        if (!vop2_zpos_splice) {
            goto out;
        }
    }
    ...
    drm_atomic_crtc_for_each_plane(plane, crtc)
    {
        ...
        if (vcstate->splice_mode) {
            ...
            splice_nr_layers++;
            DRM_DEV_DEBUG(vop2->dev, "%s active zpos:%d for vp%d from vp%d\n", splice_win->name, vpstate->zpos,
                          splice_vp->id, old_vp->id);
        }
        nr_layers++;
    }
    ...
    if (nr_layers) {
        ...
        if (vcstate->splice_mode) {
            ...
            sort(vop2_zpos_splice, splice_nr_layers, sizeof(vop2_zpos_splice[0]), vop2_zpos_cmp, NULL);
...
static void vop2_bcsh_reg_update(struct rockchip_crtc_state *vcstate, struct vop2_video_port *vp,
                                 struct rockchip_bcsh_state *bcsh_state)
{
    ...
    VOP_MODULE_SET(vop2, vp, bcsh_sat_con, bcsh_state->saturation * bcsh_state->contrast / 0x100);
...
static void vop2_tv_config_update(struct drm_crtc *crtc, struct drm_crtc_state *old_crtc_state)
{
    struct rockchip_crtc_state *vcstate = to_rockchip_crtc_state(crtc->state);
    ...
    vcstate->bcsh_en = 0;
    if (vcstate->tv_state->brightness != 0x32 || vcstate->tv_state->contrast != 0x32 ||
        vcstate->tv_state->saturation != 0x32 || vcstate->tv_state->hue != 0x32) {
        vcstate->bcsh_en = 1;
    }
    /*
    ...
    if (!memcmp(vcstate->tv_state, &vp->active_tv_state, sizeof(*vcstate->tv_state)) &&
        vcstate->yuv_overlay == old_vcstate->yuv_overlay && vcstate->bcsh_en == old_vcstate->bcsh_en &&
    ...
    if (vp_data->feature & VOP_FEATURE_OUTPUT_10BIT) {
        brightness = interpolate(0, -0x80, 0x64, 0x7f, vcstate->tv_state->brightness);
    } else {
        brightness = interpolate(0, -0x20, 0x64, 0x1f, vcstate->tv_state->brightness);
    }
    contrast = interpolate(0, 0, 0x64, 0x1ff, vcstate->tv_state->contrast);
    saturation = interpolate(0, 0, 0x64, 0x1ff, vcstate->tv_state->saturation);
    hue = interpolate(0, -0x1e, 0x64, 0x1e, vcstate->tv_state->hue);
    ...
    if (vcstate->splice_mode) {
        const struct vop2_video_port_data *vp_data_ex = &vop2->data->vp[vp->id];
        struct vop2_video_port *splice_vp = &vop2->vps[vp_data_ex->splice_vp_id];

        vop2_bcsh_reg_update(vcstate, splice_vp, &bcsh_state);
    }
}

static void vop2_cfg_update(struct drm_crtc *crtc, struct drm_crtc_state *old_crtc_state)
{
    struct vop2_video_port *vp = to_vop2_video_port(crtc);
    struct rockchip_crtc_state *vcstate = to_rockchip_crtc_state(crtc->state);
    struct vop2 *vop2 = vp->vop2;
    ...
    if (vcstate->background) {
        r = (vcstate->background & 0xff0000) >> 0x10;
        g = (vcstate->background & 0xff00) >> 0x8;
...
static void vop2_crtc_reset(struct drm_crtc *crtc)
{
    ...
    vcstate->left_margin = 0x64;
    vcstate->right_margin = 0x64;
    vcstate->top_margin = 0x64;
    vcstate->bottom_margin = 0x64;
...
static void vop2_crtc_destroy_state(struct drm_crtc *crtc, struct drm_crtc_state *state)
{
...
static int vop2_crtc_verify_crc_source(struct drm_crtc *crtc, const char *source_name, size_t *values_cnt)
{
    if (source_name && strcmp(source_name, "auto") != 0) {
        return -EINVAL;
    }
...
static int vop2_crtc_atomic_get_property(struct drm_crtc *crtc, const struct drm_crtc_state *state,
                                         struct drm_property *property, uint64_t *val)
{
    ...
    if (property == private->aclk_prop) {
        ...
        *val = clk_get_rate(vop2->aclk) / 0x3e8;
        return 0;
    }
...
static int vop2_crtc_atomic_set_property(struct drm_crtc *crtc, struct drm_crtc_state *state,
                                         struct drm_property *property, uint64_t val)
{
    struct drm_device *drm_dev = crtc->dev;
    struct rockchip_drm_private *private = drm_dev->dev_private;
...
static void vop2_wb_handler(struct vop2_video_port *vp)
{
    ...
    wb_vp_id = (vop2_readl(vop2, RK3568_LUT_PORT_SEL) >> 0x8) & 0x3;
    if (wb_vp_id != vp->id) {
        return;
    }
    /*
...
static int vop2_plane_create_name_property(struct vop2 *vop2, struct vop2_win *win)
{
    ...
    prop = drm_property_create_bitmask(vop2->drm_dev, DRM_MODE_PROP_IMMUTABLE, "NAME", props, vop2->registered_num_wins,
                                       bits);
    if (!prop) {
...
static int vop2_plane_create_feature_property(struct vop2 *vop2, struct vop2_win *win)
{
    ...
    prop = drm_property_create_bitmask(vop2->drm_dev, DRM_MODE_PROP_IMMUTABLE, "FEATURE", props, ARRAY_SIZE(props),
                                       feature);
    if (!prop) {
...
static int vop2_plane_init(struct vop2 *vop2, struct vop2_win *win, unsigned long possible_crtcs)
{
    ...
    ret = drm_universal_plane_init(vop2->drm_dev, &win->base, possible_crtcs, &vop2_plane_funcs, win->formats,
    ...
    if (win->supported_rotations) {
        drm_plane_create_rotation_property(&win->base, DRM_MODE_ROTATE_0, DRM_MODE_ROTATE_0 | win->supported_rotations);
    }
    ...
    win->input_width_prop =
        drm_property_create_range(vop2->drm_dev, DRM_MODE_PROP_IMMUTABLE, "INPUT_WIDTH", 0, max_width);
    win->input_height_prop =
        drm_property_create_range(vop2->drm_dev, DRM_MODE_PROP_IMMUTABLE, "INPUT_HEIGHT", 0, max_height);
    ...
    win->output_width_prop =
        drm_property_create_range(vop2->drm_dev, DRM_MODE_PROP_IMMUTABLE, "OUTPUT_WIDTH", 0, max_width);
    win->output_height_prop =
        drm_property_create_range(vop2->drm_dev, DRM_MODE_PROP_IMMUTABLE, "OUTPUT_HEIGHT", 0, max_height);
    win->scale_prop = drm_property_create_range(vop2->drm_dev, DRM_MODE_PROP_IMMUTABLE, "SCALE_RATE",
                                                win->max_downscale_factor, win->max_upscale_factor);
    /*
     * Support 24 bit(RGB888) or 16 bit(rgb565) color key.
     * Bit 31 is used as a flag to disable (0) or enable
     * color keying (1).
     */
    win->color_key_prop = drm_property_create_range(vop2->drm_dev, 0, "colorkey", 0, 0x80ffffff);

    if (!win->input_width_prop || !win->input_height_prop || !win->output_width_prop || !win->output_height_prop ||
...
static struct drm_plane *vop2_cursor_plane_init(struct vop2_video_port *vp, unsigned long possible_crtcs)
{
    struct vop2 *vop2 = vp->vop2;
...
static int vop2_gamma_init(struct vop2 *vop2)
{
    ...
    for (i = 0; i < vop2_data->nr_vps; i++) {
        ...
        for (j = 0; j < lut_len; j++) {
            rockchip_vop2_crtc_fb_gamma_get(crtc, &r_base[j], &g_base[j], &b_base[j], j);
        }
    }
...
static int vop2_crtc_create_plane_mask_property(struct vop2 *vop2, struct drm_crtc *crtc)
{
    struct drm_property *prop;
    struct vop2_video_port *vp = to_vop2_video_port(crtc);

    static const struct drm_prop_enum_list props[] = {
        {ROCKCHIP_VOP2_CLUSTER0, "Cluster0"}, {ROCKCHIP_VOP2_CLUSTER1, "Cluster1"}, {ROCKCHIP_VOP2_ESMART0, "Esmart0"},
        {ROCKCHIP_VOP2_ESMART1, "Esmart1"},   {ROCKCHIP_VOP2_SMART0, "Smart0"},     {ROCKCHIP_VOP2_SMART1, "Smart1"},
        {ROCKCHIP_VOP2_CLUSTER2, "Cluster2"}, {ROCKCHIP_VOP2_CLUSTER3, "Cluster3"}, {ROCKCHIP_VOP2_ESMART2, "Esmart2"},
        {ROCKCHIP_VOP2_ESMART3, "Esmart3"},
    };

    prop = drm_property_create_bitmask(vop2->drm_dev, DRM_MODE_PROP_IMMUTABLE, "PLANE_MASK", props, ARRAY_SIZE(props),
                                       0xffffffff);
    if (!prop) {
...
static int vop2_create_crtc(struct vop2 *vop2)
{
    ...
    for (i = 0; i < vop2_data->nr_vps; i++) {
        ...
        ret = snprintf(dclk_name, sizeof(dclk_name), "dclk_vp%d", vp->id);
        ...
        ret = drm_crtc_init_with_planes(drm_dev, crtc, plane, cursor, &vop2_crtc_funcs, "video_port%d", vp->id);
        if (ret) {
        ...
        if (vp_data->feature & VOP_FEATURE_OVERSCAN) {
            drm_object_attach_property(&crtc->base, drm_dev->mode_config.tv_left_margin_property, 0x64);
            drm_object_attach_property(&crtc->base, drm_dev->mode_config.tv_right_margin_property, 0x64);
            drm_object_attach_property(&crtc->base, drm_dev->mode_config.tv_top_margin_property, 0x64);
            drm_object_attach_property(&crtc->base, drm_dev->mode_config.tv_bottom_margin_property, 0x64);
        }
    ...
    for (j = 0; j < vop2->registered_num_wins; j++) {
        ...
        if (win->type == DRM_PLANE_TYPE_PRIMARY && !be_used_for_primary_plane) {
...
static int vop2_win_init(struct vop2 *vop2)
{
    ...
    for (i = 0; i < vop2_data->win_size; i++) {
        ...
        for (j = 0; j < win_data->area_size; j++) {
            ...
            area->layer_sel_id = -1;
            ret = snprintf(name, min(sizeof(name), strlen(win->name)), "%s", win->name);
            ret = snprintf(name, sizeof(name), "%s%d", name, area->area_id);
    ...
    plane_name_list = devm_kzalloc(vop2->dev, vop2->registered_num_wins * sizeof(*plane_name_list), GFP_KERNEL);
    if (!plane_name_list) {
    ...
    if (vop_out_node) {
        ...
        for_each_child_of_node(vop_out_node, child)
        {
            ...
            DRM_DEV_INFO(dev, "vp%d assign plane mask: 0x%x, primary plane phy id: %d\n", vp_id,
                         vop2->vps[vp_id].plane_mask, vop2->vps[vp_id].primary_plane_phy_id);
        }
    }
...
static void vop2_unbind(struct device *dev, struct device *master, void *data)
{
    ...
    list_for_each_entry_safe(plane, tmpp, plane_list, head) drm_plane_cleanup(plane);

    list_for_each_entry_safe(crtc, tmpc, crtc_list, head) vop2_destroy_crtc(crtc);
