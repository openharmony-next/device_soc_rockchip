...
#define drm_for_each_lessee(lessee, lessor) list_for_each_entry((lessee), &(lessor)->lessees, lessee_list)
...
static struct drm_master *_drm_find_lessee(struct drm_master *master, int lessee_id)
{
...
static bool _drm_has_leased(struct drm_master *master, int id)
{
    ...
    lockdep_assert_held(&master->dev->mode_config.idr_mutex);
    drm_for_each_lessee(lessee, master) if (_drm_lease_held_master(lessee, id)) return true;
    return false;
}
...
static void _drm_lease_revoke(struct drm_master *top)
{
    ...
    for (;;) {
        ...
        /* Evacuate the lease */
        idr_for_each_entry(&master->leases, entry, object) idr_remove(&master->leases, object);
        ...
        if (!list_empty(&master->lessees)) {
            ...
            /* Up */
            while (master != top &&
                   master == list_last_entry(&master->lessor->lessees, struct drm_master, lessee_list)) {
                master = master->lessor;
            }
...
static int validate_lease(struct drm_device *dev, int object_count, struct drm_mode_object **objects,
...
static int fill_object_idr(struct drm_device *dev, struct drm_file *lessor_priv, struct idr *leases, int object_count,
                           u32 *object_ids)
{
    ...
    objects = kcalloc(object_count, sizeof(struct drm_mode_object *), GFP_KERNEL);
    if (!objects) {
        return -ENOMEM;
    }
    ...
    for (o = 0; o < object_count; o++) {
        objects[o] = drm_mode_object_find(dev, lessor_priv, object_ids[o], DRM_MODE_OBJECT_ANY);
        if (!objects[o]) {
            ret = -ENOENT;
    ...
    for (o = 0; o < object_count; o++) {
        ...
        if (obj->type == DRM_MODE_OBJECT_CRTC && !universal_planes) {
            ...
            ret = idr_alloc(leases, &drm_lease_idr_object, crtc->primary->base.id, crtc->primary->base.id + 1,
                            GFP_KERNEL);
            if (ret < 0) {
            ...
            if (crtc->cursor) {
                ret = idr_alloc(leases, &drm_lease_idr_object, crtc->cursor->base.id, crtc->cursor->base.id + 1,
                                GFP_KERNEL);
                if (ret < 0) {
...
int drm_mode_create_lease_ioctl(struct drm_device *dev, void *data, struct drm_file *lessor_priv)
{
    ...
    object_ids = memdup_user(u64_to_user_ptr(cl->object_ids), array_size(object_count, sizeof(__u32)));
    ...
    ret = fill_object_idr(dev, lessor_priv, &leases, object_count, object_ids);
    kfree(object_ids);
    if (ret) {
