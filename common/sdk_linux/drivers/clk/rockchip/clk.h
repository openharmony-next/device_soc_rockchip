...
#define PLL(_type, _id, _name, _pnames, _flags, _con, _mode, _mshift, _lshift, _pflags, _rtable)                       \
    {                                                                                                                  \
        .id = (_id), .type = (_type), .name = (_name), .parent_names = (_pnames), .num_parents = ARRAY_SIZE(_pnames),  \
        .flags = CLK_GET_RATE_NOCACHE | (_flags), .con_offset = (_con), .mode_offset = (_mode),                        \
        .mode_shift = (_mshift), .lock_shift = (_lshift), .pll_flags = (_pflags), .rate_table = (_rtable),             \
    }
...
struct clk *rockchip_clk_register_pll(struct rockchip_clk_provider *ctx, enum rockchip_pll_type pll_type,
                                      ...
                                      struct rockchip_pll_rate_table *rate_table, unsigned long flags,
                                      u8 clk_pll_flags);
...
struct clk *rockchip_clk_register_cpuclk(const char *name, u8 num_parents, struct clk *parent, struct clk *alt_parent,
                                         const struct rockchip_cpuclk_reg_data *reg_data,
                                         const struct rockchip_cpuclk_rate_table *rates, int nrates,
                                         void __iomem *reg_base, spinlock_t *lock);

struct clk *rockchip_clk_register_mmc(const char *name, const char *const *parent_names, u8 num_parents,
...
struct clk *rockchip_clk_register_ddrclk(const char *name, int flags, const char *const *parent_names, u8 num_parents,
                                         int mux_offset, int mux_shift, int mux_width, int div_shift, int div_width,
                                         int ddr_flags, void __iomem *reg_base);

#define ROCKCHIP_INVERTER_HIWORD_MASK BIT(0)

struct clk *rockchip_clk_register_inverter(const char *name, const char *const *parent_names, u8 num_parents,
                                           void __iomem *reg, int shift, int flags, spinlock_t *lock);

struct clk *rockchip_clk_register_muxgrf(const char *name, const char *const *parent_names, u8 num_parents, int flags,
                                         struct regmap *grf, int reg, int shift, int width, int mux_flags);
...
#define COMPOSITE(_id, cname, pnames, f, mo, ms, mw, mf, ds, dw, df, go, gs, gf)                                       \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_composite, .name = (cname), .parent_names = (pnames),                       \
        .num_parents = ARRAY_SIZE(pnames), .flags = (f), .muxdiv_offset = (mo), .mux_shift = (ms), .mux_width = (mw),  \
        .mux_flags = (mf), .div_shift = (ds), .div_width = (dw), .div_flags = (df), .gate_offset = (go),               \
        .gate_shift = (gs), .gate_flags = (gf),                                                                        \
    }

#define COMPOSITE_BROTHER(_id, cname, pnames, f, mo, ms, mw, mf, ds, dw, df, go, gs, gf, bro)                          \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_composite_brother, .name = (cname), .parent_names = (pnames),               \
        .num_parents = ARRAY_SIZE(pnames), .flags = (f), .muxdiv_offset = (mo), .mux_shift = (ms), .mux_width = (mw),  \
        .mux_flags = (mf), .div_shift = (ds), .div_width = (dw), .div_flags = (df), .gate_offset = (go),               \
        .gate_shift = (gs), .gate_flags = (gf), .child = (bro),                                                        \
    }
...
#define COMPOSITE_MUXTBL(_id, cname, pnames, f, mo, ms, mw, mf, mt, ds, dw, df, go, gs, gf)                            \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_composite, .name = (cname), .parent_names = (pnames),                       \
        .num_parents = ARRAY_SIZE(pnames), .flags = (f), .muxdiv_offset = (mo), .mux_shift = (ms), .mux_width = (mw),  \
        .mux_flags = (mf), .mux_table = (mt), .div_shift = (ds), .div_width = (dw), .div_flags = (df),                 \
        .gate_offset = (go), .gate_shift = (gs), .gate_flags = (gf),                                                   \
    }
...
#define COMPOSITE_DIV_OFFSET(_id, cname, pnames, f, mo, ms, mw, mf, do, ds, dw, df, go, gs, gf)                        \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_composite, .name = (cname), .parent_names = (pnames),                       \
        .num_parents = ARRAY_SIZE(pnames), .flags = (f), .muxdiv_offset = (mo), .mux_shift = (ms), .mux_width = (mw),  \
        .mux_flags = (mf), .div_offset = do, .div_shift = (ds), .div_width = (dw), .div_flags = (df),                  \
        .gate_offset = (go), .gate_shift = (gs), .gate_flags = (gf),                                                   \
    }

#define COMPOSITE_NOMUX(_id, cname, pname, f, mo, ds, dw, df, go, gs, gf)                                              \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_composite, .name = (cname), .parent_names = (const char *[]){pname},        \
        .num_parents = 1, .flags = (f), .muxdiv_offset = (mo), .div_shift = (ds), .div_width = (dw),                   \
        .div_flags = (df), .gate_offset = (go), .gate_shift = (gs), .gate_flags = (gf),                                \
    }
...
#define COMPOSITE_NOMUX_DIVTBL(_id, cname, pname, f, mo, ds, dw, df, dt, go, gs, gf)                                   \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_composite, .name = (cname), .parent_names = (const char *[]){pname},        \
        .num_parents = 1, .flags = (f), .muxdiv_offset = (mo), .div_shift = (ds), .div_width = (dw),                   \
        .div_flags = (df), .div_table = (dt), .gate_offset = (go), .gate_shift = (gs), .gate_flags = (gf),             \
    }
...
#define COMPOSITE_NODIV(_id, cname, pnames, f, mo, ms, mw, mf, go, gs, gf)                                             \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_composite, .name = (cname), .parent_names = (pnames),                       \
        .num_parents = ARRAY_SIZE(pnames), .flags = (f), .muxdiv_offset = (mo), .mux_shift = (ms), .mux_width = (mw),  \
        .mux_flags = (mf), .gate_offset = (go), .gate_shift = (gs), .gate_flags = (gf),                                \
    }
...
#define COMPOSITE_NOGATE(_id, cname, pnames, f, mo, ms, mw, mf, ds, dw, df)                                            \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_composite, .name = (cname), .parent_names = (pnames),                       \
        .num_parents = ARRAY_SIZE(pnames), .flags = (f), .muxdiv_offset = (mo), .mux_shift = (ms), .mux_width = (mw),  \
        .mux_flags = (mf), .div_shift = (ds), .div_width = (dw), .div_flags = (df), .gate_offset = -1,                 \
    }

#define COMPOSITE_BROTHER_NOGATE(_id, cname, pnames, f, mo, ms, mw, mf, ds, dw, df, bro)                               \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_composite_brother, .name = (cname), .parent_names = (pnames),               \
        .num_parents = ARRAY_SIZE(pnames), .flags = (f), .muxdiv_offset = (mo), .mux_shift = (ms), .mux_width = (mw),  \
        .mux_flags = (mf), .div_shift = (ds), .div_width = (dw), .div_flags = (df), .gate_offset = -1, .child = (bro), \
    }
...
#define COMPOSITE_NOGATE_DIVTBL(_id, cname, pnames, f, mo, ms, mw, mf, ds, dw, df, dt)                                 \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_composite, .name = (cname), .parent_names = (pnames),                       \
        .num_parents = ARRAY_SIZE(pnames), .flags = (f), .muxdiv_offset = (mo), .mux_shift = (ms), .mux_width = (mw),  \
        .mux_flags = (mf), .div_shift = (ds), .div_width = (dw), .div_flags = (df), .div_table = (dt),                 \
        .gate_offset = -1,                                                                                             \
    }

#define COMPOSITE_FRAC(_id, cname, pname, f, mo, df, go, gs, gf, prate)                                                \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_fraction_divider, .name = (cname), .parent_names = (const char *[]){pname}, \
        .num_parents = 1, .flags = (f), .muxdiv_offset = (mo), .div_shift = 16, .div_width = 16, .div_flags = (df),    \
        .gate_offset = (go), .gate_shift = (gs), .gate_flags = (gf), .max_prate = (prate),                             \
    }
...
#define COMPOSITE_FRACMUX(_id, cname, pname, f, mo, df, go, gs, gf, ch, prate)                                         \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_fraction_divider, .name = (cname), .parent_names = (const char *[]){pname}, \
        .num_parents = 1, .flags = (f), .muxdiv_offset = (mo), .div_shift = 16, .div_width = 16, .div_flags = (df),    \
        .gate_offset = (go), .gate_shift = (gs), .gate_flags = (gf), .child = (ch), .max_prate = (prate),              \
    }
...
#define COMPOSITE_FRACMUX_NOGATE(_id, cname, pname, f, mo, df, ch, prate)                                              \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_fraction_divider, .name = (cname), .parent_names = (const char *[]){pname}, \
        .num_parents = 1, .flags = (f), .muxdiv_offset = (mo), .div_shift = 16, .div_width = 16, .div_flags = (df),    \
        .gate_offset = -1, .child = (ch), .max_prate = (prate),                                                        \
    }

#define COMPOSITE_DDRCLK(_id, cname, pnames, f, mo, ms, mw, ds, dw, df)                                                \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_ddrclk, .name = (cname), .parent_names = (pnames),                          \
        .num_parents = ARRAY_SIZE(pnames), .flags = (f), .muxdiv_offset = (mo), .mux_shift = (ms), .mux_width = (mw),  \
        .div_shift = (ds), .div_width = (dw), .div_flags = (df), .gate_offset = -1,                                    \
    }

#define MUX(_id, cname, pnames, f, o, s, w, mf)                                                                        \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_mux, .name = (cname), .parent_names = (pnames),                             \
        .num_parents = ARRAY_SIZE(pnames), .flags = (f), .muxdiv_offset = (o), .mux_shift = (s), .mux_width = (w),     \
        .mux_flags = (mf), .gate_offset = -1,                                                                          \
    }
...
#define MUXTBL(_id, cname, pnames, f, o, s, w, mf, mt)                                                                 \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_mux, .name = (cname), .parent_names = (pnames),                             \
        .num_parents = ARRAY_SIZE(pnames), .flags = (f), .muxdiv_offset = (o), .mux_shift = (s), .mux_width = (w),     \
        .mux_flags = (mf), .gate_offset = -1, .mux_table = (mt),                                                       \
    }

#define MUXGRF(_id, cname, pnames, f, o, s, w, mf)                                                                     \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_muxgrf, .name = (cname), .parent_names = (pnames),                          \
        .num_parents = ARRAY_SIZE(pnames), .flags = (f), .muxdiv_offset = (o), .mux_shift = (s), .mux_width = (w),     \
        .mux_flags = (mf), .gate_offset = -1,                                                                          \
    }
...
#define MUXPMUGRF(_id, cname, pnames, f, o, s, w, mf)                                                                  \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_muxpmugrf, .name = (cname), .parent_names = (pnames),                       \
        .num_parents = ARRAY_SIZE(pnames), .flags = (f), .muxdiv_offset = (o), .mux_shift = (s), .mux_width = (w),     \
        .mux_flags = (mf), .gate_offset = -1,                                                                          \
    }
...
#define DIV(_id, cname, pname, f, o, s, w, df)                                                                         \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_divider, .name = (cname), .parent_names = (const char *[]){pname},          \
        .num_parents = 1, .flags = (f), .muxdiv_offset = (o), .div_shift = (s), .div_width = (w), .div_flags = (df),   \
        .gate_offset = -1,                                                                                             \
    }
...
#define GATE(_id, cname, pname, f, o, b, gf)                                                                           \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_gate, .name = (cname), .parent_names = (const char *[]){pname},             \
        .num_parents = 1, .flags = (f), .gate_offset = (o), .gate_shift = (b), .gate_flags = (gf),                     \
    }

#define MMC(_id, cname, pname, offset, shift)                                                                          \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_mmc, .name = (cname), .parent_names = (const char *[]){pname},              \
        .num_parents = 1, .muxdiv_offset = (offset), .div_shift = (shift),                                             \
    }

#define INVERTER(_id, cname, pname, io, is, if)                                                                        \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_inverter, .name = (cname), .parent_names = (const char *[]){pname},         \
        .num_parents = 1, .muxdiv_offset = (io), .div_shift = (is), .div_flags = (if),                                 \
    }

#define FACTOR(_id, cname, pname, f, fm, fd)                                                                           \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_factor, .name = (cname), .parent_names = (const char *[]){pname},           \
        .num_parents = 1, .flags = (f), .div_shift = (fm), .div_width = (fd),                                          \
    }

#define FACTOR_GATE(_id, cname, pname, f, fm, fd, go, gb, gf)                                                          \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_factor, .name = (cname), .parent_names = (const char *[]){pname},           \
        .num_parents = 1, .flags = (f), .div_shift = (fm), .div_width = (fd), .gate_offset = (go), .gate_shift = (gb), \
        .gate_flags = (gf),                                                                                            \
    }

#define COMPOSITE_HALFDIV(_id, cname, pnames, f, mo, ms, mw, mf, ds, dw, df, go, gs, gf)                               \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_half_divider, .name = (cname), .parent_names = (pnames),                    \
        .num_parents = ARRAY_SIZE(pnames), .flags = (f), .muxdiv_offset = (mo), .mux_shift = (ms), .mux_width = (mw),  \
        .mux_flags = (mf), .div_shift = (ds), .div_width = (dw), .div_flags = (df), .gate_offset = (go),               \
        .gate_shift = (gs), .gate_flags = (gf),                                                                        \
    }
...
#define COMPOSITE_NOMUX_HALFDIV(_id, cname, pname, f, mo, ds, dw, df, go, gs, gf)                                      \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_half_divider, .name = (cname), .parent_names = (const char *[]){pname},     \
        .num_parents = 1, .flags = (f), .muxdiv_offset = (mo), .div_shift = (ds), .div_width = (dw),                   \
        .div_flags = (df), .gate_offset = (go), .gate_shift = (gs), .gate_flags = (gf),                                \
    }
...
#define COMPOSITE_DCLK(_id, cname, pnames, f, mo, ms, mw, mf, ds, dw, df, go, gs, gf, prate)                           \
    {                                                                                                                  \
        .id = (_id), .branch_type = branch_dclk_divider, .name = (cname), .parent_names = (pnames),                    \
        .num_parents = ARRAY_SIZE(pnames), .flags = (f), .muxdiv_offset = (mo), .mux_shift = (ms), .mux_width = (mw),  \
        .mux_flags = (mf), .div_shift = (ds), .div_width = (dw), .div_flags = (df), .gate_offset = (go),               \
        .gate_shift = (gs), .gate_flags = (gf), .max_prate = (prate),                                                  \
    }
...
struct rockchip_clk_provider *rockchip_clk_init(struct device_node *np, void __iomem *base, unsigned long nr_clks);
...
void rockchip_register_restart_notifier(struct rockchip_clk_provider *ctx, unsigned int reg, void (*cb)(void));
...
struct clk *rockchip_clk_register_dclk_branch(const char *name, const char *const *parent_names, u8 num_parents,
                                              void __iomem *base, int muxdiv_offset, u8 mux_shift, u8 mux_width,
                                              u8 mux_flags, int div_offset, u8 div_shift, u8 div_width, u8 div_flags,
                                              struct clk_div_table *div_table, int gate_offset, u8 gate_shift,
                                              u8 gate_flags, unsigned long flags, unsigned long max_prate,
                                              spinlock_t *lock);

#ifdef CONFIG_RESET_CONTROLLER
void rockchip_register_softrst(struct device_node *np, unsigned int num_regs, void __iomem *base, u8 flags);
#else
static inline void rockchip_register_softrst(struct device_node *np, unsigned int num_regs, void __iomem *base,
                                             u8 flags)
{
}
#endif
