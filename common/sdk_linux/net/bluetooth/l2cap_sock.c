...
static struct bt_sock_list l2cap_sk_list = {.lock = __RW_LOCK_UNLOCKED(l2cap_sk_list.lock)};

static const struct proto_ops l2cap_sock_ops;
static void l2cap_sock_init(struct sock *sk, struct sock *parent);
static struct sock *l2cap_sock_alloc(struct net *net, struct socket *sock, int proto, gfp_t prio, int kern);
...
static int l2cap_sock_bind(struct socket *sock, struct sockaddr *addr, int alen)
{
    ...
    if (!addr || alen < offsetofend(struct sockaddr, sa_family) || addr->sa_family != AF_BLUETOOTH) {
        return -EINVAL;
    }
    ...
    if (bdaddr_type_is_le(la.l2_bdaddr_type)) {
        ...
        if (la.l2_cid && la.l2_cid != cpu_to_le16(L2CAP_CID_ATT)) {
            return -EINVAL;
        }
    }
    ...
    switch (chan->chan_type) {
        ...
        case L2CAP_CHAN_CONN_ORIENTED:
            if (__le16_to_cpu(la.l2_psm) == L2CAP_PSM_SDP || __le16_to_cpu(la.l2_psm) == L2CAP_PSM_RFCOMM) {
                chan->sec_level = BT_SECURITY_SDP;
            }
            break;
        case L2CAP_CHAN_RAW:
...
static int l2cap_sock_connect(struct socket *sock, struct sockaddr *addr, int alen, int flags)
{
     ...
     */
    if (chan->src_type == BDADDR_BREDR && bacmp(&chan->src, BDADDR_ANY) && bdaddr_type_is_le(la.l2_bdaddr_type)) {
         ...
         */
        if (chan->scid != L2CAP_CID_ATT || la.l2_cid != cpu_to_le16(L2CAP_CID_ATT)) {
            return -EINVAL;
        }
...
static int l2cap_sock_getname(struct socket *sock, struct sockaddr *addr, int peer)
{
...
static int l2cap_sock_getsockopt_old(struct socket *sock, int optname, char __user *optval, int __user *optlen)
{
    ...
    switch (optname) {
        case L2CAP_OPTIONS:
             ...
             */
            if (bdaddr_type_is_le(chan->src_type) && chan->scid != L2CAP_CID_ATT) {
                err = -EINVAL;
                break;
            }
        ...
        case L2CAP_CONNINFO:
            if (sk->sk_state != BT_CONNECTED &&
                !(sk->sk_state == BT_CONNECT2 && test_bit(BT_SK_DEFER_SETUP, &bt_sk(sk)->flags))) {
                err = -ENOTCONN;
                break;
            }
...
static int l2cap_sock_getsockopt(struct socket *sock, int level, int optname, char __user *optval, int __user *optlen)
{
    ...
    switch (optname) {
        case BT_SECURITY:
            if (chan->chan_type != L2CAP_CHAN_CONN_ORIENTED && chan->chan_type != L2CAP_CHAN_FIXED &&
        ...
        case BT_FLUSHABLE:
            if (put_user(test_bit(FLAG_FLUSHABLE, &chan->flags), (u32 __user *)optval)) {
                err = -EFAULT;
            }
        ...
        case BT_POWER:
            if (sk->sk_type != SOCK_SEQPACKET && sk->sk_type != SOCK_STREAM && sk->sk_type != SOCK_RAW) {
                err = -EINVAL;
                break;
            }
...
static int l2cap_sock_setsockopt_old(struct socket *sock, int optname, sockptr_t optval, unsigned int optlen)
{
    struct sock *sk = sock->sk;
    struct l2cap_chan *chan = l2cap_pi(sk)->chan;
    struct l2cap_options opts;
...
static int l2cap_sock_setsockopt(struct socket *sock, int level, int optname, sockptr_t optval, unsigned int optlen)
{
    struct sock *sk = sock->sk;
    struct l2cap_chan *chan = l2cap_pi(sk)->chan;
    struct bt_security sec;
    ...
    switch (optname) {
        case BT_SECURITY:
            ...
            if (chan->scid == L2CAP_CID_ATT) {
                ...
                /* or for ACL link */
            } else if ((sk->sk_state == BT_CONNECT2 && test_bit(BT_SK_DEFER_SETUP, &bt_sk(sk)->flags)) ||
        ...
        case BT_POWER:
            if (chan->chan_type != L2CAP_CHAN_CONN_ORIENTED && chan->chan_type != L2CAP_CHAN_RAW) {
                err = -EINVAL;
                break;
            }
        ...
        case BT_CHANNEL_POLICY:
            ...
            if (sk->sk_state == BT_CONNECTED && chan->move_role == L2CAP_MOVE_ROLE_NONE) {
        ...
        case BT_RCVMTU:
            ...
            if (chan->mode == L2CAP_MODE_LE_FLOWCTL && sk->sk_state == BT_CONNECTED) {
                err = -EISCONN;
                break;
            }
...
static int l2cap_sock_recvmsg(struct socket *sock, struct msghdr *msg, size_t len, int flags)
{
    struct sock *sk = sock->sk;
...
static int __l2cap_wait_ack(struct sock *sk, struct l2cap_chan *chan)
{
    ...
    do {
        BT_DBG("Waiting for %d ACKs, timeout %04d ms", chan->unacked_frames,
               time_after(jiffies, timeout) ? 0 : jiffies_to_msecs(timeout - jiffies));
...
static int l2cap_sock_shutdown(struct socket *sock, int how)
{
    ...
    if (chan->mode == L2CAP_MODE_ERTM && chan->unacked_frames > 0 && chan->state == BT_CONNECTED) {
...
static void l2cap_sock_cleanup_listen(struct sock *parent)
{
    ...
    BT_DBG("parent %p state %s", parent, state_to_string(parent->sk_state));
...
static struct sk_buff *l2cap_sock_alloc_skb_cb(struct l2cap_chan *chan, unsigned long hdr_len, unsigned long len,
                                               int nb)
{
    struct sock *sk = chan->data;
    struct sk_buff *skb;
...
static void l2cap_skb_msg_name(struct sk_buff *skb, void *msg_name, int *msg_namelen)
{
...
static struct proto l2cap_proto = {.name = "L2CAP", .owner = THIS_MODULE, .obj_size = sizeof(struct l2cap_pinfo)};
...
static int l2cap_sock_create(struct net *net, struct socket *sock, int protocol, int kern)
{
    ...
    if (sock->type != SOCK_SEQPACKET && sock->type != SOCK_STREAM && sock->type != SOCK_DGRAM &&
        sock->type != SOCK_RAW) {
...
static const struct proto_ops l2cap_sock_ops = {.family = PF_BLUETOOTH,
                                                .owner = THIS_MODULE,
