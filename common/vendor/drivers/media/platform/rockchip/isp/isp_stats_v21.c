                ...
                struct rkisp_isp21_stat_buffer *pbuf)
{
    ...
    for (i = 0; i < ISP2X_RAWAWB_RAMDATA_NUM; i++) {
        ...
        pbuf->params.rawawb.ramdata[i].g = ((lsb & 0xFFFC0000) >> CIF_ISP_EXP_OFFSET_EIT) | (msb & 0xF) << CIF_ISP_EXP_OFFSET_FOR;
                  ...
                  struct isp2x_rawaebig_stat *ae, u32 blk_no)
{
    ...
    for (i = 0; i < ISP2X_RAWAEBIG_SUBWIN_NUM; i++)
        ae->sumr[i] = rkisp_read(stats_vdev->dev, addr + RAWAE_BIG_WND1_SUMR + i * CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT, true);

    for (i = 0; i < ISP2X_RAWAEBIG_SUBWIN_NUM; i++)
        ae->sumg[i] = rkisp_read(stats_vdev->dev, addr + RAWAE_BIG_WND1_SUMG + i * CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT, true);

    for (i = 0; i < ISP2X_RAWAEBIG_SUBWIN_NUM; i++)
        ae->sumb[i] = rkisp_read(stats_vdev->dev, addr + RAWAE_BIG_WND1_SUMB + i * CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT, true);
                ...
                struct rkisp_isp21_stat_buffer *pbuf)
{
    ...
    for (i = 0; i < ISP2X_RAWAWB_SUM_NUM; i++) {
        rawawb->ro_rawawb_sum_rgain_nor[i] =
            reg_addr[(0x20 * i + 0x0) / CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT];
        rawawb->ro_rawawb_sum_bgain_nor[i] =
            reg_addr[(0x20 * i + 0x4) / CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT];
        rawawb->ro_rawawb_wp_num_nor[i] =
            reg_addr[(0x20 * i + 0x8) / CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT];
        rawawb->ro_rawawb_sum_rgain_big[i] =
            reg_addr[(0x20 * i + 0x10) / CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT];
        rawawb->ro_rawawb_sum_bgain_big[i] =
            reg_addr[(0x20 * i + 0x14) / CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT];
        rawawb->ro_rawawb_wp_num_big[i] =
            reg_addr[(0x20 * i + 0x18) / CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT];
    }
    ...
    for (i = 0; i < ISP21_RAWAWB_HSTBIN_NUM / CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT; i++) {
        ...
        rawawb->ro_yhist_bin[CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT * i + 1] = (value & 0xFFFF0000) >> CIF_ISP_EXP_OFFSET_THI;
    }
