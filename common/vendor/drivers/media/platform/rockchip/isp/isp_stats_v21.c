...
static int rkisp_stats_get_rawawb_meas_reg(struct rkisp_isp_stats_vdev *stats_vdev,
                                           struct rkisp_isp21_stat_buffer *pbuf)
{
    ...
    for (i = 0; i < ISP2X_RAWAWB_RAMDATA_NUM; i++) {
        ...
        pbuf->params.rawawb.ramdata[i].g =
            ((lsb & 0xFFFC0000) >> CIF_ISP_EXP_OFFSET_EIT) | (msb & 0xF) << CIF_ISP_EXP_OFFSET_FOR;
        pbuf->params.rawawb.ramdata[i].r = (msb & 0x3FFFF0) >> CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT;
        pbuf->params.rawawb.ramdata[i].wp = (msb & 0xFFC00000) >> CIF_ISP_EXP_OFFSET_TT;
    }

    return 0;
}

static int rkisp_stats_get_rawaf_meas_reg(struct rkisp_isp_stats_vdev *stats_vdev, struct rkisp_isp21_stat_buffer *pbuf)
{
    ...
    for (i = 0; i < ISP2X_RAWAF_LINE_NUM; i++) {
        if (value & (ISP2X_RAWAF_INT_LINE0_EN << i)) {
            line_num[i] = (value >> (CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT * i)) & 0xF;
...
static int rkisp_stats_get_rawaebig_meas_reg(struct rkisp_isp_stats_vdev *stats_vdev, struct isp2x_rawaebig_stat *ae,
                                             u32 blk_no)
{
    u32 addr, value;
    int i;
    ...
    switch (blk_no) {
        ...
        case 1:
            ...
            break;
        case CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT:
    ...
    for (i = 0; i < ISP2X_RAWAEBIG_SUBWIN_NUM; i++) {
        ae->sumr[i] =
            rkisp_read(stats_vdev->dev, addr + RAWAE_BIG_WND1_SUMR + i * CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT, true);
    }

    for (i = 0; i < ISP2X_RAWAEBIG_SUBWIN_NUM; i++) {
        ae->sumg[i] =
            rkisp_read(stats_vdev->dev, addr + RAWAE_BIG_WND1_SUMG + i * CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT, true);
    }

    for (i = 0; i < ISP2X_RAWAEBIG_SUBWIN_NUM; i++) {
        ae->sumb[i] =
            rkisp_read(stats_vdev->dev, addr + RAWAE_BIG_WND1_SUMB + i * CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT, true);
    }
...
static int rkisp_stats_get_rawhstbig_meas_reg(struct rkisp_isp_stats_vdev *stats_vdev,
                                              struct isp2x_rawhistbig_stat *hst, u32 blk_no)
{
    ...
    for (i = 0; i < ISP2X_HIST_BIN_N_MAX; i++) {
        hst->hist_bin[i] = rkisp_read(stats_vdev->dev, addr + ISP_RAWHIST_BIG_RO_BASE_BIN, true);
    }

    return 0;
}

static int rkisp_stats_get_rawae1_meas_reg(struct rkisp_isp_stats_vdev *stats_vdev,
                                           struct rkisp_isp21_stat_buffer *pbuf)
{
    int ret = 0;
...
static int rkisp_stats_get_rawhst1_meas_reg(struct rkisp_isp_stats_vdev *stats_vdev,
                                            struct rkisp_isp21_stat_buffer *pbuf)
{
    int ret = 0;
...
static int rkisp_stats_get_rawae2_meas_reg(struct rkisp_isp_stats_vdev *stats_vdev,
                                           struct rkisp_isp21_stat_buffer *pbuf)
{
    int ret = 0;

    if (!pbuf) {
        rkisp_stats_get_rawaebig_meas_reg(stats_vdev, NULL, CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT);
    } else {
        ret = rkisp_stats_get_rawaebig_meas_reg(stats_vdev, &pbuf->params.rawae2, CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT);
        if (!ret) {
...
static int rkisp_stats_get_rawhst2_meas_reg(struct rkisp_isp_stats_vdev *stats_vdev,
                                            struct rkisp_isp21_stat_buffer *pbuf)
{
    int ret = 0;
...
static int rkisp_stats_get_rawae3_meas_reg(struct rkisp_isp_stats_vdev *stats_vdev,
                                           struct rkisp_isp21_stat_buffer *pbuf)
{
    int ret = 0;
...
static int rkisp_stats_get_rawhst3_meas_reg(struct rkisp_isp_stats_vdev *stats_vdev,
                                            struct rkisp_isp21_stat_buffer *pbuf)
{
    int ret = 0;
...
static int rkisp_stats_get_rawaelite_meas_reg(struct rkisp_isp_stats_vdev *stats_vdev,
                                              struct rkisp_isp21_stat_buffer *pbuf)
{
    ...
    for (i = 0; i < ISP2X_RAWAELITE_MEAN_NUM; i++) {
        value = rkisp_read(stats_vdev->dev, ISP_RAWAE_LITE_RO_MEAN + CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT * i, true);
        ae->data[i].channelg_xy = ISP2X_RAWAEBIG_GET_MEAN_G(value);
        ae->data[i].channelb_xy = ISP2X_RAWAEBIG_GET_MEAN_B(value);
        ae->data[i].channelr_xy = ISP2X_RAWAEBIG_GET_MEAN_R(value);
    }
...
static int rkisp_stats_get_rawhstlite_meas_reg(struct rkisp_isp_stats_vdev *stats_vdev,
                                               struct rkisp_isp21_stat_buffer *pbuf)
{
...
static int rkisp_stats_get_bls_stats(struct rkisp_isp_stats_vdev *stats_vdev, struct rkisp_isp21_stat_buffer *pbuf)
{
...
static int rkisp_stats_get_dhaz_stats(struct rkisp_isp_stats_vdev *stats_vdev, struct rkisp_isp21_stat_buffer *pbuf)
{
...
static int rkisp_stats_get_rawawb_meas_ddr(struct rkisp_isp_stats_vdev *stats_vdev,
                                           struct rkisp_isp21_stat_buffer *pbuf)
{
    ...
    for (i = 0; i < ISP2X_RAWAWB_SUM_NUM; i++) {
        rawawb->ro_rawawb_sum_rgain_nor[i] = reg_addr[(0x20 * i + 0x0) / CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT];
        rawawb->ro_rawawb_sum_bgain_nor[i] = reg_addr[(0x20 * i + 0x4) / CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT];
        rawawb->ro_rawawb_wp_num_nor[i] = reg_addr[(0x20 * i + 0x8) / CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT];
        rawawb->ro_rawawb_sum_rgain_big[i] = reg_addr[(0x20 * i + 0x10) / CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT];
        rawawb->ro_rawawb_sum_bgain_big[i] = reg_addr[(0x20 * i + 0x14) / CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT];
        rawawb->ro_rawawb_wp_num_big[i] = reg_addr[(0x20 * i + 0x18) / CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT];
    }

    for (i = 0; i < ISP21_RAWAWB_HSTBIN_NUM / CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT; i++) {
        value = reg_addr[(0x04 * i + 0xE0) / CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT];
        rawawb->ro_yhist_bin[CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT * i] = value & 0xFFFF;
        rawawb->ro_yhist_bin[CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT * i + 1] =
            (value & 0xFFFF0000) >> CIF_ISP_EXP_OFFSET_THI;
    }

    for (i = 0; i < ISP2X_RAWAWB_RAMDATA_NUM; i++) {
        lsb = raw_addr[CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT * i];
        msb = raw_addr[CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT * i + 1];
        rawawb->ramdata[i].b = lsb & 0x3FFFF;
        rawawb->ramdata[i].g = ((lsb & 0xFFFC0000) >> CIF_ISP_EXP_OFFSET_EIT) | (msb & 0xF) << CIF_ISP_EXP_OFFSET_FOR;
        rawawb->ramdata[i].r = (msb & 0x3FFFF0) >> CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT;
        rawawb->ramdata[i].wp = (msb & 0xFFC00000) >> CIF_ISP_EXP_OFFSET_TT;
    }
    return 0;
}

static int rkisp_stats_get_rawaf_meas_ddr(struct rkisp_isp_stats_vdev *stats_vdev, struct rkisp_isp21_stat_buffer *pbuf)
{
    u32 *ddr_addr;
    struct isp2x_rawaf_stat *af;
    u32 i, rd_buf_idx;
...
static int rkisp_stats_get_rawaebig_meas_ddr(struct rkisp_isp_stats_vdev *stats_vdev, struct isp2x_rawaebig_stat *ae,
                                             u32 blk_no)
{
    u32 *ddr_addr;
    ...
    switch (blk_no) {
        case 1:
            addr = RAWAE_BIG2_BASE;
            ddr_addr += 0x0390 >> CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT;
            break;
        case CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT:
            addr = RAWAE_BIG3_BASE;
            ddr_addr += 0x0720 >> CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT;
            break;
        default:
            addr = RAWAE_BIG1_BASE;
            break;
...
static int rkisp_stats_get_rawhstbig_meas_ddr(struct rkisp_isp_stats_vdev *stats_vdev,
                                              struct isp2x_rawhistbig_stat *hst, u32 blk_no)
{
    u32 *ddr_addr;
    u32 i, rd_buf_idx;
    ...
    switch (blk_no) {
        case 1:
            ddr_addr += 0x0800 >> CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT;
            break;
        case CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT:
            ddr_addr += 0x0C00 >> CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT;
            break;
        default:
            break;
    }
...
static int rkisp_stats_get_rawae1_meas_ddr(struct rkisp_isp_stats_vdev *stats_vdev,
                                           struct rkisp_isp21_stat_buffer *pbuf)
{
    int ret = 0;
...
static int rkisp_stats_get_rawhst1_meas_ddr(struct rkisp_isp_stats_vdev *stats_vdev,
                                            struct rkisp_isp21_stat_buffer *pbuf)
{
    int ret = 0;
...
static int rkisp_stats_get_rawae2_meas_ddr(struct rkisp_isp_stats_vdev *stats_vdev,
                                           struct rkisp_isp21_stat_buffer *pbuf)
{
    int ret = 0;

    if (!pbuf) {
        rkisp_stats_get_rawaebig_meas_ddr(stats_vdev, NULL, CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT);
    } else {
        ret = rkisp_stats_get_rawaebig_meas_ddr(stats_vdev, &pbuf->params.rawae2, CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT);
        if (!ret) {
            pbuf->meas_type |= ISP2X_STAT_RAWAE2;
        }
    }
...
static int rkisp_stats_get_rawhst2_meas_ddr(struct rkisp_isp_stats_vdev *stats_vdev,
                                            struct rkisp_isp21_stat_buffer *pbuf)
{
    int ret = 0;

    if (!pbuf) {
        rkisp_stats_get_rawhstbig_meas_ddr(stats_vdev, NULL, CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT);
    } else {
        ret = rkisp_stats_get_rawhstbig_meas_ddr(stats_vdev, &pbuf->params.rawhist2, CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT);
        if (!ret) {
            pbuf->meas_type |= ISP2X_STAT_RAWHST2;
        }
    }
...
static int rkisp_stats_get_rawae3_meas_ddr(struct rkisp_isp_stats_vdev *stats_vdev,
                                           struct rkisp_isp21_stat_buffer *pbuf)
{
    int ret = 0;
...
static int rkisp_stats_get_rawhst3_meas_ddr(struct rkisp_isp_stats_vdev *stats_vdev,
                                            struct rkisp_isp21_stat_buffer *pbuf)
{
    int ret = 0;
...
static int rkisp_stats_get_rawaelite_meas_ddr(struct rkisp_isp_stats_vdev *stats_vdev,
                                              struct rkisp_isp21_stat_buffer *pbuf)
{
    struct isp2x_rawaelite_stat *ae;
    u32 *ddr_addr;
...
static int rkisp_stats_get_rawhstlite_meas_ddr(struct rkisp_isp_stats_vdev *stats_vdev,
                                               struct rkisp_isp21_stat_buffer *pbuf)
{
    struct isp2x_rawhistlite_stat *hst;
...
static int rkisp_stats_restart_rawawb_meas(struct rkisp_isp_stats_vdev *stats_vdev)
{
    u32 value;
...
static int rkisp_stats_restart_rawaf_meas(struct rkisp_isp_stats_vdev *stats_vdev)
{
    u32 value;
...
static int rkisp_stats_restart_rawaebig_meas(struct rkisp_isp_stats_vdev *stats_vdev, u32 blk_no)
{
    u32 addr, value;
...
static int rkisp_stats_restart_rawhstbig_meas(struct rkisp_isp_stats_vdev *stats_vdev, u32 blk_no)
{
    u32 addr, value;
...
static int rkisp_stats_restart_rawae1_meas(struct rkisp_isp_stats_vdev *stats_vdev)
{
    return rkisp_stats_restart_rawaebig_meas(stats_vdev, 1);
}

static int rkisp_stats_restart_rawhst1_meas(struct rkisp_isp_stats_vdev *stats_vdev)
{
    return rkisp_stats_restart_rawhstbig_meas(stats_vdev, 1);
}

static int rkisp_stats_restart_rawae2_meas(struct rkisp_isp_stats_vdev *stats_vdev)
{
    return rkisp_stats_restart_rawaebig_meas(stats_vdev, CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT);
}

static int rkisp_stats_restart_rawhst2_meas(struct rkisp_isp_stats_vdev *stats_vdev)
{
    return rkisp_stats_restart_rawhstbig_meas(stats_vdev, CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT);
}

static int rkisp_stats_restart_rawae3_meas(struct rkisp_isp_stats_vdev *stats_vdev)
{
    return rkisp_stats_restart_rawaebig_meas(stats_vdev, 0);
}

static int rkisp_stats_restart_rawhst3_meas(struct rkisp_isp_stats_vdev *stats_vdev)
{
    return rkisp_stats_restart_rawhstbig_meas(stats_vdev, 0);
}

static int rkisp_stats_restart_rawae0_meas(struct rkisp_isp_stats_vdev *stats_vdev)
{
    u32 value;
...
static int rkisp_stats_restart_rawhst0_meas(struct rkisp_isp_stats_vdev *stats_vdev)
{
    u32 value;
...
static void rkisp_stats_restart_meas(struct rkisp_isp_stats_vdev *stats_vdev)
{
...
static void rkisp_stats_send_meas_v21(struct rkisp_isp_stats_vdev *stats_vdev, struct rkisp_isp_readout_work *meas_work)
{
    unsigned int cur_frame_id = -1;
    struct rkisp_buffer *cur_buf = stats_vdev->cur_buf;
    struct rkisp_isp21_stat_buffer *cur_stat_buf = NULL;
    struct rkisp_stats_v21_ops *ops = (struct rkisp_stats_v21_ops *)stats_vdev->priv_ops;
    int ret = 0;
    ...
    if (cur_buf) {
        cur_stat_buf = (struct rkisp_isp21_stat_buffer *)(cur_buf->vaddr[0]);
        cur_stat_buf->frame_id = cur_frame_id;
    }

    if (meas_work->isp_ris & ISP2X_AFM_SUM_OF) {
        v4l2_warn(stats_vdev->vnode.vdev.v4l2_dev, "ISP2X_AFM_SUM_OF\n");
    }

    if (meas_work->isp_ris & ISP2X_AFM_LUM_OF) {
        v4l2_warn(stats_vdev->vnode.vdev.v4l2_dev, "ISP2X_AFM_LUM_OF\n");
    }
    ...
    if (meas_work->isp3a_ris & ISP2X_3A_RAWAF_SUM) {
        v4l2_warn(stats_vdev->vnode.vdev.v4l2_dev, "ISP2X_3A_RAWAF_SUM\n");
    }

    if (meas_work->isp3a_ris & ISP2X_3A_RAWAF_LUM) {
        v4l2_warn(stats_vdev->vnode.vdev.v4l2_dev, "ISP2X_3A_RAWAF_LUM\n");
    }
    ...
    if (cur_buf && !ret) {
        vb2_set_plane_payload(&cur_buf->vb.vb2_buf, 0, sizeof(struct rkisp_isp2x_stat_buffer));
...
static void rkisp_stats_isr_v21(struct rkisp_isp_stats_vdev *stats_vdev, u32 isp_ris, u32 isp3a_ris)
{
    struct rkisp_device *dev = stats_vdev->dev;
    struct rkisp_isp_readout_work work;
    u32 iq_isr_mask =
        ISP2X_SIAWB_DONE | ISP2X_SIAF_FIN | ISP2X_YUVAE_END | ISP2X_SIHST_RDY | ISP2X_AFM_SUM_OF | ISP2X_AFM_LUM_OF;
    ...
    if (isp_mis_tmp) {
        ...
        if (isp_mis_tmp) {
            v4l2_err(stats_vdev->vnode.vdev.v4l2_dev, "isp icr 3A info err: 0x%x 0x%x\n", isp_mis_tmp, isp_ris);
        }
    }
    ...
    if (isp_mis_tmp) {
        ...
        if (isp_mis_tmp) {
            v4l2_err(stats_vdev->vnode.vdev.v4l2_dev, "isp3A icr 3A info err: 0x%x 0x%x\n", isp_mis_tmp, isp_ris);
        }
    }
    ...
    if ((isp_ris & ISP2X_FRAME) && stats_vdev->rd_stats_from_ddr) {
        ...
        rkisp_write(stats_vdev->dev, MI_DBR_WR_SIZE, RKISP_RD_STATS_BUF_SIZE, false);
        rkisp_write(stats_vdev->dev, MI_SWS_3A_WR_BASE, stats_vdev->stats_buf[wr_buf_idx].dma_addr, false);
        rkisp_set_bits(stats_vdev->dev, CTRL_SWS_CFG, SW_3A_DDR_WRITE_EN, SW_3A_DDR_WRITE_EN, false);
    }
...
static void rkisp_stats_rdbk_enable_v21(struct rkisp_isp_stats_vdev *stats_vdev, bool en)
{
    if (!en) {
...
void rkisp_stats_first_ddr_config_v21(struct rkisp_isp_stats_vdev *stats_vdev)
{
    ...
    if (!IS_HDR_RDBK(stats_vdev->dev->hdr.op_mode) && stats_vdev->stats_buf[0].mem_priv) {
        ...
        rkisp_write(stats_vdev->dev, MI_DBR_WR_SIZE, RKISP_RD_STATS_BUF_SIZE, false);
        rkisp_write(stats_vdev->dev, MI_SWS_3A_WR_BASE, stats_vdev->stats_buf[0].dma_addr, false);
        rkisp_set_bits(stats_vdev->dev, CTRL_SWS_CFG, SW_3A_DDR_WRITE_EN, SW_3A_DDR_WRITE_EN, false);
    }
}
...
void rkisp_init_stats_vdev_v21(struct rkisp_isp_stats_vdev *stats_vdev)
{
    ...
    stats_vdev->vdev_fmt.fmt.meta.buffersize = sizeof(struct rkisp_isp2x_stat_buffer);
