...
static void rkisp1_stats_get_awb_meas_v10(struct rkisp_isp_stats_vdev *stats_vdev, struct rkisp1_stat_buffer *pbuf)
{
    ...
    pbuf->params.awb.awb_mean[0].mean_cr_or_r = CIF_ISP_AWB_GET_MEAN_CR_R(reg_val);
    pbuf->params.awb.awb_mean[0].mean_cb_or_b = CIF_ISP_AWB_GET_MEAN_CB_B(reg_val);
    pbuf->params.awb.awb_mean[0].mean_y_or_g = CIF_ISP_AWB_GET_MEAN_Y_G(reg_val);
}

static void rkisp1_stats_get_awb_meas_v12(struct rkisp_isp_stats_vdev *stats_vdev, struct rkisp1_stat_buffer *pbuf)
{
    /* Protect against concurrent access from ISR? */
    u32 reg_val;
...
static void rkisp1_stats_get_aec_meas_v10(struct rkisp_isp_stats_vdev *stats_vdev, struct rkisp1_stat_buffer *pbuf)
{
    int i;
    void __iomem *addr = stats_vdev->dev->base_addr + CIF_ISP_EXP_MEAN_00_V10;
    struct rkisp_stats_v1x_config *config = (struct rkisp_stats_v1x_config *)stats_vdev->priv_cfg;
...
static void rkisp1_stats_get_aec_meas_v12(struct rkisp_isp_stats_vdev *stats_vdev, struct rkisp1_stat_buffer *pbuf)
{
    int i;
...
static void rkisp1_stats_get_hst_meas_v12(struct rkisp_isp_stats_vdev *stats_vdev, struct rkisp1_stat_buffer *pbuf)
{
    int i;
...
static void rkisp1_stats_get_bls_meas(struct rkisp_isp_stats_vdev *stats_vdev, struct rkisp1_stat_buffer *pbuf)
{
    struct rkisp_device *dev = stats_vdev->dev;
    const struct ispsd_in_fmt *in_fmt = rkisp_get_ispsd_in_fmt(&dev->isp_sdev);
...
static void rkisp1_stats_get_emb_data(struct rkisp_isp_stats_vdev *stats_vdev, struct rkisp1_stat_buffer *pbuf)
{
    unsigned int i;
    struct rkisp_device *dev = stats_vdev->dev;
    ...
    for (i = 0; i < mipi_kfifo_len;) {
        ...
        out = kfifo_out(&dev->emd_data_fifo[idx].mipi_kfifo, &ph, sizeof(ph));
        if (!out) {
            break;
        }
        ...
        out = kfifo_out(&dev->emd_data_fifo[idx].mipi_kfifo, fifo_data, packet_len);
        if (!out) {
            break;
        }
        ...
        v4l2_dbg(1, rkisp_debug, &dev->v4l2_dev, "packet_len: 0x%x, ph: 0x%x\n", packet_len, ph);
    }

    pbuf->meas_type |= CIFISP_STAT_EMB_DATA;

    v4l2_dbg(1, rkisp_debug, &dev->v4l2_dev, "playload_len: %d, pbuf->frame_id %d\n", playload_len, pbuf->frame_id);
}
...
static void rkisp1_stats_send_meas_v1x(struct rkisp_isp_stats_vdev *stats_vdev,
                                       struct rkisp_isp_readout_work *meas_work)
{
    unsigned int cur_frame_id = -1;
    struct rkisp1_stat_buffer *cur_stat_buf;
    struct rkisp_buffer *cur_buf = NULL;
    struct rkisp_stats_v1x_ops *ops = (struct rkisp_stats_v1x_ops *)stats_vdev->priv_ops;

    cur_frame_id = atomic_read(&stats_vdev->dev->isp_sdev.frm_sync_seq) - 1;
    if (cur_frame_id != meas_work->frame_id) {
        v4l2_warn(stats_vdev->vnode.vdev.v4l2_dev, "Measurement late(%d, %d)\n", cur_frame_id, meas_work->frame_id);
        cur_frame_id = meas_work->frame_id;
    }
    ...
    cur_stat_buf = (struct rkisp1_stat_buffer *)(cur_buf->vaddr[0]);
    ...
    if ((meas_work->isp_ris & CIF_ISP_FRAME) && ops->get_emb_data) {
        ops->get_emb_data(stats_vdev, cur_stat_buf);
    }

    vb2_set_plane_payload(&cur_buf->vb.vb2_buf, 0, sizeof(struct rkisp1_stat_buffer));
    cur_buf->vb.sequence = cur_frame_id;
    cur_buf->vb.vb2_buf.timestamp = meas_work->timestamp;
    vb2_buffer_done(&cur_buf->vb.vb2_buf, VB2_BUF_STATE_DONE);
}

static void rkisp1_stats_isr_v1x(struct rkisp_isp_stats_vdev *stats_vdev, u32 isp_ris, u32 isp3a_ris)
{
    unsigned int isp_mis_tmp = 0;
    struct rkisp_isp_readout_work work;
    unsigned int cur_frame_id = atomic_read(&stats_vdev->dev->isp_sdev.frm_sync_seq) - 1;
    ...
    isp_mis_tmp = isp_ris & (CIF_ISP_AWB_DONE | CIF_ISP_AFM_FIN | CIF_ISP_EXP_END | CIF_ISP_HIST_MEASURE_RDY);
    if (isp_mis_tmp) {
        writel(isp_mis_tmp, stats_vdev->dev->base_addr + CIF_ISP_ICR);
    ...
    if (isp_ris & (CIF_ISP_FRAME | CIF_ISP_AWB_DONE | CIF_ISP_AFM_FIN | CIF_ISP_EXP_END | CIF_ISP_HIST_MEASURE_RDY)) {
        work.readout = RKISP_ISP_READOUT_MEAS;
        work.frame_id = cur_frame_id;
        work.isp_ris = isp_ris;
        work.timestamp = ktime_get_ns();
        if (!kfifo_is_full(&stats_vdev->rd_kfifo)) {
            kfifo_in(&stats_vdev->rd_kfifo, &work, sizeof(work));
        } else {
            v4l2_err(stats_vdev->vnode.vdev.v4l2_dev, "stats kfifo is full\n");
        }
    ...
    if (isp_ris & (CIF_ISP_EXP_END | CIF_ISP_AWB_DONE | CIF_ISP_FRAME | CIF_ISP_HIST_MEASURE_RDY)) {
        ...
        v4l2_info(stats_vdev->vnode.vdev.v4l2_dev, "isp_isr time %d %d\n", diff_us, g_longest_isr_time);
    }
#endif
...
static void rkisp_stats_rdbk_enable_v1x(struct rkisp_isp_stats_vdev *stats_vdev, bool en)
{
}
...
void rkisp_init_stats_vdev_v1x(struct rkisp_isp_stats_vdev *stats_vdev)
{
    ...
    stats_vdev->vdev_fmt.fmt.meta.buffersize = sizeof(struct rkisp1_stat_buffer);

    stats_vdev->ops = &rkisp_isp_stats_ops_tbl;
    if (stats_vdev->dev->isp_ver == ISP_V12 || stats_vdev->dev->isp_ver == ISP_V13) {
