...
static inline void isp3_param_write_direct(struct rkisp_isp_params_vdev *params_vdev, u32 value, u32 addr, u32 id)
{
    if (id == ISP3_LEFT) {
...
static inline void isp3_param_write(struct rkisp_isp_params_vdev *params_vdev, u32 value, u32 addr, u32 id)
{
    if (id == ISP3_LEFT) {
...
static inline u32 isp3_param_read_direct(struct rkisp_isp_params_vdev *params_vdev, u32 addr, u32 id)
{
    u32 val;
...
static inline u32 isp3_param_read(struct rkisp_isp_params_vdev *params_vdev, u32 addr, u32 id)
{
    u32 val;
...
static inline u32 isp3_param_read_cache(struct rkisp_isp_params_vdev *params_vdev, u32 addr, u32 id)
{
    u32 val;
...
static inline void isp3_param_set_bits(struct rkisp_isp_params_vdev *params_vdev, u32 reg, u32 bit_mask, u32 id)
{
    if (id == ISP3_LEFT) {
...
static inline void isp3_param_clear_bits(struct rkisp_isp_params_vdev *params_vdev, u32 reg, u32 bit_mask, u32 id)
{
    if (id == ISP3_LEFT) {
...
static void isp_dpcc_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_dpcc_cfg *arg, u32 id)
{
    u32 value;
    int i;
    ...
    value = ISP_PACK_4BYTE(arg->line_thr_1_g, arg->line_thr_1_rb, arg->sw_mindis1_g, arg->sw_mindis1_rb);
    ...
    value =
        ISP_PACK_4BYTE(arg->line_mad_fac_1_g, arg->line_mad_fac_1_rb, arg->sw_dis_scale_max1, arg->sw_dis_scale_min1);
    ...
    value = ISP_PACK_4BYTE(arg->line_thr_2_g, arg->line_thr_2_rb, arg->sw_mindis2_g, arg->sw_mindis2_rb);
    ...
    value =
        ISP_PACK_4BYTE(arg->line_mad_fac_2_g, arg->line_mad_fac_2_rb, arg->sw_dis_scale_max2, arg->sw_dis_scale_min2);
    ...
    value = ISP_PACK_4BYTE(arg->line_thr_3_g, arg->line_thr_3_rb, arg->sw_mindis3_g, arg->sw_mindis3_rb);
    ...
    value =
        ISP_PACK_4BYTE(arg->line_mad_fac_3_g, arg->line_mad_fac_3_rb, arg->sw_dis_scale_max3, arg->sw_dis_scale_min3);
    ...
    for (i = 0; i < ISP3X_DPCC_PDAF_POINT_NUM / 2; i++) {
        value =
            ISP_PACK_4BYTE(arg->point[2 * i].x, arg->point[2 * i].y, arg->point[2 * i + 1].x, arg->point[2 * i + 1].y);
...
static void isp_dpcc_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    u32 value;
...
static void isp_bls_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp21_bls_cfg *arg, u32 id)
{
...
static void isp_bls_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    u32 new_control;
...
static void isp_sdg_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_sdg_cfg *arg, u32 id)
{
    int i;
    ...
    for (i = 0; i < ISP3X_DEGAMMA_CURVE_SIZE; i++) {
        isp3_param_write(params_vdev, arg->curve_r.gamma_y[i], ISP3X_ISP_GAMMA_R_Y_0 + i * 4, id);
        isp3_param_write(params_vdev, arg->curve_g.gamma_y[i], ISP3X_ISP_GAMMA_G_Y_0 + i * 4, id);
        isp3_param_write(params_vdev, arg->curve_b.gamma_y[i], ISP3X_ISP_GAMMA_B_Y_0 + i * 4, id);
    }
}

static void isp_sdg_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    if (en) {
        isp3_param_set_bits(params_vdev, ISP3X_ISP_CTRL0, CIF_ISP_CTRL_ISP_GAMMA_IN_ENA, id);
    } else {
        isp3_param_clear_bits(params_vdev, ISP3X_ISP_CTRL0, CIF_ISP_CTRL_ISP_GAMMA_IN_ENA, id);
    }
}

static void isp_lsc_matrix_cfg_sram(struct rkisp_isp_params_vdev *params_vdev, const struct isp3x_lsc_cfg *pconfig,
                                    bool is_check, u32 id)
{
    ...
    if (is_check && !(isp3_param_read(params_vdev, ISP3X_LSC_CTRL, id) & ISP_LSC_EN)) {
        return;
    }
    ...
    for (i = 0; i < CIF_ISP_LSC_SECTORS_MAX * CIF_ISP_LSC_SECTORS_MAX; i += CIF_ISP_LSC_SECTORS_MAX) {
        /*
         * 17 sectors with 2 values in one DWORD = 9
         * DWORDs (2nd value of last DWORD unused)
         */
        for (j = 0; j < CIF_ISP_LSC_SECTORS_MAX - 1; j += 2) {
            data = ISP_ISP_LSC_TABLE_DATA(pconfig->r_data_tbl[i + j], pconfig->r_data_tbl[i + j + 1]);
            isp3_param_write_direct(params_vdev, data, ISP3X_LSC_R_TABLE_DATA, id);

            data = ISP_ISP_LSC_TABLE_DATA(pconfig->gr_data_tbl[i + j], pconfig->gr_data_tbl[i + j + 1]);
            isp3_param_write_direct(params_vdev, data, ISP3X_LSC_GR_TABLE_DATA, id);

            data = ISP_ISP_LSC_TABLE_DATA(pconfig->gb_data_tbl[i + j], pconfig->gb_data_tbl[i + j + 1]);
            isp3_param_write_direct(params_vdev, data, ISP3X_LSC_GB_TABLE_DATA, id);

            data = ISP_ISP_LSC_TABLE_DATA(pconfig->b_data_tbl[i + j], pconfig->b_data_tbl[i + j + 1]);
...
static void isp_lsc_cfg_sram_task(unsigned long data)
{
    struct rkisp_isp_params_vdev *params_vdev = (struct rkisp_isp_params_vdev *)data;
...
static void isp_lsc_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp3x_lsc_cfg *arg, u32 id)
{
    struct rkisp_isp_params_val_v3x *priv_val = (struct rkisp_isp_params_val_v3x *)params_vdev->priv_val;
...
static void isp_lsc_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    u32 val = ISP_LSC_EN;
...
static void isp_debayer_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_debayer_cfg *arg, u32 id)
{
    u32 value;
...
static void isp_debayer_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    if (en) {
        isp3_param_set_bits(params_vdev, ISP3X_DEBAYER_CONTROL, ISP3X_MODULE_EN, id);
    } else {
        isp3_param_clear_bits(params_vdev, ISP3X_DEBAYER_CONTROL, ISP3X_MODULE_EN, id);
    }
}

static void isp_awbgain_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp21_awb_gain_cfg *arg, u32 id)
{
    struct rkisp_device *dev = params_vdev->dev;

    if (!arg->gain0_red || !arg->gain0_blue || !arg->gain1_red || !arg->gain1_blue || !arg->gain2_red ||
        !arg->gain2_blue || !arg->gain0_green_r || !arg->gain0_green_b || !arg->gain1_green_r || !arg->gain1_green_b ||
        !arg->gain2_green_r || !arg->gain2_green_b) {
        dev_err(dev->dev, "awb gain is zero!\n");
        return;
    }

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->gain0_green_b, arg->gain0_green_r), ISP3X_ISP_AWB_GAIN0_G, id);
    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->gain0_blue, arg->gain0_red), ISP3X_ISP_AWB_GAIN0_RB, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->gain1_green_b, arg->gain1_green_r), ISP3X_ISP_AWB_GAIN1_G, id);
    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->gain1_blue, arg->gain1_red), ISP3X_ISP_AWB_GAIN1_RB, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->gain2_green_b, arg->gain2_green_r), ISP3X_ISP_AWB_GAIN2_G, id);
    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->gain2_blue, arg->gain2_red), ISP3X_ISP_AWB_GAIN2_RB, id);
}

static void isp_awbgain_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    if (en) {
        isp3_param_set_bits(params_vdev, ISP3X_ISP_CTRL0, CIF_ISP_CTRL_ISP_AWB_ENA, id);
    } else {
        isp3_param_clear_bits(params_vdev, ISP3X_ISP_CTRL0, CIF_ISP_CTRL_ISP_AWB_ENA, id);
    }
}

static void isp_ccm_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp21_ccm_cfg *arg, u32 id)
{
    u32 value;
    u32 i;
...
static void isp_ccm_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    if (en) {
...
static void isp_goc_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp3x_gammaout_cfg *arg, u32 id)
{
    int i;
    u32 value;
    ...
    value |= (arg->equ_segm & 0x1) << 1 | (arg->finalx4_dense_en & 0x1) << 2;
    ...
    for (i = 0; i < ISP3X_GAMMA_OUT_MAX_SAMPLES / 2; i++) {
        value = ISP_PACK_2SHORT(arg->gamma_y[2 * i], arg->gamma_y[2 * i + 1]);
...
static void isp_goc_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    if (en) {
...
static void isp_cproc_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_cproc_cfg *arg, u32 id)
{
    ...
    if (quantization != V4L2_QUANTIZATION_FULL_RANGE || effect != V4L2_COLORFX_NONE) {
        isp3_param_clear_bits(params_vdev, ISP3X_CPROC_CTRL,
                              CIF_C_PROC_YOUT_FULL | CIF_C_PROC_YIN_FULL | CIF_C_PROC_COUT_FULL, id);
    } else {
...
static void isp_cproc_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    if (en) {
        isp3_param_set_bits(params_vdev, ISP3X_CPROC_CTRL, CIF_C_PROC_CTR_ENABLE, id);
    } else {
        isp3_param_clear_bits(params_vdev, ISP3X_CPROC_CTRL, CIF_C_PROC_CTR_ENABLE, id);
    }
}

static void isp_ie_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_ie_cfg *arg, u32 id)
{
    u32 eff_ctrl;
    ...
    switch (arg->effect) {
        ...
        case V4L2_COLORFX_AQUA:
            ...
            isp3_param_write(params_vdev, arg->color_sel, ISP3X_IMG_EFF_COLOR_SEL, id);
            break;
        case V4L2_COLORFX_EMBOSS:
            eff_ctrl |= CIF_IMG_EFF_CTRL_MODE_EMBOSS;
            isp3_param_write(params_vdev, arg->eff_mat_1, CIF_IMG_EFF_MAT_1, id);
            isp3_param_write(params_vdev, arg->eff_mat_2, CIF_IMG_EFF_MAT_2, id);
            isp3_param_write(params_vdev, arg->eff_mat_3, CIF_IMG_EFF_MAT_3, id);
            break;
        case V4L2_COLORFX_SKETCH:
            eff_ctrl |= CIF_IMG_EFF_CTRL_MODE_SKETCH;
            isp3_param_write(params_vdev, arg->eff_mat_3, CIF_IMG_EFF_MAT_3, id);
            isp3_param_write(params_vdev, arg->eff_mat_4, CIF_IMG_EFF_MAT_4, id);
            isp3_param_write(params_vdev, arg->eff_mat_5, CIF_IMG_EFF_MAT_5, id);
            break;
...
static void isp_ie_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    if (en) {
        isp3_param_set_bits(params_vdev, ISP3X_IMG_EFF_CTRL, CIF_IMG_EFF_CTRL_CFG_UPD | CIF_IMG_EFF_CTRL_ENABLE, id);
    } else {
        isp3_param_clear_bits(params_vdev, ISP3X_IMG_EFF_CTRL, CIF_IMG_EFF_CTRL_ENABLE, id);
    }
}

static void isp_rawaebig_config_foraf(struct rkisp_isp_params_vdev *params_vdev, const struct isp3x_rawaf_meas_cfg *arg,
                                      u32 id)
{
    ...
    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->win[0].h_offs, arg->win[0].v_offs),
    ...
    isp3_param_write(params_vdev, ISP_PACK_2SHORT(block_hsize, block_vsize), addr + ISP3X_RAWAE_BIG_BLK_SIZE, id);
}

static void isp_rawaf_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp3x_rawaf_meas_cfg *arg, u32 id)
{
    u32 i, var, ctrl;
    u16 h_size, v_size;
    u16 h_offs, v_offs;
    ...
    for (i = 0; i < num_of_win; i++) {
         ...
         */
        isp3_param_write(params_vdev, ISP_PACK_2SHORT(v_offs, h_offs), ISP3X_RAWAF_OFFSET_WINA + i * 8, id);

        /*
         * value must be smaller than [width of picture -2]
         * value must be lower than (number of lines -2)
         */
        isp3_param_write(params_vdev, ISP_PACK_2SHORT(v_size, h_size), ISP3X_RAWAF_SIZE_WINA + i * 8, id);
    }
    ...
    var = (arg->v2iir_var_shift & 0x7) << 12 | (arg->v1iir_var_shift & 0x7) << 8 | (arg->h2iir_var_shift & 0x7) << 4 |
          (arg->h1iir_var_shift & 0x7);
    ...
    if (arg->ldg_en) {
        ...
        for (i = 0; i < ISP3X_RAWAF_CURVE_NUM; i++) {
            isp3_param_write(params_vdev,
                             arg->curve_h[i].ldg_lumth | arg->curve_h[i].ldg_gain << 8 | arg->curve_h[i].ldg_gslp << 16,
                             ISP3X_RAWAF_H_CURVEL + i * 16, id);
            isp3_param_write(params_vdev,
                             arg->curve_v[i].ldg_lumth | arg->curve_v[i].ldg_gain << 8 | arg->curve_v[i].ldg_gslp << 16,
                             ISP3X_RAWAF_V_CURVEL + i * 16, id);
        }
    }

    ctrl |= (arg->y_mode & 0x1) << 13 | (arg->ae_mode & 0x1) << 12 | (arg->v2_fv_mode & 0x1) << 11 |
            (arg->v1_fv_mode & 0x1) << 10 | (arg->h2_fv_mode & 0x1) << 9 | (arg->h1_fv_mode & 0x1) << 8 |
            (arg->accu_8bit_mode & 0x1) << 6 | (v1_fir_sel & 0x1) << 3 | (gaus_en & 0x1) << 2 |
            (arg->gamma_en & 0x1) << 1;
...
static void isp_rawaebig_enable_foraf(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    u32 exp_ctrl;
...
static void isp_rawaf_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
...
static void isp_rawaelite_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_rawaelite_meas_cfg *arg,
                                 u32 id)
{
    ...
    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->win.h_offs, arg->win.v_offs), ISP3X_RAWAE_LITE_OFFSET, id);
    ...
    isp3_param_write(params_vdev, ISP_PACK_2SHORT(block_hsize, block_vsize), ISP3X_RAWAE_LITE_BLK_SIZ, id);
...
static void isp_rawaelite_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    u32 exp_ctrl;
...
static void isp_rawaebig_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_rawaebig_meas_cfg *arg,
                                u32 blk_no, u32 id)
{
    struct rkisp_device *ispdev = params_vdev->dev;
    struct v4l2_rect *out_crop = &ispdev->isp_sdev.out_crop;
    ...
    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->win.h_offs, arg->win.v_offs), addr + ISP3X_RAWAE_BIG_OFFSET, id);
    ...
    for (i = 0; i < ISP3X_RAWAEBIG_SUBWIN_NUM; i++) {
        isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->subwin[i].h_offs, arg->subwin[i].v_offs),
        ...
        isp3_param_write(params_vdev, ISP_PACK_2SHORT(h_size, v_size), addr + ISP3X_RAWAE_BIG_WND1_SIZE + 8 * i, id);
    }
...
static void isp_rawaebig_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 blk_no, u32 id)
{
    u32 exp_ctrl;
    u32 addr;
...
static void isp_rawae1_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_rawaebig_meas_cfg *arg,
                              u32 id)
{
    isp_rawaebig_config(params_vdev, arg, 1, id);
}

static void isp_rawae1_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    isp_rawaebig_enable(params_vdev, en, 1, id);
}

static void isp_rawae2_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_rawaebig_meas_cfg *arg,
                              u32 id)
{
    isp_rawaebig_config(params_vdev, arg, 2, id);
}

static void isp_rawae2_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
...
static void isp_rawae3_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    isp_rawaebig_enable(params_vdev, en, 0, id);
}

static void isp_rawawb_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp3x_rawawb_meas_cfg *arg,
                              u32 id)
{
    u32 i, value;
    ...
    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_h_offs, arg->sw_rawawb_v_offs), ISP3X_RAWAWB_WIN_OFFS,
                     id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_h_size, arg->sw_rawawb_v_size), ISP3X_RAWAWB_WIN_SIZE,
                     id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_r_max, arg->sw_rawawb_g_max),
                     ISP3X_RAWAWB_LIMIT_RG_MAX, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_b_max, arg->sw_rawawb_y_max),
                     ISP3X_RAWAWB_LIMIT_BY_MAX, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_r_min, arg->sw_rawawb_g_min),
                     ISP3X_RAWAWB_LIMIT_RG_MIN, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_b_min, arg->sw_rawawb_y_min),
    ...
    isp3_param_write(params_vdev,
                     ISP_PACK_4BYTE(arg->sw_rawawb_wp_luma_weicurve_y0, arg->sw_rawawb_wp_luma_weicurve_y1,
                                    arg->sw_rawawb_wp_luma_weicurve_y2, arg->sw_rawawb_wp_luma_weicurve_y3),
                     ISP3X_RAWAWB_YWEIGHT_CURVE_XCOOR03, id);

    isp3_param_write(params_vdev,
                     ISP_PACK_4BYTE(arg->sw_rawawb_wp_luma_weicurve_y4, arg->sw_rawawb_wp_luma_weicurve_y5,
                                    arg->sw_rawawb_wp_luma_weicurve_y6, arg->sw_rawawb_wp_luma_weicurve_y7),
                     ISP3X_RAWAWB_YWEIGHT_CURVE_XCOOR47, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_wp_luma_weicurve_y8, ISP3X_RAWAWB_YWEIGHT_CURVE_XCOOR8, id);

    isp3_param_write(params_vdev,
                     ISP_PACK_4BYTE(arg->sw_rawawb_wp_luma_weicurve_w0, arg->sw_rawawb_wp_luma_weicurve_w1,
                                    arg->sw_rawawb_wp_luma_weicurve_w2, arg->sw_rawawb_wp_luma_weicurve_w3),
                     ISP3X_RAWAWB_YWEIGHT_CURVE_YCOOR03, id);

    isp3_param_write(params_vdev,
                     ISP_PACK_4BYTE(arg->sw_rawawb_wp_luma_weicurve_w4, arg->sw_rawawb_wp_luma_weicurve_w5,
                                    arg->sw_rawawb_wp_luma_weicurve_w6, arg->sw_rawawb_wp_luma_weicurve_w7),
                     ISP3X_RAWAWB_YWEIGHT_CURVE_YCOOR47, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_wp_luma_weicurve_w8, arg->sw_rawawb_pre_wbgain_inv_r),
                     ISP3X_RAWAWB_YWEIGHT_CURVE_YCOOR8, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_pre_wbgain_inv_g, arg->sw_rawawb_pre_wbgain_inv_b),
                     ISP3X_RAWAWB_PRE_WBGAIN_INV, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex0_u_0, arg->sw_rawawb_vertex0_v_0),
                     ISP3X_RAWAWB_UV_DETC_VERTEX0_0, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex1_u_0, arg->sw_rawawb_vertex1_v_0),
                     ISP3X_RAWAWB_UV_DETC_VERTEX1_0, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex2_u_0, arg->sw_rawawb_vertex2_v_0),
                     ISP3X_RAWAWB_UV_DETC_VERTEX2_0, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex3_u_0, arg->sw_rawawb_vertex3_v_0),
                     ISP3X_RAWAWB_UV_DETC_VERTEX3_0, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope01_0, ISP3X_RAWAWB_UV_DETC_ISLOPE01_0, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope12_0, ISP3X_RAWAWB_UV_DETC_ISLOPE12_0, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope23_0, ISP3X_RAWAWB_UV_DETC_ISLOPE23_0, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope30_0, ISP3X_RAWAWB_UV_DETC_ISLOPE30_0, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex0_u_1, arg->sw_rawawb_vertex0_v_1),
                     ISP3X_RAWAWB_UV_DETC_VERTEX0_1, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex1_u_1, arg->sw_rawawb_vertex1_v_1),
                     ISP3X_RAWAWB_UV_DETC_VERTEX1_1, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex2_u_1, arg->sw_rawawb_vertex2_v_1),
                     ISP3X_RAWAWB_UV_DETC_VERTEX2_1, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex3_u_1, arg->sw_rawawb_vertex3_v_1),
                     ISP3X_RAWAWB_UV_DETC_VERTEX3_1, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope01_1, ISP3X_RAWAWB_UV_DETC_ISLOPE01_1, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope12_1, ISP3X_RAWAWB_UV_DETC_ISLOPE12_1, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope23_1, ISP3X_RAWAWB_UV_DETC_ISLOPE23_1, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope30_1, ISP3X_RAWAWB_UV_DETC_ISLOPE30_1, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex0_u_2, arg->sw_rawawb_vertex0_v_2),
                     ISP3X_RAWAWB_UV_DETC_VERTEX0_2, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex1_u_2, arg->sw_rawawb_vertex1_v_2),
                     ISP3X_RAWAWB_UV_DETC_VERTEX1_2, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex2_u_2, arg->sw_rawawb_vertex2_v_2),
                     ISP3X_RAWAWB_UV_DETC_VERTEX2_2, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex3_u_2, arg->sw_rawawb_vertex3_v_2),
                     ISP3X_RAWAWB_UV_DETC_VERTEX3_2, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope01_2, ISP3X_RAWAWB_UV_DETC_ISLOPE01_2, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope12_2, ISP3X_RAWAWB_UV_DETC_ISLOPE12_2, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope23_2, ISP3X_RAWAWB_UV_DETC_ISLOPE23_2, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope30_2, ISP3X_RAWAWB_UV_DETC_ISLOPE30_2, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex0_u_3, arg->sw_rawawb_vertex0_v_3),
                     ISP3X_RAWAWB_UV_DETC_VERTEX0_3, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex1_u_3, arg->sw_rawawb_vertex1_v_3),
                     ISP3X_RAWAWB_UV_DETC_VERTEX1_3, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex2_u_3, arg->sw_rawawb_vertex2_v_3),
                     ISP3X_RAWAWB_UV_DETC_VERTEX2_3, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex3_u_3, arg->sw_rawawb_vertex3_v_3),
                     ISP3X_RAWAWB_UV_DETC_VERTEX3_3, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope01_3, ISP3X_RAWAWB_UV_DETC_ISLOPE01_3, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope12_3, ISP3X_RAWAWB_UV_DETC_ISLOPE12_3, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope23_3, ISP3X_RAWAWB_UV_DETC_ISLOPE23_3, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope30_3, ISP3X_RAWAWB_UV_DETC_ISLOPE30_3, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex0_u_4, arg->sw_rawawb_vertex0_v_4),
                     ISP3X_RAWAWB_UV_DETC_VERTEX0_4, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex1_u_4, arg->sw_rawawb_vertex1_v_4),
                     ISP3X_RAWAWB_UV_DETC_VERTEX1_4, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex2_u_4, arg->sw_rawawb_vertex2_v_4),
                     ISP3X_RAWAWB_UV_DETC_VERTEX2_4, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex3_u_4, arg->sw_rawawb_vertex3_v_4),
                     ISP3X_RAWAWB_UV_DETC_VERTEX3_4, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope01_4, ISP3X_RAWAWB_UV_DETC_ISLOPE01_4, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope12_4, ISP3X_RAWAWB_UV_DETC_ISLOPE12_4, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope23_4, ISP3X_RAWAWB_UV_DETC_ISLOPE23_4, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope30_4, ISP3X_RAWAWB_UV_DETC_ISLOPE30_4, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex0_u_5, arg->sw_rawawb_vertex0_v_5),
                     ISP3X_RAWAWB_UV_DETC_VERTEX0_5, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex1_u_5, arg->sw_rawawb_vertex1_v_5),
                     ISP3X_RAWAWB_UV_DETC_VERTEX1_5, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex2_u_5, arg->sw_rawawb_vertex2_v_5),
                     ISP3X_RAWAWB_UV_DETC_VERTEX2_5, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex3_u_5, arg->sw_rawawb_vertex3_v_5),
                     ISP3X_RAWAWB_UV_DETC_VERTEX3_5, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope01_5, ISP3X_RAWAWB_UV_DETC_ISLOPE01_5, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope12_5, ISP3X_RAWAWB_UV_DETC_ISLOPE10_5, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope23_5, ISP3X_RAWAWB_UV_DETC_ISLOPE23_5, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope30_5, ISP3X_RAWAWB_UV_DETC_ISLOPE30_5, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex0_u_6, arg->sw_rawawb_vertex0_v_6),
                     ISP3X_RAWAWB_UV_DETC_VERTEX0_6, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex1_u_6, arg->sw_rawawb_vertex1_v_6),
                     ISP3X_RAWAWB_UV_DETC_VERTEX1_6, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex2_u_6, arg->sw_rawawb_vertex2_v_6),
                     ISP3X_RAWAWB_UV_DETC_VERTEX2_6, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_vertex3_u_6, arg->sw_rawawb_vertex3_v_6),
                     ISP3X_RAWAWB_UV_DETC_VERTEX3_6, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope01_6, ISP3X_RAWAWB_UV_DETC_ISLOPE01_6, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope12_6, ISP3X_RAWAWB_UV_DETC_ISLOPE10_6, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope23_6, ISP3X_RAWAWB_UV_DETC_ISLOPE23_6, id);

    isp3_param_write(params_vdev, arg->sw_rawawb_islope30_6, ISP3X_RAWAWB_UV_DETC_ISLOPE30_6, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_rgb2ryuvmat0_y, arg->sw_rawawb_rgb2ryuvmat1_y),
                     ISP3X_RAWAWB_YUV_RGB2ROTY_0, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_rgb2ryuvmat2_y, arg->sw_rawawb_rgb2ryuvofs_y),
                     ISP3X_RAWAWB_YUV_RGB2ROTY_1, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_rgb2ryuvmat0_u, arg->sw_rawawb_rgb2ryuvmat1_u),
                     ISP3X_RAWAWB_YUV_RGB2ROTU_0, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_rgb2ryuvmat2_u, arg->sw_rawawb_rgb2ryuvofs_u),
                     ISP3X_RAWAWB_YUV_RGB2ROTU_1, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_rgb2ryuvmat0_v, arg->sw_rawawb_rgb2ryuvmat1_v),
                     ISP3X_RAWAWB_YUV_RGB2ROTV_0, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_rgb2ryuvmat2_v, arg->sw_rawawb_rgb2ryuvofs_v),
                     ISP3X_RAWAWB_YUV_RGB2ROTV_1, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_coor_x1_ls0_y, arg->sw_rawawb_vec_x21_ls0_y),
                     ISP3X_RAWAWB_YUV_X_COOR_Y_0, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_coor_x1_ls0_u, arg->sw_rawawb_vec_x21_ls0_u),
                     ISP3X_RAWAWB_YUV_X_COOR_U_0, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_coor_x1_ls0_v, arg->sw_rawawb_vec_x21_ls0_v),
                     ISP3X_RAWAWB_YUV_X_COOR_V_0, id);

    isp3_param_write(params_vdev,
                     ISP_PACK_4BYTE(arg->sw_rawawb_dis_x1x2_ls0, 0, arg->sw_rawawb_rotu0_ls0, arg->sw_rawawb_rotu1_ls0),
                     ISP3X_RAWAWB_YUV_X1X2_DIS_0, id);

    isp3_param_write(params_vdev,
                     ISP_PACK_4BYTE(arg->sw_rawawb_rotu2_ls0, arg->sw_rawawb_rotu3_ls0, arg->sw_rawawb_rotu4_ls0,
                                    arg->sw_rawawb_rotu5_ls0),
                     ISP3X_RAWAWB_YUV_INTERP_CURVE_UCOOR_0, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_th0_ls0, arg->sw_rawawb_th1_ls0),
                     ISP3X_RAWAWB_YUV_INTERP_CURVE_TH0_0, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_th2_ls0, arg->sw_rawawb_th3_ls0),
                     ISP3X_RAWAWB_YUV_INTERP_CURVE_TH1_0, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_th4_ls0, arg->sw_rawawb_th5_ls0),
                     ISP3X_RAWAWB_YUV_INTERP_CURVE_TH2_0, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_coor_x1_ls1_y, arg->sw_rawawb_vec_x21_ls1_y),
                     ISP3X_RAWAWB_YUV_X_COOR_Y_1, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_coor_x1_ls1_u, arg->sw_rawawb_vec_x21_ls1_u),
                     ISP3X_RAWAWB_YUV_X_COOR_U_1, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_coor_x1_ls1_v, arg->sw_rawawb_vec_x21_ls1_v),
                     ISP3X_RAWAWB_YUV_X_COOR_V_1, id);

    isp3_param_write(params_vdev,
                     ISP_PACK_4BYTE(arg->sw_rawawb_dis_x1x2_ls1, 0, arg->sw_rawawb_rotu0_ls1, arg->sw_rawawb_rotu1_ls1),
                     ISP3X_RAWAWB_YUV_X1X2_DIS_1, id);

    isp3_param_write(params_vdev,
                     ISP_PACK_4BYTE(arg->sw_rawawb_rotu2_ls1, arg->sw_rawawb_rotu3_ls1, arg->sw_rawawb_rotu4_ls1,
                                    arg->sw_rawawb_rotu5_ls1),
                     ISP3X_RAWAWB_YUV_INTERP_CURVE_UCOOR_1, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_th0_ls1, arg->sw_rawawb_th1_ls1),
                     ISP3X_RAWAWB_YUV_INTERP_CURVE_TH0_1, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_th2_ls1, arg->sw_rawawb_th3_ls1),
                     ISP3X_RAWAWB_YUV_INTERP_CURVE_TH1_1, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_th4_ls1, arg->sw_rawawb_th5_ls1),
                     ISP3X_RAWAWB_YUV_INTERP_CURVE_TH2_1, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_coor_x1_ls2_y, arg->sw_rawawb_vec_x21_ls2_y),
                     ISP3X_RAWAWB_YUV_X_COOR_Y_2, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_coor_x1_ls2_u, arg->sw_rawawb_vec_x21_ls2_u),
                     ISP3X_RAWAWB_YUV_X_COOR_U_2, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_coor_x1_ls2_v, arg->sw_rawawb_vec_x21_ls2_v),
                     ISP3X_RAWAWB_YUV_X_COOR_V_2, id);

    isp3_param_write(params_vdev,
                     ISP_PACK_4BYTE(arg->sw_rawawb_dis_x1x2_ls2, 0, arg->sw_rawawb_rotu0_ls2, arg->sw_rawawb_rotu1_ls2),
                     ISP3X_RAWAWB_YUV_X1X2_DIS_2, id);

    isp3_param_write(params_vdev,
                     ISP_PACK_4BYTE(arg->sw_rawawb_rotu2_ls2, arg->sw_rawawb_rotu3_ls2, arg->sw_rawawb_rotu4_ls2,
                                    arg->sw_rawawb_rotu5_ls2),
                     ISP3X_RAWAWB_YUV_INTERP_CURVE_UCOOR_2, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_th0_ls2, arg->sw_rawawb_th1_ls2),
                     ISP3X_RAWAWB_YUV_INTERP_CURVE_TH0_2, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_th2_ls2, arg->sw_rawawb_th3_ls2),
                     ISP3X_RAWAWB_YUV_INTERP_CURVE_TH1_2, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_th4_ls2, arg->sw_rawawb_th5_ls2),
                     ISP3X_RAWAWB_YUV_INTERP_CURVE_TH2_2, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_coor_x1_ls3_y, arg->sw_rawawb_vec_x21_ls3_y),
                     ISP3X_RAWAWB_YUV_X_COOR_Y_3, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_coor_x1_ls3_u, arg->sw_rawawb_vec_x21_ls3_u),
                     ISP3X_RAWAWB_YUV_X_COOR_U_3, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_coor_x1_ls3_v, arg->sw_rawawb_vec_x21_ls3_v),
                     ISP3X_RAWAWB_YUV_X_COOR_V_3, id);

    isp3_param_write(params_vdev,
                     ISP_PACK_4BYTE(arg->sw_rawawb_dis_x1x2_ls3, 0, arg->sw_rawawb_rotu0_ls3, arg->sw_rawawb_rotu1_ls3),
                     ISP3X_RAWAWB_YUV_X1X2_DIS_3, id);

    isp3_param_write(params_vdev,
                     ISP_PACK_4BYTE(arg->sw_rawawb_rotu2_ls3, arg->sw_rawawb_rotu3_ls3, arg->sw_rawawb_rotu4_ls3,
                                    arg->sw_rawawb_rotu5_ls3),
                     ISP3X_RAWAWB_YUV_INTERP_CURVE_UCOOR_3, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_th0_ls3, arg->sw_rawawb_th1_ls3),
                     ISP3X_RAWAWB_YUV_INTERP_CURVE_TH0_3, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_th2_ls3, arg->sw_rawawb_th3_ls3),
                     ISP3X_RAWAWB_YUV_INTERP_CURVE_TH1_3, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_th4_ls3, arg->sw_rawawb_th5_ls3),
                     ISP3X_RAWAWB_YUV_INTERP_CURVE_TH2_3, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_wt0, arg->sw_rawawb_wt1), ISP3X_RAWAWB_RGB2XY_WT01,
                     id);

    isp3_param_write(params_vdev, arg->sw_rawawb_wt2, ISP3X_RAWAWB_RGB2XY_WT2, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_mat0_x, arg->sw_rawawb_mat0_y),
                     ISP3X_RAWAWB_RGB2XY_MAT0_XY, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_mat1_x, arg->sw_rawawb_mat1_y),
                     ISP3X_RAWAWB_RGB2XY_MAT1_XY, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_mat2_x, arg->sw_rawawb_mat2_y),
                     ISP3X_RAWAWB_RGB2XY_MAT2_XY, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_nor_x0_0, arg->sw_rawawb_nor_x1_0),
                     ISP3X_RAWAWB_XY_DETC_NOR_X_0, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_nor_y0_0, arg->sw_rawawb_nor_y1_0),
                     ISP3X_RAWAWB_XY_DETC_NOR_Y_0, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_big_x0_0, arg->sw_rawawb_big_x1_0),
                     ISP3X_RAWAWB_XY_DETC_BIG_X_0, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_big_y0_0, arg->sw_rawawb_big_y1_0),
                     ISP3X_RAWAWB_XY_DETC_BIG_Y_0, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_nor_x0_1, arg->sw_rawawb_nor_x1_1),
                     ISP3X_RAWAWB_XY_DETC_NOR_X_1, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_nor_y0_1, arg->sw_rawawb_nor_y1_1),
                     ISP3X_RAWAWB_XY_DETC_NOR_Y_1, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_big_x0_1, arg->sw_rawawb_big_x1_1),
                     ISP3X_RAWAWB_XY_DETC_BIG_X_1, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_big_y0_1, arg->sw_rawawb_big_y1_1),
                     ISP3X_RAWAWB_XY_DETC_BIG_Y_1, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_nor_x0_2, arg->sw_rawawb_nor_x1_2),
                     ISP3X_RAWAWB_XY_DETC_NOR_X_2, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_nor_y0_2, arg->sw_rawawb_nor_y1_2),
                     ISP3X_RAWAWB_XY_DETC_NOR_Y_2, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_big_x0_2, arg->sw_rawawb_big_x1_2),
                     ISP3X_RAWAWB_XY_DETC_BIG_X_2, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_big_y0_2, arg->sw_rawawb_big_y1_2),
                     ISP3X_RAWAWB_XY_DETC_BIG_Y_2, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_nor_x0_3, arg->sw_rawawb_nor_x1_3),
                     ISP3X_RAWAWB_XY_DETC_NOR_X_3, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_nor_y0_3, arg->sw_rawawb_nor_y1_3),
                     ISP3X_RAWAWB_XY_DETC_NOR_Y_3, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_big_x0_3, arg->sw_rawawb_big_x1_3),
                     ISP3X_RAWAWB_XY_DETC_BIG_X_3, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_big_y0_3, arg->sw_rawawb_big_y1_3),
                     ISP3X_RAWAWB_XY_DETC_BIG_Y_3, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_nor_x0_4, arg->sw_rawawb_nor_x1_4),
                     ISP3X_RAWAWB_XY_DETC_NOR_X_4, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_nor_y0_4, arg->sw_rawawb_nor_y1_4),
                     ISP3X_RAWAWB_XY_DETC_NOR_Y_4, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_big_x0_4, arg->sw_rawawb_big_x1_4),
                     ISP3X_RAWAWB_XY_DETC_BIG_X_4, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_big_y0_4, arg->sw_rawawb_big_y1_4),
                     ISP3X_RAWAWB_XY_DETC_BIG_Y_4, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_nor_x0_5, arg->sw_rawawb_nor_x1_5),
                     ISP3X_RAWAWB_XY_DETC_NOR_X_5, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_nor_y0_5, arg->sw_rawawb_nor_y1_5),
                     ISP3X_RAWAWB_XY_DETC_NOR_Y_5, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_big_x0_5, arg->sw_rawawb_big_x1_5),
                     ISP3X_RAWAWB_XY_DETC_BIG_X_5, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_big_y0_5, arg->sw_rawawb_big_y1_5),
                     ISP3X_RAWAWB_XY_DETC_BIG_Y_5, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_nor_x0_6, arg->sw_rawawb_nor_x1_6),
                     ISP3X_RAWAWB_XY_DETC_NOR_X_6, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_nor_y0_6, arg->sw_rawawb_nor_y1_6),
                     ISP3X_RAWAWB_XY_DETC_NOR_Y_6, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_big_x0_6, arg->sw_rawawb_big_x1_6),
                     ISP3X_RAWAWB_XY_DETC_BIG_X_6, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_big_y0_6, arg->sw_rawawb_big_y1_6),
                     ISP3X_RAWAWB_XY_DETC_BIG_Y_6, id);

    isp3_param_write(
        params_vdev,
        (arg->sw_rawawb_exc_wp_region0_excen0 & 0x1) << 0 | (arg->sw_rawawb_exc_wp_region0_excen1 & 0x1) << 1 |
            (arg->sw_rawawb_exc_wp_region0_measen & 0x1) << 2 | (arg->sw_rawawb_exc_wp_region0_domain & 0x1) << 3 |
            (arg->sw_rawawb_exc_wp_region1_excen0 & 0x1) << 4 | (arg->sw_rawawb_exc_wp_region1_excen1 & 0x1) << 5 |
            (arg->sw_rawawb_exc_wp_region1_measen & 0x1) << 6 | (arg->sw_rawawb_exc_wp_region1_domain & 0x1) << 7 |
            (arg->sw_rawawb_exc_wp_region2_excen0 & 0x1) << 8 | (arg->sw_rawawb_exc_wp_region2_excen1 & 0x1) << 9 |
            (arg->sw_rawawb_exc_wp_region2_measen & 0x1) << 10 | (arg->sw_rawawb_exc_wp_region2_domain & 0x1) << 11 |
            (arg->sw_rawawb_exc_wp_region3_excen0 & 0x1) << 12 | (arg->sw_rawawb_exc_wp_region3_excen1 & 0x1) << 13 |
            (arg->sw_rawawb_exc_wp_region3_measen & 0x1) << 14 | (arg->sw_rawawb_exc_wp_region3_domain & 0x1) << 15 |
            ...
            (arg->sw_rawawb_exc_wp_region6_domain & 0x1) << 27 | (arg->sw_rawawb_multiwindow_en & 0x1) << 31,
        ISP3X_RAWAWB_MULTIWINDOW_EXC_CTRL, id);

    isp3_param_write(params_vdev,
                     ISP_PACK_2SHORT(arg->sw_rawawb_multiwindow0_h_offs, arg->sw_rawawb_multiwindow0_v_offs),
                     ISP3X_RAWAWB_MULTIWINDOW0_OFFS, id);
    isp3_param_write(params_vdev,
                     ISP_PACK_2SHORT(arg->sw_rawawb_multiwindow0_h_size, arg->sw_rawawb_multiwindow0_v_size),
                     ISP3X_RAWAWB_MULTIWINDOW0_SIZE, id);
    isp3_param_write(params_vdev,
                     ISP_PACK_2SHORT(arg->sw_rawawb_multiwindow1_h_offs, arg->sw_rawawb_multiwindow1_v_offs),
                     ISP3X_RAWAWB_MULTIWINDOW1_OFFS, id);
    isp3_param_write(params_vdev,
                     ISP_PACK_2SHORT(arg->sw_rawawb_multiwindow1_h_size, arg->sw_rawawb_multiwindow1_v_size),
                     ISP3X_RAWAWB_MULTIWINDOW1_SIZE, id);
    isp3_param_write(params_vdev,
                     ISP_PACK_2SHORT(arg->sw_rawawb_multiwindow2_h_offs, arg->sw_rawawb_multiwindow2_v_offs),
                     ISP3X_RAWAWB_MULTIWINDOW2_OFFS, id);
    isp3_param_write(params_vdev,
                     ISP_PACK_2SHORT(arg->sw_rawawb_multiwindow2_h_size, arg->sw_rawawb_multiwindow2_v_size),
                     ISP3X_RAWAWB_MULTIWINDOW2_SIZE, id);
    isp3_param_write(params_vdev,
                     ISP_PACK_2SHORT(arg->sw_rawawb_multiwindow3_h_offs, arg->sw_rawawb_multiwindow3_v_offs),
                     ISP3X_RAWAWB_MULTIWINDOW3_OFFS, id);
    isp3_param_write(params_vdev,
                     ISP_PACK_2SHORT(arg->sw_rawawb_multiwindow3_h_size, arg->sw_rawawb_multiwindow3_v_size),
                     ISP3X_RAWAWB_MULTIWINDOW3_SIZE, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_exc_wp_region0_xu0, arg->sw_rawawb_exc_wp_region0_xu1),
                     ISP3X_RAWAWB_EXC_WP_REGION0_XU, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_exc_wp_region0_yv0, arg->sw_rawawb_exc_wp_region0_yv1),
                     ISP3X_RAWAWB_EXC_WP_REGION0_YV, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_exc_wp_region1_xu0, arg->sw_rawawb_exc_wp_region1_xu1),
                     ISP3X_RAWAWB_EXC_WP_REGION1_XU, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_exc_wp_region1_yv0, arg->sw_rawawb_exc_wp_region1_yv1),
                     ISP3X_RAWAWB_EXC_WP_REGION1_YV, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_exc_wp_region2_xu0, arg->sw_rawawb_exc_wp_region2_xu1),
                     ISP3X_RAWAWB_EXC_WP_REGION2_XU, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_exc_wp_region2_yv0, arg->sw_rawawb_exc_wp_region2_yv1),
                     ISP3X_RAWAWB_EXC_WP_REGION2_YV, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_exc_wp_region3_xu0, arg->sw_rawawb_exc_wp_region3_xu1),
                     ISP3X_RAWAWB_EXC_WP_REGION3_XU, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_exc_wp_region3_yv0, arg->sw_rawawb_exc_wp_region3_yv1),
                     ISP3X_RAWAWB_EXC_WP_REGION3_YV, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_exc_wp_region4_xu0, arg->sw_rawawb_exc_wp_region4_xu1),
                     ISP3X_RAWAWB_EXC_WP_REGION4_XU, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_exc_wp_region4_yv0, arg->sw_rawawb_exc_wp_region4_yv1),
                     ISP3X_RAWAWB_EXC_WP_REGION4_YV, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_exc_wp_region5_xu0, arg->sw_rawawb_exc_wp_region5_xu1),
                     ISP3X_RAWAWB_EXC_WP_REGION5_XU, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_exc_wp_region5_yv0, arg->sw_rawawb_exc_wp_region5_yv1),
                     ISP3X_RAWAWB_EXC_WP_REGION5_YV, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_exc_wp_region6_xu0, arg->sw_rawawb_exc_wp_region6_xu1),
                     ISP3X_RAWAWB_EXC_WP_REGION6_XU, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->sw_rawawb_exc_wp_region6_yv0, arg->sw_rawawb_exc_wp_region6_yv1),
...
static void isp_rawawb_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    u32 awb_ctrl;
...
static void isp_rawhstlite_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_rawhistlite_cfg *arg,
                                  u32 id)
{
    u32 i;
    u32 value;
    ...
    hist_ctrl = hist_ctrl | ISP3X_RAWHIST_MODE(arg->mode) | ISP3X_RAWHIST_DATASEL(arg->data_sel) |
                ISP3X_RAWHIST_WATERLINE(arg->waterline) | ISP3X_RAWHIST_STEPSIZE(arg->stepsize);
    isp3_param_write(params_vdev, hist_ctrl, ISP3X_RAWHIST_LITE_CTRL, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->win.h_offs, arg->win.v_offs), ISP3X_RAWHIST_LITE_OFFS, id);

    block_hsize = arg->win.h_size / ISP3X_RAWHISTLITE_ROW_NUM - 1;
    block_vsize = arg->win.v_size / ISP3X_RAWHISTLITE_COLUMN_NUM - 1;
    block_hsize &= 0xFFFE;
    block_vsize &= 0xFFFE;
    isp3_param_write(params_vdev, ISP_PACK_2SHORT(block_hsize, block_vsize), ISP3X_RAWHIST_LITE_SIZE, id);

    isp3_param_write(params_vdev, ISP_PACK_4BYTE(arg->rcc, arg->gcc, arg->bcc, arg->off), ISP3X_RAWHIST_LITE_RAW2Y_CC,
                     id);

    for (i = 0; i < (ISP3X_RAWHISTLITE_WEIGHT_REG_SIZE / 4); i++) {
        value = ISP_PACK_4BYTE(arg->weight[4 * i + 0], arg->weight[4 * i + 1], arg->weight[4 * i + 2],
                               arg->weight[4 * i + 3]);
        isp3_param_write(params_vdev, value, ISP3X_RAWHIST_LITE_WEIGHT + 4 * i, id);
    }
...
static void isp_rawhstlite_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    u32 hist_ctrl;
...
static void isp_rawhstbig_cfg_sram(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_rawhistbig_cfg *arg,
                                   u32 blk_no, bool is_check, u32 id)
{
    ...
    if (is_check && !(isp3_param_read(params_vdev, addr + ISP3X_RAWHIST_BIG_CTRL, id) & value)) {
        return;
    }
    ...
    for (i = 0; i < hist_wnd_num[wnd_num_idx]; i++) {
        for (j = 0; j < hist_wnd_num[wnd_num_idx]; j++) {
            weight15x15[i * ISP3X_RAWHISTBIG_ROW_NUM + j] = arg->weight[i * hist_wnd_num[wnd_num_idx] + j];
        }
    }

    for (i = 0; i < (ISP3X_RAWHISTBIG_WEIGHT_REG_SIZE / 5); i++) {
        value = (weight15x15[5 * i + 0] & 0x3f) | (weight15x15[5 * i + 1] & 0x3f) << 6 |
                (weight15x15[5 * i + 2] & 0x3f) << 12 | (weight15x15[5 * i + 3] & 0x3f) << 18 |
                (weight15x15[5 * i + 4] & 0x3f) << 24;
        isp3_param_write_direct(params_vdev, value, addr + ISP3X_RAWHIST_BIG_WEIGHT_BASE, id);
    }
}

static void isp_rawhstbig_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_rawhistbig_cfg *arg,
                                 u32 blk_no, u32 id)
{
    struct isp3x_isp_params_cfg *params_rec = params_vdev->isp3x_params + id;
    struct rkisp_device *dev = params_vdev->dev;
    ...
    hist_ctrl &= ISP3X_RAWHIST_EN;
    hist_ctrl = hist_ctrl | ISP3X_RAWHIST_MODE(arg->mode) | ISP3X_RAWHIST_DATASEL(arg->data_sel) |
                ISP3X_RAWHIST_WATERLINE(arg->waterline) | ISP3X_RAWHIST_WND_NUM(arg->wnd_num) |
                ISP3X_RAWHIST_STEPSIZE(arg->stepsize);
    isp3_param_write(params_vdev, hist_ctrl, addr + ISP3X_RAWHIST_BIG_CTRL, id);

    isp3_param_write(params_vdev, ISP_PACK_2SHORT(arg->win.h_offs, arg->win.v_offs), addr + ISP3X_RAWHIST_BIG_OFFS, id);

    block_hsize = arg->win.h_size / hist_wnd_num[wnd_num_idx] - 1;
    block_vsize = arg->win.v_size / hist_wnd_num[wnd_num_idx] - 1;
    block_hsize &= 0xFFFE;
    block_vsize &= 0xFFFE;
    isp3_param_write(params_vdev, ISP_PACK_2SHORT(block_hsize, block_vsize), addr + ISP3X_RAWHIST_BIG_SIZE, id);

    isp3_param_write(params_vdev, ISP_PACK_4BYTE(arg->rcc, arg->gcc, arg->bcc, arg->off),
...
static void isp_rawhstbig_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 blk_no, u32 id)
{
    u32 hist_ctrl;
    u32 addr;
...
static void isp_rawhst1_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_rawhistbig_cfg *arg,
                               u32 id)
{
    isp_rawhstbig_config(params_vdev, arg, 1, id);
}

static void isp_rawhst1_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
...
static void isp_rawhst2_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
...
static void isp_rawhst3_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    isp_rawhstbig_enable(params_vdev, en, 0, id);
}

static void isp_hdrmge_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp3x_hdrmge_cfg *arg,
                              enum rkisp_params_type type, u32 id)
{
    ...
    if (type == RKISP_PARAMS_IMD || type == RKISP_PARAMS_ALL) {
        value = ISP_PACK_4BYTE(arg->ms_dif_0p8, arg->ms_diff_0p15, arg->lm_dif_0p9, arg->lm_dif_0p15);
        isp3_param_write(params_vdev, value, ISP3X_HDRMGE_LIGHTZ, id);
        value = (arg->ms_scl & 0x7ff) | (arg->ms_thd0 & 0x3ff) << 12 | (arg->ms_thd1 & 0x3ff) << 22;
        isp3_param_write(params_vdev, value, ISP3X_HDRMGE_MS_DIFF, id);
        value = (arg->lm_scl & 0x7ff) | (arg->lm_thd0 & 0x3ff) << 12 | (arg->lm_thd1 & 0x3ff) << 22;
...
static void isp_hdrmge_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
}

static void isp_hdrdrc_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp3x_drc_cfg *arg,
                              enum rkisp_params_type type, u32 id)
{
    u32 i, value;
    ...
    value = (arg->offset_pow2 & 0x0F) << 28 | (arg->compres_scl & 0x1FFF) << 14 | (arg->position & 0x03FFF);
    isp3_param_write(params_vdev, value, ISP3X_DRC_CTRL1, id);

    value = (arg->delta_scalein & 0xFF) << 24 | (arg->hpdetail_ratio & 0xFFF) << 12 | (arg->lpdetail_ratio & 0xFFF);
    ...
    value = (arg->force_sgm_inv0 & 0xFFFF) << 16 | (arg->motion_scl & 0xFF) << 8 | (arg->edge_scl & 0xFF);
    ...
    value = (arg->weig_bilat & 0x1f) | (arg->weig_maxl & 0x1f) << 8 | (arg->bilat_soft_thd & 0x3fff) << 16;
    ...
    for (i = 0; i < ISP3X_DRC_Y_NUM / 2; i++) {
        value = ISP_PACK_2SHORT(arg->gain_y[2 * i], arg->gain_y[2 * i + 1]);
    ...
    for (i = 0; i < ISP3X_DRC_Y_NUM / 2; i++) {
        value = ISP_PACK_2SHORT(arg->compres_y[2 * i], arg->compres_y[2 * i + 1]);
    ...
    for (i = 0; i < ISP3X_DRC_Y_NUM / 2; i++) {
        value = ISP_PACK_2SHORT(arg->scale_y[2 * i], arg->scale_y[2 * i + 1]);
...
static void isp_hdrdrc_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    u32 value;
    bool real_en;
...
static void isp_gic_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp21_gic_cfg *arg, u32 id)
{
    u32 value;
    s32 i;

    value = (arg->regmingradthrdark2 & 0x03FF) << 20 | (arg->regmingradthrdark1 & 0x03FF) << 10 |
            (arg->regminbusythre & 0x03FF);
    isp3_param_write(params_vdev, value, ISP3X_GIC_DIFF_PARA1, id);

    value = (arg->regdarkthre & 0x07FF) << 21 | (arg->regmaxcorvboth & 0x03FF) << 11 | (arg->regdarktthrehi & 0x07FF);
    isp3_param_write(params_vdev, value, ISP3X_GIC_DIFF_PARA2, id);

    value = (arg->regkgrad2dark & 0x0F) << 28 | (arg->regkgrad1dark & 0x0F) << 24 |
            (arg->regstrengthglobal_fix & 0xFF) << 16 | (arg->regdarkthrestep & 0x0F) << 12 |
            (arg->regkgrad2 & 0x0F) << 8 | (arg->regkgrad1 & 0x0F) << 4 | (arg->reggbthre & 0x0F);
    isp3_param_write(params_vdev, value, ISP3X_GIC_DIFF_PARA3, id);

    value = (arg->regmaxcorv & 0x03FF) << 20 | (arg->regmingradthr2 & 0x03FF) << 10 | (arg->regmingradthr1 & 0x03FF);
    isp3_param_write(params_vdev, value, ISP3X_GIC_DIFF_PARA4, id);

    value = (arg->gr_ratio & 0x03) << 28 | (arg->noise_scale & 0x7F) << 12 | (arg->noise_base & 0xFFF);
...
static void isp_gic_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    u32 value = 0;
...
static void isp_dhaz_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp3x_dhaz_cfg *arg, u32 id)
{
    struct rkisp_device *dev = params_vdev->dev;
    u32 i, value, ctrl;
    ...
    ctrl |= (arg->enhance_en & 0x1) << 20 | (arg->air_lc_en & 0x1) << 16 | (arg->hpara_en & 0x1) << 12 |
            (arg->hist_en & 0x1) << 8 | (arg->dc_en & 0x1) << 4 | (arg->round_en & 0x1) << 26;
    if (arg->soft_wr_en) {
        ctrl |= (arg->soft_wr_en & 0x1) << 25;
    }
    /* merge dual unite isp params at frame end */
    if (arg->soft_wr_en && (!dev->hw_dev->is_unite || (dev->hw_dev->is_unite && !(ctrl & ISP3X_DHAZ_ENMUX)))) {
        ...
        for (i = 0; i < ISP3X_DHAZ_HIST_WR_NUM / 3; i++) {
            value = (arg->hist_wr[i * 3] & 0x3ff) | (arg->hist_wr[i * 3 + 1] & 0x3ff) << 10 |
    ...
    value = ISP_PACK_4BYTE(arg->dc_min_th, arg->dc_max_th, arg->yhist_th, arg->yblk_th);
    isp3_param_write(params_vdev, value, ISP3X_DHAZ_ADP0, id);

    value = ISP_PACK_4BYTE(arg->bright_min, arg->bright_max, arg->wt_max, 0);
    isp3_param_write(params_vdev, value, ISP3X_DHAZ_ADP1, id);

    value = ISP_PACK_4BYTE(arg->air_min, arg->air_max, arg->dark_th, arg->tmax_base);
    ...
    value = (arg->hist_min & 0xFFFF) << 16 | (arg->hist_th_off & 0xFF) << 8 | (arg->hist_k & 0x1F);
    ...
    value = (arg->iir_wt_sigma & 0x07FF) << 16 | (arg->iir_sigma & 0xFF) << 8 | (arg->stab_fnum & 0x1F);
    isp3_param_write(params_vdev, value, ISP3X_DHAZ_IIR0, id);

    value = (arg->iir_pre_wet & 0x0F) << 24 | (arg->iir_tmax_sigma & 0x7FF) << 8 | (arg->iir_air_sigma & 0xFF);
    isp3_param_write(params_vdev, value, ISP3X_DHAZ_IIR1, id);

    value = (arg->cfg_wt & 0x01FF) << 16 | (arg->cfg_air & 0xFF) << 8 | (arg->cfg_alpha & 0xFF);
    ...
    value = (arg->range_sima & 0x01FF) << 16 | (arg->space_sigma_pre & 0xFF) << 8 | (arg->space_sigma_cur & 0xFF);
    ...
    for (i = 0; i < ISP3X_DHAZ_SIGMA_IDX_NUM / 4; i++) {
        value = ISP_PACK_4BYTE(arg->sigma_idx[i * 4], arg->sigma_idx[i * 4 + 1], arg->sigma_idx[i * 4 + 2],
                               arg->sigma_idx[i * 4 + 3]);
        isp3_param_write(params_vdev, value, ISP3X_DHAZ_GAIN_IDX0 + i * 4, id);
    }
    value = ISP_PACK_4BYTE(arg->sigma_idx[i * 4], arg->sigma_idx[i * 4 + 1], arg->sigma_idx[i * 4 + 2], 0);
    isp3_param_write(params_vdev, value, ISP3X_DHAZ_GAIN_IDX0 + i * 4, id);
...
static void isp_dhaz_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    u32 value;
    bool real_en;
    ...
    if (en) {
        ...
        isp3_param_set_bits(params_vdev, ISP3X_ISP_CTRL1, ISP3X_DHAZ_FST_FRAME, id);
    } else {
...
static void isp_3dlut_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_3dlut_cfg *arg, u32 id)
{
    ...
    for (i = 0; i < arg->actual_size; i++) {
        data[i] = (arg->lut_b[i] & 0x3FF) | (arg->lut_g[i] & 0xFFF) << 10 | (arg->lut_r[i] & 0x3FF) << 22;
    }
...
static void isp_3dlut_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    u32 value;
    bool en_state;
...
static void isp_ldch_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_ldch_cfg *arg, u32 id)
{
    struct rkisp_device *dev = params_vdev->dev;
    struct rkisp_isp_params_val_v3x *priv_val;
...
static void isp_ldch_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    struct rkisp_device *dev = params_vdev->dev;
    struct rkisp_isp_params_val_v3x *priv_val;
    u32 buf_idx;
...
static void isp_ynr_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp3x_ynr_cfg *arg, u32 id)
{
    u32 i, value;
    ...
    value |= (arg->rnr_en & 0x1) << 26 | (arg->thumb_mix_cur_en & 0x1) << 24 | (arg->global_gain_alpha & 0xF) << 20 |
             (arg->global_gain & 0x3FF) << 8 | (arg->flt1x1_bypass_sel & 0x3) << 6 | (arg->sft5x5_bypass & 0x1) << 5 |
             (arg->flt1x1_bypass & 0x1) << 4 | (arg->lgft3x3_bypass & 0x1) << 3 | (arg->lbft5x5_bypass & 0x1) << 2 |
    ...
    value = (arg->low_bi_weight & 0xFF) << 24 | (arg->low_weight & 0xFF) << 16 | (arg->low_center_weight & 0xFFFF);
    ...
    value = ISP_PACK_4BYTE(arg->base_filter_weight0, arg->base_filter_weight1, arg->base_filter_weight2, 0);
    ...
    value = (arg->low_gauss1_coeff2 & 0xFFFF) << 16 | (arg->low_gauss1_coeff1 & 0xFF) << 8 |
            (arg->low_gauss1_coeff0 & 0xFF);
    isp3_param_write(params_vdev, value, ISP3X_YNR_GAUSS1_COEFF, id);

    value = (arg->low_gauss2_coeff2 & 0xFFFF) << 16 | (arg->low_gauss2_coeff1 & 0xFF) << 8 |
            (arg->low_gauss2_coeff0 & 0xFF);
    isp3_param_write(params_vdev, value, ISP3X_YNR_GAUSS2_COEFF, id);

    value =
        ISP_PACK_4BYTE(arg->direction_weight0, arg->direction_weight1, arg->direction_weight2, arg->direction_weight3);
    isp3_param_write(params_vdev, value, ISP3X_YNR_DIRECTION_W_0_3, id);

    value =
        ISP_PACK_4BYTE(arg->direction_weight4, arg->direction_weight5, arg->direction_weight6, arg->direction_weight7);
    isp3_param_write(params_vdev, value, ISP3X_YNR_DIRECTION_W_4_7, id);

    for (i = 0; i < ISP3X_YNR_XY_NUM / 2; i++) {
        value = ISP_PACK_2SHORT(arg->luma_points_x[2 * i], arg->luma_points_x[2 * i + 1]);
    ...
    for (i = 0; i < ISP3X_YNR_XY_NUM / 2; i++) {
        value = ISP_PACK_2SHORT(arg->lsgm_y[2 * i], arg->lsgm_y[2 * i + 1]);
    ...
    for (i = 0; i < ISP3X_YNR_XY_NUM / 2; i++) {
        value = ISP_PACK_2SHORT(arg->hsgm_y[2 * i], arg->hsgm_y[2 * i + 1]);
    ...
    for (i = 0; i < ISP3X_YNR_XY_NUM / 4; i++) {
        value = ISP_PACK_4BYTE(arg->rnr_strength3[4 * i], arg->rnr_strength3[4 * i + 1], arg->rnr_strength3[4 * i + 2],
...
static void isp_ynr_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    u32 ynr_ctrl;
    bool real_en;
...
static void isp_cnr_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp3x_cnr_cfg *arg, u32 id)
{
    ...
    ctrl |= (arg->thumb_mix_cur_en & 0x1) << 4 | (arg->lq_bila_bypass & 0x1) << 3 | (arg->hq_bila_bypass & 0x1) << 2 |
            (arg->exgain_bypass & 0x1) << 1;
    value = (arg->global_gain & 0x3ff) | (arg->global_gain_alpha & 0xf) << 12;
    ...
    value = ISP_PACK_4BYTE(arg->gain_1sigma, arg->gain_offset, arg->gain_iso, 0);
    ...
    value = ISP_PACK_4BYTE(arg->lbf5_weit_d0, arg->lbf5_weit_d1, arg->lbf5_weit_d2, arg->lbf5_weit_d3);
    ...
    value = (arg->hbf5_weit_src & 0xFF) << 24 | (arg->hbf5_min_wgt & 0xFF) << 16 | (arg->hbf5_sigma & 0xFFFF);
    ...
    for (i = 0; i < ISP3X_CNR_SIGMA_Y_NUM / 4; i++) {
        value = ISP_PACK_4BYTE(arg->sigma_y[i * 4], arg->sigma_y[i * 4 + 1], arg->sigma_y[i * 4 + 2],
                               arg->sigma_y[i * 4 + 3]);
...
static void isp_cnr_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    u32 cnr_ctrl;
    bool real_en;
    ...
    if (en) {
        ...
        isp3_param_set_bits(params_vdev, ISP3X_ISP_CTRL1, ISP3X_CNR_FST_FRAME, id);
    } else {
        cnr_ctrl &= ~ISP3X_MODULE_EN;
    }
...
static void isp_sharp_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp3x_sharp_cfg *arg, u32 id)
{
    u32 value;
    ...
    value |= (arg->bypass & 0x1) << 1 | (arg->center_mode & 0x1) << 2 | (arg->exgain_bypass & 0x1) << 3;
    isp3_param_write(params_vdev, value, ISP3X_SHARP_EN, id);

    value = ISP_PACK_4BYTE(arg->pbf_ratio, arg->gaus_ratio, arg->bf_ratio, arg->sharp_ratio);
    isp3_param_write(params_vdev, value, ISP3X_SHARP_RATIO, id);

    value = (arg->luma_dx[6] & 0x0F) << 24 | (arg->luma_dx[5] & 0x0F) << 20 | (arg->luma_dx[4] & 0x0F) << 16 |
            (arg->luma_dx[3] & 0x0F) << 12 | (arg->luma_dx[2] & 0x0F) << 8 | (arg->luma_dx[1] & 0x0F) << 4 |
            (arg->luma_dx[0] & 0x0F);
    isp3_param_write(params_vdev, value, ISP3X_SHARP_LUMA_DX, id);

    value =
        (arg->pbf_sigma_inv[2] & 0x3FF) << 20 | (arg->pbf_sigma_inv[1] & 0x3FF) << 10 | (arg->pbf_sigma_inv[0] & 0x3FF);
    isp3_param_write(params_vdev, value, ISP3X_SHARP_PBF_SIGMA_INV_0, id);

    value =
        (arg->pbf_sigma_inv[5] & 0x3FF) << 20 | (arg->pbf_sigma_inv[4] & 0x3FF) << 10 | (arg->pbf_sigma_inv[3] & 0x3FF);
    isp3_param_write(params_vdev, value, ISP3X_SHARP_PBF_SIGMA_INV_1, id);

    value = (arg->pbf_sigma_inv[7] & 0x3FF) << 10 | (arg->pbf_sigma_inv[6] & 0x3FF);
    isp3_param_write(params_vdev, value, ISP3X_SHARP_PBF_SIGMA_INV_2, id);

    value =
        (arg->bf_sigma_inv[2] & 0x3FF) << 20 | (arg->bf_sigma_inv[1] & 0x3FF) << 10 | (arg->bf_sigma_inv[0] & 0x3FF);
    isp3_param_write(params_vdev, value, ISP3X_SHARP_BF_SIGMA_INV_0, id);

    value =
        (arg->bf_sigma_inv[5] & 0x3FF) << 20 | (arg->bf_sigma_inv[4] & 0x3FF) << 10 | (arg->bf_sigma_inv[3] & 0x3FF);
    isp3_param_write(params_vdev, value, ISP3X_SHARP_BF_SIGMA_INV_1, id);

    value = (arg->bf_sigma_inv[7] & 0x3FF) << 10 | (arg->bf_sigma_inv[6] & 0x3FF);
    isp3_param_write(params_vdev, value, ISP3X_SHARP_BF_SIGMA_INV_2, id);

    value = (arg->bf_sigma_shift & 0x0F) << 4 | (arg->pbf_sigma_shift & 0x0F);
    isp3_param_write(params_vdev, value, ISP3X_SHARP_SIGMA_SHIFT, id);

    value = (arg->ehf_th[2] & 0x3FF) << 20 | (arg->ehf_th[1] & 0x3FF) << 10 | (arg->ehf_th[0] & 0x3FF);
    isp3_param_write(params_vdev, value, ISP3X_SHARP_EHF_TH_0, id);

    value = (arg->ehf_th[5] & 0x3FF) << 20 | (arg->ehf_th[4] & 0x3FF) << 10 | (arg->ehf_th[3] & 0x3FF);
    isp3_param_write(params_vdev, value, ISP3X_SHARP_EHF_TH_1, id);

    value = (arg->ehf_th[7] & 0x3FF) << 10 | (arg->ehf_th[6] & 0x3FF);
    isp3_param_write(params_vdev, value, ISP3X_SHARP_EHF_TH_2, id);

    value = (arg->clip_hf[2] & 0x3FF) << 20 | (arg->clip_hf[1] & 0x3FF) << 10 | (arg->clip_hf[0] & 0x3FF);
    isp3_param_write(params_vdev, value, ISP3X_SHARP_CLIP_HF_0, id);

    value = (arg->clip_hf[5] & 0x3FF) << 20 | (arg->clip_hf[4] & 0x3FF) << 10 | (arg->clip_hf[3] & 0x3FF);
    isp3_param_write(params_vdev, value, ISP3X_SHARP_CLIP_HF_1, id);

    value = (arg->clip_hf[7] & 0x3FF) << 10 | (arg->clip_hf[6] & 0x3FF);
...
static void isp_sharp_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    u32 value;
...
static void isp_baynr_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp3x_baynr_cfg *arg, u32 id)
{
    u32 i, value;
    ...
    value |= (arg->lg2_mode & 0x3) << 12 | (arg->gauss_en & 0x1) << 8 | (arg->log_bypass & 0x1) << 4;
    ...
    value = (arg->weit_d2 & 0x3FF) << 20 | (arg->weit_d1 & 0x3FF) << 10 | (arg->weit_d0 & 0x3FF);
...
static void isp_baynr_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    u32 value;
...
static void isp_bay3d_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp3x_bay3d_cfg *arg, u32 id)
{
    struct rkisp_device *dev = params_vdev->dev;
    u32 i, value;
    ...
    value |= (arg->loswitch_protect & 0x1) << 12 | (arg->glbpk_en & 0x1) << 11 | (arg->logaus3_bypass_en & 0x1) << 10 |
             (arg->logaus5_bypass_en & 0x1) << 9 | (arg->lomed_bypass_en & 0x1) << 8 | (arg->hichnsplit_en & 0x1) << 7 |
             (arg->hiabs_possel & 0x1) << 6 | (arg->higaus_bypass_en & 0x1) << 5 | (arg->himed_bypass_en & 0x1) << 4 |
             (arg->lobypass_en & 0x1) << 3 | (arg->hibypass_en & 0x1) << 2 | (arg->bypass_en & 0x1) << 1;
    ...
    for (i = 0; i < ISP3X_BAY3D_XY_NUM / 2; i++) {
        value = ISP_PACK_2SHORT(arg->sig0_x[2 * i], arg->sig0_x[2 * i + 1]);
        isp3_param_write(params_vdev, value, ISP3X_BAY3D_SIG0_X0 + 4 * i, id);

        value = ISP_PACK_2SHORT(arg->sig1_x[2 * i], arg->sig1_x[2 * i + 1]);
        isp3_param_write(params_vdev, value, ISP3X_BAY3D_SIG1_X0 + 4 * i, id);
    }

    for (i = 0; i < ISP3X_BAY3D_XY_NUM / 2; i++) {
        value = ISP_PACK_2SHORT(arg->sig0_y[2 * i], arg->sig0_y[2 * i + 1]);
        isp3_param_write(params_vdev, value, ISP3X_BAY3D_SIG0_Y0 + 4 * i, id);

        value = ISP_PACK_2SHORT(arg->sig1_y[2 * i], arg->sig1_y[2 * i + 1]);
        isp3_param_write(params_vdev, value, ISP3X_BAY3D_SIG1_Y0 + 4 * i, id);

        value = ISP_PACK_2SHORT(arg->sig2_y[2 * i], arg->sig2_y[2 * i + 1]);
        isp3_param_write(params_vdev, value, ISP3X_BAY3D_SIG2_Y0 + 4 * i, id);
    }
}

static void isp_bay3d_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    struct rkisp_device *ispdev = params_vdev->dev;
    struct rkisp_isp_params_val_v3x *priv_val;
    ...
    if ((en && (bay3d_ctrl & ISP3X_MODULE_EN)) || (!en && !(bay3d_ctrl & ISP3X_MODULE_EN))) {
        return;
    }
    ...
    if (en) {
        ...
        value = ISP3X_BAY3D_IIR_WR_AUTO_UPD | ISP3X_BAY3D_CUR_WR_AUTO_UPD | ISP3X_BAY3D_DS_WR_AUTO_UPD |
                ISP3X_BAY3D_IIRSELF_UPD | ISP3X_BAY3D_CURSELF_UPD | ISP3X_BAY3D_DSSELF_UPD | ISP3X_BAY3D_RDSELF_UPD;
...
static void isp_gain_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp3x_gain_cfg *arg, u32 id)
{
    u32 val;
...
static void isp_gain_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    struct rkisp_isp_params_val_v3x *priv_val = (struct rkisp_isp_params_val_v3x *)params_vdev->priv_val;
    ...
    val = 0;
    if (en) {
        val |= priv_val->lut3d_en << 20 | priv_val->dhaz_en << 16 | priv_val->drc_en << 12 | priv_val->lsc_en << 8 |
               priv_val->bay3d_en << 4;
...
static void isp_cac_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp3x_cac_cfg *arg, u32 id)
{
    struct rkisp_device *dev = params_vdev->dev;
    struct rkisp_isp_params_val_v3x *priv_val;
    struct isp2x_mesh_head *head;
...
static void isp_cac_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
    u32 val;
...
static void isp_csm_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp21_csm_cfg *arg, u32 id)
{
    u32 i, val, eff_ctrl, cproc_ctrl;

    for (i = 0; i < ISP3X_CSM_COEFF_NUM; i++) {
        if (i == 0) {
            val = (arg->csm_y_offset & 0x3f) << 24 | (arg->csm_c_offset & 0xff) << 16 | (arg->csm_coeff[i] & 0x1ff);
        } else {
...
static __maybe_unused void __isp_isr_other_config(struct rkisp_isp_params_vdev *params_vdev,
                                                  const struct isp3x_isp_params_cfg *new_params,
                                                  enum rkisp_params_type type, enum isp3x_unite_id id)
{
    struct rkisp_isp_params_ops_v3x *ops = (struct rkisp_isp_params_ops_v3x *)params_vdev->priv_ops;
    ...
    v4l2_dbg(4, rkisp_debug, &params_vdev->dev->v4l2_dev, "%s id:%d seq:%d module_cfg_update:0x%llx\n", __func__, id,
             new_params->frame_id, module_cfg_update);
...
static __maybe_unused void __isp_isr_other_en(struct rkisp_isp_params_vdev *params_vdev,
                                              const struct isp3x_isp_params_cfg *new_params,
                                              enum rkisp_params_type type, enum isp3x_unite_id id)
{
    struct rkisp_isp_params_ops_v3x *ops = (struct rkisp_isp_params_ops_v3x *)params_vdev->priv_ops;
    ...
    v4l2_dbg(4, rkisp_debug, &params_vdev->dev->v4l2_dev, "%s id:%d seq:%d module_en_update:0x%llx module_ens:0x%llx\n",
...
static __maybe_unused void __isp_isr_meas_config(struct rkisp_isp_params_vdev *params_vdev,
                                                 struct isp3x_isp_params_cfg *new_params, enum rkisp_params_type type,
                                                 enum isp3x_unite_id id)
{
    struct rkisp_isp_params_ops_v3x *ops = (struct rkisp_isp_params_ops_v3x *)params_vdev->priv_ops;
    u64 module_cfg_update = new_params->module_cfg_update;
...
static __maybe_unused void __isp_isr_meas_en(struct rkisp_isp_params_vdev *params_vdev,
                                             struct isp3x_isp_params_cfg *new_params, enum rkisp_params_type type,
                                             enum isp3x_unite_id id)
{
    struct rkisp_isp_params_ops_v3x *ops = (struct rkisp_isp_params_ops_v3x *)params_vdev->priv_ops;
...
static __maybe_unused void __isp_config_hdrshd(struct rkisp_isp_params_vdev *params_vdev)
{
    struct rkisp_isp_params_ops_v3x *ops = (struct rkisp_isp_params_ops_v3x *)params_vdev->priv_ops;
    struct rkisp_isp_params_val_v3x *priv_val = (struct rkisp_isp_params_val_v3x *)params_vdev->priv_val;
...
static void rkisp_params_cfgsram_v3x(struct rkisp_isp_params_vdev *params_vdev)
{
    struct isp3x_isp_params_cfg *params = params_vdev->isp3x_params;
...
static int rkisp_alloc_internal_buf(struct rkisp_isp_params_vdev *params_vdev,
                                    const struct isp3x_isp_params_cfg *new_params)
{
    struct rkisp_device *ispdev = params_vdev->dev;
    ...
    if ((module_en_update & ISP3X_MODULE_BAY3D) && (module_ens & ISP3X_MODULE_BAY3D)) {
...
static void rkisp_params_first_cfg_v3x(struct rkisp_isp_params_vdev *params_vdev)
{
    ...
    if (!params_vdev->isp3x_params->module_cfg_update && !params_vdev->isp3x_params->module_en_update) {
    ...
    if (hw->is_unite) {
        ...
        if (width > bigmode_max_w || size > bigmode_max_size) {
            ...
            rkisp_next_set_bits(params_vdev->dev, ISP3X_ISP_CTRL1, 0, ISP3X_BIGMODE_MANUAL | ISP3X_BIGMODE_FORCE_EN,
                                false);
        }
    ...
    if (width > bigmode_max_w || size > bigmode_max_size) {
        priv_val->is_bigmode = true;
        rkisp_set_bits(params_vdev->dev, ISP3X_ISP_CTRL1, 0, ISP3X_BIGMODE_MANUAL | ISP3X_BIGMODE_FORCE_EN, false);
    }
...
static void rkisp_deinit_mesh_buf(struct rkisp_isp_params_vdev *params_vdev, u64 module_id, u32 id)
{
    struct rkisp_isp_params_val_v3x *priv_val;
...
static int rkisp_init_mesh_buf(struct rkisp_isp_params_vdev *params_vdev, struct rkisp_meshbuf_size *meshsize)
{
    struct rkisp_device *ispdev = params_vdev->dev;
...
static void rkisp_get_param_size_v3x(struct rkisp_isp_params_vdev *params_vdev, unsigned int sizes[])
{
    u32 mult = params_vdev->dev->hw_dev->is_unite ? ISP3_UNITE_MAX : 1;
...
static void rkisp_params_get_meshbuf_inf_v3x(struct rkisp_isp_params_vdev *params_vdev, void *meshbuf_inf)
{
    struct rkisp_isp_params_val_v3x *priv_val;
...
static void rkisp_params_set_meshbuf_size_v3x(struct rkisp_isp_params_vdev *params_vdev, void *size)
{
...
static void rkisp_params_stream_stop_v3x(struct rkisp_isp_params_vdev *params_vdev)
{
    struct rkisp_device *ispdev = params_vdev->dev;
    struct rkisp_isp_params_val_v3x *priv_val;
    u32 id, i;
...
static void rkisp_params_fop_release_v3x(struct rkisp_isp_params_vdev *params_vdev)
{
    int id;
...
/* Not called when the camera active, thus not isr protection. */
static void rkisp_params_disable_isp_v3x(struct rkisp_isp_params_vdev *params_vdev)
{
...
static void module_data_abandon(struct rkisp_isp_params_vdev *params_vdev, struct isp3x_isp_params_cfg *params, u32 id)
{
    struct rkisp_isp_params_val_v3x *priv_val;
    struct isp2x_mesh_head *mesh_head;
    int i;
    ...
    if (params->module_cfg_update & ISP3X_MODULE_LDCH) {
        ...
        for (i = 0; i < ISP3X_MESH_BUF_NUM; i++) {
            if (arg->buf_fd == priv_val->buf_ldch[id][i].dma_fd && priv_val->buf_ldch[id][i].vaddr) {
    ...
    if (params->module_cfg_update & ISP3X_MODULE_CAC) {
        ...
        for (i = 0; i < ISP3X_MESH_BUF_NUM; i++) {
            if (arg->buf_fd == priv_val->buf_cac[id][i].dma_fd && priv_val->buf_cac[id][i].vaddr) {
...
static void rkisp_params_cfg_v3x(struct rkisp_isp_params_vdev *params_vdev, u32 frame_id, enum rkisp_params_type type)
{
    ...
    while (!list_empty(&params_vdev->params) && !cur_buf) {
        ...
        if (new_params->frame_id < frame_id) {
            ...
            if (new_params->module_cfg_update & (ISP3X_MODULE_LDCH | ISP3X_MODULE_CAC)) {
...
static void rkisp_params_clear_fstflg(struct rkisp_isp_params_vdev *params_vdev)
{
    struct rkisp_device *dev = params_vdev->dev;
    struct rkisp_hw_dev *hw_dev = dev->hw_dev;
    u32 value;
    ...
    if (value & ISP3X_YNR_FST_FRAME) {
        rkisp_clear_bits(params_vdev->dev, ISP3X_ISP_CTRL1, ISP3X_YNR_FST_FRAME, false);
    }
    if (value & ISP3X_ADRC_FST_FRAME) {
        rkisp_clear_bits(params_vdev->dev, ISP3X_ISP_CTRL1, ISP3X_ADRC_FST_FRAME, false);
    }
    if (value & ISP3X_DHAZ_FST_FRAME) {
        rkisp_clear_bits(params_vdev->dev, ISP3X_ISP_CTRL1, ISP3X_DHAZ_FST_FRAME, false);
    }
    if (value & ISP3X_CNR_FST_FRAME) {
        rkisp_clear_bits(params_vdev->dev, ISP3X_ISP_CTRL1, ISP3X_CNR_FST_FRAME, false);
    }
    if (value & ISP3X_RAW3D_FST_FRAME) {
        rkisp_clear_bits(params_vdev->dev, ISP3X_ISP_CTRL1, ISP3X_RAW3D_FST_FRAME, false);
    }
    if (hw_dev->is_unite) {
        value = rkisp_next_read(dev, ISP3X_ISP_CTRL1, false);
        if (value & ISP3X_YNR_FST_FRAME) {
            rkisp_next_clear_bits(params_vdev->dev, ISP3X_ISP_CTRL1, ISP3X_YNR_FST_FRAME, false);
        }
        if (value & ISP3X_ADRC_FST_FRAME) {
            rkisp_next_clear_bits(params_vdev->dev, ISP3X_ISP_CTRL1, ISP3X_ADRC_FST_FRAME, false);
        }
        if (value & ISP3X_DHAZ_FST_FRAME) {
            rkisp_next_clear_bits(params_vdev->dev, ISP3X_ISP_CTRL1, ISP3X_DHAZ_FST_FRAME, false);
        }
        if (value & ISP3X_CNR_FST_FRAME) {
            rkisp_next_clear_bits(params_vdev->dev, ISP3X_ISP_CTRL1, ISP3X_CNR_FST_FRAME, false);
        }
        if (value & ISP3X_RAW3D_FST_FRAME) {
            rkisp_next_clear_bits(params_vdev->dev, ISP3X_ISP_CTRL1, ISP3X_RAW3D_FST_FRAME, false);
        }
    }
}

static void rkisp_params_isr_v3x(struct rkisp_isp_params_vdev *params_vdev, u32 isp_mis)
{
    struct rkisp_device *dev = params_vdev->dev;
    u32 cur_frame_id;
...
int rkisp_init_params_vdev_v3x(struct rkisp_isp_params_vdev *params_vdev)
{
    ...
    tasklet_init(&priv_val->lsc_tasklet, isp_lsc_cfg_sram_task, (unsigned long)params_vdev);
    tasklet_disable(&priv_val->lsc_tasklet);
    return 0;
}
