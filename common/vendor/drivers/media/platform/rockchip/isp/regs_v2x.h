...
#define ISP_ISP_LSC_TABLE_DATA(v0, v1) (((v0)&0x1FFF) | (((v1)&0x1FFF) << 16))
...
static inline void raw_wr_set_pic_size(struct rkisp_stream *stream, u32 width, u32 height)
{
...
static inline void mi_raw_length(struct rkisp_stream *stream)
{
    ...
    if (stream->config->mi.length == MI_RAW0_RD_LENGTH || stream->config->mi.length == MI_RAW1_RD_LENGTH ||
