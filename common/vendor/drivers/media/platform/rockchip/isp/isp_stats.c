...
static int rkisp_stats_enum_fmt_meta_cap(struct file *file, void *priv, struct v4l2_fmtdesc *f)
{
...
static int rkisp_stats_g_fmt_meta_cap(struct file *file, void *priv, struct v4l2_format *f)
{
    struct video_device *video = video_devdata(file);
    struct rkisp_isp_stats_vdev *stats_vdev = video_get_drvdata(video);
...
static int rkisp_stats_querycap(struct file *file, void *priv, struct v4l2_capability *cap)
{
...
static const struct v4l2_ioctl_ops rkisp_stats_ioctl = {.vidioc_reqbufs = vb2_ioctl_reqbufs,
...
struct v4l2_file_operations rkisp_stats_fops = {.mmap = vb2_fop_mmap,
                                                .unlocked_ioctl = video_ioctl2,
                                                .poll = vb2_fop_poll,
                                                .open = rkisp_stats_fh_open,
                                                .release = rkisp_stats_fop_release};

static int rkisp_stats_vb2_queue_setup(struct vb2_queue *vq, unsigned int *num_buffers, unsigned int *num_planes,
                                       unsigned int sizes[], struct device *alloc_ctxs[])
{
    ...
    *num_buffers = clamp_t(u32, *num_buffers, RKISP_ISP_STATS_REQ_BUFS_MIN, RKISP_ISP_STATS_REQ_BUFS_MAX);
...
static void rkisp_stats_vb2_stop_streaming(struct vb2_queue *vq)
{
    ...
    for (i = 0; i < RKISP_ISP_STATS_REQ_BUFS_MAX; i++) {
        ...
        buf = list_first_entry(&stats_vdev->stat, struct rkisp_buffer, queue);
        list_del(&buf->queue);
...
static int rkisp_stats_vb2_start_streaming(struct vb2_queue *queue, unsigned int count)
{
...
static int rkisp_stats_init_vb2_queue(struct vb2_queue *q, struct rkisp_isp_stats_vdev *stats_vdev)
{
...
static void rkisp_stats_readout_task(unsigned long data)
{
    ...
    struct rkisp_isp_stats_vdev *vdev = (struct rkisp_isp_stats_vdev *)data;
                              ...
                              struct rkisp_device *dev)
{
    ...
    ret = kfifo_alloc(&stats_vdev->rd_kfifo, RKISP_READOUT_WORK_SIZE, GFP_KERNEL);
    if (ret) {
        dev_err(&vdev->dev, "kfifo_alloc failed with error %d\n", ret);
        goto err_unregister_video;
    }

    tasklet_init(&stats_vdev->rd_tasklet, rkisp_stats_readout_task, (unsigned long)stats_vdev);
    tasklet_disable(&stats_vdev->rd_tasklet);
