...
static int rkisp_stream_config_rsz(struct rkisp_stream *stream, bool async)
{
    ...
    if (input_isp_fmt->fmt_type == FMT_BAYER || stream->id == RKISP_STREAM_FBC || stream->id == RKISP_STREAM_BP) {
        goto disable;
    }
...
static int mp_config_mi(struct rkisp_stream *stream)
{
    ...
    val = out_fmt->plane_fmt[0x02].sizeimage;
    rkisp_unite_write(dev, stream->config->mi.cr_size_init, val, false, is_unite);

    val = is_unite ? out_fmt->width / 0x02 : out_fmt->width;
    ...
    val = ALIGN(out_fmt->plane_fmt[0].bytesperline, 0x10);
    ...
    if (stream->out_fmt.pixelformat == V4L2_PIX_FMT_NV21 || stream->out_fmt.pixelformat == V4L2_PIX_FMT_NV12 ||
        stream->out_fmt.pixelformat == V4L2_PIX_FMT_NV21M || stream->out_fmt.pixelformat == V4L2_PIX_FMT_NV12M ||
    ...
    val = calc_burst_len(stream) | CIF_MI_CTRL_INIT_BASE_EN | CIF_MI_CTRL_INIT_OFFSET_EN | CIF_MI_MP_AUTOUPDATE_ENABLE |
          stream->out_isp_fmt.write_format;
    mask = GENMASK(0x13, 0x10) | MI_CTRL_MP_FMT_MASK;
...
static int sp_config_mi(struct rkisp_stream *stream)
{
    ...
    if (mbus_code_sp_in_fmt(input_isp_fmt->mbus_code, out_fmt->pixelformat, &sp_in_fmt)) {
    ...
    val = calc_burst_len(stream) | CIF_MI_CTRL_INIT_BASE_EN | CIF_MI_CTRL_INIT_OFFSET_EN |
          stream->out_isp_fmt.write_format | sp_in_fmt | stream->out_isp_fmt.output_format |
          CIF_MI_SP_AUTOUPDATE_ENABLE;
    mask = GENMASK(0x13, 0x10) | MI_CTRL_SP_FMT_MASK;
    rkisp_unite_set_bits(dev, ISP3X_MI_WR_CTRL, mask, val, false, is_unite);
...
static int fbc_config_mi(struct rkisp_stream *stream)
{
    ...
    rkisp_unite_set_bits(stream->ispdev, ISP3X_MI_WR_CTRL, 0, CIF_MI_CTRL_INIT_BASE_EN | CIF_MI_CTRL_INIT_OFFSET_EN,
                         false, is_unite);
    mi_frame_end_int_enable(stream);
    /* set up first buffer */
    mi_frame_end(stream);
...
static int bp_config_mi(struct rkisp_stream *stream)
{
    ...
    if (out_fmt->pixelformat == V4L2_PIX_FMT_NV12 || out_fmt->pixelformat == V4L2_PIX_FMT_NV12M) {
        val |= ISP3X_SEPERATE_YUV_CFG;
    } else {
...
static void mp_enable_mi(struct rkisp_stream *stream)
{
    ...
    rkisp_unite_set_bits(stream->ispdev, ISP3X_MI_WR_CTRL, mask, val, false, stream->ispdev->hw_dev->is_unite);
}

static void sp_enable_mi(struct rkisp_stream *stream)
{
    rkisp_unite_set_bits(stream->ispdev, ISP3X_MI_WR_CTRL, 0, CIF_MI_CTRL_SP_ENABLE, false,
                         stream->ispdev->hw_dev->is_unite);
}

static void fbc_enable_mi(struct rkisp_stream *stream)
{
    u32 val, mask = ISP3X_MPFBC_FORCE_UPD | ISP3X_MPFBC_YUV_MASK | ISP3X_MPFBC_SPARSE_MODE;
    bool is_unite = stream->ispdev->hw_dev->is_unite;
...
{
    u32 val = stream->out_isp_fmt.write_format | ISP3X_BP_ENABLE | ISP3X_BP_AUTO_UPD;

    rkisp_unite_write(stream->ispdev, ISP3X_MI_BP_WR_CTRL, val, false, stream->ispdev->hw_dev->is_unite);
}
...
static void mp_disable_mi(struct rkisp_stream *stream)
{
    ...
    rkisp_unite_clear_bits(stream->ispdev, ISP3X_MI_WR_CTRL, mask, false, stream->ispdev->hw_dev->is_unite);
}
...
static void fbc_disable_mi(struct rkisp_stream *stream)
{
    ...
    rkisp_unite_clear_bits(stream->ispdev, ISP3X_MPFBC_CTRL, mask, false, stream->ispdev->hw_dev->is_unite);
}
...
static void update_mi(struct rkisp_stream *stream)
{
    ...
    if (stream->next_buf) {
        ...
        if (dev->hw_dev->is_unite) {
            u32 mult = stream->id != RKISP_STREAM_FBC ? 1 : (stream->out_isp_fmt.write_format ? 32 : 24);

            reg = stream->config->mi.y_base_ad_init;
            val = stream->next_buf->buff_addr[RKISP_PLANE_Y];
            val += ((stream->out_fmt.width / 0x02) & ~0xf);
            rkisp_next_write(dev, reg, val, false);

            reg = stream->config->mi.cb_base_ad_init;
            val = stream->next_buf->buff_addr[RKISP_PLANE_CB];
            val += ((stream->out_fmt.width / 0x02) & ~0xf) * mult;
            rkisp_next_write(dev, reg, val, false);
    ...
    v4l2_dbg(0x02, rkisp_debug, &dev->v4l2_dev, "%s stream:%d Y:0x%x CB:0x%x | Y_SHD:0x%x\n", __func__, stream->id,
    ...
    if (dev->hw_dev->is_unite) {
        v4l2_dbg(0x02, rkisp_debug, &dev->v4l2_dev, "%s stream:%d Y:0x%x CB:0x%x | Y_SHD:0x%x, right\n", __func__,
                 stream->id, rkisp_next_read(dev, stream->config->mi.y_base_ad_init, false),
...
static void rkisp_buf_queue(struct vb2_buffer *vb)
{
    ...
    if (isp_fmt->mplanes == 1) {
        for (i = 0; i < isp_fmt->cplanes - 1; i++) {
            ...
            height = (isp_fmt->fmt_type == FMT_FBC) ? ALIGN(pixm->height, 0x10) >> 0x04 : pixm->height;
            size = (i == 0) ? pixm->plane_fmt[i].bytesperline * height : pixm->plane_fmt[i].sizeimage;
            offset = (isp_fmt->fmt_type == FMT_FBC) ? ALIGN(size, RK_MPP_ALIGN) : size;
            ispbuf->buff_addr[i + 1] = ispbuf->buff_addr[i] + offset;
        }
    }
...
static void rkisp_stop_streaming(struct vb2_queue *queue)
{
    ...
    if (stream->id == RKISP_STREAM_VIR) {
        stream->stopping = true;
        wait_event_timeout(stream->done, stream->frame_end, msecs_to_jiffies(0x1F4));
        stream->streaming = false;
        stream->stopping = false;
...
static void vir_cpy_image(struct work_struct *work)
{
    ...
    v4l2_dbg(1, rkisp_debug, &vir->ispdev->v4l2_dev, "%s enter\n", __func__);
    ...
    while (src_buf || vir->streaming) {
        ...
        if (src_buf && !vir->curr_buf && !list_empty(&vir->buf_queue)) {
            vir->curr_buf = list_first_entry(&vir->buf_queue, struct rkisp_buffer, queue);
    ...
    v4l2_dbg(1, rkisp_debug, &vir->ispdev->v4l2_dev, "%s exit\n", __func__);
}
...
static int rkisp_stream_start(struct rkisp_stream *stream)
{
    ...
    async = (stream->id == RKISP_STREAM_MP) ? dev->cap_dev.stream[RKISP_STREAM_SP].streaming
...
static int rkisp_start_streaming(struct vb2_queue *queue, unsigned int count)
{
    ...
    if (atomic_read(&dev->cap_dev.refcnt) == 1 && (dev->isp_inp & INP_CIF)) {
...
void rkisp_mipi_v30_isr(unsigned int phy, unsigned int packet, unsigned int overflow, unsigned int state,
                        struct rkisp_device *dev)
{
    if (state & GENMASK(0x13, 0x11)) {
