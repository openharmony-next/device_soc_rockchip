               ...
               bool try)
{
    ...
    if (!fmt) {
        v4l2_err(&dev->v4l2_dev,
             ...
             pixm->pixelformat,
             pixm->pixelformat >> STREAM_OUTPUT_STEP_WISE,
             pixm->pixelformat >> STREAM_MIN_RSZ_OUTPUT_HEIGHT,
    ...
    for (i = 0; i < planes; i++) {
        ...
        w = (fmt->fmt_type == FMT_FBC) ?
            ALIGN(pixm->width, STREAM_MIN_RSZ_OUTPUT_HEIGHT) : pixm->width;
        h = (fmt->fmt_type == FMT_FBC) ?
            ALIGN(pixm->height, STREAM_MIN_RSZ_OUTPUT_HEIGHT) : pixm->height;
        width = i ? w / xsubs : w;
        height = i ? h / ysubs : h;
    ...
    if (!try && !stream->start_stream && !stream->streaming) {
        ...
        if (stream->id == RKISP_STREAM_SP) {
            stream->u.sp.y_stride =
                ...
                DIV_ROUND_UP(fmt->bpp[0], STREAM_OUTPUT_STEP_WISE);
