...
#define STREAM_MP_SP_IVALUE 2
#define STREAM_MP_SP_IVALUE_FO 4
#define STREAM_MIN_RSZ_TF 24
#define STREAM_MIN_RSZ_TH 30
#define STREAM_MIN_RSZ_SF 64
#define STREAM_MIN_RSZ_TFS 256
...
static int rkisp_create_hdr_buf(struct rkisp_device *dev)
{
    ...
    for (i = 0; i < max_dma; i++) {
        for (j = 0; j < max_buf; j++) {
            ...
            if (rkisp_alloc_buffer(dev, buf) < 0) {
                v4l2_err(&dev->v4l2_dev, "Failed to allocate the memory for hdr buffer\n");
                return -ENOMEM;
            }
            hdr_qbuf(&dev->hdr.q_tx[i], buf);
            v4l2_dbg(1, rkisp_debug, &dev->v4l2_dev, "hdr buf[%d][%d]:0x%x\n", i, j, (u32)buf->dma_addr);
        }
        dev->hdr.index[i] = i;
    }
    /*
    ...
    v4l2_dbg(1, rkisp_debug, &dev->v4l2_dev, "hdr:%d buf index dma0:%d dma1:%d dma2:%d\n", max_dma,
             dev->hdr.index[HDR_DMA0], dev->hdr.index[HDR_DMA1], dev->hdr.index[HDR_DMA2]);
    return 0;
}
...
void hdr_destroy_buf(struct rkisp_device *dev)
{
    ...
    if (atomic_read(&dev->cap_dev.refcnt) > 1 || !dev->active_sensor ||
        (dev->active_sensor && dev->active_sensor->mbus.type != V4L2_MBUS_CSI2_DPHY) || (dev->isp_inp & INP_CIF) ||
...
int hdr_update_dmatx_buf(struct rkisp_device *dev)
{
    ...
    if (!dev->active_sensor || (dev->active_sensor && dev->active_sensor->mbus.type != V4L2_MBUS_CSI2_DPHY) ||
        (dev->isp_inp & INP_CIF) || (dev->isp_ver != ISP_V20 && dev->isp_ver != ISP_V21)) {
        return 0;
    }
    ...
    if (buf) {
        ...
        mi_raw0_rd_set_addr(base, readl(base + MI_RAW0_WR_BASE_SHD));
    }
    ...
    if (buf) {
        ...
        mi_raw1_rd_set_addr(base, readl(base + MI_RAW1_WR_BASE_SHD));
    }
    ...
    if (buf) {
        ...
        mi_raw2_rd_set_addr(base, readl(base + MI_RAW2_WR_BASE_SHD));
    }
    ...
    v4l2_dbg(1, rkisp_debug, &dev->v4l2_dev,
             ...
             "RD CTRL:0x%x\n",
             readl(base + CSI2RX_CTRL0), readl(base + CSI2RX_CTRL1), readl(base + CSI2RX_RAW0_WR_CTRL),
             readl(base + CSI2RX_RAW1_WR_CTRL), readl(base + CSI2RX_RAW2_WR_CTRL), readl(base + CSI2RX_RAW_RD_CTRL));
    return 0;
}
...
int hdr_config_dmatx(struct rkisp_device *dev)
{
    ...
    if (atomic_inc_return(&dev->hdr.refcnt) > 1 || !dev->active_sensor ||
        (dev->active_sensor && dev->active_sensor->mbus.type != V4L2_MBUS_CSI2_DPHY) || (dev->isp_inp & INP_CIF) ||
        (dev->isp_ver != ISP_V20 && dev->isp_ver != ISP_V21)) {
        return 0;
    }
    ...
    if (dev->hdr.op_mode == HDR_FRAMEX2_DDR || dev->hdr.op_mode == HDR_LINEX2_DDR ||
        dev->hdr.op_mode == HDR_FRAMEX3_DDR || dev->hdr.op_mode == HDR_LINEX3_DDR ||
        dev->hdr.op_mode == HDR_RDBK_FRAME2 || dev->hdr.op_mode == HDR_RDBK_FRAME3) {
        stream = &dev->cap_dev.stream[RKISP_STREAM_DMATX0];
    ...
    if (dev->hdr.op_mode == HDR_FRAMEX3_DDR || dev->hdr.op_mode == HDR_LINEX3_DDR ||
    ...
    if (dev->hdr.op_mode == HDR_RDBK_FRAME1 || dev->hdr.op_mode == HDR_RDBK_FRAME2 ||
        dev->hdr.op_mode == HDR_RDBK_FRAME3) {
    ...
    if (dev->hdr.op_mode != HDR_NORMAL && !dev->dmarx_dev.trigger) {
        raw_rd_ctrl(dev->base_addr, memory << STREAM_MP_SP_IVALUE);
...
void hdr_stop_dmatx(struct rkisp_device *dev)
{
    ...
    if (atomic_dec_return(&dev->hdr.refcnt) || !dev->active_sensor ||
        (dev->active_sensor && dev->active_sensor->mbus.type != V4L2_MBUS_CSI2_DPHY) || (dev->isp_inp & INP_CIF) ||
        (dev->isp_ver != ISP_V20 && dev->isp_ver != ISP_V21)) {
        return;
    }
...
struct rkisp_dummy_buffer *hdr_dqbuf(struct list_head *q)
{
    ...
    if (!list_empty(q)) {
        buf = list_first_entry(q, struct rkisp_dummy_buffer, queue);
        list_del(&buf->queue);
    }
    return buf;
}
...
void rkisp_config_dmatx_valid_buf(struct rkisp_device *dev)
{
    ...
    if (!hw->dummy_buf.mem_priv || !dev->active_sensor ||
        (dev->active_sensor && dev->active_sensor->mbus.type != V4L2_MBUS_CSI2_DPHY) || (dev->isp_inp & INP_CIF) ||
        (dev->isp_ver != ISP_V20 && dev->isp_ver != ISP_V21)) {
        return;
    }
...
static const struct capture_fmt *find_fmt(struct rkisp_stream *stream, const u32 pixelfmt)
{
 ...
 */
static void restrict_rsz_resolution(struct rkisp_device *dev, const struct stream_config *config,
...
static int rkisp_set_fmt(struct rkisp_stream *stream, struct v4l2_pix_format_mplane *pixm, bool try)
{
    const struct capture_fmt *fmt;
    ...
    if (!fmt) {
        v4l2_err(&dev->v4l2_dev, "nonsupport pixelformat:%c%c%c%c\n", pixm->pixelformat,
                 pixm->pixelformat >> STREAM_OUTPUT_STEP_WISE, pixm->pixelformat >> STREAM_MIN_RSZ_OUTPUT_HEIGHT,
                 pixm->pixelformat >> STREAM_MIN_RSZ_TF);
        return -EINVAL;
    }
    ...
    if (stream->id == RKISP_STREAM_MP || stream->id == RKISP_STREAM_SP) {
        ...
        pixm->height = clamp_t(u32, pixm->height, config->min_rsz_height, max_rsz.height);
    } else if (dev->isp_ver == ISP_V30) {
        if (stream->id == RKISP_STREAM_BP && pixm->width != dev->isp_sdev.out_crop.width &&
            pixm->height != dev->isp_sdev.out_crop.height) {
            v4l2_warn(&dev->v4l2_dev, "fullpath %dx%d no equal to isp output %dx%d\n", pixm->width, pixm->height,
                      dev->isp_sdev.out_crop.width, dev->isp_sdev.out_crop.height);
            pixm->width = dev->isp_sdev.out_crop.width;
            pixm->height = dev->isp_sdev.out_crop.height;
        } else if (stream->id == RKISP_STREAM_FBC && pixm->width != stream->dcrop.width &&
                   pixm->height != stream->dcrop.height) {
            v4l2_warn(&dev->v4l2_dev, "fbcpatch no scale %dx%d should equal to crop %dx%d\n", pixm->width, pixm->height,
    ...
    for (i = 0; i < planes; i++) {
        ...
        w = (fmt->fmt_type == FMT_FBC) ? ALIGN(pixm->width, STREAM_MIN_RSZ_OUTPUT_HEIGHT) : pixm->width;
        h = (fmt->fmt_type == FMT_FBC) ? ALIGN(pixm->height, STREAM_MIN_RSZ_OUTPUT_HEIGHT) : pixm->height;
        width = i ? w / xsubs : w;
        height = i ? h / ysubs : h;

        if (dev->isp_ver == ISP_V20 && fmt->fmt_type == FMT_BAYER && stream->id == RKISP_STREAM_DMATX2) {
            height += RKMODULE_EXTEND_LINE;
        }

        if ((dev->isp_ver == ISP_V20 || dev->isp_ver == ISP_V21) && !stream->memory && fmt->fmt_type == FMT_BAYER &&
            stream->id != RKISP_STREAM_MP && stream->id != RKISP_STREAM_SP) {
            /* compact mode need bytesperline 4byte align */
            bytesperline = ALIGN(width * fmt->bpp[i] / STREAM_OUTPUT_STEP_WISE, STREAM_MIN_RSZ_TFS);
        } else {
            bytesperline = width * DIV_ROUND_UP(fmt->bpp[i], STREAM_OUTPUT_STEP_WISE);
        }

        /* 128bit AXI, 16byte align for bytesperline */
        if ((dev->isp_ver == ISP_V20 && stream->id == RKISP_STREAM_SP) || dev->isp_ver == ISP_V30) {
            bytesperline = ALIGN(bytesperline, STREAM_MIN_RSZ_OUTPUT_HEIGHT);
        }
        ...
        if (fmt->fmt_type == FMT_FBCGAIN && i == 0) {
            plane_fmt->sizeimage = ALIGN(plane_fmt->sizeimage, STREAM_MIN_RSZ_SF);
        } else if (fmt->fmt_type == FMT_FBC && i == 0) {
            plane_fmt->sizeimage = ALIGN(plane_fmt->sizeimage >> STREAM_MP_SP_IVALUE_FO, RK_MPP_ALIGN);
    ...
    if (!try && !stream->start_stream && !stream->streaming) {
        ...
        if (stream->id == RKISP_STREAM_SP) {
            stream->u.sp.y_stride =
                pixm->plane_fmt[0].bytesperline / DIV_ROUND_UP(fmt->bpp[0], STREAM_OUTPUT_STEP_WISE);
        } else if (stream->id == RKISP_STREAM_MP) {
            stream->u.mp.raw_enable = (fmt->fmt_type == FMT_BAYER);
        }

        v4l2_dbg(1, rkisp_debug, &stream->ispdev->v4l2_dev, "%s: stream: %d req(%d, %d) out(%d, %d)\n", __func__,
                 stream->id, pixm->width, pixm->height, stream->out_fmt.width, stream->out_fmt.height);
    }
...
static int rkisp_enum_input(struct file *file, void *priv, struct v4l2_input *input)
{
    if (input->index > 0) {
        return -EINVAL;
    }
...
static int rkisp_try_fmt_vid_cap_mplane(struct file *file, void *fh, struct v4l2_format *f)
{
    struct rkisp_stream *stream = video_drvdata(file);
...
static int rkisp_enum_framesizes(struct file *file, void *prov, struct v4l2_frmsizeenum *fsize)
{
    struct rkisp_stream *stream = video_drvdata(file);
    const struct stream_config *config = stream->config;
...
static int rkisp_get_cmsk(struct rkisp_stream *stream, struct rkisp_cmsk_cfg *cfg)
{
    ...
    *cfg = dev->cmsk_cfg;
    ...
    switch (stream->id) {
        ...
        default:
            win_en = cfg->win[STREAM_MP_SP_IVALUE].win_en;
            mode = cfg->win[STREAM_MP_SP_IVALUE].mode;
            break;
    }
...
static int rkisp_set_cmsk(struct rkisp_stream *stream, struct rkisp_cmsk_cfg *cfg)
{
    ...
    for (i = 0; i < RKISP_CMSK_WIN_MAX; i++) {
        ...
        if (cfg->win[i].win_en) {
            ...
            if (h_offs != cfg->win[i].h_offs || v_offs != cfg->win[i].v_offs || h_size != cfg->win[i].h_size ||
    ...
    switch (stream->id) {
        ...
        case RKISP_STREAM_BP:
        default:
            dev->cmsk_cfg.win[STREAM_MP_SP_IVALUE].win_en = win_en;
            dev->cmsk_cfg.win[STREAM_MP_SP_IVALUE].mode = mode;
            break;
    }
    spin_unlock_irqrestore(&dev->cmsk_lock, lock_flags);
    return 0;
...
static long rkisp_ioctl_default(struct file *file, void *fh, bool valid_prio, unsigned int cmd, void *arg)
{
    struct rkisp_stream *stream = video_drvdata(file);
    long ret = 0;
    ...
    switch (cmd) {
        case RKISP_CMD_GET_CSI_MEMORY_MODE:
            if (stream->id != RKISP_STREAM_DMATX0 && stream->id != RKISP_STREAM_DMATX1 &&
                stream->id != RKISP_STREAM_DMATX2 && stream->id != RKISP_STREAM_DMATX3) {
                ret = -EINVAL;
        ...
        case RKISP_CMD_SET_CSI_MEMORY_MODE:
            if (stream->id != RKISP_STREAM_DMATX0 && stream->id != RKISP_STREAM_DMATX1 &&
                ...
                stream->memory = SW_CSI_RWA_WR_SIMG_SWP | SW_CSI_RAW_WR_SIMG_MODE;
            }
            break;
...
static int rkisp_enum_frameintervals(struct file *file, void *fh, struct v4l2_frmivalenum *fival)
{
    ...
    if (ret && ret != -ENOIOCTLCMD) {
        ...
        fi.interval.denominator = STREAM_MIN_RSZ_TH;
    }
...
static int rkisp_enum_fmt_vid_cap_mplane(struct file *file, void *priv, struct v4l2_fmtdesc *f)
{
    struct rkisp_stream *stream = video_drvdata(file);
    ...
    switch (f->pixelformat) {
        case V4L2_PIX_FMT_FBC2:
            strscpy(f->description, "Rockchip yuv422sp fbc encoder", sizeof(f->description));
            break;
        case V4L2_PIX_FMT_FBC0:
            strscpy(f->description, "Rockchip yuv420sp fbc encoder", sizeof(f->description));
            break;
        case V4L2_PIX_FMT_FBCG:
            strscpy(f->description, "Rockchip fbc gain", sizeof(f->description));
            break;
...
static int rkisp_s_fmt_vid_cap_mplane(struct file *file, void *priv, struct v4l2_format *f)
{
    struct rkisp_stream *stream = video_drvdata(file);
...
static int rkisp_g_fmt_vid_cap_mplane(struct file *file, void *fh, struct v4l2_format *f)
{
    struct rkisp_stream *stream = video_drvdata(file);
...
static int rkisp_g_selection(struct file *file, void *prv, struct v4l2_selection *sel)
{
    struct rkisp_stream *stream = video_drvdata(file);
    struct rkisp_device *dev = stream->ispdev;
...
static struct v4l2_rect *rkisp_update_crop(struct rkisp_stream *stream, struct v4l2_rect *sel,
                                           const struct v4l2_rect *in)
{
    ...
    if ((stream->id == RKISP_STREAM_MP && stream->out_isp_fmt.fmt_type == FMT_BAYER) ||
        stream->id == RKISP_STREAM_DMATX0 || stream->id == RKISP_STREAM_DMATX1 || stream->id == RKISP_STREAM_DMATX2 ||
...
static int rkisp_s_selection(struct file *file, void *prv, struct v4l2_selection *sel)
{
    struct rkisp_stream *stream = video_drvdata(file);
    struct video_device *vdev = &stream->vnode.vdev;
    struct rkisp_vdev_node *node = vdev_to_node(vdev);
    ...
    *dcrop = *rkisp_update_crop(stream, &sel->r, input_win);
    v4l2_dbg(1, rkisp_debug, &dev->v4l2_dev, "stream %d crop(%d,%d)/%dx%d\n", stream->id, dcrop->left, dcrop->top,
...
int rkisp_register_stream_vdev(struct rkisp_stream *stream)
{
    ...
    sink = &vdev->entity;
    ret = media_create_pad_link(source, pad, sink, 0, stream->linked);
    if (ret < 0) {
        goto unreg;
    }
    return 0;
...
int rkisp_register_stream_vdevs(struct rkisp_device *dev)
{
    ...
    if (dev->isp_ver <= ISP_V13) {
        ...
        mp_cfg->max_rsz_width = dev->hw_dev->is_unite ? CIF_ISP_INPUT_W_MAX_V30_UNITE : CIF_ISP_INPUT_W_MAX_V30;
        mp_cfg->max_rsz_height = dev->hw_dev->is_unite ? CIF_ISP_INPUT_H_MAX_V30_UNITE : CIF_ISP_INPUT_H_MAX_V30;
