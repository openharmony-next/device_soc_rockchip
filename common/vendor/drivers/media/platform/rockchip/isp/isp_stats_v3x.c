...
static u32 isp3_stats_read(struct rkisp_isp_stats_vdev *stats_vdev, u32 addr, u32 id)
{
    u32 val;
...
static void isp3_stats_write(struct rkisp_isp_stats_vdev *stats_vdev, u32 addr, u32 value, u32 id)
{
    if (id == ISP3_LEFT) {
                                           ...
                                           struct rkisp3x_isp_stat_buffer *pbuf, u32 id)
{
    ...
    if (!(ctrl & ISP3X_3A_MEAS_DONE)) {
        v4l2_dbg(1, rkisp_debug, &stats_vdev->dev->v4l2_dev, "%s fail, id:%d ctrl:0x%x\n", __func__, id, ctrl);
        return -ENODATA;
    }
    ...
    for (i = 0; i < ISP3X_RAWAWB_SUM_NUM; i++) {
        ...
        pbuf->params.rawawb.ro_wp_num2[i] = isp3_stats_read(stats_vdev, ISP3X_RAWAWB_WPNUM2_0 + 4 * i, id);
    }

    for (i = 0; i < ISP3X_RAWAWB_MULWD_NUM; i++) {
        pbuf->params.rawawb.ro_sum_r_nor_multiwindow[i] =
            isp3_stats_read(stats_vdev, ISP3X_RAWAWB_SUM_R_NOR_MULTIWINDOW0 + 0x30 * i, id);
        pbuf->params.rawawb.ro_sum_b_nor_multiwindow[i] =
            isp3_stats_read(stats_vdev, ISP3X_RAWAWB_SUM_B_NOR_MULTIWINDOW0 + 0x30 * i, id);
        pbuf->params.rawawb.ro_wp_nm_nor_multiwindow[i] =
            isp3_stats_read(stats_vdev, ISP3X_RAWAWB_WP_NM_NOR_MULTIWINDOW0 + 0x30 * i, id);
        pbuf->params.rawawb.ro_sum_r_big_multiwindow[i] =
            isp3_stats_read(stats_vdev, ISP3X_RAWAWB_SUM_R_BIG_MULTIWINDOW0 + 0x30 * i, id);
        pbuf->params.rawawb.ro_sum_b_big_multiwindow[i] =
            isp3_stats_read(stats_vdev, ISP3X_RAWAWB_SUM_B_BIG_MULTIWINDOW0 + 0x30 * i, id);
        pbuf->params.rawawb.ro_wp_nm_big_multiwindow[i] =
            isp3_stats_read(stats_vdev, ISP3X_RAWAWB_WP_NM_BIG_MULTIWINDOW0 + 0x30 * i, id);
    }

    for (i = 0; i < ISP3X_RAWAWB_EXCL_STAT_NUM; i++) {
        pbuf->params.rawawb.ro_sum_r_exc[i] = isp3_stats_read(stats_vdev, ISP3X_RAWAWB_SUM_R_EXC0 + 0x10 * i, id);
        pbuf->params.rawawb.ro_sum_b_exc[i] = isp3_stats_read(stats_vdev, ISP3X_RAWAWB_SUM_B_EXC0 + 0x10 * i, id);
        pbuf->params.rawawb.ro_wp_nm_exc[i] = isp3_stats_read(stats_vdev, ISP3X_RAWAWB_WP_NM_EXC0 + 0x10 * i, id);
    }
...
static int rkisp_stats_get_rawaf_meas_reg(struct rkisp_isp_stats_vdev *stats_vdev, struct rkisp3x_isp_stat_buffer *pbuf,
                                          u32 id)
{
                                             ...
                                             u32 blk_no, u32 id)
{
    ...
    if (!(ctrl & ISP3X_3A_MEAS_DONE)) {
        v4l2_dbg(1, rkisp_debug, &stats_vdev->dev->v4l2_dev, "%s fail, id:%d addr:0x%x ctrl:0x%x\n", __func__, id, addr,
                 ctrl);
        return -ENODATA;
    }
    ...
    for (i = 0; i < ISP3X_RAWAEBIG_SUBWIN_NUM; i++) {
        ae->sumr[i] = isp3_stats_read(stats_vdev, addr + ISP3X_RAWAE_BIG_WND1_SUMR + i * 4, id);
    }

    for (i = 0; i < ISP3X_RAWAEBIG_SUBWIN_NUM; i++) {
        ae->sumg[i] = isp3_stats_read(stats_vdev, addr + ISP3X_RAWAE_BIG_WND1_SUMG + i * 4, id);
    }

    for (i = 0; i < ISP3X_RAWAEBIG_SUBWIN_NUM; i++) {
        ae->sumb[i] = isp3_stats_read(stats_vdev, addr + ISP3X_RAWAE_BIG_WND1_SUMB + i * 4, id);
    }

    for (i = 0; i < ISP3X_RAWAEBIG_MEAN_NUM; i++) {
        value = isp3_stats_read(stats_vdev, addr + ISP3X_RAWAE_BIG_RO_MEAN_BASE_ADDR, id);
...
static int rkisp_stats_get_rawhstbig_meas_reg(struct rkisp_isp_stats_vdev *stats_vdev,
                                              struct isp2x_rawhistbig_stat *hst, u32 blk_no, u32 id)
{
    u32 addr, ctrl;
    int i;
    ...
    for (i = 0; i < ISP3X_HIST_BIN_N_MAX; i++) {
        hst->hist_bin[i] = isp3_stats_read(stats_vdev, addr + ISP3X_RAWHIST_BIG_RO_BASE_BIN, id);
    }
                                           ...
                                           struct rkisp3x_isp_stat_buffer *pbuf, u32 id)
{
    ...
    if (!pbuf) {
        ...
        ret = rkisp_stats_get_rawaebig_meas_reg(stats_vdev, &pbuf->params.rawae1, 1, id);
        if (!ret) {
                                            ...
                                            struct rkisp3x_isp_stat_buffer *pbuf, u32 id)
{
    ...
    if (!pbuf) {
        ...
        ret = rkisp_stats_get_rawhstbig_meas_reg(stats_vdev, &pbuf->params.rawhist1, 1, id);
        if (!ret) {
                                           ...
                                           struct rkisp3x_isp_stat_buffer *pbuf, u32 id)
{
    ...
    if (!pbuf) {
        ...
        ret = rkisp_stats_get_rawaebig_meas_reg(stats_vdev, &pbuf->params.rawae2, 2, id);
        if (!ret) {
                                            ...
                                            struct rkisp3x_isp_stat_buffer *pbuf, u32 id)
{
    ...
    if (!pbuf) {
        ...
        ret = rkisp_stats_get_rawhstbig_meas_reg(stats_vdev, &pbuf->params.rawhist2, 2, id);
        if (!ret) {
                                           ...
                                           struct rkisp3x_isp_stat_buffer *pbuf, u32 id)
{
    ...
    if (!pbuf) {
        ...
        ret = rkisp_stats_get_rawaebig_meas_reg(stats_vdev, &pbuf->params.rawae3, 0, id);
        if (!ret) {
                                            ...
                                            struct rkisp3x_isp_stat_buffer *pbuf, u32 id)
{
    ...
    if (!pbuf) {
        ...
        ret = rkisp_stats_get_rawhstbig_meas_reg(stats_vdev, &pbuf->params.rawhist3, 0, id);
        if (!ret) {
                                               ...
                                               struct rkisp3x_isp_stat_buffer *pbuf, u32 id)
{
    ...
    for (i = 0; i < ISP3X_HIST_BIN_N_MAX; i++) {
        hst->hist_bin[i] = isp3_stats_read(stats_vdev, ISP3X_RAWHIST_LITE_RO_BASE_BIN, id);
    }
...
static int rkisp_stats_get_bls_stats(struct rkisp_isp_stats_vdev *stats_vdev, struct rkisp3x_isp_stat_buffer *pbuf,
                                     u32 id)
{
...
static int rkisp_stats_get_dhaz_stats(struct rkisp_isp_stats_vdev *stats_vdev, struct rkisp3x_isp_stat_buffer *pbuf,
                                      u32 id)
{
                                           ...
                                           struct rkisp3x_isp_stat_buffer *pbuf, u32 id)
{
    ...
    for (i = 0; i < ISP3X_RAWAWB_SUM_NUM; i++) {
        rawawb->ro_rawawb_sum_rgain_nor[i] = reg_addr[(0x20 * i + 0x0) / 4];
        rawawb->ro_rawawb_sum_bgain_nor[i] = reg_addr[(0x20 * i + 0x4) / 4];
        rawawb->ro_rawawb_wp_num_nor[i] = reg_addr[(0x20 * i + 0x8) / 4];
        rawawb->ro_wp_num2[i] = reg_addr[(0x20 * i + 0xc) / 4];
        rawawb->ro_rawawb_sum_rgain_big[i] = reg_addr[(0x20 * i + 0x10) / 4];
        rawawb->ro_rawawb_sum_bgain_big[i] = reg_addr[(0x20 * i + 0x14) / 4];
        rawawb->ro_rawawb_wp_num_big[i] = reg_addr[(0x20 * i + 0x18) / 4];
    }
    ...
    for (i = 0; i < ISP3X_RAWAWB_MULWD_NUM; i++) {
        rawawb->ro_sum_r_nor_multiwindow[i] = reg_addr[(0x20 * i + 0xF0) / 4];
        rawawb->ro_sum_b_nor_multiwindow[i] = reg_addr[(0x20 * i + 0xF4) / 4];
        rawawb->ro_wp_nm_nor_multiwindow[i] = reg_addr[(0x20 * i + 0xF8) / 4];
        rawawb->ro_sum_r_big_multiwindow[i] = reg_addr[(0x20 * i + 0x100) / 4];
        rawawb->ro_sum_b_big_multiwindow[i] = reg_addr[(0x20 * i + 0x104) / 4];
        rawawb->ro_wp_nm_big_multiwindow[i] = reg_addr[(0x20 * i + 0x108) / 4];
    }
...
static int rkisp_stats_get_rawaf_meas_ddr(struct rkisp_isp_stats_vdev *stats_vdev, struct rkisp3x_isp_stat_buffer *pbuf,
                                          u32 id)
{
    struct isp3x_rawaf_stat *af;
    u32 offs = id * ISP3X_RD_STATS_BUF_SIZE;
                                           ...
                                           struct rkisp3x_isp_stat_buffer *pbuf, u32 id)
{
    ...
    if (!pbuf) {
        ...
        ret = rkisp_stats_get_rawaebig_meas_ddr(stats_vdev, &pbuf->params.rawae1, 1, id);
        if (!ret) {
            pbuf->meas_type |= ISP3X_STAT_RAWAE1;
        }
    }
                                            ...
                                            struct rkisp3x_isp_stat_buffer *pbuf, u32 id)
{
    ...
    if (!pbuf) {
        ...
        ret = rkisp_stats_get_rawhstbig_meas_ddr(stats_vdev, &pbuf->params.rawhist1, 1, id);
        if (!ret) {
            pbuf->meas_type |= ISP3X_STAT_RAWHST1;
        }
    }
                                           ...
                                           struct rkisp3x_isp_stat_buffer *pbuf, u32 id)
{
    ...
    if (!pbuf) {
        ...
        ret = rkisp_stats_get_rawaebig_meas_ddr(stats_vdev, &pbuf->params.rawae2, 2, id);
        if (!ret) {
            pbuf->meas_type |= ISP3X_STAT_RAWAE2;
        }
    }
                                            ...
                                            struct rkisp3x_isp_stat_buffer *pbuf, u32 id)
{
    ...
    if (!pbuf) {
        ...
        ret = rkisp_stats_get_rawhstbig_meas_ddr(stats_vdev, &pbuf->params.rawhist2, 2, id);
        if (!ret) {
            pbuf->meas_type |= ISP3X_STAT_RAWHST2;
        }
    }
                                           ...
                                           struct rkisp3x_isp_stat_buffer *pbuf, u32 id)
{
    ...
    if (!pbuf) {
        ...
        ret = rkisp_stats_get_rawaebig_meas_ddr(stats_vdev, &pbuf->params.rawae3, 0, id);
        if (!ret) {
            pbuf->meas_type |= ISP3X_STAT_RAWAE3;
        }
    }
                                            ...
                                            struct rkisp3x_isp_stat_buffer *pbuf, u32 id)
{
    ...
    if (!pbuf) {
        ...
        ret = rkisp_stats_get_rawhstbig_meas_ddr(stats_vdev, &pbuf->params.rawhist3, 0, id);
        if (!ret) {
            pbuf->meas_type |= ISP3X_STAT_RAWHST3;
        }
    }
...
static void rkisp_merge_dual_dhaz_params(struct rkisp_isp_stats_vdev *stats_vdev,
                                         ...
                                         struct rkisp3x_isp_stat_buffer *right_stat)
{
    ...
    if (!left_stat || !right_stat || !(left_stat->meas_type & ISP3X_STAT_DHAZ) ||
        !(right_stat->meas_type & ISP3X_STAT_DHAZ) || !(isp3_stats_read(stats_vdev, ISP3X_DHAZ_CTRL, 0) & BIT(25))) {
        return;
    }
    ...
    if (!num || (!l_dhaz->dhaz_pic_sumh && !r_dhaz->dhaz_pic_sumh)) {
        v4l2_err(stats_vdev->vnode.vdev.v4l2_dev, "%s 0 params, num:%d sumh:%d %d\n", __func__, num,
                 l_dhaz->dhaz_pic_sumh, r_dhaz->dhaz_pic_sumh);
        return;
    }
    ...
    for (i = 0; i < ISP3X_DHAZ_HIST_WR_NUM / 3; i++) {
        hist_out0 =
            (l_dhaz->h_rgb_iir[i * 3] * l_dhaz->dhaz_pic_sumh + r_dhaz->h_rgb_iir[i * 3] * r_dhaz->dhaz_pic_sumh) /
            pic_sumh;
        ...
        hist_out1 = (l_dhaz->h_rgb_iir[i * 3 + 1] * l_dhaz->dhaz_pic_sumh +
                     r_dhaz->h_rgb_iir[i * 3 + 1] * r_dhaz->dhaz_pic_sumh) /
                    pic_sumh;
        ...
        hist_out2 = (l_dhaz->h_rgb_iir[i * 3 + 2] * l_dhaz->dhaz_pic_sumh +
                     r_dhaz->h_rgb_iir[i * 3 + 2] * r_dhaz->dhaz_pic_sumh) /
                    pic_sumh;
    ...
    val = (l_dhaz->h_rgb_iir[i * 3] * l_dhaz->dhaz_pic_sumh + r_dhaz->h_rgb_iir[i * 3] * r_dhaz->dhaz_pic_sumh) /
          pic_sumh;
...
static void rkisp_stats_send_meas_v3x(struct rkisp_isp_stats_vdev *stats_vdev, struct rkisp_isp_readout_work *meas_work)
{
    unsigned int cur_frame_id = -1;
    struct rkisp_buffer *cur_buf = stats_vdev->cur_buf;
    struct rkisp3x_isp_stat_buffer *cur_stat_buf = NULL;
    struct rkisp_stats_ops_v3x *ops = (struct rkisp_stats_ops_v3x *)stats_vdev->priv_ops;
    int ret = 0;
    ...
    if (cur_buf) {
        cur_stat_buf = (struct rkisp3x_isp_stat_buffer *)(cur_buf->vaddr[0]);
        cur_stat_buf->frame_id = cur_frame_id;
    }

    if (meas_work->isp_ris & ISP3X_AFM_SUM_OF) {
        v4l2_warn(stats_vdev->vnode.vdev.v4l2_dev, "ISP3X_AFM_SUM_OF\n");
    }

    if (meas_work->isp_ris & ISP3X_AFM_LUM_OF) {
        v4l2_warn(stats_vdev->vnode.vdev.v4l2_dev, "ISP3X_AFM_LUM_OF\n");
    }

    if (meas_work->isp3a_ris & ISP3X_3A_RAWAF_SUM) {
        v4l2_warn(stats_vdev->vnode.vdev.v4l2_dev, "ISP3X_3A_RAWAF_SUM\n");
    }
...
static void rkisp_stats_isr_v3x(struct rkisp_isp_stats_vdev *stats_vdev, u32 isp_ris, u32 isp3a_ris)
{
    struct rkisp_device *dev = stats_vdev->dev;
    ...
    u32 iq_isr_mask =
        ISP3X_SIAWB_DONE | ISP3X_SIAF_FIN | ISP3X_EXP_END | ISP3X_SIHST_RDY | ISP3X_AFM_SUM_OF | ISP3X_AFM_LUM_OF;
    ...
    if ((isp_ris & ISP3X_FRAME) && stats_vdev->rd_stats_from_ddr) {
        ...
        rkisp_write(dev, ISP3X_MI_3A_WR_BASE, stats_vdev->stats_buf[wr_buf_idx].dma_addr, false);
        if (dev->hw_dev->is_unite) {
            rkisp_next_write(dev, ISP3X_MI_3A_WR_BASE,
                             stats_vdev->stats_buf[wr_buf_idx].dma_addr + ISP3X_RD_STATS_BUF_SIZE, false);
        }
    }
...
static void rkisp_stats_rdbk_enable_v3x(struct rkisp_isp_stats_vdev *stats_vdev, bool en)
{
    if (!en) {
...
void rkisp_stats_first_ddr_config_v3x(struct rkisp_isp_stats_vdev *stats_vdev)
{
    ...
    if (!IS_HDR_RDBK(stats_vdev->dev->hdr.op_mode)) {
        ...
        rkisp_unite_write(dev, ISP3X_MI_DBR_WR_SIZE, ISP3X_RD_STATS_BUF_SIZE, false, dev->hw_dev->is_unite);
        rkisp_unite_set_bits(dev, ISP3X_SWS_CFG, 0, ISP3X_3A_DDR_WRITE_EN, false, dev->hw_dev->is_unite);
        rkisp_write(dev, ISP3X_MI_3A_WR_BASE, stats_vdev->stats_buf[0].dma_addr, false);
        if (dev->hw_dev->is_unite) {
            rkisp_next_write(dev, ISP3X_MI_3A_WR_BASE, stats_vdev->stats_buf[0].dma_addr + ISP3X_RD_STATS_BUF_SIZE,
                             false);
        }
    }
...
void rkisp_init_stats_vdev_v3x(struct rkisp_isp_stats_vdev *stats_vdev)
{
    ...
    stats_vdev->vdev_fmt.fmt.meta.buffersize = mult * sizeof(struct rkisp3x_isp_stat_buffer);
