...
static inline size_t isp_param_get_insize(struct rkisp_isp_params_vdev *params_vdev)
{
...
static void isp_dpcc_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_dpcc_cfg *arg)
{
    u32 value;
    int i;
    ...
    value = ISP2X_PACK_4BYTE(arg->line_thr_1_g, arg->line_thr_1_rb, arg->sw_mindis1_g, arg->sw_mindis1_rb);
    rkisp_iowrite32(params_vdev, value, ISP_DPCC0_LINE_THRESH_1);
    rkisp_iowrite32(params_vdev, value, ISP_DPCC1_LINE_THRESH_1);

    value =
        ISP2X_PACK_4BYTE(arg->line_mad_fac_1_g, arg->line_mad_fac_1_rb, arg->sw_dis_scale_max1, arg->sw_dis_scale_min1);
    ...
    value = ISP2X_PACK_4BYTE(arg->line_thr_2_g, arg->line_thr_2_rb, arg->sw_mindis2_g, arg->sw_mindis2_rb);
    rkisp_iowrite32(params_vdev, value, ISP_DPCC0_LINE_THRESH_2);
    rkisp_iowrite32(params_vdev, value, ISP_DPCC1_LINE_THRESH_2);

    value =
        ISP2X_PACK_4BYTE(arg->line_mad_fac_2_g, arg->line_mad_fac_2_rb, arg->sw_dis_scale_max2, arg->sw_dis_scale_min2);
    ...
    value = ISP2X_PACK_4BYTE(arg->line_thr_3_g, arg->line_thr_3_rb, arg->sw_mindis3_g, arg->sw_mindis3_rb);
    rkisp_iowrite32(params_vdev, value, ISP_DPCC0_LINE_THRESH_3);
    rkisp_iowrite32(params_vdev, value, ISP_DPCC1_LINE_THRESH_3);

    value =
        ISP2X_PACK_4BYTE(arg->line_mad_fac_3_g, arg->line_mad_fac_3_rb, arg->sw_dis_scale_max3, arg->sw_dis_scale_min3);
...
static void isp_dpcc_enable(struct rkisp_isp_params_vdev *params_vdev, bool en)
{
    u32 value;
...
static void isp_bls_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp21_bls_cfg *arg)
{
    ...
    if (!arg->enable_auto) {
        ...
        if (arg->en_windows & BIT(1)) {
            rkisp_iowrite32(params_vdev, arg->bls_window2.h_offs, ISP_BLS_H2_START);
            value = arg->bls_window2.h_offs + arg->bls_window2.h_size;
            rkisp_iowrite32(params_vdev, value, ISP_BLS_H2_STOP);
            rkisp_iowrite32(params_vdev, arg->bls_window2.v_offs, ISP_BLS_V2_START);
        ...
        if (arg->en_windows & BIT(0)) {
            rkisp_iowrite32(params_vdev, arg->bls_window1.h_offs, ISP_BLS_H1_START);
            value = arg->bls_window1.h_offs + arg->bls_window1.h_size;
            rkisp_iowrite32(params_vdev, value, ISP_BLS_H1_STOP);
            rkisp_iowrite32(params_vdev, arg->bls_window1.v_offs, ISP_BLS_V1_START);
        ...
        rkisp_iowrite32(params_vdev, arg->bls_samples, ISP_BLS_SAMPLES);
...
static void isp_bls_enable(struct rkisp_isp_params_vdev *params_vdev, bool en)
{
    u32 new_control;
...
static void isp_sdg_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_sdg_cfg *arg)
{
    int i;

    rkisp_iowrite32(params_vdev, arg->xa_pnts.gamma_dx0, ISP_GAMMA_DX_LO);
...
static void isp_sdg_enable(struct rkisp_isp_params_vdev *params_vdev, bool en)
{
    if (en) {
        isp_param_set_bits(params_vdev, CIF_ISP_CTRL, CIF_ISP_CTRL_ISP_GAMMA_IN_ENA);
    } else {
        isp_param_clear_bits(params_vdev, CIF_ISP_CTRL, CIF_ISP_CTRL_ISP_GAMMA_IN_ENA);
    }
}

static void MAYBE_UNUSED isp_lsc_matrix_cfg_sram(struct rkisp_isp_params_vdev *params_vdev,
                                                 const struct isp2x_lsc_cfg *pconfig, bool is_check)
{
    ...
    if (is_check && !(rkisp_ioread32(params_vdev, ISP_LSC_CTRL) & ISP_LSC_EN)) {
        return;
    }
...
static void MAYBE_UNUSED isp_lsc_matrix_cfg_ddr(struct rkisp_isp_params_vdev *params_vdev,
...
static void isp_lsc_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_lsc_cfg *arg)
{
...
static void isp_lsc_enable(struct rkisp_isp_params_vdev *params_vdev, bool en)
{
    struct rkisp_device *dev = params_vdev->dev;
    u32 val = ISP_LSC_EN;
...
static void isp_debayer_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_debayer_cfg *arg)
{
    u32 value;
...
static void isp_debayer_enable(struct rkisp_isp_params_vdev *params_vdev, bool en)
{
    if (en) {
        isp_param_set_bits(params_vdev, ISP_DEBAYER_CONTROL, ISP_DEBAYER_EN);
    } else {
        isp_param_clear_bits(params_vdev, ISP_DEBAYER_CONTROL, ISP_DEBAYER_EN);
    }
}

static void isp_awbgain_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp21_awb_gain_cfg *arg)
{
    struct rkisp_device *dev = params_vdev->dev;
    ...
    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->gain0_green_b, arg->gain0_green_r), ISP21_AWB_GAIN0_G);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->gain0_blue, arg->gain0_red), ISP21_AWB_GAIN0_RB);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->gain1_green_b, arg->gain1_green_r), ISP21_AWB_GAIN1_G);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->gain1_blue, arg->gain1_red), ISP21_AWB_GAIN1_RB);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->gain2_green_b, arg->gain2_green_r), ISP21_AWB_GAIN2_G);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->gain2_blue, arg->gain2_red), ISP21_AWB_GAIN2_RB);
}

static void isp_awbgain_enable(struct rkisp_isp_params_vdev *params_vdev, bool en)
{
    if (en) {
        isp_param_set_bits(params_vdev, CIF_ISP_CTRL, CIF_ISP_CTRL_ISP_AWB_ENA);
    } else {
        isp_param_clear_bits(params_vdev, CIF_ISP_CTRL, CIF_ISP_CTRL_ISP_AWB_ENA);
    }
}

static void isp_ccm_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp21_ccm_cfg *arg)
{
    u32 value;
    u32 i;
...
static void isp_ccm_enable(struct rkisp_isp_params_vdev *params_vdev, bool en)
{
    if (en) {
...
static void isp_goc_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_gammaout_cfg *arg)
{
    int i;
    u32 value;
...
static void isp_cproc_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_cproc_cfg *arg)
{
    ...
    if (quantization != V4L2_QUANTIZATION_FULL_RANGE) {
        isp_param_clear_bits(params_vdev, CPROC_CTRL,
                             CIF_C_PROC_YOUT_FULL | CIF_C_PROC_YIN_FULL | CIF_C_PROC_COUT_FULL);
    } else {
        isp_param_set_bits(params_vdev, CPROC_CTRL, CIF_C_PROC_YOUT_FULL | CIF_C_PROC_YIN_FULL | CIF_C_PROC_COUT_FULL);
    }
}

static void isp_cproc_enable(struct rkisp_isp_params_vdev *params_vdev, bool en)
{
    if (en) {
        isp_param_set_bits(params_vdev, CPROC_CTRL, CIF_C_PROC_CTR_ENABLE);
    } else {
        isp_param_clear_bits(params_vdev, CPROC_CTRL, CIF_C_PROC_CTR_ENABLE);
    }
}
...
static void isp_ie_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_ie_cfg *arg)
{
    ...
    switch (arg->effect) {
        ...
        case V4L2_COLORFX_AQUA:
            ...
            rkisp_iowrite32(params_vdev, arg->color_sel, CIF_IMG_EFF_COLOR_SEL);
            break;
        case V4L2_COLORFX_EMBOSS:
            eff_ctrl |= CIF_IMG_EFF_CTRL_MODE_EMBOSS;
            rkisp_iowrite32(params_vdev, arg->eff_mat_1, CIF_IMG_EFF_MAT_1);
            rkisp_iowrite32(params_vdev, arg->eff_mat_2, CIF_IMG_EFF_MAT_2);
            rkisp_iowrite32(params_vdev, arg->eff_mat_3, CIF_IMG_EFF_MAT_3);
            break;
        case V4L2_COLORFX_SKETCH:
            eff_ctrl |= CIF_IMG_EFF_CTRL_MODE_SKETCH;
            rkisp_iowrite32(params_vdev, arg->eff_mat_3, CIF_IMG_EFF_MAT_3);
            rkisp_iowrite32(params_vdev, arg->eff_mat_4, CIF_IMG_EFF_MAT_4);
            rkisp_iowrite32(params_vdev, arg->eff_mat_5, CIF_IMG_EFF_MAT_5);
            break;
...
{
    if (en) {
        isp_param_set_bits(params_vdev, CIF_IMG_EFF_CTRL, CIF_IMG_EFF_CTRL_ENABLE);
        isp_param_set_bits(params_vdev, CIF_IMG_EFF_CTRL, CIF_IMG_EFF_CTRL_CFG_UPD);
    } else {
        isp_param_clear_bits(params_vdev, CIF_IMG_EFF_CTRL, CIF_IMG_EFF_CTRL_ENABLE);
    }
}

static void isp_rawaf_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_rawaf_meas_cfg *arg)
{
    u32 i, var;
    u16 h_size, v_size;
    u16 h_offs, v_offs;
    ...
    rkisp_iowrite32(params_vdev, ISP2X_PACK_4BYTE(arg->gaus_coe_h0, arg->gaus_coe_h1, arg->gaus_coe_h2, 0),
                    ISP_RAWAF_GAUS_COE);
...
static void isp_rawaf_enable(struct rkisp_isp_params_vdev *params_vdev, bool en)
{
...
static void isp_rawaelite_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_rawaelite_meas_cfg *arg)
{
    ...
    rkisp_iowrite32(params_vdev,
                    ISP2X_RAWAE_LITE_V_OFFSET_SET(arg->win.v_offs) | ISP2X_RAWAE_LITE_H_OFFSET_SET(arg->win.h_offs),
...
static void isp_rawaelite_enable(struct rkisp_isp_params_vdev *params_vdev, bool en)
{
    u32 exp_ctrl;
                                ...
                                u32 blk_no)
{
    ...
    value &= ~(ISP2X_RAWAEBIG_WNDNUM_SET(0x3) | ISP2X_RAWAEBIG_SUBWIN1_EN | ISP2X_RAWAEBIG_SUBWIN2_EN |
               ISP2X_RAWAEBIG_SUBWIN3_EN | ISP2X_RAWAEBIG_SUBWIN4_EN | ISP2X_REG_WR_MASK);
    ...
    rkisp_iowrite32(params_vdev,
                    ISP2X_RAWAEBIG_V_OFFSET_SET(arg->win.v_offs) | ISP2X_RAWAEBIG_H_OFFSET_SET(arg->win.h_offs),
...
static void isp_rawae1_enable(struct rkisp_isp_params_vdev *params_vdev, bool en)
{
...
static void isp_rawae3_enable(struct rkisp_isp_params_vdev *params_vdev, bool en)
{
    isp_rawaebig_enable(params_vdev, en, 0);
}

static void isp_rawawb_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp21_rawawb_meas_cfg *arg)
{
    u32 i, value;
    ...
    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_h_size, arg->sw_rawawb_v_size),
                    ISP21_RAWAWB_WIN_SIZE);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_r_max, arg->sw_rawawb_g_max),
                    ISP21_RAWAWB_LIMIT_RG_MAX);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_b_max, arg->sw_rawawb_y_max),
                    ISP21_RAWAWB_LIMIT_BY_MAX);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_r_min, arg->sw_rawawb_g_min),
                    ISP21_RAWAWB_LIMIT_RG_MIN);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_b_min, arg->sw_rawawb_y_min),
    ...
    rkisp_iowrite32(params_vdev,
                    ISP2X_PACK_4BYTE(arg->sw_rawawb_wp_luma_weicurve_y0, arg->sw_rawawb_wp_luma_weicurve_y1,
                                     arg->sw_rawawb_wp_luma_weicurve_y2, arg->sw_rawawb_wp_luma_weicurve_y3),
                    ISP21_RAWAWB_YWEIGHT_CURVE_XCOOR03);

    rkisp_iowrite32(params_vdev,
                    ISP2X_PACK_4BYTE(arg->sw_rawawb_wp_luma_weicurve_y4, arg->sw_rawawb_wp_luma_weicurve_y5,
                                     arg->sw_rawawb_wp_luma_weicurve_y6, arg->sw_rawawb_wp_luma_weicurve_y7),
                    ISP21_RAWAWB_YWEIGHT_CURVE_XCOOR47);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_wp_luma_weicurve_y8, ISP21_RAWAWB_YWEIGHT_CURVE_XCOOR8);

    rkisp_iowrite32(params_vdev,
                    ISP2X_PACK_4BYTE(arg->sw_rawawb_wp_luma_weicurve_w0, arg->sw_rawawb_wp_luma_weicurve_w1,
                                     arg->sw_rawawb_wp_luma_weicurve_w2, arg->sw_rawawb_wp_luma_weicurve_w3),
                    ISP21_RAWAWB_YWEIGHT_CURVE_YCOOR03);

    rkisp_iowrite32(params_vdev,
                    ISP2X_PACK_4BYTE(arg->sw_rawawb_wp_luma_weicurve_w4, arg->sw_rawawb_wp_luma_weicurve_w5,
                                     arg->sw_rawawb_wp_luma_weicurve_w6, arg->sw_rawawb_wp_luma_weicurve_w7),
                    ISP21_RAWAWB_YWEIGHT_CURVE_YCOOR47);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_wp_luma_weicurve_w8, arg->sw_rawawb_pre_wbgain_inv_r),
                    ISP21_RAWAWB_YWEIGHT_CURVE_YCOOR8);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_pre_wbgain_inv_g, arg->sw_rawawb_pre_wbgain_inv_b),
                    ISP21_RAWAWB_PRE_WBGAIN_INV);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex0_u_0, arg->sw_rawawb_vertex0_v_0),
                    ISP21_RAWAWB_UV_DETC_VERTEX0_0);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex1_u_0, arg->sw_rawawb_vertex1_v_0),
                    ISP21_RAWAWB_UV_DETC_VERTEX1_0);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex2_u_0, arg->sw_rawawb_vertex2_v_0),
                    ISP21_RAWAWB_UV_DETC_VERTEX2_0);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex3_u_0, arg->sw_rawawb_vertex3_v_0),
                    ISP21_RAWAWB_UV_DETC_VERTEX3_0);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope01_0, ISP21_RAWAWB_UV_DETC_ISLOPE01_0);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope12_0, ISP21_RAWAWB_UV_DETC_ISLOPE12_0);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope23_0, ISP21_RAWAWB_UV_DETC_ISLOPE23_0);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope30_0, ISP21_RAWAWB_UV_DETC_ISLOPE30_0);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex0_u_1, arg->sw_rawawb_vertex0_v_1),
                    ISP21_RAWAWB_UV_DETC_VERTEX0_1);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex1_u_1, arg->sw_rawawb_vertex1_v_1),
                    ISP21_RAWAWB_UV_DETC_VERTEX1_1);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex2_u_1, arg->sw_rawawb_vertex2_v_1),
                    ISP21_RAWAWB_UV_DETC_VERTEX2_1);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex3_u_1, arg->sw_rawawb_vertex3_v_1),
                    ISP21_RAWAWB_UV_DETC_VERTEX3_1);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope01_1, ISP21_RAWAWB_UV_DETC_ISLOPE01_1);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope12_1, ISP21_RAWAWB_UV_DETC_ISLOPE12_1);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope23_1, ISP21_RAWAWB_UV_DETC_ISLOPE23_1);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope30_1, ISP21_RAWAWB_UV_DETC_ISLOPE30_1);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex0_u_2, arg->sw_rawawb_vertex0_v_2),
                    ISP21_RAWAWB_UV_DETC_VERTEX0_2);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex1_u_2, arg->sw_rawawb_vertex1_v_2),
                    ISP21_RAWAWB_UV_DETC_VERTEX1_2);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex2_u_2, arg->sw_rawawb_vertex2_v_2),
                    ISP21_RAWAWB_UV_DETC_VERTEX2_2);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex3_u_2, arg->sw_rawawb_vertex3_v_2),
                    ISP21_RAWAWB_UV_DETC_VERTEX3_2);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope01_2, ISP21_RAWAWB_UV_DETC_ISLOPE01_2);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope12_2, ISP21_RAWAWB_UV_DETC_ISLOPE12_2);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope23_2, ISP21_RAWAWB_UV_DETC_ISLOPE23_2);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope30_2, ISP21_RAWAWB_UV_DETC_ISLOPE30_2);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex0_u_3, arg->sw_rawawb_vertex0_v_3),
                    ISP21_RAWAWB_UV_DETC_VERTEX0_3);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex1_u_3, arg->sw_rawawb_vertex1_v_3),
                    ISP21_RAWAWB_UV_DETC_VERTEX1_3);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex2_u_3, arg->sw_rawawb_vertex2_v_3),
                    ISP21_RAWAWB_UV_DETC_VERTEX2_3);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex3_u_3, arg->sw_rawawb_vertex3_v_3),
                    ISP21_RAWAWB_UV_DETC_VERTEX3_3);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope01_3, ISP21_RAWAWB_UV_DETC_ISLOPE01_3);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope12_3, ISP21_RAWAWB_UV_DETC_ISLOPE12_3);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope23_3, ISP21_RAWAWB_UV_DETC_ISLOPE23_3);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope30_3, ISP21_RAWAWB_UV_DETC_ISLOPE30_3);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex0_u_4, arg->sw_rawawb_vertex0_v_4),
                    ISP21_RAWAWB_UV_DETC_VERTEX0_4);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex1_u_4, arg->sw_rawawb_vertex1_v_4),
                    ISP21_RAWAWB_UV_DETC_VERTEX1_4);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex2_u_4, arg->sw_rawawb_vertex2_v_4),
                    ISP21_RAWAWB_UV_DETC_VERTEX2_4);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex3_u_4, arg->sw_rawawb_vertex3_v_4),
                    ISP21_RAWAWB_UV_DETC_VERTEX3_4);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope01_4, ISP21_RAWAWB_UV_DETC_ISLOPE01_4);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope12_4, ISP21_RAWAWB_UV_DETC_ISLOPE12_4);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope23_4, ISP21_RAWAWB_UV_DETC_ISLOPE23_4);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope30_4, ISP21_RAWAWB_UV_DETC_ISLOPE30_4);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex0_u_5, arg->sw_rawawb_vertex0_v_5),
                    ISP21_RAWAWB_UV_DETC_VERTEX0_5);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex1_u_5, arg->sw_rawawb_vertex1_v_5),
                    ISP21_RAWAWB_UV_DETC_VERTEX1_5);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex2_u_5, arg->sw_rawawb_vertex2_v_5),
                    ISP21_RAWAWB_UV_DETC_VERTEX2_5);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex3_u_5, arg->sw_rawawb_vertex3_v_5),
                    ISP21_RAWAWB_UV_DETC_VERTEX3_5);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope01_5, ISP21_RAWAWB_UV_DETC_ISLOPE01_5);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope12_5, ISP21_RAWAWB_UV_DETC_ISLOPE10_5);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope23_5, ISP21_RAWAWB_UV_DETC_ISLOPE23_5);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope30_5, ISP21_RAWAWB_UV_DETC_ISLOPE30_5);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex0_u_6, arg->sw_rawawb_vertex0_v_6),
                    ISP21_RAWAWB_UV_DETC_VERTEX0_6);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex1_u_6, arg->sw_rawawb_vertex1_v_6),
                    ISP21_RAWAWB_UV_DETC_VERTEX1_6);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex2_u_6, arg->sw_rawawb_vertex2_v_6),
                    ISP21_RAWAWB_UV_DETC_VERTEX2_6);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_vertex3_u_6, arg->sw_rawawb_vertex3_v_6),
                    ISP21_RAWAWB_UV_DETC_VERTEX3_6);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope01_6, ISP21_RAWAWB_UV_DETC_ISLOPE01_6);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope12_6, ISP21_RAWAWB_UV_DETC_ISLOPE10_6);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope23_6, ISP21_RAWAWB_UV_DETC_ISLOPE23_6);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_islope30_6, ISP21_RAWAWB_UV_DETC_ISLOPE30_6);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_rgb2ryuvmat0_y, arg->sw_rawawb_rgb2ryuvmat1_y),
                    ISP21_RAWAWB_YUV_RGB2ROTY_0);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_rgb2ryuvmat2_y, arg->sw_rawawb_rgb2ryuvofs_y),
                    ISP21_RAWAWB_YUV_RGB2ROTY_1);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_rgb2ryuvmat0_u, arg->sw_rawawb_rgb2ryuvmat1_u),
                    ISP21_RAWAWB_YUV_RGB2ROTU_0);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_rgb2ryuvmat2_u, arg->sw_rawawb_rgb2ryuvofs_u),
                    ISP21_RAWAWB_YUV_RGB2ROTU_1);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_rgb2ryuvmat0_v, arg->sw_rawawb_rgb2ryuvmat1_v),
                    ISP21_RAWAWB_YUV_RGB2ROTV_0);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_rgb2ryuvmat2_v, arg->sw_rawawb_rgb2ryuvofs_v),
                    ISP21_RAWAWB_YUV_RGB2ROTV_1);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_coor_x1_ls0_y, arg->sw_rawawb_vec_x21_ls0_y),
                    ISP21_RAWAWB_YUV_X_COOR_Y_0);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_coor_x1_ls0_u, arg->sw_rawawb_vec_x21_ls0_u),
                    ISP21_RAWAWB_YUV_X_COOR_U_0);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_coor_x1_ls0_v, arg->sw_rawawb_vec_x21_ls0_v),
                    ISP21_RAWAWB_YUV_X_COOR_V_0);

    rkisp_iowrite32(
        params_vdev,
        ISP2X_PACK_4BYTE(arg->sw_rawawb_dis_x1x2_ls0, 0, arg->sw_rawawb_rotu0_ls0, arg->sw_rawawb_rotu1_ls0),
        ISP21_RAWAWB_YUV_X1X2_DIS_0);

    rkisp_iowrite32(params_vdev,
                    ISP2X_PACK_4BYTE(arg->sw_rawawb_rotu2_ls0, arg->sw_rawawb_rotu3_ls0, arg->sw_rawawb_rotu4_ls0,
                                     arg->sw_rawawb_rotu5_ls0),
                    ISP21_RAWAWB_YUV_INTERP_CURVE_UCOOR_0);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_th0_ls0, arg->sw_rawawb_th1_ls0),
                    ISP21_RAWAWB_YUV_INTERP_CURVE_TH0_0);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_th2_ls0, arg->sw_rawawb_th3_ls0),
                    ISP21_RAWAWB_YUV_INTERP_CURVE_TH1_0);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_th4_ls0, arg->sw_rawawb_th5_ls0),
                    ISP21_RAWAWB_YUV_INTERP_CURVE_TH2_0);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_coor_x1_ls1_y, arg->sw_rawawb_vec_x21_ls1_y),
                    ISP21_RAWAWB_YUV_X_COOR_Y_1);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_coor_x1_ls1_u, arg->sw_rawawb_vec_x21_ls1_u),
                    ISP21_RAWAWB_YUV_X_COOR_U_1);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_coor_x1_ls1_v, arg->sw_rawawb_vec_x21_ls1_v),
                    ISP21_RAWAWB_YUV_X_COOR_V_1);

    rkisp_iowrite32(
        params_vdev,
        ISP2X_PACK_4BYTE(arg->sw_rawawb_dis_x1x2_ls1, 0, arg->sw_rawawb_rotu0_ls1, arg->sw_rawawb_rotu1_ls1),
        ISP21_RAWAWB_YUV_X1X2_DIS_1);

    rkisp_iowrite32(params_vdev,
                    ISP2X_PACK_4BYTE(arg->sw_rawawb_rotu2_ls1, arg->sw_rawawb_rotu3_ls1, arg->sw_rawawb_rotu4_ls1,
                                     arg->sw_rawawb_rotu5_ls1),
                    ISP21_RAWAWB_YUV_INTERP_CURVE_UCOOR_1);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_th0_ls1, arg->sw_rawawb_th1_ls1),
                    ISP21_RAWAWB_YUV_INTERP_CURVE_TH0_1);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_th2_ls1, arg->sw_rawawb_th3_ls1),
                    ISP21_RAWAWB_YUV_INTERP_CURVE_TH1_1);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_th4_ls1, arg->sw_rawawb_th5_ls1),
                    ISP21_RAWAWB_YUV_INTERP_CURVE_TH2_1);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_coor_x1_ls2_y, arg->sw_rawawb_vec_x21_ls2_y),
                    ISP21_RAWAWB_YUV_X_COOR_Y_2);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_coor_x1_ls2_u, arg->sw_rawawb_vec_x21_ls2_u),
                    ISP21_RAWAWB_YUV_X_COOR_U_2);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_coor_x1_ls2_v, arg->sw_rawawb_vec_x21_ls2_v),
                    ISP21_RAWAWB_YUV_X_COOR_V_2);

    rkisp_iowrite32(
        params_vdev,
        ISP2X_PACK_4BYTE(arg->sw_rawawb_dis_x1x2_ls2, 0, arg->sw_rawawb_rotu0_ls2, arg->sw_rawawb_rotu1_ls2),
        ISP21_RAWAWB_YUV_X1X2_DIS_2);

    rkisp_iowrite32(params_vdev,
                    ISP2X_PACK_4BYTE(arg->sw_rawawb_rotu2_ls2, arg->sw_rawawb_rotu3_ls2, arg->sw_rawawb_rotu4_ls2,
                                     arg->sw_rawawb_rotu5_ls2),
                    ISP21_RAWAWB_YUV_INTERP_CURVE_UCOOR_2);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_th0_ls2, arg->sw_rawawb_th1_ls2),
                    ISP21_RAWAWB_YUV_INTERP_CURVE_TH0_2);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_th2_ls2, arg->sw_rawawb_th3_ls2),
                    ISP21_RAWAWB_YUV_INTERP_CURVE_TH1_2);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_th4_ls2, arg->sw_rawawb_th5_ls2),
                    ISP21_RAWAWB_YUV_INTERP_CURVE_TH2_2);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_coor_x1_ls3_y, arg->sw_rawawb_vec_x21_ls3_y),
                    ISP21_RAWAWB_YUV_X_COOR_Y_3);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_coor_x1_ls3_u, arg->sw_rawawb_vec_x21_ls3_u),
                    ISP21_RAWAWB_YUV_X_COOR_U_3);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_coor_x1_ls3_v, arg->sw_rawawb_vec_x21_ls3_v),
                    ISP21_RAWAWB_YUV_X_COOR_V_3);

    rkisp_iowrite32(
        params_vdev,
        ISP2X_PACK_4BYTE(arg->sw_rawawb_dis_x1x2_ls3, 0, arg->sw_rawawb_rotu0_ls3, arg->sw_rawawb_rotu1_ls3),
        ISP21_RAWAWB_YUV_X1X2_DIS_3);

    rkisp_iowrite32(params_vdev,
                    ISP2X_PACK_4BYTE(arg->sw_rawawb_rotu2_ls3, arg->sw_rawawb_rotu3_ls3, arg->sw_rawawb_rotu4_ls3,
                                     arg->sw_rawawb_rotu5_ls3),
                    ISP21_RAWAWB_YUV_INTERP_CURVE_UCOOR_3);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_th0_ls3, arg->sw_rawawb_th1_ls3),
                    ISP21_RAWAWB_YUV_INTERP_CURVE_TH0_3);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_th2_ls3, arg->sw_rawawb_th3_ls3),
                    ISP21_RAWAWB_YUV_INTERP_CURVE_TH1_3);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_th4_ls3, arg->sw_rawawb_th5_ls3),
                    ISP21_RAWAWB_YUV_INTERP_CURVE_TH2_3);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_wt0, arg->sw_rawawb_wt1), ISP21_RAWAWB_RGB2XY_WT01);

    rkisp_iowrite32(params_vdev, arg->sw_rawawb_wt2, ISP21_RAWAWB_RGB2XY_WT2);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_mat0_x, arg->sw_rawawb_mat0_y),
                    ISP21_RAWAWB_RGB2XY_MAT0_XY);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_mat1_x, arg->sw_rawawb_mat1_y),
    ...
    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_nor_x0_0, arg->sw_rawawb_nor_x1_0),
                    ISP21_RAWAWB_XY_DETC_NOR_X_0);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_nor_y0_0, arg->sw_rawawb_nor_y1_0),
                    ISP21_RAWAWB_XY_DETC_NOR_Y_0);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_big_x0_0, arg->sw_rawawb_big_x1_0),
                    ISP21_RAWAWB_XY_DETC_BIG_X_0);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_big_y0_0, arg->sw_rawawb_big_y1_0),
                    ISP21_RAWAWB_XY_DETC_BIG_Y_0);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_nor_x0_1, arg->sw_rawawb_nor_x1_1),
                    ISP21_RAWAWB_XY_DETC_NOR_X_1);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_nor_y0_1, arg->sw_rawawb_nor_y1_1),
                    ISP21_RAWAWB_XY_DETC_NOR_Y_1);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_big_x0_1, arg->sw_rawawb_big_x1_1),
                    ISP21_RAWAWB_XY_DETC_BIG_X_1);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_big_y0_1, arg->sw_rawawb_big_y1_1),
                    ISP21_RAWAWB_XY_DETC_BIG_Y_1);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_nor_x0_2, arg->sw_rawawb_nor_x1_2),
                    ISP21_RAWAWB_XY_DETC_NOR_X_2);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_nor_y0_2, arg->sw_rawawb_nor_y1_2),
                    ISP21_RAWAWB_XY_DETC_NOR_Y_2);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_big_x0_2, arg->sw_rawawb_big_x1_2),
                    ISP21_RAWAWB_XY_DETC_BIG_X_2);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_big_y0_2, arg->sw_rawawb_big_y1_2),
                    ISP21_RAWAWB_XY_DETC_BIG_Y_2);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_nor_x0_3, arg->sw_rawawb_nor_x1_3),
                    ISP21_RAWAWB_XY_DETC_NOR_X_3);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_nor_y0_3, arg->sw_rawawb_nor_y1_3),
                    ISP21_RAWAWB_XY_DETC_NOR_Y_3);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_big_x0_3, arg->sw_rawawb_big_x1_3),
                    ISP21_RAWAWB_XY_DETC_BIG_X_3);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_big_y0_3, arg->sw_rawawb_big_y1_3),
                    ISP21_RAWAWB_XY_DETC_BIG_Y_3);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_nor_x0_4, arg->sw_rawawb_nor_x1_4),
                    ISP21_RAWAWB_XY_DETC_NOR_X_4);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_nor_y0_4, arg->sw_rawawb_nor_y1_4),
                    ISP21_RAWAWB_XY_DETC_NOR_Y_4);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_big_x0_4, arg->sw_rawawb_big_x1_4),
                    ISP21_RAWAWB_XY_DETC_BIG_X_4);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_big_y0_4, arg->sw_rawawb_big_y1_4),
                    ISP21_RAWAWB_XY_DETC_BIG_Y_4);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_nor_x0_5, arg->sw_rawawb_nor_x1_5),
                    ISP21_RAWAWB_XY_DETC_NOR_X_5);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_nor_y0_5, arg->sw_rawawb_nor_y1_5),
                    ISP21_RAWAWB_XY_DETC_NOR_Y_5);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_big_x0_5, arg->sw_rawawb_big_x1_5),
                    ISP21_RAWAWB_XY_DETC_BIG_X_5);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_big_y0_5, arg->sw_rawawb_big_y1_5),
                    ISP21_RAWAWB_XY_DETC_BIG_Y_5);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_nor_x0_6, arg->sw_rawawb_nor_x1_6),
                    ISP21_RAWAWB_XY_DETC_NOR_X_6);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_nor_y0_6, arg->sw_rawawb_nor_y1_6),
                    ISP21_RAWAWB_XY_DETC_NOR_Y_6);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_big_x0_6, arg->sw_rawawb_big_x1_6),
                    ISP21_RAWAWB_XY_DETC_BIG_X_6);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_2SHORT(arg->sw_rawawb_big_y0_6, arg->sw_rawawb_big_y1_6),
...
static void isp_rawawb_enable(struct rkisp_isp_params_vdev *params_vdev, bool en)
{
    u32 awb_ctrl;
...
static void isp_rawhstlite_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_rawhistlite_cfg *arg)
{
    ...
    /* avoid to override the old enable value */
    hist_ctrl = rkisp_ioread32(params_vdev, ISP_RAWHIST_LITE_CTRL);
    hist_ctrl &= ISP2X_RAWHSTLITE_CTRL_EN_MASK;
    hist_ctrl &= ~ISP2X_REG_WR_MASK;
    hist_ctrl = hist_ctrl | ISP2X_RAWHSTLITE_CTRL_MODE_SET(arg->mode) |
                ISP2X_RAWHSTLITE_CTRL_DATASEL_SET(arg->data_sel) | ISP2X_RAWHSTLITE_CTRL_WATERLINE_SET(arg->waterline) |
                ISP2X_RAWHSTLITE_CTRL_STEPSIZE_SET(arg->stepsize);
    rkisp_iowrite32(params_vdev, hist_ctrl, ISP_RAWHIST_LITE_CTRL);

    rkisp_iowrite32(params_vdev, ISP2X_RAWHSTLITE_OFFS_SET(arg->win.h_offs & 0xFFFE, arg->win.v_offs & 0xFFFE),
    ...
    block_hsize &= 0xFFFE;
    block_vsize &= 0xFFFE;
    rkisp_iowrite32(params_vdev, ISP2X_RAWHSTLITE_SIZE_SET(block_hsize, block_vsize), ISP_RAWHIST_LITE_SIZE);
                                   ...
                                   u32 blk_no, bool is_check)
{
    ...
    if (is_check && !(rkisp_ioread32(params_vdev, addr + ISP_RAWHIST_BIG_CTRL) & value)) {
        return;
    }
    ...
    for (i = 0; i < hist_wnd_num[wnd_num_idx]; i++) {
        for (j = 0; j < hist_wnd_num[wnd_num_idx]; j++) {
            weight15x15[i * ISP2X_RAWHSTBIG_ROW_NUM + j] = arg->weight[i * hist_wnd_num[wnd_num_idx] + j];
        }
    }
                                 ...
                                 u32 blk_no)
{
    ...
    hist_ctrl &= ~ISP2X_REG_WR_MASK;
    hist_ctrl = hist_ctrl | ISP2X_RAWHSTBIG_CTRL_MODE_SET(arg->mode) | ISP2X_RAWHSTBIG_CTRL_DATASEL_SET(arg->data_sel) |
                ISP2X_RAWHSTBIG_CTRL_WATERLINE_SET(arg->waterline) | ISP2X_RAWHSTBIG_CTRL_WNDNUM_SET(arg->wnd_num) |
                ISP2X_RAWHSTBIG_CTRL_STEPSIZE_SET(arg->stepsize);
    rkisp_iowrite32(params_vdev, hist_ctrl, addr + ISP_RAWHIST_BIG_CTRL);

    rkisp_iowrite32(params_vdev, ISP2X_RAWHSTBIG_OFFS_SET(arg->win.h_offs & 0xFFFE, arg->win.v_offs & 0xFFFE),
    ...
    block_hsize &= 0xFFFE;
    block_vsize &= 0xFFFE;
    rkisp_iowrite32(params_vdev, ISP2X_RAWHSTBIG_SIZE_SET(block_hsize, block_vsize), addr + ISP_RAWHIST_BIG_SIZE);

    rkisp_iowrite32(params_vdev, ISP2X_PACK_4BYTE(arg->rcc, arg->gcc, arg->bcc, arg->off),
...
static void isp_rawhstbig_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 blk_no)
{
    u32 hist_ctrl;
    u32 addr;
...
static void isp_rawhst1_enable(struct rkisp_isp_params_vdev *params_vdev, bool en)
{
...
static void isp_rawhst3_enable(struct rkisp_isp_params_vdev *params_vdev, bool en)
{
    isp_rawhstbig_enable(params_vdev, en, 0);
}

static void isp_hdrmge_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_hdrmge_cfg *arg,
                              enum rkisp_params_type type)
{
    ...
    if (type == RKISP_PARAMS_IMD || type == RKISP_PARAMS_ALL) {
        value = ISP2X_PACK_4BYTE(arg->ms_dif_0p8, arg->ms_diff_0p15, arg->lm_dif_0p9, arg->lm_dif_0p15);
...
static void isp_hdrmge_enable(struct rkisp_isp_params_vdev *params_vdev, bool en)
{
}

static void isp_hdrdrc_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp21_drc_cfg *arg,
                              enum rkisp_params_type type)
{
    u32 i, value;
    ...
    value = (arg->sw_drc_offset_pow2 & 0x0F) << 28 | (arg->sw_drc_compres_scl & 0x1FFF) << 14 |
            (arg->sw_drc_position & 0x03FFF);
    rkisp_iowrite32(params_vdev, value, ISP21_DRC_CTRL1);

    value = (arg->sw_drc_delta_scalein & 0xFF) << 24 | (arg->sw_drc_hpdetail_ratio & 0xFFF) << 12 |
    ...
    value = (arg->sw_drc_force_sgm_inv0 & 0xFFFF) << 16 | (arg->sw_drc_motion_scl & 0xFF) << 8 |
    ...
    for (i = 0; i < ISP21_DRC_Y_NUM / 2; i++) {
        value = ISP2X_PACK_2SHORT(arg->sw_drc_gain_y[2 * i], arg->sw_drc_gain_y[2 * i + 1]);
    ...
    for (i = 0; i < ISP21_DRC_Y_NUM / 2; i++) {
        value = ISP2X_PACK_2SHORT(arg->sw_drc_compres_y[2 * i], arg->sw_drc_compres_y[2 * i + 1]);
    ...
    for (i = 0; i < ISP21_DRC_Y_NUM / 2; i++) {
        value = ISP2X_PACK_2SHORT(arg->sw_drc_scale_y[2 * i], arg->sw_drc_scale_y[2 * i + 1]);
...
static void isp_hdrdrc_enable(struct rkisp_isp_params_vdev *params_vdev, bool en)
{
    u32 value;
    bool real_en;
    ...
    if (en) {
        value |= ISP_DRC_EN;
        rkisp_set_bits(params_vdev->dev, ISP_CTRL1, ISP2X_SYS_ADRC_FST, ISP2X_SYS_ADRC_FST, false);
    } else {
        value = 0;
    }
...
static void isp_gic_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp21_gic_cfg *arg)
{
    u32 value;
    s32 i;
...
static void isp_gic_enable(struct rkisp_isp_params_vdev *params_vdev, bool en)
{
    u32 value = 0;
...
static void isp_dhaz_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp21_dhaz_cfg *arg)
{
    u32 i, value;
    ...
    value |= (arg->enhance_en & 0x1) << 20 | (arg->air_lc_en & 0x1) << 16 | (arg->hpara_en & 0x1) << 12 |
             (arg->hist_en & 0x1) << 8 | (arg->dc_en & 0x1) << 4;
    rkisp_iowrite32(params_vdev, value, ISP21_DHAZ_CTRL);

    value = ISP2X_PACK_4BYTE(arg->dc_min_th, arg->dc_max_th, arg->yhist_th, arg->yblk_th);
    rkisp_iowrite32(params_vdev, value, ISP21_DHAZ_ADP0);

    value = ISP2X_PACK_4BYTE(arg->bright_min, arg->bright_max, arg->wt_max, 0);
    rkisp_iowrite32(params_vdev, value, ISP21_DHAZ_ADP1);

    value = ISP2X_PACK_4BYTE(arg->air_min, arg->air_max, arg->dark_th, arg->tmax_base);
...
static void isp_dhaz_enable(struct rkisp_isp_params_vdev *params_vdev, bool en)
{
    u32 value;
    bool real_en;
    ...
    if (en) {
        ...
        rkisp_set_bits(params_vdev->dev, ISP_CTRL1, ISP2X_SYS_DHAZ_FST, ISP2X_SYS_DHAZ_FST, false);
    } else {
        value &= ~ISP_DHAZ_ENMUX;
    }
...
static void isp_3dlut_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_3dlut_cfg *arg)
{
    struct rkisp_isp_params_val_v21 *priv_val;
    u32 value, buf_idx, i;
    u32 *data;
...
static void isp_3dlut_enable(struct rkisp_isp_params_vdev *params_vdev, bool en)
{
    u32 value;
    bool en_state;
...
static void isp_ldch_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp2x_ldch_cfg *arg)
{
    struct rkisp_device *dev = params_vdev->dev;
    struct rkisp_isp_params_val_v21 *priv_val;
...
static void isp_ldch_enable(struct rkisp_isp_params_vdev *params_vdev, bool en)
{
    struct rkisp_device *dev = params_vdev->dev;
    struct rkisp_isp_params_val_v21 *priv_val;
    u32 buf_idx;
...
static void isp_ynr_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp21_ynr_cfg *arg)
{
    u32 i, value;
    ...
    value |= (arg->sw_ynr_thumb_mix_cur_en & 0x1) << 24 | (arg->sw_ynr_global_gain_alpha & 0xF) << 20 |
             (arg->sw_ynr_global_gain & 0x3FF) << 8 | (arg->sw_ynr_flt1x1_bypass_sel & 0x3) << 6 |
             (arg->sw_ynr_sft5x5_bypass & 0x1) << 5 | (arg->sw_ynr_flt1x1_bypass & 0x1) << 4 |
             (arg->sw_ynr_lgft3x3_bypass & 0x1) << 3 | (arg->sw_ynr_lbft5x5_bypass & 0x1) << 2 |
    ...
    value = (arg->sw_ynr_low_bi_weight & 0xFF) << 24 | (arg->sw_ynr_low_weight & 0xFF) << 16 |
    ...
    value = ISP2X_PACK_4BYTE(arg->sw_ynr_base_filter_weight0, arg->sw_ynr_base_filter_weight1,
                             arg->sw_ynr_base_filter_weight2, 0);
    rkisp_iowrite32(params_vdev, value, ISP21_YNR_HIGHNR_BASE_FILTER_WEIGHT);

    value = (arg->sw_ynr_low_gauss1_coeff2 & 0xFFFF) << 16 | (arg->sw_ynr_low_gauss1_coeff1 & 0xFF) << 8 |
            (arg->sw_ynr_low_gauss1_coeff0 & 0xFF);
    rkisp_iowrite32(params_vdev, value, ISP21_YNR_GAUSS1_COEFF);

    value = (arg->sw_ynr_low_gauss2_coeff2 & 0xFFFF) << 16 | (arg->sw_ynr_low_gauss2_coeff1 & 0xFF) << 8 |
            (arg->sw_ynr_low_gauss2_coeff0 & 0xFF);
    rkisp_iowrite32(params_vdev, value, ISP21_YNR_GAUSS2_COEFF);

    value = ISP2X_PACK_4BYTE(arg->sw_ynr_direction_weight0, arg->sw_ynr_direction_weight1,
                             arg->sw_ynr_direction_weight2, arg->sw_ynr_direction_weight3);
    rkisp_iowrite32(params_vdev, value, ISP21_YNR_DIRECTION_W_0_3);

    value = ISP2X_PACK_4BYTE(arg->sw_ynr_direction_weight4, arg->sw_ynr_direction_weight5,
                             arg->sw_ynr_direction_weight6, arg->sw_ynr_direction_weight7);
    rkisp_iowrite32(params_vdev, value, ISP21_YNR_DIRECTION_W_4_7);

    for (i = 0; i < ISP21_YNR_XY_NUM / 2; i++) {
        value = ISP2X_PACK_2SHORT(arg->sw_ynr_luma_points_x[2 * i], arg->sw_ynr_luma_points_x[2 * i + 1]);
    ...
    for (i = 0; i < ISP21_YNR_XY_NUM / 2; i++) {
        value = ISP2X_PACK_2SHORT(arg->sw_ynr_lsgm_y[2 * i], arg->sw_ynr_lsgm_y[2 * i + 1]);
    ...
    for (i = 0; i < ISP21_YNR_XY_NUM / 2; i++) {
        value = ISP2X_PACK_2SHORT(arg->sw_ynr_hsgm_y[2 * i], arg->sw_ynr_hsgm_y[2 * i + 1]);
    ...
    for (i = 0; i < ISP21_YNR_XY_NUM / 4; i++) {
        value = ISP2X_PACK_4BYTE(arg->sw_ynr_rnr_strength3[4 * i], arg->sw_ynr_rnr_strength3[4 * i + 1],
                                 arg->sw_ynr_rnr_strength3[4 * i + 2], arg->sw_ynr_rnr_strength3[4 * i + 3]);
...
static void isp_ynr_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, const struct isp21_ynr_cfg *arg)
{
    u32 ynr_ctrl, value = 0;
    bool real_en;

    if (arg) {
        value = (arg->sw_ynr_thumb_mix_cur_en & 0x1) << CIF_ISP_EXP_OFFSET_TWF |
                (arg->sw_ynr_global_gain_alpha & 0xF) << CIF_ISP_EXP_OFFSET_TW |
                (arg->sw_ynr_global_gain & 0x3FF) << CIF_DUAL_CROP_SP_MODE_RAW |
                (arg->sw_ynr_flt1x1_bypass_sel & 0x3) << CIF_IMG_EFF_CTRL_MODE_SHARPEN_SHIFT |
                (arg->sw_ynr_sft5x5_bypass & 0x1) << CIF_IMG_EFF_CTRL_MODE_SKETCH_SHIFT |
                (arg->sw_ynr_flt1x1_bypass & 0x1) << CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT |
                (arg->sw_ynr_lgft3x3_bypass & 0x1) << CIF_IMG_EFF_CTRL_MODE_COLOR_SEL_SHIFT |
                (arg->sw_ynr_lbft5x5_bypass & 0x1) << CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT |
                (arg->sw_ynr_bft3x3_bypass & 0x1) << 1;
    }
    ...
    if (en) {
        value |= ISP21_YNR_EN;
        rkisp_set_bits(params_vdev->dev, ISP_CTRL1, ISP2X_SYS_YNR_FST, ISP2X_SYS_YNR_FST, false);
    }

    rkisp_iowrite32(params_vdev, value, ISP21_YNR_GLOBAL_CTRL);
}

static void isp_cnr_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp21_cnr_cfg *arg)
{
    u32 value;
    ...
    value |= (arg->sw_cnr_thumb_mix_cur_en & 0x1) << CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT |
             (arg->sw_cnr_lq_bila_bypass & 0x1) << CIF_IMG_EFF_CTRL_MODE_COLOR_SEL_SHIFT |
             (arg->sw_cnr_hq_bila_bypass & 0x1) << CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT |
    ...
    value = ISP2X_PACK_4BYTE(arg->sw_cnr_gain_1sigma, arg->sw_cnr_gain_offset, arg->sw_cnr_gain_iso, 0);
    ...
    value = ISP2X_PACK_4BYTE(arg->sw_cnr_lbf5_weit_d0, arg->sw_cnr_lbf5_weit_d1, arg->sw_cnr_lbf5_weit_d2,
                             arg->sw_cnr_lbf5_weit_d3);
    ...
    value = (arg->sw_cnr_hbf5_weit_src & 0xFF) << CIF_ISP_EXP_OFFSET_TWF |
            (arg->sw_cnr_hbf5_min_wgt & 0xFF) << CIF_ISP_EXP_OFFSET_THI | (arg->sw_cnr_hbf5_sigma & 0xFFFF);
...
static void isp_cnr_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, const struct isp21_cnr_cfg *arg)
{
    ...
    if (en) {
        value |= ISP21_CNR_EN;
        rkisp_set_bits(params_vdev->dev, ISP_CTRL1, ISP2X_SYS_CNR_FST, ISP2X_SYS_CNR_FST, false);
    }

    rkisp_iowrite32(params_vdev, value, ISP21_CNR_CTRL);
}

static void isp_sharp_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp21_sharp_cfg *arg)
{
    u32 value;
    ...
    value = ISP2X_PACK_4BYTE(arg->sw_sharp_pbf_ratio, arg->sw_sharp_gaus_ratio, arg->sw_sharp_bf_ratio,
                             arg->sw_sharp_sharp_ratio);
    rkisp_iowrite32(params_vdev, value, ISP21_SHARP_SHARP_RATIO);

    value = (arg->sw_sharp_luma_dx[CIF_IMG_EFF_CTRL_MODE_SHARPEN_SHIFT] & 0x0F) << CIF_ISP_EXP_OFFSET_TWF |
            (arg->sw_sharp_luma_dx[CIF_IMG_EFF_CTRL_MODE_SKETCH_SHIFT] & 0x0F) << CIF_ISP_EXP_OFFSET_TW |
            (arg->sw_sharp_luma_dx[CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT] & 0x0F) << CIF_ISP_EXP_OFFSET_THI |
            (arg->sw_sharp_luma_dx[CIF_IMG_EFF_CTRL_MODE_COLOR_SEL_SHIFT] & 0x0F) << CIF_ISP_EXP_OFFSET_TWE |
            (arg->sw_sharp_luma_dx[CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT] & 0x0F) << CIF_DUAL_CROP_SP_MODE_RAW |
            (arg->sw_sharp_luma_dx[1] & 0x0F) << CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT | (arg->sw_sharp_luma_dx[0] & 0x0F);
    rkisp_iowrite32(params_vdev, value, ISP21_SHARP_SHARP_LUMA_DX);

    value = (arg->sw_sharp_pbf_sigma_inv[CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT] & 0x3FF) << CIF_ISP_EXP_OFFSET_TW |
            (arg->sw_sharp_pbf_sigma_inv[1] & 0x3FF) << CIF_ISP_EXP_OFFSET_TEN |
            (arg->sw_sharp_pbf_sigma_inv[0] & 0x3FF);
    rkisp_iowrite32(params_vdev, value, ISP21_SHARP_SHARP_PBF_SIGMA_INV_0);

    value = (arg->sw_sharp_pbf_sigma_inv[CIF_IMG_EFF_CTRL_MODE_SKETCH_SHIFT] & 0x3FF) << CIF_ISP_EXP_OFFSET_TW |
            (arg->sw_sharp_pbf_sigma_inv[CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT] & 0x3FF) << CIF_ISP_EXP_OFFSET_TEN |
            (arg->sw_sharp_pbf_sigma_inv[CIF_IMG_EFF_CTRL_MODE_COLOR_SEL_SHIFT] & 0x3FF);
    rkisp_iowrite32(params_vdev, value, ISP21_SHARP_SHARP_PBF_SIGMA_INV_1);

    value = (arg->sw_sharp_pbf_sigma_inv[CIF_ISP_EXP_OFFSET_SEV] & 0x3FF) << CIF_ISP_EXP_OFFSET_TEN |
            (arg->sw_sharp_pbf_sigma_inv[CIF_IMG_EFF_CTRL_MODE_SHARPEN_SHIFT] & 0x3FF);
    rkisp_iowrite32(params_vdev, value, ISP21_SHARP_SHARP_PBF_SIGMA_INV_2);

    value = (arg->sw_sharp_bf_sigma_inv[CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT] & 0x3FF) << CIF_ISP_EXP_OFFSET_TW |
            (arg->sw_sharp_bf_sigma_inv[1] & 0x3FF) << CIF_ISP_EXP_OFFSET_TEN | (arg->sw_sharp_bf_sigma_inv[0] & 0x3FF);
    rkisp_iowrite32(params_vdev, value, ISP21_SHARP_SHARP_BF_SIGMA_INV_0);

    value = (arg->sw_sharp_bf_sigma_inv[CIF_IMG_EFF_CTRL_MODE_SKETCH_SHIFT] & 0x3FF) << CIF_ISP_EXP_OFFSET_TW |
            (arg->sw_sharp_bf_sigma_inv[CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT] & 0x3FF) << CIF_ISP_EXP_OFFSET_TEN |
            (arg->sw_sharp_bf_sigma_inv[CIF_IMG_EFF_CTRL_MODE_COLOR_SEL_SHIFT] & 0x3FF);
    rkisp_iowrite32(params_vdev, value, ISP21_SHARP_SHARP_BF_SIGMA_INV_1);

    value = (arg->sw_sharp_bf_sigma_inv[CIF_ISP_EXP_OFFSET_SEV] & 0x3FF) << CIF_ISP_EXP_OFFSET_TEN |
            (arg->sw_sharp_bf_sigma_inv[CIF_IMG_EFF_CTRL_MODE_SHARPEN_SHIFT] & 0x3FF);
    rkisp_iowrite32(params_vdev, value, ISP21_SHARP_SHARP_BF_SIGMA_INV_2);

    value = (arg->sw_sharp_bf_sigma_shift & 0x0F) << CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT |
            (arg->sw_sharp_pbf_sigma_shift & 0x0F);
    rkisp_iowrite32(params_vdev, value, ISP21_SHARP_SHARP_SIGMA_SHIFT);

    value = (arg->sw_sharp_ehf_th[CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT] & 0x3FF) << CIF_ISP_EXP_OFFSET_TW |
            (arg->sw_sharp_ehf_th[1] & 0x3FF) << CIF_ISP_EXP_OFFSET_TEN | (arg->sw_sharp_ehf_th[0] & 0x3FF);
    rkisp_iowrite32(params_vdev, value, ISP21_SHARP_SHARP_EHF_TH_0);

    value = (arg->sw_sharp_ehf_th[CIF_IMG_EFF_CTRL_MODE_SKETCH_SHIFT] & 0x3FF) << CIF_ISP_EXP_OFFSET_TW |
            (arg->sw_sharp_ehf_th[CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT] & 0x3FF) << CIF_ISP_EXP_OFFSET_TEN |
            (arg->sw_sharp_ehf_th[CIF_IMG_EFF_CTRL_MODE_COLOR_SEL_SHIFT] & 0x3FF);
    rkisp_iowrite32(params_vdev, value, ISP21_SHARP_SHARP_EHF_TH_1);

    value = (arg->sw_sharp_ehf_th[CIF_ISP_EXP_OFFSET_SEV] & 0x3FF) << CIF_ISP_EXP_OFFSET_TEN |
            (arg->sw_sharp_ehf_th[CIF_IMG_EFF_CTRL_MODE_SHARPEN_SHIFT] & 0x3FF);
    rkisp_iowrite32(params_vdev, value, ISP21_SHARP_SHARP_EHF_TH_2);

    value = (arg->sw_sharp_clip_hf[CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT] & 0x3FF) << CIF_ISP_EXP_OFFSET_TW |
            (arg->sw_sharp_clip_hf[1] & 0x3FF) << CIF_ISP_EXP_OFFSET_TEN | (arg->sw_sharp_clip_hf[0] & 0x3FF);
    rkisp_iowrite32(params_vdev, value, ISP21_SHARP_SHARP_CLIP_HF_0);

    value = (arg->sw_sharp_clip_hf[CIF_IMG_EFF_CTRL_MODE_SKETCH_SHIFT] & 0x3FF) << CIF_ISP_EXP_OFFSET_TW |
            (arg->sw_sharp_clip_hf[CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT] & 0x3FF) << CIF_ISP_EXP_OFFSET_TEN |
            (arg->sw_sharp_clip_hf[CIF_IMG_EFF_CTRL_MODE_COLOR_SEL_SHIFT] & 0x3FF);
    rkisp_iowrite32(params_vdev, value, ISP21_SHARP_SHARP_CLIP_HF_1);

    value = (arg->sw_sharp_clip_hf[CIF_ISP_EXP_OFFSET_SEV] & 0x3FF) << CIF_ISP_EXP_OFFSET_TEN |
            (arg->sw_sharp_clip_hf[CIF_IMG_EFF_CTRL_MODE_SHARPEN_SHIFT] & 0x3FF);
    rkisp_iowrite32(params_vdev, value, ISP21_SHARP_SHARP_CLIP_HF_2);

    value = ISP2X_PACK_4BYTE(arg->sw_sharp_pbf_coef_0, arg->sw_sharp_pbf_coef_1, arg->sw_sharp_pbf_coef_2, 0);
    rkisp_iowrite32(params_vdev, value, ISP21_SHARP_SHARP_PBF_COEF);

    value = ISP2X_PACK_4BYTE(arg->sw_sharp_bf_coef_0, arg->sw_sharp_bf_coef_1, arg->sw_sharp_bf_coef_2, 0);
    rkisp_iowrite32(params_vdev, value, ISP21_SHARP_SHARP_BF_COEF);

    value = ISP2X_PACK_4BYTE(arg->sw_sharp_gaus_coef_0, arg->sw_sharp_gaus_coef_1, arg->sw_sharp_gaus_coef_2, 0);
    rkisp_iowrite32(params_vdev, value, ISP21_SHARP_SHARP_GAUS_COEF);
}

static void isp_sharp_enable(struct rkisp_isp_params_vdev *params_vdev, bool en)
{
    u32 value;
...
static void isp_baynr_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp21_baynr_cfg *arg)
{
    u32 i, value;
    ...
    value |= (arg->sw_baynr_gauss_en & 0x1) << CIF_DUAL_CROP_SP_MODE_RAW | (arg->sw_baynr_log_bypass & 0x1)
                                                                               << CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT;
    ...
    for (i = 0; i < ISP21_BAYNR_XY_NUM / CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT; i++) {
        value = ISP2X_PACK_2SHORT(arg->sw_sigma_x[CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT * i],
                                  arg->sw_sigma_x[CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT * i + 1]);
        rkisp_iowrite32(params_vdev, value, ISP21_BAYNR_SIGMAX01 + CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT * i);
    }

    for (i = 0; i < ISP21_BAYNR_XY_NUM / CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT; i++) {
        value = ISP2X_PACK_2SHORT(arg->sw_sigma_y[CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT * i],
                                  arg->sw_sigma_y[CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT * i + 1]);
        rkisp_iowrite32(params_vdev, value, ISP21_BAYNR_SIGMAY01 + CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT * i);
    }

    value = (arg->weit_d2 & 0x3FF) << CIF_ISP_EXP_OFFSET_TW | (arg->weit_d1 & 0x3FF) << CIF_ISP_EXP_OFFSET_TEN |
            (arg->weit_d0 & 0x3FF);
    rkisp_iowrite32(params_vdev, value, ISP21_BAYNR_WRIT_D);
}

static void isp_baynr_enable(struct rkisp_isp_params_vdev *params_vdev, bool en)
{
    u32 value;
...
static void isp_bay3d_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp21_bay3d_cfg *arg)
{
    u32 i, value;
    ...
    value |= (arg->sw_bay3d_exp_sel & 0x1) << CIF_ISP_EXP_OFFSET_THI |
             (arg->sw_bay3d_bypass_en & 0x1) << CIF_ISP_EXP_OFFSET_TWE |
             (arg->sw_bay3d_pk_en & 0x1) << CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT;
    ...
    for (i = 0; i < ISP21_BAY3D_XY_NUM / CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT; i++) {
        value = ISP2X_PACK_2SHORT(arg->sw_bay3d_sig_x[CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT * i],
                                  arg->sw_bay3d_sig_x[CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT * i + 1]);
        rkisp_iowrite32(params_vdev, value, ISP21_BAY3D_SIG_X0 + CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT * i);
    }

    for (i = 0; i < ISP21_BAY3D_XY_NUM / CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT; i++) {
        value = ISP2X_PACK_2SHORT(arg->sw_bay3d_sig_y[CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT * i],
                                  arg->sw_bay3d_sig_y[CIF_IMG_EFF_CTRL_MODE_SEPIA_SHIFT * i + 1]);
        rkisp_iowrite32(params_vdev, value, ISP21_BAY3D_SIG_Y0 + CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT * i);
    }
}

static void isp_bay3d_enable(struct rkisp_isp_params_vdev *params_vdev, bool en)
{
    struct rkisp_device *ispdev = params_vdev->dev;
    struct rkisp_isp_params_val_v21 *priv_val;
    ...
    if ((en && (bay3d_ctrl & ISP21_BAY3D_EN)) || (!en && !(bay3d_ctrl & ISP21_BAY3D_EN))) {
        return;
    }
    ...
    if (en) {
        ...
        rkisp_set_bits(params_vdev->dev, MI_RD_CTRL2, BAY3D_RW_ONEADDR_EN, BAY3D_RW_ONEADDR_EN, false);
        rkisp_set_bits(params_vdev->dev, MI_WR_CTRL2, SW_BAY3D_WR_AUTOUPD | SW_BAY3D_FORCEUPD,
                       SW_BAY3D_WR_AUTOUPD | SW_BAY3D_FORCEUPD, false);

        rkisp_set_bits(params_vdev->dev, ISP_CTRL1, ISP2X_SYS_BAY3D_FST, ISP2X_SYS_BAY3D_FST, false);
...
static void isp_csm_config(struct rkisp_isp_params_vdev *params_vdev, const struct isp21_csm_cfg *arg)
{
    u32 i, val, eff_ctrl, cproc_ctrl;

    for (i = 0; i < ISP21_CSM_COEFF_NUM; i++) {
        if (i == 0) {
            val = (arg->csm_y_offset & 0x3f) << CIF_ISP_EXP_OFFSET_TWF |
                  (arg->csm_c_offset & 0xff) << CIF_ISP_EXP_OFFSET_THI | (arg->csm_coeff[i] & 0x1ff);
        } else {
            val = arg->csm_coeff[i] & 0x1ff;
        }
        rkisp_iowrite32(params_vdev, val, ISP_CC_COEFF_0 + i * CIF_IMG_EFF_CTRL_MODE_EMBOSS_SHIFT);
    }
...
static MAYBE_UNUSED void _isp_isr_other_config(struct rkisp_isp_params_vdev *params_vdev,
                                               const struct isp21_isp_params_cfg *new_params,
                                               enum rkisp_params_type type)
{
    struct rkisp_isp_params_v21_ops *ops = (struct rkisp_isp_params_v21_ops *)params_vdev->priv_ops;
...
static MAYBE_UNUSED void _isp_isr_other_en(struct rkisp_isp_params_vdev *params_vdev,
                                           const struct isp21_isp_params_cfg *new_params, enum rkisp_params_type type)
{
    struct rkisp_isp_params_v21_ops *ops = (struct rkisp_isp_params_v21_ops *)params_vdev->priv_ops;
    struct rkisp_isp_params_val_v21 *priv_val = (struct rkisp_isp_params_val_v21 *)params_vdev->priv_val;
    struct rkisp_device *ispdev = params_vdev->dev;
    ...
    if (is_feature_on) {
        ...
        if (module_en_update & ~iq_feature) {
            dev_err(ispdev->dev, "some iq features(0x%llx, 0x%llx) are not supported\n", module_en_update, iq_feature);
            module_en_update &= iq_feature;
        }
    }
...
static MAYBE_UNUSED void _isp_isr_meas_config(struct rkisp_isp_params_vdev *params_vdev,
                                              struct isp21_isp_params_cfg *new_params, enum rkisp_params_type type)
{
    struct rkisp_isp_params_v21_ops *ops = (struct rkisp_isp_params_v21_ops *)params_vdev->priv_ops;
    u64 module_cfg_update = new_params->module_cfg_update;
...
static MAYBE_UNUSED void _isp_isr_meas_en(struct rkisp_isp_params_vdev *params_vdev,
                                          struct isp21_isp_params_cfg *new_params, enum rkisp_params_type type)
{
    struct rkisp_isp_params_v21_ops *ops = (struct rkisp_isp_params_v21_ops *)params_vdev->priv_ops;
    struct rkisp_device *ispdev = params_vdev->dev;
...
static MAYBE_UNUSED void _isp_config_hdrshd(struct rkisp_isp_params_vdev *params_vdev)
{
    struct rkisp_isp_params_v21_ops *ops = (struct rkisp_isp_params_v21_ops *)params_vdev->priv_ops;
    struct rkisp_isp_params_val_v21 *priv_val = (struct rkisp_isp_params_val_v21 *)params_vdev->priv_val;
...
static void rkisp_params_cfgsram_v21(struct rkisp_isp_params_vdev *params_vdev)
{
...
static void rkisp_alloc_bay3d_buf(struct rkisp_isp_params_vdev *params_vdev,
                                  const struct isp21_isp_params_cfg *new_params)
{
    struct rkisp_device *ispdev = params_vdev->dev;
    ...
    if ((module_en_update & ISP2X_MODULE_BAY3D) && (module_ens & ISP2X_MODULE_BAY3D)) {
...
static void rkisp_params_first_cfg_v2x(struct rkisp_isp_params_vdev *params_vdev)
{
    ...
    if (!params_vdev->isp21_params->module_cfg_update && !params_vdev->isp21_params->module_en_update) {
    ...
    priv_val->lsc_en = 0;
    priv_val->mge_en = 0;
    _isp_isr_other_config(params_vdev, params_vdev->isp21_params, RKISP_PARAMS_ALL);
    _isp_isr_other_en(params_vdev, params_vdev->isp21_params, RKISP_PARAMS_ALL);
    _isp_isr_meas_config(params_vdev, params_vdev->isp21_params, RKISP_PARAMS_ALL);
    _isp_isr_meas_en(params_vdev, params_vdev->isp21_params, RKISP_PARAMS_ALL);
    if (width <= ISP2X_AUTO_BIGMODE_WIDTH && size > ISP2X_NOBIG_OVERFLOW_SIZE) {
        rkisp_set_bits(params_vdev->dev, ISP_CTRL1, ISP2X_SYS_BIGMODE_MANUAL | ISP2X_SYS_BIGMODE_FORCEEN,
...
static u32 rkisp_get_ldch_meshsize(struct rkisp_isp_params_vdev *params_vdev, struct rkisp_ldchbuf_size *ldchsize)
{
...
static int rkisp_init_ldch_buf(struct rkisp_isp_params_vdev *params_vdev, struct rkisp_ldchbuf_size *ldchsize)
{
    struct device *dev = params_vdev->dev->dev;
    struct rkisp_isp_params_val_v21 *priv_val;
    struct isp2x_ldch_head *ldch_head;
...
static void rkisp_get_param_size_v2x(struct rkisp_isp_params_vdev *params_vdev, unsigned int sizes[])
{
    sizes[0] = sizeof(struct isp2x_isp_params_cfg);
}

static void rkisp_params_get_ldchbuf_inf_v2x(struct rkisp_isp_params_vdev *params_vdev, void *buf)
{
    struct rkisp_isp_params_val_v21 *priv_val;
...
static void rkisp_params_set_ldchbuf_size_v2x(struct rkisp_isp_params_vdev *params_vdev, void *size)
{
...
static void rkisp_params_stream_stop_v2x(struct rkisp_isp_params_vdev *params_vdev)
{
    struct rkisp_device *ispdev = params_vdev->dev;
    struct rkisp_isp_params_val_v21 *priv_val;
...
static void rkisp_params_fop_release_v2x(struct rkisp_isp_params_vdev *params_vdev)
{
    rkisp_deinit_ldch_buf(params_vdev);
}

/* Not called when the camera active, thus not isr protection. */
static void rkisp_params_disable_isp_v2x(struct rkisp_isp_params_vdev *params_vdev)
{
    struct rkisp_isp_params_v21_ops *ops = (struct rkisp_isp_params_v21_ops *)params_vdev->priv_ops;
...
static void ldch_data_abandon(struct rkisp_isp_params_vdev *params_vdev, struct isp21_isp_params_cfg *params)
{
    ...
    priv_val = (struct rkisp_isp_params_val_v21 *)params_vdev->priv_val;
    for (i = 0; i < ISP2X_LDCH_BUF_NUM; i++) {
        if (arg->buf_fd == priv_val->buf_ldch[i].dma_fd && priv_val->buf_ldch[i].vaddr) {
            ldch_head = (struct isp2x_ldch_head *)priv_val->buf_ldch[i].vaddr;
            ldch_head->stat = LDCH_BUF_CHIPINUSE;
            break;
        }
...
static void rkisp_params_cfg_v2x(struct rkisp_isp_params_vdev *params_vdev, u32 frame_id, enum rkisp_params_type type)
{
    ...
    while (!list_empty(&params_vdev->params) && !cur_buf) {
        ...
        if (new_params->frame_id < frame_id) {
            ...
            if (list_empty(&params_vdev->params)) {
                ...
                _isp_isr_other_en(params_vdev, new_params, type);
                _isp_isr_meas_en(params_vdev, new_params, type);
            }
    ...
    new_params = (struct isp21_isp_params_cfg *)(cur_buf->vaddr[0]);
    _isp_isr_other_config(params_vdev, new_params, type);
    _isp_isr_other_en(params_vdev, new_params, type);
    _isp_isr_meas_config(params_vdev, new_params, type);
    _isp_isr_meas_en(params_vdev, new_params, type);
    if (!hw_dev->is_single && type != RKISP_PARAMS_SHD) {
        _isp_config_hdrshd(params_vdev);
    }
...
static void rkisp_params_clear_fstflg(struct rkisp_isp_params_vdev *params_vdev)
{
    ...
    if (value & ISP2X_SYS_YNR_FST) {
        rkisp_clear_bits(params_vdev->dev, ISP_CTRL1, ISP2X_SYS_YNR_FST, false);
    }
    if (value & ISP2X_SYS_ADRC_FST) {
        rkisp_clear_bits(params_vdev->dev, ISP_CTRL1, ISP2X_SYS_ADRC_FST, false);
    }
    if (value & ISP2X_SYS_DHAZ_FST) {
        rkisp_clear_bits(params_vdev->dev, ISP_CTRL1, ISP2X_SYS_DHAZ_FST, false);
    }
    if (value & ISP2X_SYS_CNR_FST) {
        rkisp_clear_bits(params_vdev->dev, ISP_CTRL1, ISP2X_SYS_CNR_FST, false);
    }
    if (value & ISP2X_SYS_BAY3D_FST) {
        rkisp_clear_bits(params_vdev->dev, ISP_CTRL1, ISP2X_SYS_BAY3D_FST, false);
    }
}

static void rkisp_params_isr_v2x(struct rkisp_isp_params_vdev *params_vdev, u32 isp_mis)
{
    struct rkisp_device *dev = params_vdev->dev;
    u32 cur_frame_id;
