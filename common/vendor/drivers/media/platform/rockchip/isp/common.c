...
{
    void IO_MEM *base = dev->hw_dev->base_addr;
    u32 i;

    if (end > RKISP_ISP_SW_REG_SIZE - RKISP_MAX_PIPELINE) {
        dev_err(dev->dev, "%s out of range\n", __func__);
        return;
    }
    for (i = start; i <= end; i += RKISP_MAX_PIPELINE) {
...
int rkisp_alloc_common_dummy_buf(struct rkisp_device *dev)
{
    ...
    if (hw->max_in.w && hw->max_in.h)
        size = hw->max_in.w * hw->max_in.h * RKISP_MAX_SENSOR;
