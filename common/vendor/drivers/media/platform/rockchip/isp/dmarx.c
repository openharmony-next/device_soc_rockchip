...
static int dmarx_config_mi(struct rkisp_stream *stream)
{
    ...
    void IO_MEM *base = dev->base_addr;
    struct capture_fmt *dmarx_in_fmt = &stream->out_isp_fmt;

    v4l2_dbg(1, rkisp_debug, &dev->v4l2_dev, "%s %dx%d y_stride:%d\n", __func__, stream->out_fmt.width,
             stream->out_fmt.height, stream->u.dmarx.y_stride);

    mi_set_y_size(stream, stream->out_fmt.width * stream->out_fmt.height);
    ...
    dmarx_ctrl(base, dmarx_in_fmt->write_format | dmarx_in_fmt->output_format | CIF_MI_DMA_CTRL_BURST_LEN_LUM_16 |
...
{
    void IO_MEM *base = stream->ispdev->base_addr;

    if (stream->curr_buf) {
        mi_set_y_addr(stream, stream->curr_buf->buff_addr[RKISP_PLANE_Y]);
        mi_set_cb_addr(stream, stream->curr_buf->buff_addr[RKISP_PLANE_CB]);
        mi_set_cr_addr(stream, stream->curr_buf->buff_addr[RKISP_PLANE_CR]);
        mi_dmarx_start(base);
...
static int rawrd_config_mi(struct rkisp_stream *stream)
{
    ...
    v4l2_dbg(1, rkisp_debug, &dev->v4l2_dev, "%s id:%d 0x%x %dx%d\n", __func__, stream->id, val, stream->out_fmt.width,
...
static int dmarx_frame_end(struct rkisp_stream *stream)
{
    ...
    if (!list_empty(&stream->buf_queue)) {
        stream->curr_buf = list_first_entry(&stream->buf_queue, struct rkisp_buffer, queue);
...
static int dmarx_start_streaming(struct vb2_queue *queue, unsigned int count)
{
    struct rkisp_stream *stream = queue->drv_priv;
    struct rkisp_device *dev = stream->ispdev;
    struct v4l2_device *v4l2_dev = &dev->v4l2_dev;
...
static int rkisp_set_fmt(struct rkisp_stream *stream, struct v4l2_pix_format_mplane *pixm, bool try)
{
    ...
    for (i = 0; i < planes; i++) {
        ...
        if (stream->ispdev->isp_ver == ISP_V20 && stream->id == RKISP_STREAM_RAWRD2 && fmt->fmt_type == FMT_BAYER) {
...
static int rkisp_try_fmt_vid_out_mplane(struct file *file, void *fh, struct v4l2_format *f)
{
...
static int rkisp_enum_fmt_vid_out_mplane(struct file *file, void *priv, struct v4l2_fmtdesc *f)
{
    struct rkisp_stream *stream = video_drvdata(file);
...
static int rkisp_s_fmt_vid_out_mplane(struct file *file, void *priv, struct v4l2_format *f)
{
    struct rkisp_stream *stream = video_drvdata(file);
...
static int rkisp_g_fmt_vid_out_mplane(struct file *file, void *fh, struct v4l2_format *f)
{
    struct rkisp_stream *stream = video_drvdata(file);
...
static long rkisp_ioctl_default(struct file *file, void *fh, bool valid_prio, unsigned int cmd, void *arg)
{
    ...
    switch (cmd) {
        case RKISP_CMD_GET_CSI_MEMORY_MODE:
            if (stream->id != RKISP_STREAM_RAWRD0 && stream->id != RKISP_STREAM_RAWRD1 &&
...
static int rkisp_register_dmarx_video(struct rkisp_stream *stream)
{
    ...
    rkisp_init_vb2_queue(&node->buf_queue, stream, V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE);
    vdev->queue = &node->buf_queue;

    ret = video_register_device(vdev, VFL_TYPE_VIDEO, -1);
    if (ret < 0) {
        v4l2_err(v4l2_dev, "%s failed with error %d\n", __func__, ret);
        return ret;
    }
...
static int dmarx_init(struct rkisp_device *dev, u32 id)
{
    ...
    switch (id) {
        case RKISP_STREAM_DMARX:
            ...
            break;
        case RKISP_STREAM_RAWRD0:
            strlcpy(vdev->name, DMARX0_VDEV_NAME, sizeof(vdev->name));
            stream->ops = &rkisp2_dmarx_streams_ops;
            stream->config = &rkisp2_dmarx0_stream_config;
            break;
        case RKISP_STREAM_RAWRD1:
            strlcpy(vdev->name, DMARX1_VDEV_NAME, sizeof(vdev->name));
            stream->ops = &rkisp2_dmarx_streams_ops;
            stream->config = &rkisp2_dmarx1_stream_config;
            break;
        case RKISP_STREAM_RAWRD2:
            strlcpy(vdev->name, DMARX2_VDEV_NAME, sizeof(vdev->name));
            stream->ops = &rkisp2_dmarx_streams_ops;
    ...
    return media_create_pad_link(source, 0, sink, RKISP_ISP_PAD_SINK, stream->linked);
}
...
void rkisp_rawrd_set_pic_size(struct rkisp_device *dev, u32 width, u32 height)
{
    ...
    if (dev->isp_ver == ISP_V20 && sdev->in_fmt.fmt_type == FMT_BAYER && sdev->out_fmt.fmt_type != FMT_BAYER &&
