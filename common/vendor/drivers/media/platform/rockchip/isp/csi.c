...
static void get_remote_mipi_sensor(struct rkisp_device *dev, struct v4l2_subdev **sensor_sd, u32 function)
{
...
static int rkisp_csi_link_setup(struct media_entity *entity, const struct media_pad *local,
...
static int rkisp_csi_g_mbus_config(struct v4l2_subdev *sd, unsigned int pad_id, struct v4l2_mbus_config *config)
{
...
static int rkisp_csi_get_set_fmt(struct v4l2_subdev *sd, struct v4l2_subdev_pad_config *cfg,
...
static int csi_config(struct rkisp_csi_device *csi)
{
    ...
    if (mipi_sensor) {
        ctrl = v4l2_ctrl_find(mipi_sensor->ctrl_handler, CIFISP_CID_EMB_VC);
        if (ctrl) {
            emd_vc = v4l2_ctrl_g_ctrl(ctrl);
        }

        ctrl = v4l2_ctrl_find(mipi_sensor->ctrl_handler, CIFISP_CID_EMB_DT);
        if (ctrl) {
    ...
    if (emd_vc <= CIF_ISP_ADD_DATA_VC_MAX) {
        for (i = 0; i < RKISP_EMDDATA_FIFO_MAX; i++) {
            ret = kfifo_alloc(&dev->emd_data_fifo[i].mipi_kfifo, CIFISP_ADD_DATA_FIFO_SIZE, GFP_ATOMIC);
            if (ret) {
                v4l2_err(&dev->v4l2_dev, "kfifo_alloc failed with error %d\n", ret);
                return ret;
            }
        }
    }

    if (dev->isp_ver == ISP_V13 || dev->isp_ver == ISP_V12) {
        /* lanes */
        ...
        rkisp_write(dev, CIF_ISP_CSI0_MASK3,
                    CIF_ISP_CSI0_IMASK_FRAME_END(0x3F) | CIF_ISP_CSI0_IMASK_RAW0_OUT_V_END |
                        CIF_ISP_CSI0_IMASK_RAW1_OUT_V_END | CIF_ISP_CSI0_IMASK_LINECNT,
                    true);
        ...
        v4l2_dbg(1, rkisp_debug, &dev->v4l2_dev,
                 ...
                 "CSI0_IDS 0x%08x\n"
                 "CSI0_MASK3 0x%08x\n",
                 rkisp_read(dev, CIF_ISP_CSI0_CTRL1, true), rkisp_read(dev, CIF_ISP_CSI0_DATA_IDS_1, true),
        ...
        if (mipi_sensor) {
            ret = v4l2_subdev_call(mipi_sensor, core, ioctl, RKMODULE_GET_HDR_CFG, &hdr_cfg);
            if (!ret) {
        ...
        if (dev->hdr.op_mode == HDR_NORMAL && (dev->isp_inp & INP_RAWRD2 || !dev->hw_dev->is_single)) {
        ...
        if (!dev->hw_dev->is_mi_update) {
            rkisp_write(dev, CSI2RX_CTRL0, SW_IBUF_OP_MODE(dev->hdr.op_mode) | SW_HDR_ESP_MODE(dev->hdr.esp_mode),
                        true);
        }
        ...
        /* set interrupts mask */
        val = PHY_ERR_SOTHS | PHY_ERR_SOTSYNCHS | PHY_ERR_EOTSYNCHS | PHY_ERR_ESC | PHY_ERR_CTL;
        rkisp_write(dev, CSI2RX_MASK_PHY, val, true);
        val = PACKET_ERR_F_BNDRY_MATCG | PACKET_ERR_F_SEQ | PACKET_ERR_FRAME_DATA | PACKET_ERR_ECC_1BIT |
              PACKET_ERR_ECC_2BIT | PACKET_ERR_CHECKSUM;
        rkisp_write(dev, CSI2RX_MASK_PACKET, val, true);
        val = AFIFO0_OVERFLOW | AFIFO1X_OVERFLOW | LAFIFO1X_OVERFLOW | AFIFO2X_OVERFLOW | IBUFX3_OVERFLOW |
              IBUF3R_OVERFLOW | Y_STAT_AFIFOX3_OVERFLOW;
        rkisp_write(dev, CSI2RX_MASK_OVERFLOW, val, true);
        val = RAW0_WR_FRAME | RAW1_WR_FRAME | RAW2_WR_FRAME | MIPI_DROP_FRM | RAW_WR_SIZE_ERR | MIPI_LINECNT |
              RAW_RD_SIZE_ERR | RAW0_Y_STATE | RAW1_Y_STATE | RAW2_Y_STATE;
        ...
        v4l2_dbg(1, rkisp_debug, &dev->v4l2_dev, "CSI2RX_IDS 0x%08x 0x%08x\n", rkisp_read(dev, CSI2RX_DATA_IDS_1, true),
                 rkisp_read(dev, CSI2RX_DATA_IDS_2, true));
    } else {
        mipi_ctrl = CIF_MIPI_CTRL_NUM_LANES(lanes - 1) | CIF_MIPI_CTRL_SHUTDOWNLANES(0xf) |
                    CIF_MIPI_CTRL_ERR_SOT_SYNC_HS_SKIP | CIF_MIPI_CTRL_CLOCKLANE_ENA;

        rkisp_write(dev, CIF_MIPI_CTRL, mipi_ctrl, true);

        /* Configure Data Type and Virtual Channel */
        rkisp_write(dev, CIF_MIPI_IMG_DATA_SEL, csi->mipi_di[0], true);

        rkisp_write(dev, CIF_MIPI_ADD_DATA_SEL_1, CIF_MIPI_DATA_SEL_DT(emd_dt) | CIF_MIPI_DATA_SEL_VC(emd_vc), true);
        rkisp_write(dev, CIF_MIPI_ADD_DATA_SEL_2, CIF_MIPI_DATA_SEL_DT(emd_dt) | CIF_MIPI_DATA_SEL_VC(emd_vc), true);
        rkisp_write(dev, CIF_MIPI_ADD_DATA_SEL_3, CIF_MIPI_DATA_SEL_DT(emd_dt) | CIF_MIPI_DATA_SEL_VC(emd_vc), true);
        rkisp_write(dev, CIF_MIPI_ADD_DATA_SEL_4, CIF_MIPI_DATA_SEL_DT(emd_dt) | CIF_MIPI_DATA_SEL_VC(emd_vc), true);
        ...
        rkisp_write(dev, CIF_MIPI_IMSC,
                    CIF_MIPI_FRAME_END | CIF_MIPI_ERR_CSI | CIF_MIPI_ERR_DPHY | CIF_MIPI_SYNC_FIFO_OVFLW(0x0F) |
        ...
        v4l2_dbg(1, rkisp_debug, &dev->v4l2_dev,
                 ...
                 "  MIPI_STATUS 0x%08x\n"
                 "  MIPI_IMSC 0x%08x\n",
                 rkisp_read(dev, CIF_MIPI_CTRL, true), rkisp_read(dev, CIF_MIPI_IMG_DATA_SEL, true),
                 rkisp_read(dev, CIF_MIPI_STATUS, true), rkisp_read(dev, CIF_MIPI_IMSC, true));
    }
...
int rkisp_csi_config_patch(struct rkisp_device *dev)
{
    ...
    if (dev->isp_inp & INP_CSI) {
        ...
        if (dev->isp_inp & INP_CIF) {
            ...
            if (dev->isp_ver == ISP_V30 && !mode.is_rdbk) {
                ...
                if (dev->hdr.op_mode != HDR_NORMAL && mipi_sensor) {
                    ...
                    v4l2_subdev_call(mipi_sensor, core, ioctl, RKISP_VICAP_CMD_INIT_BUF, &cnt);
                }
            }
        } else {
        ...
        if (!dev->hw_dev->is_mi_update) {
            rkisp_unite_write(dev, CSI2RX_CTRL0, SW_IBUF_OP_MODE(dev->hdr.op_mode), true, dev->hw_dev->is_unite);
        }
        ...
        rkisp_unite_set_bits(dev, CSI2RX_MASK_STAT, 0, RAW_RD_SIZE_ERR, true, dev->hw_dev->is_unite);
    }

    if (IS_HDR_RDBK(dev->hdr.op_mode)) {
        rkisp_unite_set_bits(dev, CTRL_SWS_CFG, 0, SW_MPIP_DROP_FRM_DIS, true, dev->hw_dev->is_unite);
    }

    if (dev->isp_ver == ISP_V30) {
        rkisp_unite_set_bits(dev, CTRL_SWS_CFG, 0, ISP3X_SW_ACK_FRM_PRO_DIS, true, dev->hw_dev->is_unite);
    }
...
int rkisp_register_csi_subdev(struct rkisp_device *dev, struct v4l2_device *v4l2_dev)
{
    ...
    csi_dev->pads[CSI_SINK].flags = MEDIA_PAD_FL_SINK | MEDIA_PAD_FL_MUST_CONNECT;
    csi_dev->pads[CSI_SRC_CH0].flags = MEDIA_PAD_FL_SOURCE | MEDIA_PAD_FL_MUST_CONNECT;
    ...
    ret = media_entity_pads_init(&sd->entity, csi_dev->max_pad, csi_dev->pads);
    if (ret < 0) {
        return ret;
    }
