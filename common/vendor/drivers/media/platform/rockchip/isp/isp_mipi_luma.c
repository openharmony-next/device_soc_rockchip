...
static int rkisp_luma_enum_fmt_meta_cap(struct file *file, void *priv, struct v4l2_fmtdesc *f)
{
...
static int rkisp_luma_g_fmt_meta_cap(struct file *file, void *priv, struct v4l2_format *f)
{
    struct video_device *video = video_devdata(file);
    struct rkisp_luma_vdev *luma_vdev = video_get_drvdata(video);
...
static const struct v4l2_ioctl_ops rkisp_luma_ioctl = {.vidioc_reqbufs = vb2_ioctl_reqbufs,
...
struct v4l2_file_operations rkisp_luma_fops = {.mmap = vb2_fop_mmap,
                                               .unlocked_ioctl = video_ioctl2,
                                               .poll = vb2_fop_poll,
                                               .open = rkisp_luma_fh_open,
                                               .release = rkisp_luma_fop_release};

static int rkisp_luma_vb2_queue_setup(struct vb2_queue *vq, unsigned int *num_buffers, unsigned int *num_planes,
                                      unsigned int sizes[], struct device *alloc_ctxs[])
{
    ...
    *num_buffers = clamp_t(u32, *num_buffers, RKISP_ISP_LUMA_REQ_BUFS_MIN, RKISP_ISP_LUMA_REQ_BUFS_MAX);
...
static void rkisp_luma_vb2_stop_streaming(struct vb2_queue *vq)
{
    ...
    for (i = 0; i < RKISP_ISP_LUMA_REQ_BUFS_MAX; i++) {
        ...
        buf = list_first_entry(&luma_vdev->stat, struct rkisp_buffer, queue);
        list_del(&buf->queue);
...
static int rkisp_luma_vb2_start_streaming(struct vb2_queue *queue, unsigned int count)
{
...
static int rkisp_luma_init_vb2_queue(struct vb2_queue *q, struct rkisp_luma_vdev *luma_vdev)
{
...
static void rkisp_stats_send_luma(struct rkisp_luma_vdev *vdev, struct rkisp_luma_readout_work *work)
{
    ...
    if (!list_empty(&vdev->stat)) {
        cur_buf = list_first_entry(&vdev->stat, struct rkisp_buffer, queue);
        list_del(&cur_buf->queue);
    }
    spin_unlock(&vdev->rd_lock);

    if (!cur_buf) {
        v4l2_warn(vdev->vnode.vdev.v4l2_dev, "no luma buffer available\n");
        return;
    }

    cur_stat_buf = (struct rkisp_isp2x_luma_buffer *)(cur_buf->vaddr[0]);
    if (!cur_stat_buf) {
        v4l2_err(vdev->vnode.vdev.v4l2_dev, "cur_stat_buf is NULL\n");
        return;
    }
    ...
    for (i = 0; i < ISP2X_MIPI_RAW_MAX; i++) {
        for (j = 0; j < ISP2X_MIPI_LUMA_MEAN_MAX; j++) {
            cur_stat_buf->luma[i].exp_mean[j] = work->luma[i].exp_mean[j];
        }
    }

    cur_frame_id = cur_stat_buf->frame_id;
    vb2_set_plane_payload(&cur_buf->vb.vb2_buf, 0, sizeof(struct rkisp_isp2x_luma_buffer));
...
static void rkisp_luma_readout_task(unsigned long data)
{
    ...
    struct rkisp_luma_vdev *vdev = (struct rkisp_luma_vdev *)data;

    while (!kfifo_is_empty(&vdev->rd_kfifo)) {
        out = kfifo_out(&vdev->rd_kfifo, &work, sizeof(work));
        if (!out) {
            break;
        }
...
void rkisp_luma_isr(struct rkisp_luma_vdev *luma_vdev, u32 isp_stat)
{
    ...
    unsigned int cur_frame_id = atomic_read(&luma_vdev->dev->isp_sdev.frm_sync_seq) - 1;
    ...
    if (isp_stat & RAW0_WR_FRAME) {
        if (luma_vdev->ystat_isrcnt[0] != RKISP_LUMA_YSTAT_ISR_NUM) {
            v4l2_dbg(1, rkisp_debug, luma_vdev->vnode.vdev.v4l2_dev, "missing raw0 y state isr, %d\n",
                     luma_vdev->ystat_isrcnt[0]);
    ...
    if (isp_stat & RAW1_WR_FRAME) {
        if (luma_vdev->ystat_isrcnt[1] != RKISP_LUMA_YSTAT_ISR_NUM) {
            v4l2_dbg(1, rkisp_debug, luma_vdev->vnode.vdev.v4l2_dev, "missing raw1 y state isr, %d\n",
                     luma_vdev->ystat_isrcnt[1]);
    ...
    if (isp_stat & RAW2_WR_FRAME) {
        if (luma_vdev->ystat_isrcnt[2] != RKISP_LUMA_YSTAT_ISR_NUM) {
            v4l2_dbg(1, rkisp_debug, luma_vdev->vnode.vdev.v4l2_dev, "missing raw2 y state isr, %d\n",
                     luma_vdev->ystat_isrcnt[2]);
    ...
    if (luma_vdev->ystat_isrcnt[0] == RKISP_LUMA_YSTAT_ISR_NUM && !luma_vdev->ystat_rdflg[0]) {
        ...
        for (i = 0; i < ISP2X_MIPI_LUMA_MEAN_MAX; i++) {
            luma_vdev->work.luma[0].exp_mean[i] = readl(base + CSI2RX_Y_STAT_RO);
        }
    ...
    if (luma_vdev->ystat_isrcnt[1] == RKISP_LUMA_YSTAT_ISR_NUM && !luma_vdev->ystat_rdflg[1]) {
        ...
        writel(value, base + CSI2RX_Y_STAT_CTRL);
        for (i = 0; i < ISP2X_MIPI_LUMA_MEAN_MAX; i++) {
            luma_vdev->work.luma[1].exp_mean[i] = readl(base + CSI2RX_Y_STAT_RO);
        }
    ...
    if (luma_vdev->ystat_isrcnt[2] == RKISP_LUMA_YSTAT_ISR_NUM && !luma_vdev->ystat_rdflg[2]) {
        ...
        writel(value, base + CSI2RX_Y_STAT_CTRL);
        for (i = 0; i < ISP2X_MIPI_LUMA_MEAN_MAX; i++) {
            luma_vdev->work.luma[2].exp_mean[i] = readl(base + CSI2RX_Y_STAT_RO);
        }
    ...
    if (frm_mode == RKISP_LUMA_THREEFRM) {
        if (luma_vdev->ystat_rdflg[0] && luma_vdev->ystat_rdflg[1] && luma_vdev->ystat_rdflg[2]) {
            send_task = true;
        }
    ...
    if (send_task) {
        ...
        if (frm_mode == RKISP_LUMA_THREEFRM) {
            luma_vdev->work.meas_type = ISP2X_RAW0_Y_STATE | ISP2X_RAW1_Y_STATE | ISP2X_RAW2_Y_STATE;
        } else if (frm_mode == RKISP_LUMA_TWOFRM) {
        ...
        if (!kfifo_is_full(&luma_vdev->rd_kfifo)) {
            kfifo_in(&luma_vdev->rd_kfifo, &luma_vdev->work, sizeof(luma_vdev->work));
        } else {
            v4l2_err(luma_vdev->vnode.vdev.v4l2_dev, "stats kfifo is full\n");
        }
...
{
    luma_vdev->vdev_fmt.fmt.meta.dataformat = V4L2_META_FMT_RK_ISP1_STAT_LUMA;
    luma_vdev->vdev_fmt.fmt.meta.buffersize = sizeof(struct rkisp_isp2x_luma_buffer);
}
...
int rkisp_register_luma_vdev(struct rkisp_luma_vdev *luma_vdev, struct v4l2_device *v4l2_dev, struct rkisp_device *dev)
{
    ...
    ret = media_create_pad_link(source, RKISP_ISP_PAD_SOURCE_STATS, sink, 0, MEDIA_LNK_FL_ENABLED);
    if (ret < 0) {
        goto err_unregister_video;
    }

    ret = kfifo_alloc(&luma_vdev->rd_kfifo, RKISP_LUMA_READOUT_WORK_SIZE, GFP_KERNEL);
    if (ret) {
        dev_err(&vdev->dev, "kfifo_alloc failed with error %d\n", ret);
        goto err_unregister_video;
    }

    tasklet_init(&luma_vdev->rd_tasklet, rkisp_luma_readout_task, (unsigned long)luma_vdev);
    tasklet_disable(&luma_vdev->rd_tasklet);
