...
static int rkcif_scale_g_fmt_vid_cap_mplane(struct file *file, void *priv, struct v4l2_format *f)
{
...
static u32 rkcif_scale_align_bits_per_pixel(struct rkcif_device *cif_dev, const struct cif_output_fmt *fmt,
                                            int plane_index)
{
    u32 bpp = 0, i;
...
static const struct cif_output_fmt *find_output_fmt(u32 pixelfmt)
{
...
static int rkcif_scale_set_fmt(struct rkcif_scale_vdev *scale_vdev, struct v4l2_pix_format_mplane *pixm, bool try)
{
    ...
    if (cif_dev->terminal_sensor.sd) {
        ...
        if (ret) {
            v4l2_err(&scale_vdev->cifdev->v4l2_dev, "%s: get sensor format failed\n", __func__);
            return ret;
        }
    ...
    if (fmt == NULL) {
        v4l2_err(&scale_vdev->cifdev->v4l2_dev, "format of source channel are not bayer raw, not support scale\n");
        return -1;
    }
...
static int rkcif_scale_s_fmt_vid_cap_mplane(struct file *file, void *priv, struct v4l2_format *f)
{
    struct rkcif_scale_vdev *scale_vdev = video_drvdata(file);
    int ret = 0;
...
static int rkcif_scale_querycap(struct file *file, void *priv, struct v4l2_capability *cap)
{
    struct rkcif_scale_vdev *scale_vdev = video_drvdata(file);
...
static long rkcif_scale_ioctl_default(struct file *file, void *fh, bool valid_prio, unsigned int cmd, void *arg)
{
    struct rkcif_scale_vdev *scale_vdev = video_drvdata(file);
...
static int rkcif_scale_try_fmt_vid_cap_mplane(struct file *file, void *fh, struct v4l2_format *f)
{
    struct rkcif_scale_vdev *scale_vdev = video_drvdata(file);
    int ret = 0;
...
static int rkcif_scale_enum_frameintervals(struct file *file, void *fh, struct v4l2_frmivalenum *fival)
{
    struct rkcif_scale_vdev *scale_vdev = video_drvdata(file);
    struct rkcif_device *dev = scale_vdev->cifdev;
...
static int rkcif_scale_enum_framesizes(struct file *file, void *prov, struct v4l2_frmsizeenum *fsize)
{
    ...
    if (terminal_sensor && terminal_sensor->sd) {
        get_input_fmt(terminal_sensor->sd, &input_rect, 0, &csi_info);
    }
...
struct v4l2_file_operations rkcif_scale_fops = {.mmap = vb2_fop_mmap,
                                                .unlocked_ioctl = video_ioctl2,
                                                .poll = vb2_fop_poll,
                                                .open = rkcif_scale_fh_open,
                                                .release = rkcif_scale_fop_release};

static int rkcif_scale_vb2_queue_setup(struct vb2_queue *queue, unsigned int *num_buffers, unsigned int *num_planes,
...
static void rkcif_scale_vb2_buf_queue(struct vb2_buffer *vb)
{
    ...
    for (i = 0; i < fmt->mplanes; i++) {
        ...
        if (rkcif_debug && addr && !hw_dev->iommu_en) {
            ...
            v4l2_dbg(1, rkcif_debug, &scale_vdev->cifdev->v4l2_dev, "Clear buffer, size: 0x%08x\n",
...
static int rkcif_scale_stop(struct rkcif_scale_vdev *scale_vdev)
{
    ...
    rkcif_write_register_and(dev, CIF_REG_SCL_CH_CTRL,
                             ~(CIF_SCALE_EN(ch) | CIF_SCALE_SW_SRC_CH(0x1f, ch) | CIF_SCALE_SW_MODE(0x03, ch)));
...
static void rkcif_scale_vb2_stop_streaming(struct vb2_queue *vq)
{
    ...
    if (scale_vdev->next_buf && scale_vdev->next_buf != scale_vdev->curr_buf) {
    ...
    while (!list_empty(&scale_vdev->buf_head)) {
        buf = list_first_entry(&scale_vdev->buf_head, struct rkcif_buffer, queue);
        list_del(&buf->queue);
...
static void rkcif_assign_scale_buffer_init(struct rkcif_scale_vdev *scale_vdev, int ch)
{
    ...
    if (!scale_vdev->curr_buf) {
        if (!list_empty(&scale_vdev->buf_head)) {
            scale_vdev->curr_buf = list_first_entry(&scale_vdev->buf_head, struct rkcif_buffer, queue);
            list_del(&scale_vdev->curr_buf->queue);
        }
    }

    if (scale_vdev->curr_buf) {
        rkcif_write_register(dev, frm0_addr, scale_vdev->curr_buf->buff_addr[RKCIF_PLANE_Y]);
    }

    if (!scale_vdev->next_buf) {
        if (!list_empty(&scale_vdev->buf_head)) {
            scale_vdev->next_buf = list_first_entry(&scale_vdev->buf_head, struct rkcif_buffer, queue);
            list_del(&scale_vdev->next_buf->queue);
        }
    }

    if (scale_vdev->next_buf) {
        rkcif_write_register(dev, frm1_addr, scale_vdev->next_buf->buff_addr[RKCIF_PLANE_Y]);
    }

    spin_unlock_irqrestore(&scale_vdev->vbq_lock, flags);
}

static int rkcif_assign_scale_buffer_update(struct rkcif_scale_vdev *scale_vdev, int channel_id)
{
    struct rkcif_device *dev = scale_vdev->cifdev;
    ...
    frm_addr = scale_vdev->frame_phase & CIF_CSI_FRAME0_READY ? get_reg_index_of_scale_frm0_addr(channel_id)
    ...
    if (buffer) {
        rkcif_write_register(dev, frm_addr, buffer->buff_addr[RKCIF_PLANE_Y]);
    } else {
        ret = -EINVAL;
        v4l2_info(&dev->v4l2_dev, "not active buffer,skip frame, scale ch[%d]\n", scale_vdev->ch);
    }
    return ret;
}

static int rkcif_assign_scale_buffer_pingpong(struct rkcif_scale_vdev *scale_vdev, int init, int channel_id)
{
    int ret = 0;
...
static int rkcif_scale_channel_set(struct rkcif_scale_vdev *scale_vdev)
{
    ...
    rkcif_write_register_and(dev, CIF_REG_GLB_INTST, ~(SCALE_END_INTSTAT(ch) | SCALE_FIFO_OVERFLOW(ch)));
    rkcif_write_register_or(
        dev, CIF_REG_GLB_INTEN,
        (SCALE_END_INTSTAT(ch) | SCALE_FIFO_OVERFLOW(ch) | SCALE_TOISP_AXI0_ERR | SCALE_TOISP_AXI1_ERR));
    val = CIF_SCALE_SW_PRESS_ENABLE | CIF_SCALE_SW_PRESS_VALUE(7) | CIF_SCALE_SW_HURRY_ENABLE |
          CIF_SCALE_SW_HURRY_VALUE(7) | CIF_SCALE_SW_WATER_LINE(1);

    rkcif_write_register(dev, CIF_REG_SCL_CTRL, val);
    val = scale_vdev->blc.pattern00 | (scale_vdev->blc.pattern01 << 8) | (scale_vdev->blc.pattern02 << 16) |
          (scale_vdev->blc.pattern03 << 24);
    rkcif_write_register(dev, CIF_REG_SCL_BLC_CH0 + ch, val);
    rkcif_write_register(dev, get_reg_index_of_scale_vlw(ch), scale_vdev->ch_info.vir_width);
    val =
        CIF_SCALE_SW_SRC_CH(scale_vdev->ch_src, ch) | CIF_SCALE_SW_MODE(scale_vdev->scale_mode, ch) | CIF_SCALE_EN(ch);
...
destroy_buf:
    if (scale_vdev->next_buf) {
        vb2_buffer_done(&scale_vdev->next_buf->vb.vb2_buf, VB2_BUF_STATE_QUEUED);
    }
    if (scale_vdev->curr_buf) {
        vb2_buffer_done(&scale_vdev->curr_buf->vb.vb2_buf, VB2_BUF_STATE_QUEUED);
    }
    while (!list_empty(&scale_vdev->buf_head)) {
...
static int rkcif_scale_vb2_start_streaming(struct vb2_queue *queue, unsigned int count)
{
    struct rkcif_scale_vdev *scale_vdev = queue->drv_priv;
    struct rkcif_stream *stream = scale_vdev->stream;
    int ret = 0;
...
static int rkcif_scale_init_vb2_queue(struct vb2_queue *q, struct rkcif_scale_vdev *scale_vdev,
...
static int rkcif_scale_g_ch(struct v4l2_device *v4l2_dev, unsigned int intstat)
{
    if (intstat & SCALE_END_INTSTAT(0)) {
        if ((intstat & SCALE_END_INTSTAT(0)) == SCALE_END_INTSTAT(0)) {
    ...
    if (intstat & SCALE_END_INTSTAT(1)) {
        if ((intstat & SCALE_END_INTSTAT(1)) == SCALE_END_INTSTAT(1)) {
    ...
    if (intstat & SCALE_END_INTSTAT(2)) {
        if ((intstat & SCALE_END_INTSTAT(2)) == SCALE_END_INTSTAT(2)) {
    ...
    if (intstat & SCALE_END_INTSTAT(3)) {
        if ((intstat & SCALE_END_INTSTAT(3)) == SCALE_END_INTSTAT(3)) {
...
static void rkcif_scale_vb_done_oneframe(struct rkcif_scale_vdev *scale_vdev, struct vb2_v4l2_buffer *vb_done)
{
    const struct cif_output_fmt *fmt = scale_vdev->scale_out_fmt;
    u32 i;
    ...
    for (i = 0; i < fmt->mplanes; i++) {
        vb2_set_plane_payload(&vb_done->vb2_buf, i, scale_vdev->pixm.plane_fmt[i].sizeimage);
    }
...
void rkcif_irq_handle_scale(struct rkcif_device *cif_dev, unsigned int intstat_glb)
{
    ...
    val = SCALE_FIFO_OVERFLOW(0) | SCALE_FIFO_OVERFLOW(1) | SCALE_FIFO_OVERFLOW(2) | SCALE_FIFO_OVERFLOW(3);
    if (intstat_glb & val) {
        v4l2_err(&cif_dev->v4l2_dev, "ERROR: scale channel, overflow intstat_glb:0x%x !!\n", intstat_glb);
        return;
    }

    ch = rkcif_scale_g_ch(&cif_dev->v4l2_dev, intstat_glb);
    if (ch < 0) {
        return;
    }
...
static int rkcif_register_scale_vdev(struct rkcif_scale_vdev *scale_vdev, bool is_multi_input)
{
    ...
    rkcif_scale_init_vb2_queue(&node->buf_queue, scale_vdev, V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE);
    vdev->queue = &node->buf_queue;
...
int rkcif_register_scale_vdevs(struct rkcif_device *cif_dev, int stream_num, bool is_multi_input)
{
    struct rkcif_scale_vdev *scale_vdev;
    int i, j, ret;
...
void rkcif_unregister_scale_vdevs(struct rkcif_device *cif_dev, int stream_num)
{
    struct rkcif_scale_vdev *scale_vdev;
    int i;
