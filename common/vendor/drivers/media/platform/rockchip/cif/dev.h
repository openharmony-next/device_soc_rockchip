...
enum rkcif_workmode { RKCIF_WORKMODE_ONEFRAME = 0x00, RKCIF_WORKMODE_PINGPONG = 0x01, RKCIF_WORKMODE_LINELOOP = 0x02 };
...
enum rkcif_yuvaddr_state { RKCIF_YUV_ADDR_STATE_UPDATE = 0x0, RKCIF_YUV_ADDR_STATE_INIT = 0x1 };
 ...
 * for distinguishing cropping from senosr or usr
 */
enum rkcif_crop_src { CROP_SRC_ACT = 0x0, CROP_SRC_SENSOR, CROP_SRC_USR, CROP_SRC_MAX };
...
struct rkcif_pipeline {
    ...
    int (*open)(struct rkcif_pipeline *p, struct media_entity *me, bool prepare);
 ...
 * in CIF_MIPI_INTEN
 */
enum cif_frame_ready { CIF_CSI_FRAME0_READY = 0x1, CIF_CSI_FRAME1_READY };
...
#define RKCIF_LUMA_READOUT_WORK_SIZE (9 * sizeof(struct rkcif_luma_readout_work))
...
static inline struct rkcif_vdev_node *vdev_to_node(struct video_device *vdev)
{
    return container_of(vdev, struct rkcif_vdev_node, vdev);
}

static inline struct rkcif_stream *to_rkcif_stream(struct rkcif_vdev_node *vnode)
{
...
static inline struct rkcif_scale_vdev *to_rkcif_scale_vdev(struct rkcif_vdev_node *vnode)
{
...
int rkcif_register_luma_vdev(struct rkcif_luma_vdev *luma_vdev, struct v4l2_device *v4l2_dev, struct rkcif_device *dev);
