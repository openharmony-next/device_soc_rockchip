...
static void rkcif_show_clks(struct rkcif_device *dev, struct seq_file *f)
{
    ...
    for (i = 0; i < hw->clk_size; i++) {
        seq_printf(f, "%s:%ld\n", hw->match_data->clks[i], clk_get_rate(hw->clks[i]));
    }
}
    ...
    if (sensor) {
        ...
        if (sensor->mbus.type == V4L2_MBUS_PARALLEL || sensor->mbus.type == V4L2_MBUS_BT656) {
            ...
            seq_printf(f, "\tinterface:%s\n",
                       ...
                       : sensor->mbus.type == V4L2_MBUS_CCP2      ? "lvds"
                                                                  : "unknown");
        ...
        seq_printf(f, "\thdr mode: %s\n",
                   ...
                   : dev->hdr.hdr_mode == HDR_X2 ? "hdr_x2"
                                                 : "hdr_x3");

        seq_printf(f, "\tformat:%s/%ux%u@%d\n", rkcif_pixelcode_to_string(stream->cif_fmt_in->mbus_code), rect->width,
                   rect->height, interval->interval.denominator / interval->interval.numerator);
        ...
        seq_printf(f, "\tformat:%s/%ux%u(%u,%u)\n", rkcif_pixelcode_to_string(stream->cif_fmt_out->fourcc),
                   dev->channels[0].width, dev->channels[0].height, dev->channels[0].crop_st_x,
                   dev->channels[0].crop_st_y);
