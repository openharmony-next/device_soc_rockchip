...
#define CIF_DVP_PCLK_DUAL_EDGE (V4L2_MBUS_PCLK_SAMPLE_RISING | V4L2_MBUS_PCLK_SAMPLE_FALLING)
...
static inline struct rkcif_rx_buffer *to_cif_rx_buf(struct rkisp_rx_buf *dbufs)
{
...
static struct v4l2_subdev *get_remote_sensor(struct rkcif_stream *stream, u16 *index)
{
    ...
    if (!local) {
        v4l2_err(&stream->cifdev->v4l2_dev, "%s: video pad[0] is null\n", __func__);
        return NULL;
    }

    remote = media_entity_remote_pad(local);
    if (!remote) {
        v4l2_err(&stream->cifdev->v4l2_dev, "%s: remote pad is null\n", __func__);
        return NULL;
    }
...
static void get_remote_terminal_sensor(struct rkcif_stream *stream, struct v4l2_subdev **sensor_sd)
{
...
static struct rkcif_sensor_info *sd_to_sensor(struct rkcif_device *dev, struct v4l2_subdev *sd)
{
    u32 i;
...
const struct cif_input_fmt *get_input_fmt(struct v4l2_subdev *sd, struct v4l2_rect *rect, u32 pad_id,
                                          struct csi_channel_info *csi_info)
{
    ...
    if (ret < 0) {
        v4l2_warn(sd->v4l2_dev, "sensor fmt invalid, set to default size\n");
        goto set_default;
    }
    ch_info.index = pad_id;
    ret = v4l2_subdev_call(sd, core, ioctl, RKMODULE_GET_CHANNEL_INFO, &ch_info);
    if (!ret) {
        ...
        if (ch_info.bus_fmt == MEDIA_BUS_FMT_SPD_2X8 || ch_info.bus_fmt == MEDIA_BUS_FMT_EBD_1X8) {
    ...
    v4l2_dbg(1, rkcif_debug, sd->v4l2_dev, "remote fmt: mbus code:0x%x, size:%dx%d, field: %d\n", fmt.format.code,
    ...
    for (i = 0; i < ARRAY_SIZE(in_fmts); i++) {
        if (fmt.format.code == in_fmts[i].mbus_code && fmt.format.field == in_fmts[i].field) {
            return &in_fmts[i];
        }
    }
...
static const struct cif_output_fmt *find_output_fmt(struct rkcif_stream *stream, u32 pixelfmt)
{
...
/***************************** stream operations ******************************/
static int rkcif_assign_new_buffer_oneframe(struct rkcif_stream *stream, enum rkcif_yuvaddr_state stat)
{
    ...
    if (stat == RKCIF_YUV_ADDR_STATE_INIT) {
        if (!stream->curr_buf) {
            if (!list_empty(&stream->buf_head)) {
                stream->curr_buf = list_first_entry(&stream->buf_head, struct rkcif_buffer, queue);
                list_del(&stream->curr_buf->queue);
            }
        }

        if (stream->curr_buf) {
            rkcif_write_register(dev, CIF_REG_DVP_FRM0_ADDR_Y, stream->curr_buf->buff_addr[RKCIF_PLANE_Y]);
            rkcif_write_register(dev, CIF_REG_DVP_FRM0_ADDR_UV, stream->curr_buf->buff_addr[RKCIF_PLANE_CBCR]);
        } else {
            if (dummy_buf->vaddr) {
                rkcif_write_register(dev, CIF_REG_DVP_FRM0_ADDR_Y, dummy_buf->dma_addr);
                rkcif_write_register(dev, CIF_REG_DVP_FRM0_ADDR_UV, dummy_buf->dma_addr);
            }
        }

        if (!stream->next_buf) {
            if (!list_empty(&stream->buf_head)) {
                stream->next_buf = list_first_entry(&stream->buf_head, struct rkcif_buffer, queue);
                list_del(&stream->next_buf->queue);
            }
        }

        if (stream->next_buf) {
            rkcif_write_register(dev, CIF_REG_DVP_FRM1_ADDR_Y, stream->next_buf->buff_addr[RKCIF_PLANE_Y]);
            rkcif_write_register(dev, CIF_REG_DVP_FRM1_ADDR_UV, stream->next_buf->buff_addr[RKCIF_PLANE_CBCR]);
        } else {
            if (dummy_buf->vaddr) {
                rkcif_write_register(dev, CIF_REG_DVP_FRM1_ADDR_Y, dummy_buf->dma_addr);
                rkcif_write_register(dev, CIF_REG_DVP_FRM1_ADDR_UV, dummy_buf->dma_addr);
            }
        }
        ...
        if (buffer) {
            rkcif_write_register(dev, frm_addr_y, buffer->buff_addr[RKCIF_PLANE_Y]);
            rkcif_write_register(dev, frm_addr_uv, buffer->buff_addr[RKCIF_PLANE_CBCR]);
        } else {
            if (dummy_buf->vaddr) {
            ...
            v4l2_dbg(1, rkcif_debug, &dev->v4l2_dev, "not active buffer, frame Drop\n");
        }
    }
...
static void rkcif_assign_new_buffer_init_toisp(struct rkcif_stream *stream, int channel_id)
{
    struct rkcif_device *dev = stream->cifdev;
    ...
    if (mbus_cfg->type == V4L2_MBUS_CSI2_DPHY || mbus_cfg->type == V4L2_MBUS_CSI2_CPHY ||
    ...
    if (!stream->curr_buf_toisp) {
        if (!list_empty(&stream->rx_buf_head)) {
            dbufs = list_first_entry(&stream->rx_buf_head, struct rkisp_rx_buf, list);
            if (dbufs) {
                list_del(&dbufs->list);
            }
    ...
    if (stream->curr_buf_toisp) {
        rkcif_write_register(dev, frm0_addr_y, stream->curr_buf_toisp->dummy.dma_addr);
    }
    ...
    if (stream->next_buf_toisp) {
        rkcif_write_register(dev, frm1_addr_y, stream->next_buf_toisp->dummy.dma_addr);
    }

    spin_unlock_irqrestore(&stream->vbq_lock, flags);
}

static int rkcif_assign_new_buffer_update_toisp(struct rkcif_stream *stream, int channel_id)
{
    struct rkcif_device *dev = stream->cifdev;
    struct v4l2_mbus_config *mbus_cfg = &dev->active_sensor->mbus;
    ...
    if (mbus_cfg->type == V4L2_MBUS_CSI2_DPHY || mbus_cfg->type == V4L2_MBUS_CSI2_CPHY ||
        mbus_cfg->type == V4L2_MBUS_CCP2) {
        frm_addr_y = stream->frame_phase & CIF_CSI_FRAME0_READY ? get_reg_index_of_frm0_y_addr(channel_id)
                     : get_reg_index_of_frm1_y_addr(channel_id);
    } else {
        frm_addr_y = stream->frame_phase & CIF_CSI_FRAME0_READY ? get_dvp_reg_index_of_frm0_y_addr(channel_id)
    ...
    if (buffer) {
        rkcif_write_register(dev, frm_addr_y, buffer->dummy.dma_addr);
    }
    return ret;
}

static int rkcif_assign_new_buffer_pingpong_toisp(struct rkcif_stream *stream, int init, int channel_id)
{
    int ret = 0;
...
static void rkcif_assign_new_buffer_init(struct rkcif_stream *stream, int channel_id)
{
    struct rkcif_device *dev = stream->cifdev;
    struct v4l2_mbus_config *mbus_cfg = &dev->active_sensor->mbus;
    ...
    if (stream->curr_buf) {
        rkcif_write_register(dev, frm0_addr_y, stream->curr_buf->buff_addr[RKCIF_PLANE_Y]);
        if (stream->cif_fmt_out->fmt_type != CIF_FMT_TYPE_RAW) {
            rkcif_write_register(dev, frm0_addr_uv, stream->curr_buf->buff_addr[RKCIF_PLANE_CBCR]);
        }
    } else {
        if (dummy_buf->vaddr) {
    ...
    if (stream->cif_fmt_in->field == V4L2_FIELD_INTERLACED) {
        ...
        if (stream->next_buf) {
            rkcif_write_register(dev, frm1_addr_y, stream->next_buf->buff_addr[RKCIF_PLANE_Y]);
            if (stream->cif_fmt_out->fmt_type != CIF_FMT_TYPE_RAW) {
                rkcif_write_register(dev, frm1_addr_uv, stream->next_buf->buff_addr[RKCIF_PLANE_CBCR]);
            }
        } else {
            if (dummy_buf->vaddr) {
    ...
    if (mbus_cfg->type == V4L2_MBUS_BT656) {
        ...
        for (ch_id = 0; ch_id < RKCIF_MAX_STREAM_DVP; ch_id++) {
            ...
            if (dummy_buf->dma_addr) {
                rkcif_write_register(dev, get_dvp_reg_index_of_frm0_y_addr(ch_id), dummy_buf->dma_addr);
                rkcif_write_register(dev, get_dvp_reg_index_of_frm0_uv_addr(ch_id), dummy_buf->dma_addr);
                rkcif_write_register(dev, get_dvp_reg_index_of_frm1_y_addr(ch_id), dummy_buf->dma_addr);
                rkcif_write_register(dev, get_dvp_reg_index_of_frm1_uv_addr(ch_id), dummy_buf->dma_addr);
            }
        }
    }
}

static int rkcif_assign_new_buffer_update(struct rkcif_stream *stream, int channel_id)
{
    struct rkcif_device *dev = stream->cifdev;
    struct rkcif_dummy_buffer *dummy_buf = &dev->dummy_buf;
    struct v4l2_mbus_config *mbus_cfg = &dev->active_sensor->mbus;
    ...
    if (mbus_cfg->type == V4L2_MBUS_CSI2_DPHY || mbus_cfg->type == V4L2_MBUS_CSI2_CPHY ||
        ...
        frm_addr_y = stream->frame_phase & CIF_CSI_FRAME0_READY ? get_reg_index_of_frm0_y_addr(channel_id)
                     : get_reg_index_of_frm1_y_addr(channel_id);
        frm_addr_uv = stream->frame_phase & CIF_CSI_FRAME0_READY ? get_reg_index_of_frm0_uv_addr(channel_id)
                      : get_reg_index_of_frm1_uv_addr(channel_id);
    } else {
        frm_addr_y = stream->frame_phase & CIF_CSI_FRAME0_READY ? get_dvp_reg_index_of_frm0_y_addr(channel_id)
                     : get_dvp_reg_index_of_frm1_y_addr(channel_id);
        frm_addr_uv = stream->frame_phase & CIF_CSI_FRAME0_READY ? get_dvp_reg_index_of_frm0_uv_addr(channel_id)
    ...
    if (buffer) {
        if (stream->cif_fmt_in->field == V4L2_FIELD_INTERLACED && stream->frame_phase == CIF_CSI_FRAME1_READY) {
            rkcif_write_register(dev, frm_addr_y, buffer->buff_addr[RKCIF_PLANE_Y] + (channel->virtual_width / 2));
            if (stream->cif_fmt_out->fmt_type != CIF_FMT_TYPE_RAW) {
        ...
        v4l2_info(&dev->v4l2_dev, "not active buffer, skip current frame, %s stream[%d]\n",
                  (mbus_cfg->type == V4L2_MBUS_CSI2_DPHY || mbus_cfg->type == V4L2_MBUS_CSI2_CPHY ||
                   mbus_cfg->type == V4L2_MBUS_CCP2)
                      ? "mipi/lvds"
                      : "dvp",
                  stream->id);
...
static void rkcif_assign_dummy_buffer(struct rkcif_stream *stream)
{
    ...
    if (mbus_cfg->type == V4L2_MBUS_BT656 && dummy_buf->vaddr) {
        rkcif_write_register(dev, get_dvp_reg_index_of_frm0_y_addr(stream->id), dummy_buf->dma_addr);
        rkcif_write_register(dev, get_dvp_reg_index_of_frm0_uv_addr(stream->id), dummy_buf->dma_addr);
        rkcif_write_register(dev, get_dvp_reg_index_of_frm1_y_addr(stream->id), dummy_buf->dma_addr);
        rkcif_write_register(dev, get_dvp_reg_index_of_frm1_uv_addr(stream->id), dummy_buf->dma_addr);
    }

    spin_unlock_irqrestore(&stream->vbq_lock, flags);
}

static int rkcif_assign_new_buffer_pingpong(struct rkcif_stream *stream, int init, int channel_id)
{
    int ret = 0;
...
static void rkcif_csi_get_vc_num(struct rkcif_device *dev, unsigned int mbus_flags)
{
    int i, vc_num = 0;
...
static void rkcif_csi_set_lvds_sav_eav(struct rkcif_stream *stream, struct csi_channel_info *channel)
{
    struct rkcif_device *dev = stream->cifdev;
    ...
    if (odd_sync_code && even_sync_code) {
        rkcif_write_register(stream->cifdev, get_reg_index_of_lvds_sav_eav_act0(channel->id),
                             SW_LVDS_EAV_ACT(odd_sync_code->act.eav) | SW_LVDS_SAV_ACT(odd_sync_code->act.sav));

        rkcif_write_register(stream->cifdev, get_reg_index_of_lvds_sav_eav_blk0(channel->id),
                             SW_LVDS_EAV_BLK(odd_sync_code->blk.eav) | SW_LVDS_SAV_BLK(odd_sync_code->blk.sav));

        rkcif_write_register(stream->cifdev, get_reg_index_of_lvds_sav_eav_act1(channel->id),
                             SW_LVDS_EAV_ACT(even_sync_code->act.eav) | SW_LVDS_SAV_ACT(even_sync_code->act.sav));

        rkcif_write_register(stream->cifdev, get_reg_index_of_lvds_sav_eav_blk1(channel->id),
                             SW_LVDS_EAV_BLK(even_sync_code->blk.eav) | SW_LVDS_SAV_BLK(even_sync_code->blk.sav));
    }
}

static unsigned char get_csi_fmt_val(const struct cif_input_fmt *cif_fmt_in, struct csi_channel_info *csi_info)
{
    unsigned char csi_fmt_val = 0;

    if (cif_fmt_in->mbus_code == MEDIA_BUS_FMT_SPD_2X8 || cif_fmt_in->mbus_code == MEDIA_BUS_FMT_EBD_1X8) {
...
static int rkcif_csi_channel_init(struct rkcif_stream *stream, struct csi_channel_info *channel)
{
    ...
    if (!fmt) {
        v4l2_err(&dev->v4l2_dev, "can not find output format: 0x%x", stream->pixm.pixelformat);
        return -EINVAL;
    }
    ...
    if (fourcc == V4L2_PIX_FMT_YUYV || fourcc == V4L2_PIX_FMT_YVYU || fourcc == V4L2_PIX_FMT_UYVY ||
        fourcc == V4L2_PIX_FMT_VYUY) {
    ...
    if (stream->cif_fmt_in->mbus_code == MEDIA_BUS_FMT_EBD_1X8 ||
        ...
        if (dev->channels[stream->id].data_type) {
            ...
            channel->data_type = get_data_type(stream->cif_fmt_in->mbus_code, channel->cmd_mode_en);
        }
    } else {
        channel->data_type = get_data_type(stream->cif_fmt_in->mbus_code, channel->cmd_mode_en);
    }
    channel->csi_fmt_val = get_csi_fmt_val(stream->cif_fmt_in, &dev->channels[stream->id]);

    if (dev->hdr.hdr_mode == NO_HDR || (dev->hdr.hdr_mode == HDR_X2 && stream->id > 1) ||
        (dev->hdr.hdr_mode == HDR_X3 && stream->id > 2)) {
        channel->vc = vc >= 0 ? vc : channel->id;
    } else {
        channel->vc = channel->id;
    }
    v4l2_dbg(3, rkcif_debug, &dev->v4l2_dev, "%s: channel width %d, height %d, virtual_width %d, vc %d\n", __func__,
             channel->width, channel->height, channel->virtual_width, channel->vc);
    return 0;
}

static int rkcif_csi_channel_set(struct rkcif_stream *stream, struct csi_channel_info *channel,
                                 enum v4l2_mbus_type mbus_type)
{
    ...
    if (!channel->enable) {
        rkcif_write_register(dev, get_reg_index_of_id_ctrl0(channel->id), CSI_DISABLE_CAPTURE);
        return 0;
    }

    rkcif_write_register_and(
        dev, CIF_REG_MIPI_LVDS_INTSTAT,
        ~(CSI_START_INTSTAT(channel->id) | CSI_DMA_END_INTSTAT(channel->id) | CSI_LINE_INTSTAT(channel->id)));

    /* enable id0 frame start int for sof(long frame, for hdr) */
    if (channel->id == RKCIF_STREAM_MIPI_ID0) {
        rkcif_write_register_or(dev, CIF_REG_MIPI_LVDS_INTEN, CSI_START_INTEN(channel->id));
    }
    if (detect_stream->is_line_wake_up) {
        rkcif_write_register_or(dev, CIF_REG_MIPI_LVDS_INTEN, CSI_LINE_INTEN(channel->id));
        wait_line = dev->wait_line;
    }
    rkcif_write_register(dev, CIF_REG_MIPI_LVDS_LINE_INT_NUM_ID0_1, (wait_line << 16) | wait_line);
    rkcif_write_register(dev, CIF_REG_MIPI_LVDS_LINE_INT_NUM_ID2_3, (wait_line << 16) | wait_line);

    rkcif_write_register_or(dev, CIF_REG_MIPI_LVDS_INTEN, CSI_DMA_END_INTEN(channel->id));
    rkcif_write_register(dev, CIF_REG_MIPI_WATER_LINE,
                         CIF_MIPI_LVDS_SW_WATER_LINE_25_RK1808 | CIF_MIPI_LVDS_SW_WATER_LINE_ENABLE_RK1808 |
                             CIF_MIPI_LVDS_SW_HURRY_VALUE_RK1808(0x3) | CIF_MIPI_LVDS_SW_HURRY_ENABLE_RK1808);

    val = CIF_MIPI_LVDS_SW_PRESS_VALUE(0x3) | CIF_MIPI_LVDS_SW_PRESS_ENABLE | CIF_MIPI_LVDS_SW_HURRY_VALUE(0x3) |
          CIF_MIPI_LVDS_SW_HURRY_ENABLE | CIF_MIPI_LVDS_SW_WATER_LINE_25 | CIF_MIPI_LVDS_SW_WATER_LINE_ENABLE;
    ...
    rkcif_write_register_or(dev, CIF_REG_MIPI_LVDS_INTEN, CSI_ALL_ERROR_INTEN);

    rkcif_write_register(dev, get_reg_index_of_id_ctrl1(channel->id), channel->width | (channel->height << 16));

    rkcif_write_register(dev, get_reg_index_of_frm0_y_vlw(channel->id), channel->virtual_width);
    rkcif_write_register(dev, get_reg_index_of_frm1_y_vlw(channel->id), channel->virtual_width);
    rkcif_write_register(dev, get_reg_index_of_frm0_uv_vlw(channel->id), channel->virtual_width);
    rkcif_write_register(dev, get_reg_index_of_frm1_uv_vlw(channel->id), channel->virtual_width);
    ...
    rkcif_assign_new_buffer_pingpong(stream, RKCIF_YUV_ADDR_STATE_INIT, channel->id);

    if (mbus_type == V4L2_MBUS_CSI2_DPHY) {
        // need always enable crop
        val = CSI_ENABLE_CAPTURE | channel->fmt_val | channel->cmd_mode_en << 4 | CSI_ENABLE_CROP | channel->vc << 8 |
              channel->data_type << 10;
        ...
        val = LVDS_ENABLE_CAPTURE | LVDS_MODE(channel->lvds_cfg.mode) | LVDS_MAIN_LANE(0) | LVDS_FID(0) |
...
static int rkcif_csi_channel_set_v1(struct rkcif_stream *stream, struct csi_channel_info *channel,
                                    enum v4l2_mbus_type mbus_type, unsigned int mode)
{
    ...
    rkcif_write_register_and(
        dev, CIF_REG_MIPI_LVDS_INTSTAT,
        ~(CSI_START_INTSTAT(channel->id) | CSI_DMA_END_INTSTAT(channel->id) | CSI_LINE_INTSTAT_V1(channel->id)));
    ...
    if (priv && priv->toisp_inf.link_mode == TOISP_NONE && detect_stream->is_line_wake_up) {
        rkcif_write_register_or(dev, CIF_REG_MIPI_LVDS_INTEN, CSI_LINE_INTEN_RK3588(channel->id));
        wait_line = dev->wait_line;
    }
    rkcif_write_register(dev, CIF_REG_MIPI_LVDS_LINE_INT_NUM_ID0_1, (wait_line << 16) | wait_line);
    rkcif_write_register(dev, CIF_REG_MIPI_LVDS_LINE_INT_NUM_ID2_3, (wait_line << 16) | wait_line);
    ...
    if (stream->cifdev->id_use_cnt == 0) {
        val = CIF_MIPI_LVDS_SW_PRESS_VALUE_RK3588(0x3) | CIF_MIPI_LVDS_SW_PRESS_ENABLE |
              CIF_MIPI_LVDS_SW_HURRY_VALUE_RK3588(0x3) | CIF_MIPI_LVDS_SW_HURRY_ENABLE |
              CIF_MIPI_LVDS_SW_WATER_LINE_25 | CIF_MIPI_LVDS_SW_WATER_LINE_ENABLE;
        rkcif_write_register(dev, CIF_REG_MIPI_LVDS_CTRL, val);

        rkcif_write_register_or(dev, CIF_REG_MIPI_LVDS_INTEN, CSI_ALL_ERROR_INTEN_V1);
    }
    rkcif_write_register(dev, get_reg_index_of_id_ctrl1(channel->id), channel->width | (channel->height << 16));
    ...
    if (stream->dma_en & RKCIF_DMAEN_BY_VICAP) {
        ...
        rkcif_assign_new_buffer_pingpong_toisp(stream, RKCIF_YUV_ADDR_STATE_INIT, channel->id);
    }
    if (mbus_type == V4L2_MBUS_CSI2_DPHY || mbus_type == V4L2_MBUS_CSI2_CPHY) {
        val = CSI_ENABLE_CAPTURE | dma_en | channel->cmd_mode_en << 26 | CSI_ENABLE_CROP_V1 | channel->vc << 8 |
        ...
        if (stream->cifdev->hdr.hdr_mode != NO_HDR && stream->cifdev->hdr.esp.mode == HDR_NORMAL_VC) {
...
static int rkcif_csi_stream_start(struct rkcif_stream *stream, unsigned int mode)
{
    ...
    if (mbus_type == V4L2_MBUS_CSI2_DPHY || mbus_type == V4L2_MBUS_CSI2_CPHY) {
        ...
        if (stream->state != RKCIF_STATE_STREAMING) {
            ...
            if (stream->cifdev->chip_id >= CHIP_RK3588_CIF) {
                if ((mode & RKCIF_STREAM_MODE_CAPTURE) == RKCIF_STREAM_MODE_CAPTURE && (!stream->dma_en)) {
                    stream->to_en_dma = RKCIF_DMAEN_BY_VICAP;
                } else if ((mode & RKCIF_STREAM_MODE_TOISP) == RKCIF_STREAM_MODE_TOISP) {
                    if (dev->hdr.hdr_mode == HDR_X2 && stream->id == 0 && (!stream->dma_en)) {
        ...
        ret = v4l2_subdev_call(dev->terminal_sensor.sd, core, ioctl, RKMODULE_GET_LVDS_CFG, &channel->lvds_cfg);
        if (ret) {
...
static void rkcif_stream_stop(struct rkcif_stream *stream)
{
    ...
    if (mbus_cfg->type == V4L2_MBUS_CSI2_DPHY || mbus_cfg->type == V4L2_MBUS_CSI2_CPHY ||
        ...
        rkcif_write_register_or(cif_dev, CIF_REG_MIPI_LVDS_INTSTAT,
                                CSI_START_INTSTAT(id) | CSI_DMA_END_INTSTAT(id) | CSI_LINE_INTSTAT(id));

        rkcif_write_register_and(cif_dev, CIF_REG_MIPI_LVDS_INTEN,
                                 ~(CSI_START_INTEN(id) | CSI_DMA_END_INTEN(id) | CSI_LINE_INTEN(id)));

        if (stream->cifdev->chip_id < CHIP_RK3588_CIF) {
            rkcif_write_register_and(cif_dev, CIF_REG_MIPI_LVDS_INTEN, ~CSI_ALL_ERROR_INTEN);
        } else {
            if (stream->cifdev->id_use_cnt == 0) {
                rkcif_write_register_and(cif_dev, CIF_REG_MIPI_LVDS_INTEN, ~CSI_ALL_ERROR_INTEN_V1);
        ...
        if (atomic_read(&cif_dev->pipe.stream_cnt) == 1) {
            ...
            rkcif_write_register(cif_dev, CIF_REG_DVP_CTRL, val & (~ENABLE_CAPTURE));
...
static bool rkcif_is_extending_line_for_height(struct rkcif_device *dev, struct rkcif_stream *stream,
                                               const struct cif_input_fmt *fmt)
{
    ...
    if (dev->chip_id == CHIP_RV1126_CIF || dev->chip_id == CHIP_RV1126_CIF_LITE) {
        if (dev->terminal_sensor.sd) {
            ret = v4l2_subdev_call(dev->terminal_sensor.sd, core, ioctl, RKMODULE_GET_HDR_CFG, &hdr_cfg);
            if (!ret) {
                dev->hdr = hdr_cfg;
            } else {
        ...
        if (fmt && fmt->fmt_type == CIF_FMT_TYPE_RAW) {
            if ((dev->hdr.hdr_mode == HDR_X2 && stream->id == RKCIF_STREAM_MIPI_ID1) ||
                (dev->hdr.hdr_mode == HDR_X3 && stream->id == RKCIF_STREAM_MIPI_ID2) || (dev->hdr.hdr_mode == NO_HDR)) {
                is_extended = true;
            }
        }
    }
...
static int rkcif_queue_setup(struct vb2_queue *queue, unsigned int *num_buffers, unsigned int *num_planes,
                             unsigned int sizes[], struct device *alloc_ctxs[])
{
    ...
    *num_planes = cif_fmt->mplanes;
    ...
    v4l2_dbg(1, rkcif_debug, &dev->v4l2_dev, "%s count %d, size %d, extended(%d, %d)\n", v4l2_type_names[queue->type],
             *num_buffers, sizes[0], is_extended, extend_line->is_extended);

    return 0;
}

static void rkcif_check_buffer_update_pingpong(struct rkcif_stream *stream, int channel_id)
{
    struct rkcif_device *dev = stream->cifdev;
    struct v4l2_mbus_config *mbus_cfg = &dev->active_sensor->mbus;
    struct rkcif_buffer *buffer = NULL;
    ...
    if (stream->state == RKCIF_STATE_STREAMING && stream->curr_buf == stream->next_buf &&
        stream->cif_fmt_in->field != V4L2_FIELD_INTERLACED && (!dummy_buf->vaddr)) {
        if (!stream->is_line_wake_up) {
            if (mbus_cfg->type == V4L2_MBUS_CSI2_DPHY || mbus_cfg->type == V4L2_MBUS_CSI2_CPHY ||
                mbus_cfg->type == V4L2_MBUS_CCP2) {
...
static void rkcif_buf_queue(struct vb2_buffer *vb)
{
    ...
    for (i = 0; i < fmt->mplanes; i++) {
        ...
        if (rkcif_debug && addr && !hw_dev->iommu_en) {
            ...
            v4l2_dbg(1, rkcif_debug, &stream->cifdev->v4l2_dev, "Clear buffer, size: 0x%08x\n",
    ...
    if (fmt->mplanes == 1) {
        for (i = 0; i < fmt->cplanes - 1; i++) {
            cifbuf->buff_addr[i + 1] = cifbuf->buff_addr[i] + pixm->plane_fmt[i].bytesperline * pixm->height;
        }
    }
    spin_lock_irqsave(&stream->vbq_lock, flags);
    ...
    v4l2_dbg(1, rkcif_debug, &stream->cifdev->v4l2_dev, "stream[%d] buf queue, index: %d\n", stream->id, vb->index);
}
...
void rkcif_free_rx_buf(struct rkcif_stream *stream, int buf_num)
{
    ...
    stream->dma_en &= ~RKCIF_DMAEN_BY_ISP;
    v4l2_dbg(3, rkcif_debug, &stream->cifdev->v4l2_dev, "free rx_buf, buf_num %d\n", buf_num);
}
...
int rkcif_init_rx_buf(struct rkcif_stream *stream, int buf_num)
{
    ...
    for (i = 0; i < buf_num; i++) {
        ...
        v4l2_dbg(3, rkcif_debug, &dev->v4l2_dev, "init rx_buf,dma_addr 0x%llx size: 0x%x\n", (u64)dummy->dma_addr,
...
static int rkcif_create_dummy_buf(struct rkcif_stream *stream)
{
    ...
    dummy_buf->size = max3(stream->pixm.plane_fmt[0].bytesperline * stream->pixm.height,
                           stream->pixm.plane_fmt[1].sizeimage, stream->pixm.plane_fmt[2].sizeimage);
    /*
     * rk cif don't support output yuyv fmt data
     * if user request yuyv fmt, the input mode must be RAW8
     * and the width is double Because the real input fmt is
    ...
    if (ret) {
        v4l2_err(&dev->v4l2_dev, "Failed to allocate the memory for dummy buffer\n");
        return -ENOMEM;
    }
...
{
    if (dev->active_sensor->mbus.type == V4L2_MBUS_CSI2_DPHY || dev->active_sensor->mbus.type == V4L2_MBUS_CSI2_CPHY ||
    ...
    usleep_range(10, 20);
    v4l2_dbg(3, rkcif_debug, &dev->v4l2_dev, "vicap do soft reset 0x%x\n", 0x000A0000);
}
...
static void rkcif_release_rdbk_buf(struct rkcif_stream *stream)
{
    ...
    if (rdbk_buf) {
        if (rdbk_buf != stream->curr_buf && rdbk_buf != stream->next_buf) {
            has_added = false;
...
static void rkcif_detach_sync_mode(struct rkcif_device *cif_dev)
{
    ...
    if ((!cif_dev->sync_type) || (atomic_read(&cif_dev->pipe.stream_cnt) != 0)) {
        return;
    }
...
void rkcif_do_stop_stream(struct rkcif_stream *stream, enum rkcif_stream_mode mode)
{
    ...
    if (mode == stream->cur_stream_mode) {
        ...
        ret = wait_event_timeout(stream->wq_stopped, stream->state != RKCIF_STATE_STREAMING, msecs_to_jiffies(1000));
        if (!ret) {
        ...
        wait_event_timeout(stream->wq_stopped, stream->to_stop_dma != RKCIF_DMAEN_BY_VICAP, msecs_to_jiffies(1000));
    } else if (mode == RKCIF_STREAM_MODE_TOISP) {
        // only stop dma
        stream->to_stop_dma = RKCIF_DMAEN_BY_ISP;
        wait_event_timeout(stream->wq_stopped, stream->to_stop_dma != RKCIF_DMAEN_BY_ISP, msecs_to_jiffies(1000));
    }
    if ((mode & RKCIF_STREAM_MODE_CAPTURE) == RKCIF_STREAM_MODE_CAPTURE) {
        /* release buffers */
        if (stream->curr_buf) {
        ...
        while (!list_empty(&stream->buf_head)) {
            buf = list_first_entry(&stream->buf_head, struct rkcif_buffer, queue);
            list_del(&buf->queue);
    ...
    if (ret == 0) {
        ...
        switch (std) {
            ...
            case V4L2_STD_ATSC:
                ...
                break;
            default:
                v4l2_err(&dev->v4l2_dev, "std: %lld is not supported", std);
        }
    } else {
        ...
        switch (stream->cif_fmt_in->fmt_type) {
            ...
            case CIF_FMT_TYPE_RAW:
                if (dev->chip_id >= CHIP_RK3568_CIF) {
                    ret = v4l2_subdev_call(terminal_sensor->sd, core, ioctl, RKMODULE_GET_BT656_INTF_TYPE, &intf);
                    if (!ret) {
...
static inline u32 rkcif_scl_ctl(struct rkcif_stream *stream)
{
    ...
    return (fmt_type == CIF_FMT_TYPE_YUV) ? ENABLE_YUV_16BIT_BYPASS : ENABLE_RAW_16BIT_BYPASS;
}
 ...
 */
static u32 rkcif_align_bits_per_pixel(struct rkcif_stream *stream, const struct cif_output_fmt *fmt, int plane_index)
{
    u32 bpp = 0, i, cal = 0;
 ...
 */
static u32 rkcif_cal_raw_vir_line_ratio(struct rkcif_stream *stream, const struct cif_output_fmt *fmt)
{
    u32 ratio = 0, bpp = 0;
...
static void rkcif_sync_crop_info(struct rkcif_stream *stream)
{
    ...
    if (dev->terminal_sensor.sd) {
        ...
        input_sel.pad = 0;
        ret = v4l2_subdev_call(dev->terminal_sensor.sd, pad, get_selection, NULL, &input_sel);
        if (!ret) {
    ...
    if ((stream->crop_mask & 0x3) == (CROP_SRC_USR_MASK | CROP_SRC_SENSOR_MASK)) {
        if (stream->crop[CROP_SRC_USR].left + stream->crop[CROP_SRC_USR].width > stream->crop[CROP_SRC_SENSOR].width ||
            stream->crop[CROP_SRC_USR].top + stream->crop[CROP_SRC_USR].height > stream->crop[CROP_SRC_SENSOR].height) {
    ...
    if (stream->crop_mask & CROP_SRC_USR_MASK) {
        ...
        if (stream->crop_mask & CROP_SRC_SENSOR_MASK) {
            stream->crop[CROP_SRC_ACT].left = stream->crop[CROP_SRC_USR].left + stream->crop[CROP_SRC_SENSOR].left;
            stream->crop[CROP_SRC_ACT].top = stream->crop[CROP_SRC_USR].top + stream->crop[CROP_SRC_SENSOR].top;
        }
    } else {
 ...
 */
static int rkcif_sanity_check_fmt(struct rkcif_stream *stream, const struct v4l2_rect *s_crop)
{
    struct rkcif_device *dev = stream->cifdev;
    struct v4l2_device *v4l2_dev = &dev->v4l2_dev;
    struct v4l2_rect input, *crop;

    stream->cif_fmt_in = get_input_fmt(dev->terminal_sensor.sd, &input, stream->id, &dev->channels[stream->id]);
    ...
    if (crop->width + crop->left > input.width || crop->height + crop->top > input.height) {
        v4l2_err(v4l2_dev, "crop size is bigger than input\n");
        return -EINVAL;
    }

    if (dev->active_sensor && (dev->active_sensor->mbus.type == V4L2_MBUS_CSI2_DPHY ||
        ...
        if (crop->left > 0) {
            ...
            if (align_x > 0 && crop->left % align_x != 0) {
                v4l2_err(v4l2_dev, "ERROR: crop left must align %d\n", align_x);
                return -EINVAL;
            }
        }
    } else if (dev->active_sensor && dev->active_sensor->mbus.type == V4L2_MBUS_CCP2) {
        if (crop->left % 4 != 0 && crop->width % 4 != 0) {
            v4l2_err(v4l2_dev, "ERROR: lvds crop left and width must align %d\n", 4);
            return -EINVAL;
        }
    }
...
int rkcif_update_sensor_info(struct rkcif_stream *stream)
{
    ...
    if (!sensor_sd) {
        v4l2_err(&stream->cifdev->v4l2_dev, "%s: stream[%d] get remote sensor_sd failed!\n", __func__, stream->id);
        return -ENODEV;
    }

    sensor = sd_to_sensor(stream->cifdev, sensor_sd);
    if (!sensor) {
        v4l2_err(&stream->cifdev->v4l2_dev, "%s: stream[%d] get remote sensor failed!\n", __func__, stream->id);
        return -ENODEV;
    }
    ret = v4l2_subdev_call(sensor->sd, pad, get_mbus_config, 0, &sensor->mbus);
    if (ret && ret != -ENOIOCTLCMD) {
        v4l2_err(&stream->cifdev->v4l2_dev, "%s: get remote %s mbus failed!\n", __func__, sensor->sd->name);
        return ret;
    }
    ...
    if (terminal_sensor->sd) {
        ret = v4l2_subdev_call(terminal_sensor->sd, pad, get_mbus_config, 0, &terminal_sensor->mbus);
        if (ret && ret != -ENOIOCTLCMD) {
            v4l2_err(&stream->cifdev->v4l2_dev, "%s: get terminal %s mbus failed!\n", __func__,
                     terminal_sensor->sd->name);
            return ret;
        }
    } else {
        v4l2_err(&stream->cifdev->v4l2_dev, "%s: stream[%d] get remote terminal sensor failed!\n", __func__,
                 stream->id);
        return -ENODEV;
    }

    if (terminal_sensor->mbus.type == V4L2_MBUS_CSI2_DPHY || terminal_sensor->mbus.type == V4L2_MBUS_CSI2_CPHY ||
        ...
        switch (terminal_sensor->mbus.flags & V4L2_MBUS_CSI2_LANES) {
            ...
            default:
                v4l2_err(&stream->cifdev->v4l2_dev, "%s:get sd:%s lane num failed!\n", __func__,
                         terminal_sensor->sd ? terminal_sensor->sd->name : "null");
                return -EINVAL;
        }
    }
...
static int rkcif_dvp_get_output_type_mask(struct rkcif_stream *stream)
{
    ...
    switch (fmt->fourcc) {
        case V4L2_PIX_FMT_NV16:
            mask = (CSI_WRDDR_TYPE_YUV422SP_RK3588 << 11) | (CSI_YUV_OUTPUT_ORDER_UYVY << 1);
            break;
        case V4L2_PIX_FMT_NV61:
            mask = (CSI_WRDDR_TYPE_YUV422SP_RK3588 << 11) | (CSI_YUV_OUTPUT_ORDER_VYUY << 1);
            break;
        case V4L2_PIX_FMT_NV12:
            mask = (CSI_WRDDR_TYPE_YUV420SP_RK3588 << 11) | (CSI_YUV_OUTPUT_ORDER_UYVY << 1);
            break;
        case V4L2_PIX_FMT_NV21:
            mask = (CSI_WRDDR_TYPE_YUV420SP_RK3588 << 11) | (CSI_YUV_OUTPUT_ORDER_VYUY << 1);
            break;
        case V4L2_PIX_FMT_YUYV:
            mask = (CSI_WRDDR_TYPE_YUV_PACKET << 11) | (CSI_YUV_OUTPUT_ORDER_YUYV << 1);
            break;
        case V4L2_PIX_FMT_YVYU:
            mask = (CSI_WRDDR_TYPE_YUV_PACKET << 11) | (CSI_YUV_OUTPUT_ORDER_YVYU << 1);
            break;
        case V4L2_PIX_FMT_UYVY:
            mask = (CSI_WRDDR_TYPE_YUV_PACKET << 11) | (CSI_YUV_OUTPUT_ORDER_UYVY << 1);
            break;
        case V4L2_PIX_FMT_VYUY:
            mask = (CSI_WRDDR_TYPE_YUV_PACKET << 11) | (CSI_YUV_OUTPUT_ORDER_VYUY << 1);
            break;
        case V4L2_PIX_FMT_RGB24:
        case V4L2_PIX_FMT_RGB565:
        case V4L2_PIX_FMT_BGR666:
...
static int rkcif_stream_start(struct rkcif_stream *stream, unsigned int mode)
{
    u32 val, mbus_flags, href_pol, vsync_pol,
        xfer_mode = 0, yc_swap = 0, inputmode = 0, mipimode = 0, workmode = 0, multi_id = 0,
        multi_id_en = BT656_1120_MULTI_ID_DISABLE, multi_id_mode = BT656_1120_MULTI_ID_MODE_1,
        multi_id_sel = BT656_1120_MULTI_ID_SEL_LSB, bt1120_edge_mode = BT1120_CLOCK_SINGLE_EDGES, bt1120_flags = 0,
    ...
    if (sensor_info->sd && mbus->type == V4L2_MBUS_BT656) {
        ...
        ret = v4l2_subdev_call(sensor_info->sd, core, ioctl, RKMODULE_GET_BT656_MBUS_INFO, &bt1120_info);
        if (ret) {
            v4l2_warn(&dev->v4l2_dev, "waring: no muti channel info for BT.656\n");
        } else {
    ...
    if ((mbus_flags & CIF_DVP_PCLK_DUAL_EDGE) == CIF_DVP_PCLK_DUAL_EDGE) {
        bt1120_edge_mode =
            (dev->chip_id < CHIP_RK3588_CIF ? BT1120_CLOCK_DOUBLE_EDGES : BT1120_CLOCK_DOUBLE_EDGES_RK3588);
        rkcif_enable_dvp_clk_dual_edge(dev, true);
    } else {
        bt1120_edge_mode =
            dev->chip_id < CHIP_RK3588_CIF ? BT1120_CLOCK_SINGLE_EDGES : BT1120_CLOCK_SINGLE_EDGES_RK3588;
    ...
    href_pol = (mbus_flags & V4L2_MBUS_HSYNC_ACTIVE_HIGH) ? HSY_HIGH_ACTIVE : HSY_LOW_ACTIVE;
    vsync_pol = (mbus_flags & V4L2_MBUS_VSYNC_ACTIVE_HIGH) ? VSY_HIGH_ACTIVE : VSY_LOW_ACTIVE;
    ...
    if (dev->chip_id < CHIP_RK3588_CIF) {
        val = vsync_pol | href_pol | inputmode | mipimode | stream->cif_fmt_out->fmt_val |
              stream->cif_fmt_in->dvp_fmt_val | xfer_mode | yc_swap | multi_id_en | multi_id_sel | multi_id_mode |
              bt1120_edge_mode;
        if (stream->is_high_align) {
            val |= CIF_HIGH_ALIGN;
        } else {
        ...
        val = vsync_pol | href_pol | inputmode | yc_swap | out_fmt_mask | in_fmt_yuv_order | multi_id_en | sav_detect |
    ...
    rkcif_write_register(dev, CIF_REG_DVP_SET_SIZE,
                         dev->channels[stream->id].width | (dev->channels[stream->id].height << 16));
    rkcif_write_register(dev, CIF_REG_DVP_CROP,
                         dev->channels[stream->id].crop_st_y << CIF_CROP_Y_SHIFT | dev->channels[stream->id].crop_st_x);
    ...
    if (dev->chip_id < CHIP_RK3588_CIF) {
        ...
        rkcif_write_register_or(dev, CIF_REG_DVP_INTEN,
                                DVP_DMA_END_INTSTAT(stream->id) | INTSTAT_ERR | PST_INF_FRAME_END);
    ...
    if (dma_en) {
        if (dev->chip_id < CHIP_RK1808_CIF) {
            rkcif_assign_new_buffer_oneframe(stream, RKCIF_YUV_ADDR_STATE_INIT);
        } else {
            rkcif_assign_new_buffer_pingpong(stream, RKCIF_YUV_ADDR_STATE_INIT, stream->id);
        }
    }
    rkcif_write_register_or(dev, CIF_REG_DVP_INTEN, DVP_DMA_END_INTSTAT(stream->id) | INTSTAT_ERR | PST_INF_FRAME_END);
    ...
    if (dev->chip_id < CHIP_RK3588_CIF) {
        rkcif_write_register(dev, CIF_REG_DVP_CTRL, AXI_BURST_16 | workmode | ENABLE_CAPTURE);
    } else {
        rkcif_write_register(dev, CIF_REG_DVP_CTRL,
                             DVP_SW_WATER_LINE_25 | dma_en | DVP_PRESS_EN | DVP_HURRY_EN | DVP_SW_WATER_LINE_25 |
                                 DVP_SW_PRESS_VALUE(3) | DVP_SW_HURRY_VALUE(3) | ENABLE_CAPTURE);
    }
...
static void rkcif_attach_sync_mode(struct rkcif_hw *hw)
{
    ...
    for (i = 0; i < hw->dev_num; i++) {
        dev = hw->cif_dev[i];
        ret = v4l2_subdev_call(dev->terminal_sensor.sd, core, ioctl, RKMODULE_GET_SYNC_MODE, &sync_type);
        if (!ret) {
    ...
    if (hw->sync_config.int_master.count == 1) {
        if (hw->sync_config.ext_master.count) {
            ...
            dev_info(hw->dev, "Missing slave device, do not use sync mode\n");
        }
        if (hw->sync_config.ext_master.count && hw->sync_config.slave.count) {
            dev_info(hw->dev, "There are two types of slave devices, it may cause problems\n");
        }
    } else {
        dev_info(hw->dev, "Only support one master device, master device count %d\n", hw->sync_config.int_master.count);
    }
}
...
int rkcif_do_start_stream(struct rkcif_stream *stream, enum rkcif_stream_mode mode)
{
    ...
    if (terminal_sensor->sd) {
        ret = v4l2_subdev_call(terminal_sensor->sd, core, ioctl, RKMODULE_GET_HDR_CFG, &hdr_cfg);
        if (!ret) {
            dev->hdr = hdr_cfg;
        } else {
            dev->hdr.hdr_mode = NO_HDR;
    ...
    if (((dev->active_sensor && dev->active_sensor->mbus.type == V4L2_MBUS_BT656) || dev->is_use_dummybuf) &&
        (!dev->dummy_buf.vaddr)) {
    ...
    if (stream->cur_stream_mode == RKCIF_STREAM_MODE_NONE) {
         ...
         */
        if (sensor_info->mbus.type == V4L2_MBUS_PARALLEL || rkmodule_stream_seq == RKMODULE_START_STREAM_FRONT) {
    ...
    if (dev->chip_id >= CHIP_RK1808_CIF) {
        if (dev->active_sensor &&
            (dev->active_sensor->mbus.type == V4L2_MBUS_CSI2_DPHY ||
             dev->active_sensor->mbus.type == V4L2_MBUS_CSI2_CPHY || dev->active_sensor->mbus.type == V4L2_MBUS_CCP2)) {
    ...
    if (dev->chip_id == CHIP_RV1126_CIF || dev->chip_id == CHIP_RV1126_CIF_LITE || dev->chip_id == CHIP_RK3568_CIF) {
        if (dev->hdr.hdr_mode == NO_HDR) {
            if (dev->stream[RKCIF_STREAM_MIPI_ID0].state == RKCIF_STATE_STREAMING) {
                rkcif_start_luma(&dev->luma_vdev, dev->stream[RKCIF_STREAM_MIPI_ID0].cif_fmt_in);
            }
...
destroy_buf:
    if (stream->next_buf) {
        vb2_buffer_done(&stream->next_buf->vb.vb2_buf, VB2_BUF_STATE_QUEUED);
    }
    if (stream->curr_buf) {
        vb2_buffer_done(&stream->curr_buf->vb.vb2_buf, VB2_BUF_STATE_QUEUED);
    }
    while (!list_empty(&stream->buf_head)) {
...
int rkcif_set_fmt(struct rkcif_stream *stream, struct v4l2_pix_format_mplane *pixm, bool try)
{
    struct rkcif_device *dev = stream->cifdev;
    const struct cif_output_fmt *fmt;
    ...
    if (dev->terminal_sensor.sd) {
        cif_fmt_in = get_input_fmt(dev->terminal_sensor.sd, &input_rect, stream->id, &dev->channels[stream->id]);
        stream->cif_fmt_in = cif_fmt_in;
    } else {
        v4l2_err(&stream->cifdev->v4l2_dev, "terminal subdev does not exist\n");
        return -EINVAL;
    }
    ...
    if (cif_fmt_in &&
        (cif_fmt_in->mbus_code == MEDIA_BUS_FMT_SPD_2X8 || cif_fmt_in->mbus_code == MEDIA_BUS_FMT_EBD_1X8)) {
        stream->crop_enable = false;
    }
    ...
    for (i = 0; i < planes; i++) {
         ...
         */
        if (fmt->fmt_type == CIF_FMT_TYPE_RAW && (stream->cif_fmt_in->mbus_code == MEDIA_BUS_FMT_EBD_1X8 ||
                                                  stream->cif_fmt_in->mbus_code == MEDIA_BUS_FMT_SPD_2X8)) {
        ...
        if (fmt->fmt_type == CIF_FMT_TYPE_RAW && stream->is_compact &&
            (dev->active_sensor->mbus.type == V4L2_MBUS_CSI2_DPHY ||
             dev->active_sensor->mbus.type == V4L2_MBUS_CSI2_CPHY || dev->active_sensor->mbus.type == V4L2_MBUS_CCP2)) {
            bpl = ALIGN(width * fmt->raw_bpp / 0x08, 0x100);
        } else {
            if (fmt->fmt_type == CIF_FMT_TYPE_RAW && stream->is_compact && dev->chip_id >= CHIP_RK3588_CIF) {
                bpl = ALIGN(width * fmt->raw_bpp / 0x08, 0x100);
            } else {
    ...
    if (!try) {
        ...
        v4l2_dbg(1, rkcif_debug, &stream->cifdev->v4l2_dev, "%s: req(%d, %d) out(%d, %d)\n", __func__, pixm->width,
...
static int rkcif_fh_open(struct file *filp)
{
     ...
     */
    if (cifdev->chip_id == CHIP_RK1808_CIF || cifdev->chip_id == CHIP_RV1126_CIF ||
        cifdev->chip_id == CHIP_RV1126_CIF_LITE || cifdev->chip_id == CHIP_RK3568_CIF ||
...
static int rkcif_enum_input(struct file *file, void *priv, struct v4l2_input *input)
{
    if (input->index > 0) {
        return -EINVAL;
    }
...
static int rkcif_try_fmt_vid_cap_mplane(struct file *file, void *fh, struct v4l2_format *f)
{
...
static int rkcif_enum_framesizes(struct file *file, void *prov, struct v4l2_frmsizeenum *fsize)
{
    ...
    if (dev->terminal_sensor.sd) {
        get_input_fmt(dev->terminal_sensor.sd, &input_rect, stream->id, &csi_info);
    }
...
static int rkcif_enum_frameintervals(struct file *file, void *fh, struct v4l2_frmivalenum *fival)
{
    struct rkcif_stream *stream = video_drvdata(file);
    struct rkcif_device *dev = stream->cifdev;
...
static int rkcif_enum_fmt_vid_cap_mplane(struct file *file, void *priv, struct v4l2_fmtdesc *f)
{
    ...
    switch (f->pixelformat) {
        case V4l2_PIX_FMT_EBD8:
            strscpy(f->description, "Embedded data 8-bit", sizeof(f->description));
            break;
        case V4l2_PIX_FMT_SPD16:
            strscpy(f->description, "Shield pix data 16-bit", sizeof(f->description));
            break;
        default:
            break;
    }
...
static int rkcif_s_fmt_vid_cap_mplane(struct file *file, void *priv, struct v4l2_format *f)
{
    struct rkcif_stream *stream = video_drvdata(file);
    struct rkcif_device *dev = stream->cifdev;
    int ret = 0;
...
static int rkcif_g_fmt_vid_cap_mplane(struct file *file, void *fh, struct v4l2_format *f)
{
    struct rkcif_stream *stream = video_drvdata(file);
...
static int rkcif_querycap(struct file *file, void *priv, struct v4l2_capability *cap)
{
    int ret = 0;
    struct rkcif_stream *stream = video_drvdata(file);
...
static __maybe_unused int rkcif_cropcap(struct file *file, void *fh, struct v4l2_cropcap *cap)
{
    struct rkcif_stream *stream = video_drvdata(file);
    struct rkcif_device *dev = stream->cifdev;
...
static __maybe_unused int rkcif_s_crop(struct file *file, void *fh, const struct v4l2_crop *a)
{
    ...
    if (stream->crop_mask & CROP_SRC_SENSOR) {
        ...
        if (rect->left + rect->width > sensor_crop.width || rect->top + rect->height > sensor_crop.height) {
        ...
        if (rect->left + rect->width > raw_rect->width || rect->top + rect->height > raw_rect->height) {
            v4l2_err(&dev->v4l2_dev, "crop size is bigger than sensor raw input:left:%d, top:%d, width:%d, height:%d\n",
...
static int rkcif_s_selection(struct file *file, void *fh, struct v4l2_selection *s)
{
    struct rkcif_stream *stream = video_drvdata(file);
    struct rkcif_device *dev = stream->cifdev;
    struct v4l2_subdev *sensor_sd;
...
static int rkcif_g_selection(struct file *file, void *fh, struct v4l2_selection *s)
{
    struct rkcif_stream *stream = video_drvdata(file);
    struct rkcif_device *dev = stream->cifdev;
    struct v4l2_subdev *sensor_sd;
    ...
    if (s->target == V4L2_SEL_TGT_CROP_BOUNDS) {
        ...
        v4l2_dbg(1, rkcif_debug, &dev->v4l2_dev, "%s(line:%d): sd:%s pad:%d, which:%d, target:%d\n", __func__, __LINE__,
                 sensor_sd->name, pad, sd_sel.which, sd_sel.target);
...
static long rkcif_ioctl_default(struct file *file, void *fh, bool valid_prio, unsigned int cmd, void *arg)
{
    struct rkcif_stream *stream = video_drvdata(file);
    struct rkcif_device *dev = stream->cifdev;
    const struct cif_input_fmt *in_fmt;
...
static int rkcif_register_stream_vdev(struct rkcif_stream *stream, bool is_multi_input)
{
    ...
    rkcif_init_vb2_queue(&node->buf_queue, stream, V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE);
...
void rkcif_unregister_stream_vdevs(struct rkcif_device *dev, int stream_num)
{
...
int rkcif_register_stream_vdevs(struct rkcif_device *dev, int stream_num, bool is_multi_input)
{
    struct rkcif_stream *stream;
    int i, j, ret;
...
static int rkcif_lvds_subdev_link_setup(struct media_entity *entity, const struct media_pad *local,
                                        const struct media_pad *remote, u32 flags)
{
    return 0;
}

static int rkcif_lvds_sd_set_fmt(struct v4l2_subdev *sd, struct v4l2_subdev_pad_config *cfg,
                                 struct v4l2_subdev_format *fmt)
{
...
static int rkcif_lvds_sd_get_fmt(struct v4l2_subdev *sd, struct v4l2_subdev_pad_config *cfg,
                                 struct v4l2_subdev_format *fmt)
{
...
static struct v4l2_rect *rkcif_lvds_sd_get_crop(struct rkcif_lvds_subdev *subdev, struct v4l2_subdev_pad_config *cfg,
...
static int rkcif_lvds_sd_set_selection(struct v4l2_subdev *sd, struct v4l2_subdev_pad_config *cfg,
...
static int rkcif_lvds_sd_get_selection(struct v4l2_subdev *sd, struct v4l2_subdev_pad_config *cfg,
                                       struct v4l2_subdev_selection *sel)
{
    struct rkcif_lvds_subdev *subdev = container_of(sd, struct rkcif_lvds_subdev, sd);
    struct v4l2_subdev *sensor = get_lvds_remote_sensor(sd);
...
static int rkcif_lvds_g_mbus_config(struct v4l2_subdev *sd, unsigned int pad_id, struct v4l2_mbus_config *mbus)
{
...
static int rkcif_sof_subscribe_event(struct v4l2_subdev *sd, struct v4l2_fh *fh, struct v4l2_event_subscription *sub)
{
...
static void rkcif_lvds_event_inc_sof(struct rkcif_device *dev)
{
    ...
    if (subdev) {
        struct v4l2_event event = {
            ...
            .u.frame_sync.frame_sequence = atomic_inc_return(&subdev->frm_sync_seq) - 1,
        };
...
int rkcif_register_lvds_subdev(struct rkcif_device *dev)
{
    ...
    lvds_subdev->pads[RKCIF_LVDS_PAD_SINK].flags = MEDIA_PAD_FL_SINK | MEDIA_PAD_FL_MUST_CONNECT;
    ...
    ret = media_entity_pads_init(&sd->entity, RKCIF_LVDS_PAD_MAX, lvds_subdev->pads);
    if (ret < 0) {
        return ret;
    }
...
int rkcif_register_dvp_sof_subdev(struct rkcif_device *dev)
{
    ...
    sd->owner = THIS_MODULE;
    sd->flags |= V4L2_SUBDEV_FL_HAS_DEVNODE | V4L2_SUBDEV_FL_HAS_EVENTS;
    ret = snprintf(sd->name, sizeof(sd->name), "rkcif-dvp-sof");
...
static void rkcif_vb_done_oneframe(struct rkcif_stream *stream, struct vb2_v4l2_buffer *vb_done)
{
    const struct cif_output_fmt *fmt = stream->cif_fmt_out;
    u32 i;
    ...
    v4l2_dbg(1, rkcif_debug, &stream->cifdev->v4l2_dev, "stream[%d] vb done, index: %d, sequence %d\n", stream->id,
...
void rkcif_irq_oneframe(struct rkcif_device *cif_dev)
{
    ...
    if ((intstat & PST_INF_FRAME_END)) {
        rkcif_write_register(cif_dev, CIF_REG_DVP_INTSTAT, PST_INF_FRAME_END_CLR);

        if (stream->stopping) {
            /* To stop CIF ASAP, before FRAME_END irq */
            rkcif_write_register(cif_dev, CIF_REG_DVP_CTRL, ctl & (~ENABLE_CAPTURE));
        }
    }
    ...
    if ((intstat & FRAME_END)) {
        ...
        rkcif_write_register(cif_dev, CIF_REG_DVP_INTSTAT, FRAME_END_CLR);
        ...
        if (lastline != stream->pixm.height || !(cif_frmst & CIF_F0_READY)) {
             ...
             */
            rkcif_write_register(cif_dev, CIF_REG_DVP_FRAME_STATUS, FRM0_STAT_CLS);
            v4l2_err(&cif_dev->v4l2_dev, "Bad frame, irq:0x%x frmst:0x%x size:%dx%d\n", intstat, cif_frmst, lastline,
                     lastpix);
         ...
         */
        rkcif_write_register(cif_dev, CIF_REG_DVP_FRAME_STATUS, cif_frmst & FRM0_STAT_CLS);
        ret = rkcif_assign_new_buffer_oneframe(stream, RKCIF_YUV_ADDR_STATE_UPDATE);
        if (vb_done && (!ret)) {
...
static int rkcif_csi_g_mipi_id(struct v4l2_device *v4l2_dev, unsigned int intstat)
{
...
static int rkcif_dvp_g_ch_id(struct v4l2_device *v4l2_dev, u32 *intstat, u32 frm_stat)
{
    if (*intstat & DVP_FRAME_END_ID0) {
        if ((frm_stat & DVP_CHANNEL0_FRM_READ) == DVP_CHANNEL0_FRM_READ) {
    ...
    if (*intstat & DVP_FRAME_END_ID1) {
        if ((frm_stat & DVP_CHANNEL1_FRM_READ) == DVP_CHANNEL1_FRM_READ) {
    ...
    if (*intstat & DVP_FRAME_END_ID2) {
        if ((frm_stat & DVP_CHANNEL2_FRM_READ) == DVP_CHANNEL2_FRM_READ) {
    ...
    if (*intstat & DVP_FRAME_END_ID3) {
        if ((frm_stat & DVP_CHANNEL3_FRM_READ) == DVP_CHANNEL3_FRM_READ) {
...
static int rkcif_dvp_g_ch_id_by_fe(struct v4l2_device *v4l2_dev, u32 intstat)
{
    if (intstat & DVP_ALL_END_ID0) {
        if ((intstat & DVP_ALL_END_ID0) == DVP_ALL_END_ID0) {
            v4l2_warn(v4l2_dev, "frame0/1 trigger simultaneously in DVP ID0\n");
        }
        return RKCIF_STREAM_MIPI_ID0;
    }
    ...
    if (intstat & DVP_ALL_END_ID1) {
        if ((intstat & DVP_ALL_END_ID1) == DVP_ALL_END_ID1) {
            v4l2_warn(v4l2_dev, "frame0/1 trigger simultaneously in DVP ID1\n");
        }
        return RKCIF_STREAM_MIPI_ID1;
    }
    ...
    if (intstat & DVP_ALL_END_ID2) {
        if ((intstat & DVP_ALL_END_ID2) == DVP_ALL_END_ID2) {
            v4l2_warn(v4l2_dev, "frame0/1 trigger simultaneously in DVP ID2\n");
        }
        return RKCIF_STREAM_MIPI_ID2;
    }
    ...
    if (intstat & DVP_ALL_END_ID3) {
        if ((intstat & DVP_ALL_END_ID3) == DVP_ALL_END_ID3) {
            v4l2_warn(v4l2_dev, "frame0/1 trigger simultaneously in DVP ID3\n");
        }
        return RKCIF_STREAM_MIPI_ID3;
    }
...
static bool rkcif_is_csi2_err_trigger_reset(struct rkcif_timer *timer)
{
    ...
    if (!is_first_err) {
        if (timer->csi2_err_triggered_cnt >= 1) {
            ...
            diff_time = div_u64(diff_time, DIFF_TIME);
            ...
            if (!is_assign_triggered && (timer->csi2_err_cnt_odd == 0 || timer->csi2_err_cnt_even == 0)) {
                is_triggered = true;
     ...
     */
    if (stream->is_fs_fe_not_paired || stream->fs_cnt_in_single_frame > RKCIF_FS_DETECTED_NUM) {
        is_triggered = true;
...
static bool rkcif_is_triggered_monitoring(struct rkcif_device *dev)
{
    ...
    if (timer->monitor_mode == RKCIF_MONITOR_MODE_CONTINUE || timer->monitor_mode == RKCIF_MONITOR_MODE_HOTPLUG) {
...
static void rkcif_cal_csi_crop_width_vwidth(struct rkcif_stream *stream, u32 raw_width, u32 *crop_width,
                                            u32 *crop_vwidth)
{
    struct rkcif_device *dev = stream->cifdev;
    ...
    *crop_width = raw_width;
     ...
     */
    if (fmt->fmt_type == CIF_FMT_TYPE_RAW && stream->is_compact) {
        *crop_vwidth = ALIGN(raw_width * fmt->raw_bpp / 0x08, 0x100);
    } else {
        if (fmt->fmt_type == CIF_FMT_TYPE_RAW) {
            *crop_vwidth = ALIGN(raw_width * 0x02, 0x08);
        } else {
            *crop_vwidth = ALIGN(raw_width * fmt->bpp[0] / 0x08, 0x08);
        }
    }

    if (channel->fmt_val == CSI_WRDDR_TYPE_RGB888) {
        *crop_width = raw_width * fmt->bpp[0] / 0x08;
    }
    /*
     * rk cif don't support output yuyv fmt data
     * if user request yuyv fmt, the input mode must be RAW8
...
static void rkcif_dynamic_crop(struct rkcif_stream *stream)
{
    ...
    struct v4l2_mbus_config *mbus;
    const struct cif_output_fmt *fmt;
    u32 raw_width, crop_width = 64, crop_vwidth = 64, crop_height = 64, crop_x = 0, crop_y = 0;
    ...
    if (mbus->type == V4L2_MBUS_CSI2_DPHY || mbus->type == V4L2_MBUS_CSI2_CPHY || mbus->type == V4L2_MBUS_CCP2) {
        struct csi_channel_info *channel = &cif_dev->channels[stream->id];

        if (channel->fmt_val == CSI_WRDDR_TYPE_RGB888) {
            crop_x = 0x03 * stream->crop[CROP_SRC_ACT].left;
        } else {
        ...
        rkcif_cal_csi_crop_width_vwidth(stream, raw_width, &crop_width, &crop_vwidth);
        rkcif_write_register(cif_dev, get_reg_index_of_id_crop_start(channel->id), (crop_y << 0x10) | crop_x);
        rkcif_write_register(cif_dev, get_reg_index_of_id_ctrl1(channel->id), (crop_height << 0x10) | crop_width);

        rkcif_write_register(cif_dev, get_reg_index_of_frm0_y_vlw(channel->id), crop_vwidth);
        rkcif_write_register(cif_dev, get_reg_index_of_frm1_y_vlw(channel->id), crop_vwidth);
        rkcif_write_register(cif_dev, get_reg_index_of_frm0_uv_vlw(channel->id), crop_vwidth);
        rkcif_write_register(cif_dev, get_reg_index_of_frm1_uv_vlw(channel->id), crop_vwidth);
    } else {
        raw_width = stream->crop[CROP_SRC_ACT].width;
        crop_width = raw_width;
        ...
        rkcif_write_register(cif_dev, CIF_REG_DVP_CROP, (crop_y << CIF_CROP_Y_SHIFT) | crop_x);
        ...
        rkcif_write_register(cif_dev, CIF_REG_DVP_SET_SIZE, (crop_height << 0x10) | crop_width);
    }
...
static void rkcif_monitor_reset_event(struct rkcif_device *dev)
{
    ...
    if (timer->is_triggered) {
        ...
        fps = timestamp0 > timestamp1 ? timestamp0 - timestamp1 : timestamp1 - timestamp0;
        ...
        if (mode == RKCIF_MONITOR_MODE_CONTINUE || mode == RKCIF_MONITOR_MODE_HOTPLUG) {
        ...
        v4l2_dbg(1, rkcif_debug, &dev->v4l2_dev, "%s:mode:%d, raw height:%d,vblank:%d, cycle:%ld, fps:%llu\n", __func__,
                 timer->monitor_mode, raw_rect->height, vblank, timer->cycle, div_u64(1000, fps));
    }
}
...
static void rkcif_rdbk_frame_end(struct rkcif_stream *stream)
{
    ...
    if (dev->hdr.hdr_mode == HDR_X2) {
        if (stream->id != RKCIF_STREAM_MIPI_ID1 || dev->stream[RKCIF_STREAM_MIPI_ID0].state != RKCIF_STATE_STREAMING ||
            dev->stream[RKCIF_STREAM_MIPI_ID1].state != RKCIF_STATE_STREAMING) {
            return;
        }
    } else if (dev->hdr.hdr_mode == HDR_X3) {
        if (stream->id != RKCIF_STREAM_MIPI_ID2 || dev->stream[RKCIF_STREAM_MIPI_ID0].state != RKCIF_STATE_STREAMING ||
            dev->stream[RKCIF_STREAM_MIPI_ID1].state != RKCIF_STATE_STREAMING ||
            dev->stream[RKCIF_STREAM_MIPI_ID2].state != RKCIF_STATE_STREAMING) {
            return;
        }
    ...
    if (dev->hdr.hdr_mode == HDR_X3) {
        if (dev->rdbk_buf[RDBK_L] && dev->rdbk_buf[RDBK_M] && dev->rdbk_buf[RDBK_S]) {
            ...
            if (m_ts < l_ts || s_ts < m_ts) {
                v4l2_err(&dev->v4l2_dev, "s/m/l frame err, timestamp s:%lld m:%lld l:%lld\n", s_ts, m_ts, l_ts);
            ...
            if ((m_ts - l_ts) > time || (s_ts - m_ts) > time) {
                ...
                if ((m_ts - l_ts) > time || (s_ts - m_ts) > time) {
                    v4l2_err(&dev->v4l2_dev, "timestamp no match, s:%lld m:%lld l:%lld, fps:%d\n", s_ts, m_ts, l_ts,
                             fps);
                    goto RDBK_FRM_UNMATCH;
                }
            }
            ...
            rkcif_vb_done_oneframe(&dev->stream[RKCIF_STREAM_MIPI_ID0], &dev->rdbk_buf[RDBK_L]->vb);
            rkcif_vb_done_oneframe(&dev->stream[RKCIF_STREAM_MIPI_ID1], &dev->rdbk_buf[RDBK_M]->vb);
            rkcif_vb_done_oneframe(&dev->stream[RKCIF_STREAM_MIPI_ID2], &dev->rdbk_buf[RDBK_S]->vb);
        } else {
        ...
        if (dev->rdbk_buf[RDBK_L] && dev->rdbk_buf[RDBK_M]) {
            ...
            if (s_ts < l_ts) {
                v4l2_err(&dev->v4l2_dev, "s/l frame err, timestamp s:%lld l:%lld\n", s_ts, l_ts);
                goto RDBK_FRM_UNMATCH;
            }
            ...
            if ((s_ts - l_ts) > time) {
                ...
                if ((s_ts - l_ts) > time) {
                    v4l2_err(&dev->v4l2_dev, "timestamp no match, s:%lld l:%lld, fps:%d\n", s_ts, l_ts, fps);
                    goto RDBK_FRM_UNMATCH;
                }
            }
...
static void rkcif_buf_done_prepare(struct rkcif_stream *stream, struct rkcif_buffer *active_buf, int mipi_id, u32 mode)
{
    unsigned long flags;
    struct vb2_v4l2_buffer *vb_done = NULL;
    struct rkcif_device *cif_dev = stream->cifdev;
        ...
        if (cif_dev->is_start_hdr) {
            ...
            if (mipi_id == RKCIF_STREAM_MIPI_ID0) {
                if (cif_dev->rdbk_buf[RDBK_L]) {
                    v4l2_err(&cif_dev->v4l2_dev, "multiple long data in %s frame,frm_idx:%d,state:0x%x\n",
                             cif_dev->hdr.hdr_mode == HDR_X2 ? "hdr_x2" : "hdr_x3", stream->frame_idx,
                ...
                if (cif_dev->rdbk_buf[RDBK_M]) {
                    v4l2_err(&cif_dev->v4l2_dev, "multiple %s frame,frm_idx:%d,state:0x%x\n",
                             cif_dev->hdr.hdr_mode == HDR_X2 ? "short data in hdr_x2" : "medium data in hdr_x3",
                             stream->frame_idx, cif_dev->rdbk_buf[RDBK_M]->vb.vb2_buf.state);
                ...
                if (cif_dev->rdbk_buf[RDBK_S]) {
                    v4l2_err(&cif_dev->v4l2_dev, "multiple %s frame, frm_idx:%d,state:0x%x\n",
                             ...
                             stream->frame_idx, cif_dev->rdbk_buf[RDBK_S]->vb.vb2_buf.state);
...
static void rkcif_update_stream(struct rkcif_device *cif_dev, struct rkcif_stream *stream, int mipi_id)
{
    struct rkcif_buffer *active_buf = NULL;
    unsigned long flags;
    int ret = 0;
    ...
    if (cif_dev->chip_id == CHIP_RV1126_CIF || cif_dev->chip_id == CHIP_RV1126_CIF_LITE ||
        cif_dev->chip_id == CHIP_RK3568_CIF) {
        rkcif_luma_isr(&cif_dev->luma_vdev, mipi_id, stream->frame_idx);
    }

    if (!stream->is_line_wake_up) {
        ret = rkcif_assign_new_buffer_pingpong(stream, RKCIF_YUV_ADDR_STATE_UPDATE, mipi_id);
        if (ret) {
            goto end;
        }
    } else {
...
static void rkcif_update_stream_toisp(struct rkcif_device *cif_dev, struct rkcif_stream *stream, int mipi_id)
{
    if (stream->frame_phase == (CIF_CSI_FRAME0_READY | CIF_CSI_FRAME1_READY)) {
        v4l2_err(&cif_dev->v4l2_dev, "stream[%d], frm0/frm1 end simultaneously,frm id:%d\n", stream->id,
                 stream->frame_idx);
    ...
    rkcif_assign_new_buffer_pingpong_toisp(stream, RKCIF_YUV_ADDR_STATE_UPDATE, mipi_id);
    stream->frame_idx++;
}
...
static u32 rkcif_get_sof(struct rkcif_device *cif_dev)
{
    ...
    if (sensor->mbus.type == V4L2_MBUS_CSI2_DPHY || sensor->mbus.type == V4L2_MBUS_CSI2_CPHY) {
    ...
    } else if (sensor->mbus.type == V4L2_MBUS_PARALLEL || sensor->mbus.type == V4L2_MBUS_BT656) {
...
static int rkcif_do_reset_work(struct rkcif_device *cif_dev, enum rkmodule_reset_src reset_src)
{
    ...
    for (i = 0, j = 0; i < RKCIF_MAX_STREAM_MIPI; i++) {
        ...
        if (stream->state == RKCIF_STATE_STREAMING) {
            v4l2_dbg(1, rkcif_debug, &cif_dev->v4l2_dev, "stream[%d] stopping\n", stream->id);
            stream->stopping = true;

            ret = wait_event_timeout(stream->wq_stopped, stream->state != RKCIF_STATE_STREAMING, msecs_to_jiffies(fps));
            if (!ret) {
                rkcif_stream_stop(stream);
                stream->stopping = false;
            }
            ...
            if (stream->id == RKCIF_STREAM_MIPI_ID0) {
                ...
                v4l2_err(&cif_dev->v4l2_dev, "%s: stream[%d] sync frmid & csi_sof, frm_id:%d, csi_sof:%d\n", __func__,
                         stream->id, stream->frame_idx, sof_cnt);
            ...
            v4l2_dbg(1, rkcif_debug, &cif_dev->v4l2_dev, "%s stop stream[%d] in streaming, frm_id:%d, csi_sof:%d\n",
    ...
    for (i = 0; i < p->num_subdevs; i++) {
        if (p->subdevs[i] == terminal_sensor->sd) {
            if (reset_src == RKCIF_RESET_SRC_ERR_CSI2 || reset_src == RKCIF_RESET_SRC_ERR_HOTPLUG ||
    ...
    for (i = 0; i < j; i++) {
        ...
        v4l2_dbg(1, rkcif_debug, &cif_dev->v4l2_dev, "resume stream[%d], frm_idx:%d, csi_sof:%d\n", stream->id,
                 stream->frame_idx, rkcif_get_sof(cif_dev));
    }
    ...
    for (i = 0; i < p->num_subdevs; i++) {
        if (p->subdevs[i] == terminal_sensor->sd) {
            ...
            if (reset_src == RKCIF_RESET_SRC_ERR_CSI2 || reset_src == RKCIF_RESET_SRC_ERR_HOTPLUG ||
                ...
                ret = v4l2_subdev_call(p->subdevs[i], core, ioctl, RKMODULE_SET_QUICK_STREAM, &on);
                if (ret) {
                    v4l2_err(&cif_dev->v4l2_dev, "quick stream on subdev:%s failed\n", p->subdevs[i]->name);
                }
            }
        } else {
            if (p->subdevs[i] == terminal_sensor->sd) {
    ...
    rkcif_start_luma(&cif_dev->luma_vdev, cif_dev->stream[RKCIF_STREAM_MIPI_ID0].cif_fmt_in);
...
{
    struct rkcif_work_struct *reset_work = container_of(work, struct rkcif_work_struct, work);
    struct rkcif_device *dev = container_of(reset_work, struct rkcif_device, reset_work);
    struct rkcif_timer *timer = &dev->reset_watchdog_timer;
    int ret;
...
static bool rkcif_is_reduced_frame_rate(struct rkcif_device *dev)
{
    ...
    fps = timestamp0 > timestamp1 ? timestamp0 - timestamp1 : timestamp1 - timestamp0;
    fps = div_u64(fps, 1000);
    diff_time = fps > timer->frame_end_cycle_us ? fps - timer->frame_end_cycle_us : 0;
    ...
    if (diff_time > timer->line_end_cycle * deviation) {
        ...
        v4l2_dbg(1, rkcif_debug, &dev->v4l2_dev,
                 ...
                 rkcif_get_sensor_vblank(dev), raw_rect->height, fps, timer->frame_end_cycle_us, timer->line_end_cycle,
                 diff_time);
...
static void rkcif_init_reset_work(struct rkcif_timer *timer)
{
    ...
    v4l2_info(&dev->v4l2_dev, "do reset work schedule, run_cnt:%d, reset source:%d\n", timer->run_cnt,
              timer->reset_src);
    ...
    if (schedule_work(&dev->reset_work.work)) {
        ...
        v4l2_info(&dev->v4l2_dev, "schedule reset work successfully\n");
    } else {
        v4l2_info(&dev->v4l2_dev, "schedule reset work failed\n");
    }
}
...
void rkcif_reset_watchdog_timer_handler(struct timer_list *t)
{
    ...
    if (timer->last_buf_wakeup_cnt < dev->buf_wake_up_cnt) {
        v4l2_dbg(1, rkcif_debug, &dev->v4l2_dev, "info: frame end still update(%d, %d) in detecting cnt:%d, mode:%d\n",
                 timer->last_buf_wakeup_cnt, dev->buf_wake_up_cnt, timer->run_cnt, timer->monitor_mode);

        timer->last_buf_wakeup_cnt = dev->buf_wake_up_cnt;
        rkcif_is_reduced_frame_rate(dev);
        if (timer->monitor_mode == RKCIF_MONITOR_MODE_HOTPLUG) {
            ret = v4l2_subdev_call(terminal_sensor->sd, core, ioctl, RKMODULE_GET_VICAP_RST_INFO, &rst_info);
            if (ret) {
                is_reset = 0;
            } else {
            ...
            v4l2_subdev_call(terminal_sensor->sd, core, ioctl, RKMODULE_SET_VICAP_RST_INFO, &rst_info);
            if (!is_reset) {
...
end_detect:
    ...
    timer->is_running = false;
    spin_unlock_irqrestore(&timer->timer_lock, flags);
    v4l2_info(&dev->v4l2_dev, "stream[%d] is stopped, stop reset detect!\n", dev->stream[i].id);
}
...
static void rkcif_modify_line_int(struct rkcif_stream *stream, bool en)
{
    ...
    if (en) {
        ...
        rkcif_write_register_or(cif_dev, CIF_REG_MIPI_LVDS_INTEN, CSI_LINE_INTEN(stream->id));
    } else {
        rkcif_write_register_and(cif_dev, CIF_REG_MIPI_LVDS_INTEN, ~(CSI_LINE_INTEN(stream->id)));
    }
}
...
void rkcif_enable_dma_capture(struct rkcif_stream *stream)
{
    ...
    if (stream->to_en_dma == RKCIF_DMAEN_BY_VICAP) {
        rkcif_assign_new_buffer_pingpong(stream, RKCIF_YUV_ADDR_STATE_INIT, stream->id);
        rkcif_write_register(cif_dev, get_reg_index_of_frm0_y_vlw(stream->id), channel->virtual_width);
        if (stream->is_compact) {
            val |= LVDS_COMPACT;
        } else {
            val &= ~LVDS_COMPACT;
        ...
        rkcif_assign_new_buffer_pingpong_toisp(stream, RKCIF_YUV_ADDR_STATE_INIT, stream->id);
    }
    if (mbus_cfg->type == V4L2_MBUS_CSI2_DPHY || mbus_cfg->type == V4L2_MBUS_CSI2_CPHY ||
        mbus_cfg->type == V4L2_MBUS_CCP2) {
        if (mbus_cfg->type == V4L2_MBUS_CSI2_DPHY || mbus_cfg->type == V4L2_MBUS_CSI2_CPHY) {
...
static void rkcif_toisp_check_stop_status(struct sditf_priv *priv, unsigned int intstat_glb, int index)
{
    int ch = 0;
    struct rkcif_stream *stream;
    int src_id = 0;
    ...
    for (i = 0; i < TOISP_CH_MAX; i++) {
        ...
        if (stream->stopping) {
            v4l2_dbg(3, rkcif_debug, &priv->cif_dev->v4l2_dev, "stream[%d] stop\n", stream->id);
            rkcif_stream_stop(stream);
            stream->stopping = false;
            wake_up(&stream->wq_stopped);
        }
...
static void rkcif_deal_sof(struct rkcif_device *cif_dev)
{
    ...
    if (mbus->type == V4L2_MBUS_CSI2_DPHY || mbus->type == V4L2_MBUS_CSI2_CPHY) {
...
unsigned int rkcif_irq_global(struct rkcif_device *cif_dev)
{
    ...
    if (intstat_glb & SCALE_TOISP_AXI0_ERR) {
        v4l2_err(&cif_dev->v4l2_dev, "ERROR: scale channel, AXI0 bus err intstat_glb:0x%x !!\n", intstat_glb);
        return 0;
    }
    if (intstat_glb & SCALE_TOISP_AXI1_ERR) {
        v4l2_err(&cif_dev->v4l2_dev, "ERROR: scale channel, AXI1 bus err intstat_glb:0x%x !!\n", intstat_glb);
        return 0;
    }
    rkcif_irq_handle_scale(cif_dev, intstat_glb);
    v4l2_dbg(3, rkcif_debug, &cif_dev->v4l2_dev, "intstat_glb 0x%x\n", intstat_glb);
    return intstat_glb;
}
...
void rkcif_irq_pingpong_v1(struct rkcif_device *cif_dev)
{
    ...
    if (mbus->type == V4L2_MBUS_CSI2_DPHY || mbus->type == V4L2_MBUS_CSI2_CPHY || mbus->type == V4L2_MBUS_CCP2) {
        ...
        if (intstat) {
            ...
            v4l2_dbg(3, rkcif_debug, &cif_dev->v4l2_dev, "intstat 0x%x\n", intstat);
        } else {
            return;
        }
        ...
        if (intstat & CSI_SIZE_ERR) {
            ...
            v4l2_err(&cif_dev->v4l2_dev, "ERROR: csi size err, intstat:0x%x, lastline:%d!!\n", intstat, lastline);
            return;
        }

        if (intstat & CSI_FIFO_OVERFLOW_V1) {
            cif_dev->irq_stats.csi_overflow_cnt++;
            v4l2_err(&cif_dev->v4l2_dev, "ERROR: csi fifo overflow, intstat:0x%x, lastline:%d!!\n", intstat, lastline);
            return;
        }

        if (intstat & CSI_BANDWIDTH_LACK_V1) {
            cif_dev->irq_stats.csi_bwidth_lack_cnt++;
            v4l2_err(&cif_dev->v4l2_dev, "ERROR: csi bandwidth lack, intstat:0x%x!!\n", intstat);
            return;
        }

        if (intstat & CSI_ALL_ERROR_INTEN_V1) {
            cif_dev->irq_stats.all_err_cnt++;
            v4l2_err(&cif_dev->v4l2_dev, "ERROR: CSI_ALL_ERROR_INTEN:0x%x!!\n", intstat);
            return;
        }
        ...
        for (i = 0; i < RKCIF_MAX_STREAM_MIPI; i++) {
            if (intstat & CSI_LINE_INTSTAT(i)) {
                ...
                v4l2_dbg(1, rkcif_debug, &cif_dev->v4l2_dev, "%s: id0 cur line:%d\n", __func__, lastline & 0x3fff);
            }
        }
        ...
        for (i = 0; i < RKCIF_MAX_STREAM_MIPI; i++) {
            ...
            if (stream->stopping && !(stream->cur_stream_mode & RKCIF_STREAM_MODE_TOISP)) {
                rkcif_stream_stop(stream);
                stream->stopping = false;
                wake_up(&stream->wq_stopped);
                continue;
            ...
            if (mipi_id == RKCIF_STREAM_MIPI_ID0) {
                if ((intstat & (CSI_FRAME1_START_ID0 | CSI_FRAME0_START_ID0)) == 0 &&
                    ...
                    v4l2_err(&cif_dev->v4l2_dev, "%s ERR: multi fs in oneframe, bak_int:0x%x, fs_num:%u\n", __func__,
                             bak_intstat, detect_stream->fs_cnt_in_single_frame);
        ...
        if (intstat & DVP_BANDWIDTH_LACK) {
            ...
            v4l2_info(&cif_dev->v4l2_dev, "dvp bandwidth lack err intstat 0x%x\n", intstat);
        }

        if (intstat & INTSTAT_ERR_RK3588) {
            cif_dev->irq_stats.all_err_cnt++;
            v4l2_err(&cif_dev->v4l2_dev, "ERROR: DVP_ALL_ERROR_INTEN:0x%x!!\n", intstat);
        }
...
void rkcif_irq_pingpong(struct rkcif_device *cif_dev)
{
    ...
    mbus = &cif_dev->active_sensor->mbus;
    if ((mbus->type == V4L2_MBUS_CSI2_DPHY || mbus->type == V4L2_MBUS_CSI2_CPHY || mbus->type == V4L2_MBUS_CCP2) &&
        (cif_dev->chip_id == CHIP_RK1808_CIF || cif_dev->chip_id == CHIP_RV1126_CIF ||
         cif_dev->chip_id == CHIP_RK3568_CIF)) {
        int mipi_id;
        u32 lastline = 0;
        ...
        if ((intstat & LINE_INT_END) && !(intstat & FRAME_END) && (cif_dev->dvp_sof_in_oneframe == 0)) {
        ...
        if (cif_dev->chip_id <= CHIP_RK1808_CIF) {
            ...
            if ((intstat & FRAME_END)) {
                ...
                if ((cif_frmst == 0xfffd0002) || (cif_frmst == 0xfffe0002)) {
                    ...
                    rkcif_write_register(cif_dev, CIF_REG_DVP_FRAME_STATUS, FRAME_STAT_CLS);
                }

                if (lastline != stream->pixm.height || (!(cif_frmst & CIF_F0_READY) && !(cif_frmst & CIF_F1_READY))) {
                    cif_dev->dvp_sof_in_oneframe = 1;
                    v4l2_err(&cif_dev->v4l2_dev, "Bad frame, pp irq:0x%x frmst:0x%x size:%dx%d\n", intstat, cif_frmst,
                             lastpix, lastline);
                    return;
                }
...
void rkcif_irq_lite_lvds(struct rkcif_device *cif_dev)
{
    ...
    if (mbus->type == V4L2_MBUS_CCP2 && cif_dev->chip_id == CHIP_RV1126_CIF_LITE) {
        int mipi_id;
        u32 lastline = 0;
        ...
        if (intstat & CSI_FIFO_OVERFLOW) {
            v4l2_err(&cif_dev->v4l2_dev, "ERROR: cif lite lvds fifo overflow, intstat:0x%x, lastline:%d!!\n", intstat,
                     lastline);
            return;
        }

        if (intstat & CSI_BANDWIDTH_LACK) {
            v4l2_err(&cif_dev->v4l2_dev, "ERROR: cif lite lvds bandwidth lack, intstat:0x%x!!\n", intstat);
            return;
        }

        if (intstat & CSI_ALL_ERROR_INTEN) {
            v4l2_err(&cif_dev->v4l2_dev, "ERROR: cif lite lvds all err:0x%x!!\n", intstat);
            return;
        }
