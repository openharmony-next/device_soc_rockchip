...
int rkispp_allow_buffer(struct rkispp_device *dev, struct rkispp_dummy_buffer *buf)
{
    ...
    mem_priv = g_ops->alloc(dev->hw_dev->dev, attrs, buf->size, DMA_BIDIRECTIONAL, GFP_KERNEL | GFP_DMA32);
    ...
    v4l2_dbg(1, rkispp_debug, &dev->v4l2_dev, "%s buf:0x%x~0x%x size:%d\n", __func__, (u32)buf->dma_addr,
             (u32)buf->dma_addr + buf->size, buf->size);
    return ret;
err:
...
void rkispp_free_buffer(struct rkispp_device *dev, struct rkispp_dummy_buffer *buf)
{
    ...
    if (buf && buf->mem_priv) {
        v4l2_dbg(1, rkispp_debug, &dev->v4l2_dev, "%s buf:0x%x~0x%x\n", __func__, (u32)buf->dma_addr,
...
static int rkispp_init_pool(struct rkispp_hw_dev *hw, struct rkisp_ispp_buf *dbufs)
{
    ...
    for (i = 0; i < hw->pool[0].group_buf_max; i++) {
        mem = g_ops->attach_dmabuf(hw->dev, dbufs->dbuf[i], dbufs->dbuf[i]->size, DMA_BIDIRECTIONAL);
        if (IS_ERR(mem)) {
            ret = PTR_ERR(mem);
            goto err;
        }
...
static void rkispp_queue_dmabuf(struct rkispp_hw_dev *hw, struct rkisp_ispp_buf *dbufs)
{
    ...
    if (buf) {
        ...
        val = (vdev->module_ens & ISPP_MODULE_TNR)
                  ? ISPP_MODULE_TNR
...
static int rkispp_alloc_page_dummy_buf(struct rkispp_device *dev, u32 size)
{
    ...
    ret = sg_alloc_table_from_pages(sg, pages, n_pages, 0, n_pages << PAGE_SHIFT, GFP_KERNEL);
    if (ret) {
        goto free_sg;
    }
    ...
    v4l2_dbg(1, rkispp_debug, &dev->v4l2_dev, "%s buf:0x%x map cnt:%d\n", __func__, (u32)dummy_buf->dma_addr, ret);
    return 0;
free_sg:
    kfree(sg);
free_pages:
...
int rkispp_alloc_common_dummy_buf(struct rkispp_device *dev)
{
    ...
    if (!ret) {
        v4l2_dbg(1, rkispp_debug, &dev->v4l2_dev, "%s buf:0x%x size:%d\n", __func__, (u32)dummy_buf->dma_addr,
                 dummy_buf->size);
    }
end:
    if (ret < 0) {
...
void rkispp_free_common_dummy_buf(struct rkispp_device *dev)
{
    ...
    mutex_lock(&hw->dev_lock);
    if (atomic_read(&hw->refcnt) || atomic_read(&dev->stream_vdev.refcnt) > 1) {
        goto end;
    }
    if (hw->is_mmu) {
...
void rkispp_release_regbuf(struct rkispp_device *ispp, struct rkisp_ispp_reg *freebuf)
{
    ...
    for (i = 0; i < RKISP_ISPP_REGBUF_NUM; i++) {
        if (reg_buf[i].dev_id == freebuf->dev_id && reg_buf[i].frame_timestamp < freebuf->frame_timestamp) {
            reg_buf[i].frame_id = 0;
            reg_buf[i].stat = ISP_ISPP_FREE;
        }
    }
...
bool rkispp_is_reg_withstream_local(struct device *dev)
{
    ...
    if (!memcmp(rkispp_reg_withstream_video_name, node_name, strlen(node_name))) {
        return true;
    } else {
        return false;
    }
