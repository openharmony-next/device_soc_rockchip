...
static int rkispp_subdev_link_setup(struct media_entity *entity, const struct media_pad *local,
                                    const struct media_pad *remote, u32 flags)
{
    ...
    if (local->index != RKISPP_PAD_SINK && local->index != RKISPP_PAD_SOURCE) {
        return 0;
    }
    ...
    if (!strcmp(remote->entity->name, II_VDEV_NAME)) {
        ...
        v4l2_dbg(1, rkispp_debug, &dev->v4l2_dev, "input:%d\n", dev->inp);
    ...
    if (stream && dev->stream_sync) {
        stream->linked = flags & MEDIA_LNK_FL_ENABLED;
        v4l2_dbg(1, rkispp_debug, &dev->v4l2_dev, "stream:%d linked:%d\n", stream->id, stream->linked);
    }
    return 0;
}

static int rkispp_sd_get_fmt(struct v4l2_subdev *sd, struct v4l2_subdev_pad_config *cfg, struct v4l2_subdev_format *fmt)
{
    ...
    if (fmt->pad != RKISPP_PAD_SINK && fmt->pad != RKISPP_PAD_SOURCE) {
        goto err;
    }
    ...
    *mf = ispp_sdev->in_fmt;
    if (fmt->pad == RKISPP_PAD_SINK && ispp_sdev->dev->inp == INP_ISP) {
        ret = v4l2_subdev_call(ispp_sdev->remote_sd, pad, get_fmt, cfg, fmt);
        if (!ret) {
...
static int rkispp_sd_set_fmt(struct v4l2_subdev *sd, struct v4l2_subdev_pad_config *cfg, struct v4l2_subdev_format *fmt)
{
    struct rkispp_subdev *ispp_sdev = v4l2_get_subdevdata(sd);
    struct v4l2_mbus_framefmt *mf;
...
static int rkispp_sd_get_selection(struct v4l2_subdev *sd, struct v4l2_subdev_pad_config *cfg,
                                   struct v4l2_subdev_selection *sel)
{
    ...
    ret = v4l2_subdev_call(ispp_sdev->remote_sd, pad, get_selection, cfg, sel);
...
static int rkispp_sd_set_selection(struct v4l2_subdev *sd, struct v4l2_subdev_pad_config *cfg,
                                   struct v4l2_subdev_selection *sel)
{
    struct rkispp_subdev *ispp_sdev = v4l2_get_subdevdata(sd);
    struct v4l2_rect *crop;
    ...
    if (sel->pad != RKISPP_PAD_SINK || sel->target != V4L2_SEL_TGT_CROP) {
        goto err;
    }
    ...
    ret = v4l2_subdev_call(ispp_sdev->remote_sd, pad, set_selection, cfg, sel);
    if (!ret) {
        ispp_sdev->out_fmt.width = crop->width;
        ispp_sdev->out_fmt.height = crop->height;
    }
...
static int rkispp_sd_s_stream(struct v4l2_subdev *sd, int on)
{
    ...
    v4l2_dbg(1, rkispp_debug, &ispp_sdev->dev->v4l2_dev, "s_stream on:%d\n", on);
...
static int rkispp_sd_s_rx_buffer(struct v4l2_subdev *sd, void *buf, unsigned int *size)
{
    struct rkispp_subdev *ispp_sdev = v4l2_get_subdevdata(sd);
    struct rkispp_device *dev = ispp_sdev->dev;
...
static int rkispp_sd_s_power(struct v4l2_subdev *sd, int on)
{
    ...
    v4l2_dbg(1, rkispp_debug, &ispp_dev->v4l2_dev, "s_power on:%d\n", on);
    if (on) {
        if (ispp_dev->inp == INP_ISP) {
            ...
            if (ret) {
                v4l2_err(&ispp_dev->v4l2_dev, "%s get format fail:%d\n", __func__, ret);
                return ret;
            }
            ...
            if (ret) {
                v4l2_err(&ispp_dev->v4l2_dev, "%s get crop fail:%d\n", __func__, ret);
                return ret;
            }

            ret = v4l2_subdev_call(ispp_sdev->remote_sd, core, s_power, 1);
            if (ret < 0) {
                v4l2_err(&ispp_dev->v4l2_dev, "%s set isp power on fail:%d\n", __func__, ret);
                return ret;
            }
        }
        ret = pm_runtime_get_sync(ispp_dev->dev);
        if (ret < 0) {
            v4l2_err(&ispp_dev->v4l2_dev, "%s runtime get failed:%d\n", __func__, ret);
            if (ispp_dev->inp == INP_ISP) {
        ...
        if (ret < 0) {
            v4l2_err(&ispp_dev->v4l2_dev, "%s runtime put failed:%d\n", __func__, ret);
        }
    }
...
#ifdef CONFIG_COMPAT
static long rkispp_compat_ioctl32(struct v4l2_subdev *sd, unsigned int cmd, unsigned long arg)
{
...
static int rkispp_subscribe_event(struct v4l2_subdev *sd, struct v4l2_fh *fh, struct v4l2_event_subscription *sub)
{
    switch (sub->type) {
...
int rkispp_register_subdev(struct rkispp_device *dev, struct v4l2_device *v4l2_dev)
{
    ...
    ret = media_entity_pads_init(&sd->entity, RKISPP_PAD_MAX, ispp_sdev->pads);
    if (ret < 0) {
        return ret;
    }
