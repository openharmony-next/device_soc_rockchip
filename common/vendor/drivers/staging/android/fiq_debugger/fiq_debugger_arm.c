...
void fiq_debugger_dump_regs(struct fiq_debugger_output *output, const struct pt_regs *regs)
{
    ...
    output->printf(output, " r4 %08x  r5 %08x  r6 %08x  r7 %08x\n", regs->ARM_r4, regs->ARM_r5, regs->ARM_r6,
                   regs->ARM_r7);
    output->printf(output, " r8 %08x  r9 %08x r10 %08x r11 %08x  mode %s\n", regs->ARM_r8, regs->ARM_r9, regs->ARM_r10,
                   regs->ARM_fp, mode_name(regs->ARM_cpsr));
    output->printf(output, " ip %08x  sp %08x  lr %08x  pc %08x cpsr %08x\n", regs->ARM_ip, regs->ARM_sp, regs->ARM_lr,
...
void fiq_debugger_dump_allregs(struct fiq_debugger_output *output, const struct pt_regs *regs)
{
    ...
    output->printf(output, "%csvc: sp %08x  lr %08x  spsr %08x\n", mode == SVC_MODE ? '*' : ' ', mode_regs.sp_svc,
                   mode_regs.lr_svc, mode_regs.spsr_svc);
    output->printf(output, "%cabt: sp %08x  lr %08x  spsr %08x\n", mode == ABT_MODE ? '*' : ' ', mode_regs.sp_abt,
                   mode_regs.lr_abt, mode_regs.spsr_abt);
    output->printf(output, "%cund: sp %08x  lr %08x  spsr %08x\n", mode == UND_MODE ? '*' : ' ', mode_regs.sp_und,
                   mode_regs.lr_und, mode_regs.spsr_und);
    output->printf(output, "%cirq: sp %08x  lr %08x  spsr %08x\n", mode == IRQ_MODE ? '*' : ' ', mode_regs.sp_irq,
                   mode_regs.lr_irq, mode_regs.spsr_irq);
    output->printf(output, "%cfiq: r8 %08x  r9 %08x  r10 %08x  r11 %08x  r12 %08x\n", mode == FIQ_MODE ? '*' : ' ',
                   mode_regs.r8_fiq, mode_regs.r9_fiq, mode_regs.r10_fiq, mode_regs.r11_fiq, mode_regs.r12_fiq);
    output->printf(output, " fiq: sp %08x  lr %08x  spsr %08x\n", mode_regs.sp_fiq, mode_regs.lr_fiq,
                   mode_regs.spsr_fiq);
}
...
static int report_trace(struct stackframe *frame, void *d)
{
    ...
    if (sts->depth) {
        sts->output->printf(sts->output, "  pc: %px (%pF), lr %px (%pF), sp %px, fp %px\n", frame->pc, frame->pc,
                            frame->lr, frame->lr, frame->sp, frame->fp);
        sts->depth--;
        return 0;
    }
...
static struct frame_tail *user_backtrace(struct fiq_debugger_output *output, struct frame_tail *tail)
{
    ...
    if (__copy_from_user_inatomic(buftail, tail, sizeof(buftail))) {
        output->printf(output, "  failed to copy frame pointer %px\n", tail);
        return NULL;
    }
                                  ...
                                  void *ssp)
{
    ...
    *current_thread_info() = *real_thread_info;
    ...
    if (!user_mode(regs)) {
        ...
        output->printf(output, "  pc: %px (%pF), lr %px (%pF), sp %px, fp %px\n", regs->ARM_pc, regs->ARM_pc,
                       regs->ARM_lr, regs->ARM_lr, regs->ARM_sp, regs->ARM_fp);
