        ...
        const struct pt_regs *regs)
{
    ...
    output->printf(output, " r4 %08x  r5 %08x  r6 %08x  r7 %08x\n",
            regs->compat_usr(DEBUGER_INDEX_FO), regs->compat_usr(DEBUGER_INDEX_FI),
            regs->compat_usr(DEBUGER_INDEX_SI), regs->compat_usr(DEBUGER_INDEX_SE));
    output->printf(output, " r8 %08x  r9 %08x r10 %08x r11 %08x\n",
            regs->compat_usr(DEBUGER_INDEX_EI), regs->compat_usr(DEBUGER_INDEX_NI),
            regs->compat_usr(DEBUGER_INDEX_TE), regs->compat_usr(DEBUGER_INDEX_EIE));
    output->printf(output, " ip %08x  sp %08x  lr %08x  pc %08x\n",
            regs->compat_usr(DEBUGER_INDEX_TWE), regs->compat_sp,
        ...
        const struct pt_regs *regs)
{
    ...
    output->printf(output, " x22 %016lx  x23 %016lx\n",
            regs->regs[DEBUGER_INDEX_TWET], regs->regs[DEBUGER_INDEX_TWETH]);
    output->printf(output, " x24 %016lx  x25 %016lx\n",
            regs->regs[DEBUGER_INDEX_TWEF], regs->regs[DEBUGER_INDEX_TWEFI]);
    output->printf(output, " x26 %016lx  x27 %016lx\n",
            regs->regs[DEBUGER_INDEX_TWES], regs->regs[DEBUGER_INDEX_TWESE]);
    output->printf(output, " x28 %016lx  x29 %016lx\n",
            regs->regs[DEBUGER_INDEX_TWEE], regs->regs[DEBUGER_INDEX_TWENI]);
