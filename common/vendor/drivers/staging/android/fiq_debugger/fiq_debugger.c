...
static inline void fiq_debugger_enable_wakeup_irq(struct fiq_debugger_state *state)
{
}
static inline void fiq_debugger_disable_wakeup_irq(struct fiq_debugger_state *state)
{
}
#else
static inline void fiq_debugger_enable_wakeup_irq(struct fiq_debugger_state *state)
...
__printf(2, 3) static void fiq_debugger_printf(struct fiq_debugger_output *output, const char *fmt, ...)
{
...
__printf(2, 3) static int fiq_debugger_printf_nfiq(void *cookie, const char *fmt, ...)
{
...
static void fiq_debugger_dump_irqs(struct fiq_debugger_state *state)
{
    ...
    for_each_irq_desc(n, desc)
    {
        ...
        fiq_debugger_printf(&state->output, "%5d: %10u %11u %8x  %s\n", n, kstat_irqs(n),
                            kstat_irqs(n) - state->last_irqs[n], desc->status_use_accessors,
...
static void fiq_debugger_do_ps(struct fiq_debugger_state *state)
{
    ...
    do_each_thread(g, p)
    {
        ...
        fiq_debugger_printf(&state->output, "%5d %5d %4d ", p->pid, p->parent->pid, p->prio);
...
static void fiq_debugger_schedule_work(struct fiq_debugger_state *state, char *cmd)
{
    unsigned long flags;
...
static void fiq_debugger_irq_exec(struct fiq_debugger_state *state, char *cmd)
{
    ...
    if (!strncmp(cmd, "sysrq ", 0x6)) {
        fiq_debugger_do_sysrq(state, cmd[0x6]);
    }
#endif
#ifdef CONFIG_KGDB
    if (!strcmp(cmd, "kgdb")) {
...
static char cmd_buf[][16] = {
    {"pc"},    {"regs"},    {"allregs"}, {"bt"},      {"reboot"},
#ifdef CONFIG_ROCKCHIP_DEBUG
    {"pcsr"},
#endif
    {"sleep"}, {"nosleep"}, {"console"}, {"cpu"},
#ifndef CONFIG_FIQ_DEBUGGER_MODULE
    {"reset"}, {"irqs"},    {"kmsg"},    {"version"}, {"ps"},     {"sysrq"},
#endif
...
#ifndef CONFIG_FIQ_DEBUGGER_MODULE
    fiq_debugger_printf(&state->output, " reset [<c>]   Hard reset with command <c>\n"
...
#ifdef CONFIG_FIQ_DEBUGGER_TRUST_ZONE
        if (sip_fiq_debugger_is_enabled() || sdei_fiq_debugger_is_enabled()) {
...
static bool fiq_debugger_fiq_exec(struct fiq_debugger_state *state, const char *cmd, const struct pt_regs *regs,
                                  void *svc_sp)
{
    ...
    if (!strcmp(cmd, "help") || !strcmp(cmd, "?")) {
        ...
        if (user_mode((struct pt_regs *)regs) || ((unsigned long)svc_sp < va_start) ||
            ((unsigned long)svc_sp > -256UL)) {
            fiq_debugger_printf(&state->output, "User mode\n");
        } else {
            fiq_debugger_dump_stacktrace(&state->output, regs, 0x64, svc_sp);
        }
    } else if (!strncmp(cmd, "reset", 0x5)) {
        cmd += 0x5;
        while (*cmd == ' ') {
            cmd++;
        }
    ...
    } else if (!strncmp(cmd, "cpu ", 0x4)) {
        unsigned long cpu = 0;
        if (kstrtoul(cmd + 0x4, 0xA, &cpu) == 0) {
        ...
        if (state->debug_busy) {
            fiq_debugger_printf(&state->output, "command processor busy. trying to abort.\n");
            state->debug_abort = -1;
        } else {
...
static void fiq_debugger_handle_wakeup(struct fiq_debugger_state *state)
{
    ...
    if (state->wakeup_irq >= 0 && state->ignore_next_wakeup_irq) {
        ...
        mod_timer(&state->sleep_timer, jiffies + HZ / 0x2);
    }
    spin_unlock_irqrestore(&state->sleep_timer_lock, flags);
}
...
static void fiq_debugger_handle_console_irq_context(struct fiq_debugger_state *state)
{
...
static void fiq_debugger_handle_irq_context(struct fiq_debugger_state *state)
{
    if (!state->no_sleep) {
        ...
        spin_lock_irqsave(&state->sleep_timer_lock, flags);
        __pm_stay_awake(&state->debugger_wake_src);
        mod_timer(&state->sleep_timer, jiffies + HZ * 0x5);
        spin_unlock_irqrestore(&state->sleep_timer_lock, flags);
    }
...
static bool fiq_debugger_handle_uart_interrupt(struct fiq_debugger_state *state, int this_cpu,
                                               const struct pt_regs *regs, void *svc_sp)
{
    ...
    if (this_cpu != state->current_cpu) {
        ...
        if (atomic_inc_return(&state->unhandled_fiq_count) != MAX_UNHANDLED_FIQ_COUNT) {
            return false;
        }
    ...
    while ((c = fiq_debugger_getc(state)) != FIQ_DEBUGGER_NO_CHAR) {
        ...
        if (!state->debug_enable) {
            ...
            if (fiq_debugger_have_fiq(state)) {
                ...
                fiq_debugger_puts(state, "irq debugger mode\n");
            }
            state->debug_count = 0;
#ifdef CONFIG_ARCH_ROCKCHIP
...
#endif
            fiq_debugger_prompt(state);
            fiq_debugger_ringbuf_push(state->tty_rbuf, 0x8);
            fiq_debugger_ringbuf_push(state->tty_rbuf, 0x8);
#ifdef CONFIG_FIQ_DEBUGGER_CONSOLE
            ...
            if (state->debug_count) {
                ...
                state->debug_count = 0;
                signal_helper |= fiq_debugger_fiq_exec(state, state->debug_buf, regs, svc_sp);
#ifdef CONFIG_ARCH_ROCKCHIP
...
#ifdef CONFIG_FIQ_GLUE
static void fiq_debugger_fiq(struct fiq_glue_handler *h, const struct pt_regs *regs, void *svc_sp)
{
    struct fiq_debugger_state *state = container_of(h, struct fiq_debugger_state, handler);
    unsigned int this_cpu = THREAD_INFO(svc_sp)->cpu;
    bool need_irq;

    need_irq = fiq_debugger_handle_uart_interrupt(state, this_cpu, regs, svc_sp);
    if (need_irq) {
...
void fiq_debugger_fiq(void *regs, u32 cpu)
{
    ...
    need_irq = fiq_debugger_handle_uart_interrupt(state, cpu, regs, current_thread_info());
    if (need_irq) {
        fiq_debugger_force_irq(state);
    }
}
...
static irqreturn_t fiq_debugger_uart_irq(int irq, void *dev)
{
    ...
    not_done = fiq_debugger_handle_uart_interrupt(state, smp_processor_id(), get_irq_regs(), current_thread_info());
    if (not_done) {
...
static void fiq_debugger_console_write(struct console *co, const char *s, unsigned int count)
{
    struct fiq_debugger_state *state;
    unsigned long flags;
...
static int fiq_debugger_tty_init(void)
{
    ...
    fiq_tty_driver->flags = TTY_DRIVER_REAL_RAW | TTY_DRIVER_DYNAMIC_DEV;
    fiq_tty_driver->driver_state = states;

    fiq_tty_driver->init_termios.c_cflag = B115200 | CS8 | CREAD | HUPCL | CLOCAL;
    fiq_tty_driver->init_termios.c_ispeed = 0x1C200;
    fiq_tty_driver->init_termios.c_ospeed = 0x1C200;
...
static int fiq_debugger_tty_init_one(struct fiq_debugger_state *state)
{
    ...
    state->tty_rbuf = fiq_debugger_ringbuf_alloc(0x400);
    if (!state->tty_rbuf) {
    ...
    tty_dev = tty_port_register_device(&state->tty_port, fiq_tty_driver, state->pdev->id, &state->pdev->dev);
    if (IS_ERR(tty_dev)) {
...
static int fiq_debugger_probe(struct platform_device *pdev)
{
    ...
    if ((pdata->uart_enable && !pdata->uart_disable) || (!pdata->uart_enable && pdata->uart_disable)) {
        return -EINVAL;
    }
    ...
    if (state->signal_irq >= 0) {
        ret = request_irq(state->signal_irq, fiq_debugger_signal_irq, IRQF_TRIGGER_RISING, "debug-signal", state);
        if (ret) {
    ...
    if (state->wakeup_irq >= 0) {
        ret = request_irq(state->wakeup_irq, fiq_debugger_wakeup_irq_handler, IRQF_TRIGGER_FALLING, "debug-wakeup",
                          state);
        if (ret) {
...
console_out:
    ...
    if (!console_set_on_cmdline) {
        add_preferred_console(state->console.name, state->console.index, NULL);
    }
#endif
