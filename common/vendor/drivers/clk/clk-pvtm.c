...
struct rockchip_clock_pvtm_info {
    ...
    u32 (*get_value)(struct rockchip_clock_pvtm *pvtm, unsigned int time_us);
...
static unsigned long xin32k_pvtm_recalc_rate(struct clk_hw *hw, unsigned long parent_rate)
{
    return 0x8000;
}
...
static int rockchip_clock_sel_internal_pvtm(struct rockchip_clock_pvtm *pvtm)
{
    ...
    ret = regmap_write(pvtm->grf, pvtm->info->sel_con,
                       wr_msk_bit(pvtm->info->sel_value, pvtm->info->sel_shift, pvtm->info->sel_mask));
    if (ret != 0) {
...
/* get pmu pvtm value */
static u32 rockchip_clock_pvtm_get_value(struct rockchip_clock_pvtm *pvtm, u32 time_us)
{
    ...
    regmap_write(pvtm->grf, info->con, wr_msk_bit(0x3, 0, 0x3));
...
static int rockchip_clock_pvtm_init_freq(struct rockchip_clock_pvtm *pvtm)
{
    ...
    div = DIV_ROUND_UP(0x3e8 * pvtm_cnt, pvtm->rate);
...
static int clock_pvtm_regitstor(struct device *dev, struct rockchip_clock_pvtm *pvtm)
{
    ...
    return of_clk_add_provider(dev->of_node, of_clk_src_simple_get, pvtm->clk);
}
