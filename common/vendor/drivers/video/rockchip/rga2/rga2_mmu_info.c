...
dma_addr_t rga2_dma_flush_page(struct page *page, int map)
{
    ...
    if (map & MMU_MAP_MASK) {
        switch (map) {
            case MMU_MAP_CLEAN:
                paddr = dma_map_page(rga2_drvdata->dev, page, 0, PAGE_SIZE, DMA_TO_DEVICE);
                break;
            case MMU_MAP_INVALID:
                paddr = dma_map_page(rga2_drvdata->dev, page, 0, PAGE_SIZE, DMA_FROM_DEVICE);
                break;
            case MMU_MAP_CLEAN | MMU_MAP_INVALID:
                paddr = dma_map_page(rga2_drvdata->dev, page, 0, PAGE_SIZE, DMA_BIDIRECTIONAL);
                break;
            default:
                paddr = 0;
        ...
        switch (map) {
            case MMU_UNMAP_CLEAN:
                dma_unmap_page(rga2_drvdata->dev, paddr, PAGE_SIZE, DMA_TO_DEVICE);
                break;
            case MMU_UNMAP_INVALID:
                dma_unmap_page(rga2_drvdata->dev, paddr, PAGE_SIZE, DMA_FROM_DEVICE);
                break;
            case MMU_UNMAP_CLEAN | MMU_UNMAP_INVALID:
                dma_unmap_page(rga2_drvdata->dev, paddr, PAGE_SIZE, DMA_BIDIRECTIONAL);
                break;
            default:
                pr_err("unknown map cmd 0x%x\n", map);
                break;
...
static int rga2_user_memory_check(struct page **pages, u32 w, u32 h, u32 format, int flag)
{
    ...
    taipage_num = w * h * bits / 0x8 / (0x400 * 0x4);
    taidata_num = w * h * bits / 0x8 % (0x400 * 0x4);
    if (taidata_num == 0) {
        vaddr = kmap(pages[taipage_num - 1]);
        tai_vaddr = (int *)vaddr + 0x3ff;
    } else {
        vaddr = kmap(pages[taipage_num]);
        tai_vaddr = (int *)vaddr + taidata_num / 0x4 - 1;
    }
...
static int rga2_virtual_memory_check(void *vaddr, u32 w, u32 h, u32 format, int fd)
{
    ...
    one_line = kzalloc(w * 0x4, GFP_KERNEL);
    if (!one_line) {
        ERR("kzalloc fail %s[%d]\n", __func__, __LINE__);
        return 0;
    }

    temp_data = w * (h - 1) * bits >> 0x3;
    if (fd > 0) {
        INFO("vaddr is%p, bits is %d, fd check\n", vaddr, bits);
        memcpy(one_line, (char *)vaddr + temp_data, w * bits >> 0x3);
        INFO("fd check ok\n");
    } else {
        INFO("vir addr memory check.\n");
        memcpy((void *)((char *)vaddr + temp_data), one_line, w * bits >> 0x3);
...
static int rga2_dma_memory_check(struct rga_dma_buffer_t *buffer, struct rga_img_info_t *img)
{
    int ret = 0;
    void *vaddr;
    ...
    if (!IS_ERR_OR_NULL(dma_buffer)) {
        ...
        if (vaddr) {
            ret = rga2_virtual_memory_check(vaddr, img->vir_w, img->vir_h, img->format, img->yrgb_addr);
        } else {
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 4, 0))
static int rga2_map_dma_buffer(int fd, struct rga_dma_buffer_t *rga_dma_buffer, enum dma_data_direction dir)
{
...
static void rga2_unmap_dma_buffer(struct rga_dma_buffer_t *rga_dma_buffer)
{
    if (rga_dma_buffer->attach && rga_dma_buffer->sgt) {
        dma_buf_unmap_attachment(rga_dma_buffer->attach, rga_dma_buffer->sgt, rga_dma_buffer->dir);
    }
...
static void rga2_convert_addr(struct rga_img_info_t *img)
{
    ...
    if (rga2_is_yuv422p_format(img->format)) {
        img->v_addr = img->uv_addr + (img->vir_w * img->vir_h) / 0x2;
    } else {
        img->v_addr = img->uv_addr + (img->vir_w * img->vir_h) / 0x4;
    }
}
...
static int rga2_get_dma_info(struct rga2_reg *reg, struct rga2_req *req)
{
    ...
    if ((int)req->src.yrgb_addr > 0) {
        ...
        if (RGA2_CHECK_MODE) {
            ...
            if (vaddr) {
                rga2_memory_check(vaddr, req->src.vir_w, req->src.vir_h, req->src.format, req->src.yrgb_addr);
            }
            ion_unmap_kernel(rga2_drvdata->ion_client, hdl);
        }
#endif
        if (req->mmu_info.src0_mmu_flag) {
            buffer_src0.sgt = ion_sg_table(rga2_drvdata->ion_client, hdl);
            req->src.yrgb_addr = req->src.uv_addr;
            req->src.uv_addr = req->src.yrgb_addr + (src_vir_w * req->src.vir_h);
            req->src.v_addr = req->src.uv_addr + (src_vir_w * req->src.vir_h) / 0x4;
        } else {
    ...
    if ((int)req->dst.yrgb_addr > 0) {
        ...
        if (RGA2_CHECK_MODE) {
            vaddr = ion_map_kernel(rga2_drvdata->ion_client, hdl);
            if (vaddr) {
                rga2_memory_check(vaddr, req->dst.vir_w, req->dst.vir_h, req->dst.format, req->dst.yrgb_addr);
            }
            ion_unmap_kernel(rga2_drvdata->ion_client, hdl);
        }
#endif
        if (req->mmu_info.dst_mmu_flag) {
            buffer_dst.sgt = ion_sg_table(rga2_drvdata->ion_client, hdl);
            req->dst.yrgb_addr = req->dst.uv_addr;
            req->dst.uv_addr = req->dst.yrgb_addr + (dst_vir_w * req->dst.vir_h);
            req->dst.v_addr = req->dst.uv_addr + (dst_vir_w * req->dst.vir_h) / 0x4;
        } else {
            ion_phys(rga2_drvdata->ion_client, hdl, &phy_addr, &len);
    ...
    if ((int)req->src1.yrgb_addr > 0) {
        ...
        if (req->mmu_info.dst_mmu_flag) {
            buffer_src1.sgt = ion_sg_table(rga2_drvdata->ion_client, hdl);
            req->src1.yrgb_addr = req->src1.uv_addr;
            req->src1.uv_addr = req->src1.yrgb_addr + (req->src1.vir_w * req->src1.vir_h);
            req->src1.v_addr = req->src1.uv_addr + (req->src1.vir_w * req->src1.vir_h) / 0x4;
        } else {
            ion_phys(rga2_drvdata->ion_client, hdl, &phy_addr, &len);
    ...
    if (rga2_is_yuv422p_format(req->src.format)) {
        req->src.v_addr = req->src.uv_addr + (req->src.vir_w * req->src.vir_h) / 0x2;
    }
    if (rga2_is_yuv422p_format(req->dst.format)) {
        req->dst.v_addr = req->dst.uv_addr + (req->dst.vir_w * req->dst.vir_h) / 0x2;
    }
    if (rga2_is_yuv422p_format(req->src1.format)) {
        req->src1.v_addr = req->src1.uv_addr + (req->src1.vir_w * req->dst.vir_h) / 0x2;
    }
...
static int rga2_buf_size_cal(unsigned long yrgb_addr, unsigned long uv_addr, unsigned long v_addr, int format,
                             uint32_t w, uint32_t h, unsigned long *StartAddr)
{
...
static int rga2_MapUserMemory(struct page **pages, uint32_t *pageTable, unsigned long Memory, uint32_t pageCount,
                              int writeFlag, int map)
{
    ...
    result =
        get_user_pages(current, current->mm, Memory << PAGE_SHIFT, pageCount, writeFlag ? FOLL_WRITE : 0, pages, NULL);
#elif LINUX_VERSION_CODE < KERNEL_VERSION(4, 6, 0)
    result = get_user_pages(current, current->mm, Memory << PAGE_SHIFT, pageCount, writeFlag, 0, pages, NULL);
#elif LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0)
    result = get_user_pages_remote(current, current->mm, Memory << PAGE_SHIFT, pageCount, writeFlag, pages, NULL, NULL);
#else
...
static int rga2_MapION(struct sg_table *sg, uint32_t *Memory, int32_t pageCount)
{
    uint32_t i;
    uint32_t status;
    unsigned long Address;
...
static int rga2_mmu_flush_cache(struct rga2_reg *reg, struct rga2_req *req)
{
    ...
    if (req->mmu_info.dst_mmu_flag & 1) {
        DstPageCount = rga2_buf_size_cal(req->dst.yrgb_addr, req->dst.uv_addr, req->dst.v_addr, req->dst.format,
                                         req->dst.vir_w, req->dst.vir_h, &DstStart);
        if (DstPageCount == 0) {
            return -EINVAL;
        }
    }
    /* Cal out the needed mem size */
    DstMemSize = (DstPageCount + 0xf) & (~0xf);
...
static int rga2_mmu_info_BitBlt_mode(struct rga2_reg *reg, struct rga2_req *req)
{
    ...
    if (req->mmu_info.src0_mmu_flag & 1) {
        Src0PageCount = rga2_buf_size_cal(req->src.yrgb_addr, req->src.uv_addr, req->src.v_addr, req->src.format,
                                          req->src.vir_w, (req->src.vir_h), &Src0Start);
        if (Src0PageCount == 0) {
            return -EINVAL;
        }
    }
    ...
    if (req->mmu_info.src1_mmu_flag & 1) {
        Src1PageCount = rga2_buf_size_cal(req->src1.yrgb_addr, req->src1.uv_addr, req->src1.v_addr, req->src1.format,
                                          req->src1.vir_w, (req->src1.vir_h), &Src1Start);
        if (Src1PageCount == 0) {
            return -EINVAL;
        }
    }
    ...
    /* Cal out the needed mem size */
    Src0MemSize = (Src0PageCount + 0xf) & (~0xf);
    Src1MemSize = (Src1PageCount + 0xf) & (~0xf);
    DstMemSize = (DstPageCount + 0xf) & (~0xf);
    ...
    if (Src0MemSize) {
        ...
        if (dma_buffer->sgt) {
            ret = rga2_MapION(dma_buffer->sgt, &MMU_Base[0], Src0MemSize);
        } else {
            ret = rga2_MapUserMemory(&pages[0], &MMU_Base[0], Src0Start, Src0PageCount, 0, MMU_MAP_CLEAN);
#ifdef CONFIG_ROCKCHIP_RGA2_DEBUGGER
            if (RGA2_CHECK_MODE) {
                rga2_user_memory_check(&pages[0], req->src.vir_w, req->src.vir_h, req->src.format, 1);
            }
#endif
    ...
    if (Src1MemSize) {
        ...
        if (dma_buffer->sgt) {
            ret = rga2_MapION(dma_buffer->sgt, MMU_Base + Src0MemSize, Src1MemSize);
        } else {
            ret = rga2_MapUserMemory(&pages[0], MMU_Base + Src0MemSize, Src1Start, Src1PageCount, 0, MMU_MAP_CLEAN);
    ...
    if (DstMemSize) {
        ...
        if (dma_buffer->sgt) {
            ret = rga2_MapION(dma_buffer->sgt, MMU_Base + Src0MemSize + Src1MemSize, DstMemSize);
        } else if (req->alpha_mode_0 != 0 && req->bitblt_mode == 0) {
            /* The blend mode of src + dst => dst requires clean and invalidate */
            ret = rga2_MapUserMemory(&pages[0], MMU_Base + Src0MemSize + Src1MemSize, DstStart, DstPageCount, 1,
...
static int rga2_mmu_info_color_palette_mode(struct rga2_reg *reg, struct rga2_req *req)
{
    ...
    shift = 0x3 - (req->palette_mode & 0x3);
    sw = req->src.vir_w * req->src.vir_h;
    byte_num = sw >> shift;
    stride = (byte_num + 0x3) & (~0x3);
    ...
    do {
        ...
        SrcMemSize = (SrcPageCount + 0xf) & (~0xf);
        DstMemSize = (DstPageCount + 0xf) & (~0xf);
        ...
        if (SrcMemSize) {
            ...
            if (dma_buffer->sgt) {
                ret = rga2_MapION(dma_buffer->sgt, &MMU_Base[0], SrcMemSize);
            } else {
                ret = rga2_MapUserMemory(&pages[0], &MMU_Base[0], SrcStart, SrcPageCount, 0, MMU_MAP_CLEAN);
#ifdef CONFIG_ROCKCHIP_RGA2_DEBUGGER
                if (RGA2_CHECK_MODE) {
                    rga2_user_memory_check(&pages[0], req->src.vir_w, req->src.vir_h, req->src.format, 1);
                }
        ...
        if (DstMemSize) {
            ...
            if (dma_buffer->sgt) {
                ret = rga2_MapION(dma_buffer->sgt, MMU_Base + SrcMemSize, DstMemSize);
            } else {
                ret = rga2_MapUserMemory(&pages[0], MMU_Base + SrcMemSize, DstStart, DstPageCount, 1, MMU_MAP_INVALID);
#ifdef CONFIG_ROCKCHIP_RGA2_DEBUGGER
                if (RGA2_CHECK_MODE) {
                    rga2_user_memory_check(&pages[0], req->dst.vir_w, req->dst.vir_h, req->dst.format, 1);
                }
#endif
            }
            if (ret < 0) {
    ...
    do {
        ...
        if (DstMemSize) {
            ...
            if (dma_buffer->sgt) {
                ...
                ret = rga2_MapUserMemory(&pages[0], &MMU_Base[0], DstStart, DstPageCount, 1, MMU_MAP_INVALID);
            }
            if (ret < 0) {
                pr_err("rga2 map dst memory failed\n");
                status = ret;
...
static int rga2_mmu_info_update_palette_table_mode(struct rga2_reg *reg, struct rga2_req *req)
{
    ...
    do {
        ...
        if (req->mmu_info.els_mmu_flag & 1) {
            ...
            LutPageCount = rga2_buf_size_cal(req->pat.yrgb_addr, req->pat.uv_addr, req->pat.v_addr, req->pat.format,
                                             req->pat.vir_w, req->pat.vir_h, &LutStart);
            if (LutPageCount == 0) {
                return -EINVAL;
            }
        }

        LutMemSize = (LutPageCount + 0xf) & (~0xf);
        AllSize = LutMemSize;
        ...
        if (LutMemSize) {
            ...
            if (dma_buffer->sgt) {
                ret = rga2_MapION(dma_buffer->sgt, &MMU_Base[0], LutMemSize);
            } else {
                ret = rga2_MapUserMemory(&pages[0], &MMU_Base[0], LutStart, LutPageCount, 0, MMU_MAP_CLEAN);
            }
            if (ret < 0) {
            ...
            uv_size = (req->pat.uv_addr - (LutStart << PAGE_SHIFT)) >> PAGE_SHIFT;
            v_size = (req->pat.v_addr - (LutStart << PAGE_SHIFT)) >> PAGE_SHIFT;
            req->pat.uv_addr = (req->pat.uv_addr & (~PAGE_MASK)) | ((uv_size) << PAGE_SHIFT);
            req->pat.v_addr = (req->pat.v_addr & (~PAGE_MASK)) | ((v_size) << PAGE_SHIFT);
        }
