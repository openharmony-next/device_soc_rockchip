 ...
 */
static const u16 trans_tbl_h264d[] = {128, 129, 130, 131, 132, 133, 134, 135, 136, 137, 138, 139, 140,
                                      141, 142, 161, 163, 164, 165, 166, 167, 168, 169, 170, 171, 172,
                                      173, 174, 175, 176, 177, 178, 179, 180, 181, 182, 183, 184, 185,
                                      186, 187, 188, 189, 190, 191, 192, 193, 194, 195, 196, 197};

static const u16 trans_tbl_h265d[] = {128, 129, 130, 131, 132, 133, 134, 135, 136, 137, 138, 139, 140,
                                      141, 142, 161, 163, 164, 165, 166, 167, 168, 169, 170, 171, 172,
                                      173, 174, 175, 176, 177, 178, 179, 180, 181, 182, 183, 184, 185,
                                      186, 187, 188, 189, 190, 191, 192, 193, 194, 195, 196, 197};

static const u16 trans_tbl_vp9d[] = {128, 129, 130, 131, 132, 133, 134, 135, 136, 137, 138, 139, 140, 141, 142,
...
static int mpp_extract_rcb_info(struct rkvdec2_rcb_info *rcb_inf, struct mpp_request *req)
{
...
static int rkvdec2_extract_task_msg(struct mpp_session *session, struct rkvdec2_task *task, struct mpp_task_msgs *msgs)
{
    u32 i;
    int ret;
    ...
    if (priv && dec->rcb_iova) {
        ...
        for (i = 0; i < rcb_inf->cnt; i++) {
            ...
            if ((rcb_offset + rcb_size) > dec->rcb_size) {
                mpp_debug(DEBUG_SRAM_INFO, "rcb: reg %d use original buffer\n", reg_idx);
                continue;
            }
                      ...
                      struct mpp_task_msgs *msgs)
{
    ...
    if (!(msgs->flags & MPP_FLAGS_REG_FD_NO_TRANS)) {
        ...
        ret = mpp_translate_reg_address(session, mpp_task, fmt, task->reg, &task->off_inf);
        if (ret) {
            goto fail;
        }
...
static void *rkvdec2_rk3568_alloc_task(struct mpp_session *session, struct mpp_task_msgs *msgs)
{
    u32 fmt;
...
static int rkvdec2_isr(struct mpp_dev *mpp)
{
    ...
    err_mask = RKVDEC_COLMV_REF_ERR_STA | RKVDEC_BUF_EMPTY_STA | RKVDEC_TIMEOUT_STA | RKVDEC_ERROR_STA;
...
int rkvdec2_result(struct mpp_dev *mpp, struct mpp_task *mpp_task, struct mpp_task_msgs *msgs)
{
    ...
    for (i = 0; i < task->r_req_cnt; i++) {
        ...
        if (req->offset >= RKVDEC_PERF_SEL_OFFSET) {
            ...
            if (copy_to_user(req->data, (u8 *)task->reg_sel + off, req->size)) {
    ...
    switch (req->cmd) {
        case MPP_CMD_SEND_CODEC_INFO: {
            ...
            for (i = 0; i < cnt; i++) {
                ...
                if (elem.type > DEC_INFO_BASE && elem.type < DEC_INFO_BUTT && elem.flag > CODEC_INFO_FLAG_NULL &&
...
static int rkvdec2_procfs_init(struct mpp_dev *mpp)
{
    ...
    mpp_procfs_create_u32("aclk", MPP_PROCFS_FILE, dec->procfs, &dec->aclk_info.debug_rate_hz);
    mpp_procfs_create_u32("clk_core", MPP_PROCFS_FILE, dec->procfs, &dec->core_clk_info.debug_rate_hz);
...
static int rkvdec2_init(struct mpp_dev *mpp)
{
    ...
    mpp_set_clk_info_rate_hz(&dec->core_clk_info, CLK_MODE_DEFAULT, 0xc8 * MHZ);
    mpp_set_clk_info_rate_hz(&dec->cabac_clk_info, CLK_MODE_DEFAULT, 0xc8 * MHZ);
    mpp_set_clk_info_rate_hz(&dec->hevc_cabac_clk_info, CLK_MODE_DEFAULT, 0x12c * MHZ);
...
static int rkvdec2_get_freq(struct mpp_dev *mpp, struct mpp_task *mpp_task)
{
    u32 task_cnt;
    u32 workload;
...
static int rkvdec2_set_freq(struct mpp_dev *mpp, struct mpp_task *mpp_task)
{
    struct rkvdec2_dev *dec = to_rkvdec2_dev(mpp);
    struct rkvdec2_task *task = to_rkvdec2_task(mpp_task);
...
static int rkvdec2_core_probe(struct platform_device *pdev)
{
    ...
    /* get irq request */
    ret = devm_request_threaded_irq(dev, mpp->irq, rkvdec2_soft_ccu_irq, NULL, IRQF_SHARED, dev_name(dev), mpp);
    if (ret) {
...
static int rkvdec2_probe_default(struct platform_device *pdev)
{
    ...
    if (dec->link_dec) {
        ret = devm_request_threaded_irq(dev, mpp->irq, rkvdec2_link_irq_proc, NULL, IRQF_SHARED, dev_name(dev), mpp);
