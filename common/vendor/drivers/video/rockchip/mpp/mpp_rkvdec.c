...
static unsigned long model_static_power(struct devfreq *devfreq, unsigned long voltage)
{
    ...
    const unsigned long voltage_cubed = (voltage * voltage * voltage) >> DEVIATION_TEN;
...
static int rkvdec_process_scl_fd(struct mpp_session *session, struct rkvdec_task *task, struct mpp_task_msgs *msgs)
{
    ...
    if (session->msg_flags & MPP_FLAGS_REG_NO_OFFSET) {
        ...
        pps_offset = task->reg[idx] >> 10; /* Reduce RKVDEC_V2_REG_NUM 10 */
    }
...
static int rkvdec_init(struct mpp_dev *mpp)
{
    ...
    mpp_set_clk_info_rate_hz(&dec->cabac_clk_info, CLK_MODE_DEFAULT, CLK_MODE_TW * MHZ);
                                 ...
                                 void *arg)
{
     ...
     */
    if (IOMMU_GET_BUS_ID(status) == STATE_BUS) {
