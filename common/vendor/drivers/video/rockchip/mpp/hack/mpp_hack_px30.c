...
static int mpp_iommu_enable(struct mpp_rk_iommu *iommu)
{
    ...
    for (i = 0; i < iommu->mmu_num; i++) {
        ...
        writel(iommu->dte_addr, iommu->bases[i] + RK_MMU_DTE_ADDR);
        /* zap cache */
        writel(RK_MMU_CMD_ZAP_CACHE, iommu->bases[i] + RK_MMU_COMMAND);
        /* irq mask */
        writel(RK_MMU_IRQ_MASK, iommu->bases[i] + RK_MMU_INT_MASK);
    }
    udelay(0x2);
    /* enable paging */
    for (i = 0; i < iommu->mmu_num; i++) {
        writel(RK_MMU_CMD_ENABLE_PAGING, iommu->bases[i] + RK_MMU_COMMAND);
    }
    udelay(0x2);
    /* disable stall */
    for (i = 0; i < iommu->mmu_num; i++) {
