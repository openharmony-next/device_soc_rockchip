...
{
    struct kbase_debug_copy_buffer *buffers = (struct kbase_debug_copy_buffer *)(uintptr_t)katom->jc;
    unsigned int i;
    ...
    for (i = 0; i < nr; i++) {
        ...
        switch (reg->gpu_alloc->type) {
            ...
            case KBASE_MEM_TYPE_IMPORTED_UMP: {
                dev_warn(katom->kctx->kbdev->dev, "UMP is not supported for debug_copy jobs\n");
                ret = -EINVAL;
                goto out_unlock;
            }
            default:
...
static void kbase_mem_copy_from_extres_page(struct kbase_context *kctx, void *extres_page, struct page **pages,
...
static int kbase_mem_copy_from_extres(struct kbase_context *kctx, struct kbase_debug_copy_buffer *buf_data)
{
    unsigned int i;
    ...
    switch (gpu_alloc->type) {
        case KBASE_MEM_TYPE_IMPORTED_USER_BUF: {
            for (i = 0; i < buf_data->nr_extres_pages; i++) {
                ...
                if (extres_page) {
                    kbase_mem_copy_from_extres_page(kctx, extres_page, pages, buf_data->nr_pages, &target_page_nr,
                                                    offset, &to_copy);
                }
        ...
        case KBASE_MEM_TYPE_IMPORTED_UMM: {
            ...
            KBASE_DEBUG_ASSERT(dma_buf->size == buf_data->nr_extres_pages * PAGE_SIZE);
...
static int kbase_jit_allocate_process(struct kbase_jd_atom *katom)
{
    ...
    if (!reg) {
        ...
        jit_atom = list_first_entry(&kctx->jit_atoms_head, struct kbase_jd_atom, jit_node);
    ...
    *ptr = new_addr;
    KBASE_TLSTREAM_TL_ATTRIB_ATOM_JIT(katom, info->gpu_alloc_addr, new_addr);
...
static void kbase_jit_free_finish(struct kbase_jd_atom *katom)
{
    ...
    list_for_each_safe(i, tmp, &kctx->jit_pending_alloc)
    {
        ...
        if (kbase_jit_allocate_process(pending_atom) == 0) {
            /* Atom has completed */
            INIT_WORK(&pending_atom->work, kbasep_jit_free_finish_worker);
...
static void kbase_ext_res_process(struct kbase_jd_atom *katom, bool map)
{
    ...
    for (i = 0; i < ext_res->count; i++) {
        ...
        } else if (!kbase_sticky_resource_release(katom->kctx, NULL, gpu_addr)) {
            failed = true;
        }
    }
...
void kbase_resume_suspended_soft_jobs(struct kbase_device *kbdev)
{
    ...
    list_for_each_entry_safe(katom_iter, tmp_iter, &local_suspended_soft_jobs, dep_item[1])
    {
        ...
        if (kbase_process_soft_job(katom_iter) == 0) {
            ...
            KBASE_DEBUG_ASSERT((katom_iter->core_req & BASE_JD_REQ_SOFT_JOB_TYPE) != BASE_JD_REQ_SOFT_REPLAY);
        }
