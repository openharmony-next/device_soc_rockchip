...
static bool is_action_of_powering_off_l2(enum kbase_pm_core_type core_type, enum kbasep_pm_action active)
{
    return (KBASE_PM_CORE_L2 == core_type) && (ACTION_PWROFF == active);
}

static bool is_action_of_powering_off_shader(enum kbase_pm_core_type core_type, enum kbasep_pm_action active)
{
    return (KBASE_PM_CORE_SHADER == core_type) && (ACTION_PWROFF == active);
}

static bool is_action_of_powering_off_tiler(enum kbase_pm_core_type core_type, enum kbasep_pm_action active)
{
...
static void mali_cci_flush_l2(struct kbase_device *kbdev)
{
    ...
    raw = kbase_reg_read(kbdev, GPU_CONTROL_REG(GPU_IRQ_RAWSTAT), NULL);
 ...
 */
static void kbase_pm_invoke(struct kbase_device *kbdev, enum kbase_pm_core_type core_type, u64 cores,
                            enum kbasep_pm_action action)
{
    u32 reg;
    ...
    if (is_action_of_powering_off_shader(core_type, action)) {
        D("not to power off shader actually. cores_lo : 0x%x, hi : 0x%x.", lo, hi);
        return;
    }
 ...
 */
static bool kbase_pm_transition_core_type(struct kbase_device *kbdev, enum kbase_pm_core_type type, u64 desired_state,
 ...
 */
static u64 get_desired_cache_status(u64 present, u64 cores_powered, u64 tilers_powered)
{
    u64 desired = 0;
...
u64 kbase_pm_core_stack_mask(u64 cores)
{
    ...
    for (i = 0; i <= MAX_CORE_ID; ++i) {
        if (test_bit(i, (unsigned long *)&cores)) {
             ...
             * instead of 0-3 */
            size_t const stack_num = (i > 16) ? (i % NUM_CORES_PER_STACK) + 4 : (i % NUM_CORES_PER_STACK);
...
bool MOCKABLE(kbase_pm_check_transitions_nolock)(struct kbase_device *kbdev)
{
    ...
    if (kbdev->pm.backend.gpu_powered == false) {
        ...
        if (kbdev->pm.backend.desired_shader_state == 0 && kbdev->pm.backend.desired_tiler_state == 0) {
            return true;
        }
        return false;
    }
    ...
    stacks_powered = kbase_pm_get_ready_cores(kbdev, KBASE_PM_CORE_STACK) | desired_stack_state;
#endif /* CONFIG_MALI_CORESTACK */
    ...
    desired_l2_state =
        get_desired_cache_status(kbdev->gpu_props.props.raw_props.l2_present, cores_powered, tilers_powered);

    l2_inuse_bitmap = get_desired_cache_status(kbdev->gpu_props.props.raw_props.l2_present,
    ...
    in_desired_state &= kbase_pm_transition_core_type(kbdev, KBASE_PM_CORE_L2, desired_l2_state, l2_inuse_bitmap,
    ...
    if (in_desired_state) {
        in_desired_state &=
            kbase_pm_transition_core_type(kbdev, KBASE_PM_CORE_STACK, desired_stack_state, 0,
                                          &kbdev->stack_available_bitmap, &kbdev->pm.backend.powering_on_stack_state);
    }
#endif /* CONFIG_MALI_CORESTACK */

    if (in_desired_state) {
        in_desired_state &=
            kbase_pm_transition_core_type(kbdev, KBASE_PM_CORE_TILER, kbdev->pm.backend.desired_tiler_state, 0,
                                          &tiler_available_bitmap, &kbdev->pm.backend.powering_on_tiler_state);
        in_desired_state &= kbase_pm_transition_core_type(
            kbdev, KBASE_PM_CORE_SHADER, kbdev->pm.backend.desired_shader_state, kbdev->shader_inuse_bitmap,
    ...
    if ((kbdev->shader_available_bitmap & kbdev->pm.backend.desired_shader_state) ==
        ...
        KBASE_TRACE_ADD(kbdev, PM_CORES_AVAILABLE, NULL, NULL, 0u,
                        (u32)(kbdev->shader_available_bitmap & kbdev->pm.backend.desired_shader_state));
        KBASE_TRACE_ADD(kbdev, PM_CORES_AVAILABLE_TILER, NULL, NULL, 0u,
                        (u32)(kbdev->tiler_available_bitmap & kbdev->pm.backend.desired_tiler_state));
...
#ifdef CONFIG_MALI_CORESTACK
        kbase_trace_mali_pm_status(KBASE_PM_CORE_STACK, kbase_pm_get_ready_cores(kbdev, KBASE_PM_CORE_STACK));
#endif /* CONFIG_MALI_CORESTACK */
#endif

        KBASE_TLSTREAM_AUX_PM_STATE(KBASE_PM_CORE_L2, kbase_pm_get_ready_cores(kbdev, KBASE_PM_CORE_L2));
        KBASE_TLSTREAM_AUX_PM_STATE(KBASE_PM_CORE_SHADER, kbase_pm_get_ready_cores(kbdev, KBASE_PM_CORE_SHADER));
        KBASE_TLSTREAM_AUX_PM_STATE(KBASE_PM_CORE_TILER, kbase_pm_get_ready_cores(kbdev, KBASE_PM_CORE_TILER));
#ifdef CONFIG_MALI_CORESTACK
        KBASE_TLSTREAM_AUX_PM_STATE(KBASE_PM_CORE_STACK, kbase_pm_get_ready_cores(kbdev, KBASE_PM_CORE_STACK));
#endif /* CONFIG_MALI_CORESTACK */

        KBASE_TRACE_ADD(kbdev, PM_DESIRED_REACHED, NULL, NULL, kbdev->pm.backend.gpu_in_desired_state,
                        (u32)kbdev->pm.backend.desired_shader_state);
        KBASE_TRACE_ADD(kbdev, PM_DESIRED_REACHED_TILER, NULL, NULL, 0u, (u32)kbdev->pm.backend.desired_tiler_state);
...
void kbase_pm_check_transitions_sync(struct kbase_device *kbdev)
{
    ...
    if (ret < 0 && time_after(jiffies, timeout)) {
        ...
        dev_err(kbdev->dev, "\tShader=%08x%08x\n", kbase_reg_read(kbdev, GPU_CONTROL_REG(SHADER_READY_HI), NULL),
                kbase_reg_read(kbdev, GPU_CONTROL_REG(SHADER_READY_LO), NULL));
        dev_err(kbdev->dev, "\tTiler =%08x%08x\n", kbase_reg_read(kbdev, GPU_CONTROL_REG(TILER_READY_HI), NULL),
                kbase_reg_read(kbdev, GPU_CONTROL_REG(TILER_READY_LO), NULL));
        dev_err(kbdev->dev, "\tL2    =%08x%08x\n", kbase_reg_read(kbdev, GPU_CONTROL_REG(L2_READY_HI), NULL),
                kbase_reg_read(kbdev, GPU_CONTROL_REG(L2_READY_LO), NULL));
        dev_err(kbdev->dev, "Cores transitioning :\n");
        dev_err(kbdev->dev, "\tShader=%08x%08x\n", kbase_reg_read(kbdev, GPU_CONTROL_REG(SHADER_PWRTRANS_HI), NULL),
                kbase_reg_read(kbdev, GPU_CONTROL_REG(SHADER_PWRTRANS_LO), NULL));
        dev_err(kbdev->dev, "\tTiler =%08x%08x\n", kbase_reg_read(kbdev, GPU_CONTROL_REG(TILER_PWRTRANS_HI), NULL),
                kbase_reg_read(kbdev, GPU_CONTROL_REG(TILER_PWRTRANS_LO), NULL));
        dev_err(kbdev->dev, "\tL2    =%08x%08x\n", kbase_reg_read(kbdev, GPU_CONTROL_REG(L2_PWRTRANS_HI), NULL),
                kbase_reg_read(kbdev, GPU_CONTROL_REG(L2_PWRTRANS_LO), NULL));
...
static void kbase_pm_hw_issues_detect(struct kbase_device *kbdev)
{
    ...
    const u32 prod_id = (gpu_id & GPU_ID_VERSION_PRODUCT_ID) >> GPU_ID_VERSION_PRODUCT_ID_SHIFT;
    const u32 major = (gpu_id & GPU_ID_VERSION_MAJOR) >> GPU_ID_VERSION_MAJOR_SHIFT;
    ...
    if (kbase_hw_has_issue(kbdev, BASE_HW_ISSUE_8443) || kbase_hw_has_issue(kbdev, BASE_HW_ISSUE_11035)) {
    ...
    if (!kbdev->hw_quirks_sc) {
        kbdev->hw_quirks_sc = kbase_reg_read(kbdev, GPU_CONTROL_REG(SHADER_CONFIG), NULL);
    }

    kbdev->hw_quirks_tiler = kbase_reg_read(kbdev, GPU_CONTROL_REG(TILER_CONFIG), NULL);
    ...
    kbdev->hw_quirks_mmu = kbase_reg_read(kbdev, GPU_CONTROL_REG(L2_MMU_CONFIG), NULL);

    /* Limit read ID width for AXI */
    kbdev->hw_quirks_mmu &= ~(L2_MMU_CONFIG_LIMIT_EXTERNAL_READS);
    kbdev->hw_quirks_mmu |= (DEFAULT_ARID_LIMIT & 0x3) << L2_MMU_CONFIG_LIMIT_EXTERNAL_READS_SHIFT;
    ...
    if (!kbdev->hw_quirks_jm) {
        kbdev->hw_quirks_jm = kbase_reg_read(kbdev, GPU_CONTROL_REG(JM_CONFIG), NULL);
    }
...
{
    kbase_reg_write(kbdev, GPU_CONTROL_REG(SHADER_CONFIG), kbdev->hw_quirks_sc, NULL);

    kbase_reg_write(kbdev, GPU_CONTROL_REG(TILER_CONFIG), kbdev->hw_quirks_tiler, NULL);

    kbase_reg_write(kbdev, GPU_CONTROL_REG(L2_MMU_CONFIG), kbdev->hw_quirks_mmu, NULL);

    kbase_reg_write(kbdev, GPU_CONTROL_REG(JM_CONFIG), kbdev->hw_quirks_jm, NULL);
}
...
static int kbasep_protected_mode_enable(struct protected_mode_device *pdev)
{
    ...
    kbase_reg_write(kbdev, GPU_CONTROL_REG(GPU_COMMAND), GPU_COMMAND_SET_PROTECTED_MODE, NULL);
    return 0;
}
...
struct protected_mode_ops kbase_native_protected_ops = {.protected_mode_enable = kbasep_protected_mode_enable,
...
int kbase_pm_init_hw(struct kbase_device *kbdev, unsigned int flags)
{
    ...
    if (kbase_hw_has_feature(kbdev, BASE_HW_FEATURE_PROTECTED_MODE)) {
        u32 gpu_status = kbase_reg_read(kbdev, GPU_CONTROL_REG(GPU_STATUS), NULL);
