...
/* MMU_AS_REG(n,r) */
static int as_reg_snapshot[] = {AS_TRANSTAB_LO, AS_TRANSTAB_HI,     AS_MEMATTR_LO,      AS_MEMATTR_HI,
                                AS_FAULTSTATUS, AS_FAULTADDRESS_LO, AS_FAULTADDRESS_HI, AS_STATUS};
...
bool kbase_job_fault_get_reg_snapshot(struct kbase_context *kctx)
{
    ...
    while (kctx->reg_dump[offset] != REGISTER_DUMP_TERMINATION_FLAG) {
        kctx->reg_dump[offset + 1] = kbase_reg_read(kctx->kbdev, kctx->reg_dump[offset], NULL);
        offset += 0x2;
    }
    return true;
}
