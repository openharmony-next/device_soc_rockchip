...
static inline void kbase_timeline_pm_cores_func(struct kbase_device *kbdev, enum kbase_pm_func_id func_id,
...
static void kbasep_pm_do_poweroff_cores(struct kbase_device *kbdev)
{
    ...
    kbdev->pm.backend.desired_shader_state &= ~kbdev->pm.backend.shader_poweroff_pending;
    kbdev->pm.backend.desired_tiler_state &= ~kbdev->pm.backend.tiler_poweroff_pending;
    ...
    if (prev_shader_state != kbdev->pm.backend.desired_shader_state ||
        prev_tiler_state != kbdev->pm.backend.desired_tiler_state || kbdev->pm.backend.ca_in_transition) {
...
static enum hrtimer_restart kbasep_pm_do_gpu_poweroff_callback(struct hrtimer *timer)
{
    ...
    kbdev = container_of(timer, struct kbase_device, pm.backend.gpu_poweroff_timer);
    ...
    if (kbdev->pm.backend.gpu_poweroff_pending) {
        queue_work(kbdev->pm.backend.gpu_poweroff_wq, &kbdev->pm.backend.gpu_poweroff_work);
    }
    ...
    if (kbdev->pm.backend.shader_poweroff_pending || kbdev->pm.backend.tiler_poweroff_pending) {
        ...
        KBASE_DEBUG_ASSERT(kbdev->pm.backend.shader_poweroff_pending_time >= 0);
...
static void kbasep_pm_do_gpu_poweroff_wq(struct work_struct *data)
{
    ...
    kbdev = container_of(data, struct kbase_device, pm.backend.gpu_poweroff_work);
...
int kbase_pm_policy_init(struct kbase_device *kbdev)
{
    ...
    wq = alloc_workqueue("kbase_pm_do_poweroff", WQ_HIGHPRI | WQ_UNBOUND, 1);
    if (!wq) {
        return -ENOMEM;
    }

    kbdev->pm.backend.gpu_poweroff_wq = wq;
    INIT_WORK(&kbdev->pm.backend.gpu_poweroff_work, kbasep_pm_do_gpu_poweroff_wq);
    hrtimer_init(&kbdev->pm.backend.gpu_poweroff_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
    kbdev->pm.backend.gpu_poweroff_timer.function = kbasep_pm_do_gpu_poweroff_callback;
    kbdev->pm.backend.pm_current_policy = policy_list[0];
    kbdev->pm.backend.pm_current_policy->init(kbdev);
    kbdev->pm.gpu_poweroff_time = HR_TIMER_DELAY_NSEC(DEFAULT_PM_GPU_POWEROFF_TICK_NS);
...
void kbase_pm_update_active(struct kbase_device *kbdev)
{
    ...
    if (active) {
        ...
        if (!backend->poweroff_timer_running && !backend->gpu_powered &&
            (pm->poweroff_gpu_ticks || pm->poweroff_shader_ticks)) {
            backend->poweroff_timer_needed = true;
            backend->poweroff_timer_running = true;
            hrtimer_start(&backend->gpu_poweroff_timer, pm->gpu_poweroff_time, HRTIMER_MODE_REL);
        }
        ...
        if (backend->shader_poweroff_pending || backend->tiler_poweroff_pending) {
        ...
        if (pm->backend.gpu_powered) {
            if (pm->poweroff_gpu_ticks) {
                backend->gpu_poweroff_pending = pm->poweroff_gpu_ticks;
                backend->poweroff_timer_needed = true;
...
void kbase_pm_update_cores_state_nolock(struct kbase_device *kbdev)
{
    ...
    if (kbdev->protected_mode_transition && !kbdev->shader_needed_bitmap && !kbdev->shader_inuse_bitmap &&
        ...
        desired_bitmap = kbdev->pm.backend.pm_current_policy->get_core_mask(kbdev);
    ...
    if (~kbdev->pm.backend.desired_shader_state & desired_bitmap ||
        ~kbdev->pm.backend.desired_tiler_state & desired_tiler_bitmap || kbdev->pm.backend.ca_in_transition) {
        ...
        if (kbdev->pm.backend.desired_shader_state & ~desired_bitmap ||
            ...
            kbdev->pm.backend.shader_poweroff_pending |= (kbdev->pm.backend.desired_shader_state & ~desired_bitmap);
            kbdev->pm.backend.tiler_poweroff_pending |= (kbdev->pm.backend.desired_tiler_state & ~desired_tiler_bitmap);

            if (kbdev->pm.poweroff_shader_ticks && !kbdev->protected_mode_transition) {
                kbdev->pm.backend.shader_poweroff_pending_time = kbdev->pm.poweroff_shader_ticks;
            } else {
                do_poweroff = true;
            }
        }
    ...
    } else if (kbdev->pm.active_count == 0 && desired_bitmap != 0 && desired_tiler_bitmap != 0 &&
...
void kbase_pm_request_cores(struct kbase_device *kbdev, bool tiler_required, u64 shader_cores)
{
    u64 cores;
...
void kbase_pm_unrequest_cores(struct kbase_device *kbdev, bool tiler_required, u64 shader_cores)
{
    kbase_pm_change_state change_gpu_state = 0u;
...
enum kbase_pm_cores_ready kbase_pm_register_inuse_cores(struct kbase_device *kbdev, bool tiler_required,
                                                        u64 shader_cores)
{
     ...
     * be chosen */
    if ((kbdev->pm.backend.desired_shader_state & shader_cores) != shader_cores) {
        return (kbdev->pm.backend.poweroff_wait_in_progress || kbdev->pm.backend.pm_current_policy == NULL)
                   ? KBASE_CORES_NOT_READY
                   : KBASE_NEW_AFFINITY;
    }
...
void kbase_pm_release_cores(struct kbase_device *kbdev, bool tiler_required, u64 shader_cores)
{
    kbase_pm_change_state change_gpu_state = 0u;
...
void kbase_pm_request_cores_sync(struct kbase_device *kbdev, bool tiler_required, u64 shader_cores)
{
    unsigned long flags;
...
void kbase_pm_request_l2_caches(struct kbase_device *kbdev)
{
    ...
    spin_unlock_irqrestore(&kbdev->hwaccess_lock, flags);
    wait_event(kbdev->pm.backend.l2_powered_wait, kbdev->pm.backend.l2_powered == 1);
