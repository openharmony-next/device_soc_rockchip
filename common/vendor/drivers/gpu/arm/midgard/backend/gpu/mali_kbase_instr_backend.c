                                      ...
                                      struct kbase_uk_hwcnt_setup *setup)
{
    ...
    /* alignment failure */
    if ((setup->dump_buffer == 0ULL) || (setup->dump_buffer & (0x800 - 1))) {
        goto out_err;
    }
    ...
    kbase_reg_write(kbdev, GPU_CONTROL_REG(PRFCNT_CONFIG), prfcnt_config | PRFCNT_CONFIG_MODE_OFF, kctx);

    kbase_reg_write(kbdev, GPU_CONTROL_REG(PRFCNT_BASE_LO), setup->dump_buffer & 0xFFFFFFFF, kctx);
    kbase_reg_write(kbdev, GPU_CONTROL_REG(PRFCNT_BASE_HI), setup->dump_buffer >> 0x20, kctx);
    ...
    kbase_reg_write(kbdev, GPU_CONTROL_REG(PRFCNT_CONFIG), prfcnt_config | PRFCNT_CONFIG_MODE_MANUAL, kctx);
...
int kbase_instr_hwcnt_disable_internal(struct kbase_context *kctx)
{
    ...
    kbase_pm_unrequest_cores(kbdev, true, kbase_pm_get_present_cores(kbdev, KBASE_PM_CORE_SHADER));
...
int kbase_instr_hwcnt_request_dump(struct kbase_context *kctx)
{
    ...
    kbase_reg_write(kbdev, GPU_CONTROL_REG(PRFCNT_BASE_HI), kbdev->hwcnt.addr >> 0x20, NULL);
...
void kbasep_cache_clean_worker(struct work_struct *data)
{
    ...
    while (kbdev->hwcnt.backend.state == KBASE_INSTR_STATE_CLEANING) {
        spin_unlock_irqrestore(&kbdev->hwcnt.lock, flags);
        wait_event(kbdev->hwcnt.backend.cache_clean_wait, kbdev->hwcnt.backend.state != KBASE_INSTR_STATE_CLEANING);
        spin_lock_irqsave(&kbdev->hwcnt.lock, flags);
    }
    KBASE_DEBUG_ASSERT(kbdev->hwcnt.backend.state == KBASE_INSTR_STATE_CLEANED);
