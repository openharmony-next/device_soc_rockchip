...
#define MALI_UTILIZATION_HALF_PERIOD 50000
...
static void kbase_pm_reset_dvfs_utilisation_unlocked(struct kbase_device *kbdev, ktime_t now)
{
    ...
    kbdev->pm.backend.metrics.prev_idle = kbdev->pm.backend.metrics.time_idle;
    kbdev->pm.backend.metrics.prev_busy = kbdev->pm.backend.metrics.time_busy;
...
void kbase_pm_get_dvfs_utilisation(struct kbase_device *kbdev, unsigned long *total_out, unsigned long *busy_out)
{
 ...
 * function
 */
int kbase_pm_get_dvfs_utilisation_old(struct kbase_device *kbdev, int *util_gl_share, int util_cl_share[2], ktime_t now)
{
    int utilisation;
    int busy;
    ...
    busy =
        kbdev->pm.backend.metrics.busy_gl + kbdev->pm.backend.metrics.busy_cl[0] + kbdev->pm.backend.metrics.busy_cl[1];
