...
static void kbase_gpu_release_atom(struct kbase_device *kbdev, struct kbase_jd_atom *katom, ktime_t *end_timestamp);
 ...
 */
static struct kbase_jd_atom *kbase_gpu_dequeue_atom(struct kbase_device *kbdev, int js, ktime_t *end_timestamp)
{
...
static bool kbase_gpu_atoms_submitted(struct kbase_device *kbdev, int js)
{
    ...
    for (i = 0; i < SLOT_RB_SIZE; i++) {
        ...
        if (katom->gpu_rb_state == KBASE_ATOM_GPU_RB_SUBMITTED || katom->gpu_rb_state == KBASE_ATOM_GPU_RB_READY) {
            return true;
        }
    }
...
static int kbase_gpu_nr_atoms_on_slot_min(struct kbase_device *kbdev, int js, enum kbase_atom_gpu_rb_state min_rb_state)
{
    int nr = 0;
    int i;
...
static bool check_secure_atom(struct kbase_jd_atom *katom, bool secure)
{
    if (katom->gpu_rb_state >= KBASE_ATOM_GPU_RB_WAITING_PROTECTED_MODE_TRANSITION &&
        ((kbase_jd_katom_is_protected(katom) && secure) || (!kbase_jd_katom_is_protected(katom) && !secure))) {
        return true;
    }
...
static void kbasep_js_job_check_deref_cores(struct kbase_device *kbdev, struct kbase_jd_atom *katom);

static bool kbasep_js_job_check_ref_cores(struct kbase_device *kbdev, int js, struct kbase_jd_atom *katom)
{
    ...
    do {
        ...
        switch (katom->coreref_state) {
            ...
            case KBASE_ATOM_COREREF_STATE_NO_CORES_REQUESTED:
                ...
                /* Compute affinity */
                if (false == kbase_js_choose_affinity(&recently_chosen_affinity, kbdev, katom, js)) {
            ...
            case KBASE_ATOM_COREREF_STATE_WAITING_FOR_REQUESTED_CORES: {
                ...
                if (cores_ready == KBASE_NEW_AFFINITY) {
                    ...
                    KBASE_TRACE_ADD_SLOT_INFO(kbdev, JS_CORE_REF_REGISTER_INUSE_FAILED, katom->kctx, katom, katom->jc,
                                              js, (u32)katom->affinity);
            ...
            case KBASE_ATOM_COREREF_STATE_RECHECK_AFFINITY:
                ...
                if (recently_chosen_affinity != katom->affinity) {
                    ...
                    cores_ready =
                        kbase_pm_register_inuse_cores(kbdev, katom->core_req & BASE_JD_REQ_T, recently_chosen_affinity);
                    kbasep_js_job_check_deref_cores(kbdev, katom);
            ...
            case KBASE_ATOM_COREREF_STATE_CHECK_AFFINITY_VIOLATIONS:
                 ...
                 * later */
                if (kbase_js_affinity_would_violate(kbdev, js, katom->affinity) != false) {
                    /* Return to previous state */
                    katom->coreref_state = KBASE_ATOM_COREREF_STATE_RECHECK_AFFINITY;
                    /* *** BREAK OUT: Transition to lower state ***
                     */
                    KBASE_TRACE_ADD_SLOT_INFO(kbdev, JS_CORE_REF_AFFINITY_WOULD_VIOLATE, katom->kctx, katom, katom->jc,
                                              js, (u32)katom->affinity);
                    break;
                }
            ...
            default:
                KBASE_DEBUG_ASSERT_MSG(false, "Unhandled kbase_atom_coreref_state %d", katom->coreref_state);
                break;
        }
    } while (retry != false);
...
static void kbasep_js_job_check_deref_cores(struct kbase_device *kbdev, struct kbase_jd_atom *katom)
{
    ...
    switch (katom->coreref_state) {
        ...
        default:
            KBASE_DEBUG_ASSERT_MSG(false, "Unhandled coreref_state: %d", katom->coreref_state);
            break;
    }
...
static void kbasep_js_job_check_deref_cores_nokatom(struct kbase_device *kbdev, base_jd_core_req core_req, u64 affinity,
                                                    enum kbase_atom_coreref_state coreref_state)
{
    ...
    switch (coreref_state) {
        case KBASE_ATOM_COREREF_STATE_READY:
            /* State where atom was submitted to the HW - just proceed to
             * power-down */
            KBASE_DEBUG_ASSERT(affinity != 0 || (core_req & BASE_JD_REQ_T));
        ...
        case KBASE_ATOM_COREREF_STATE_RECHECK_AFFINITY:
            /* State where cores were registered */
            KBASE_DEBUG_ASSERT(affinity != 0 || (core_req & BASE_JD_REQ_T));
            kbase_pm_release_cores(kbdev, core_req & BASE_JD_REQ_T, affinity);
        ...
        case KBASE_ATOM_COREREF_STATE_WAITING_FOR_REQUESTED_CORES:
            /* State where cores were requested, but not registered */
            KBASE_DEBUG_ASSERT(affinity != 0 || (core_req & BASE_JD_REQ_T));
            kbase_pm_unrequest_cores(kbdev, core_req & BASE_JD_REQ_T, affinity);
            break;
        ...
        default:
            KBASE_DEBUG_ASSERT_MSG(false, "Unhandled coreref_state: %d", coreref_state);
            break;
    }
}
...
static void kbase_gpu_release_atom(struct kbase_device *kbdev, struct kbase_jd_atom *katom, ktime_t *end_timestamp)
{
    ...
    switch (katom->gpu_rb_state) {
        ...
        case KBASE_ATOM_GPU_RB_SUBMITTED:
            ...
            KBASE_TLSTREAM_TL_NRET_ATOM_LPU(katom, &kbdev->gpu_props.props.raw_props.js_features[katom->slot_nr]);
            KBASE_TLSTREAM_TL_NRET_ATOM_AS(katom, &kbdev->as[kctx->as_nr]);
            KBASE_TLSTREAM_TL_NRET_CTX_LPU(kctx, &kbdev->gpu_props.props.raw_props.js_features[katom->slot_nr]);
            /* fallthrough */
        case KBASE_ATOM_GPU_RB_READY:
            /* ***FALLTHROUGH: TRANSITION TO LOWER STATE*** */
            /* fallthrough */
        case KBASE_ATOM_GPU_RB_WAITING_AFFINITY:
            kbase_js_affinity_release_slot_cores(kbdev, katom->slot_nr, katom->affinity);
            /* ***FALLTHROUGH: TRANSITION TO LOWER STATE*** */
            /* fallthrough */
...
static inline bool kbase_gpu_rmu_workaround(struct kbase_device *kbdev, int js)
{
    ...
    slot_busy[0] = kbase_gpu_nr_atoms_on_slot_min(kbdev, 0, KBASE_ATOM_GPU_RB_WAITING_AFFINITY);
    slot_busy[1] = kbase_gpu_nr_atoms_on_slot_min(kbdev, 1, KBASE_ATOM_GPU_RB_WAITING_AFFINITY);
    slot_busy[2] = kbase_gpu_nr_atoms_on_slot_min(kbdev, 2, KBASE_ATOM_GPU_RB_WAITING_AFFINITY);

    if ((js == 2 && !(slot_busy[0] || slot_busy[1])) || (js != 2 && !slot_busy[2])) {
        return true;
    }

    /* Don't submit slot 2 atom while GPU has jobs on slots 0/1 */
    if (js == 2 &&
        (kbase_gpu_atoms_submitted(kbdev, 0) || kbase_gpu_atoms_submitted(kbdev, 1) || backend->rmu_workaround_flag)) {
        return false;
    }

    /* Don't submit slot 0/1 atom while GPU has jobs on slot 2 */
    if (js != 2 && (kbase_gpu_atoms_submitted(kbdev, 2) || !backend->rmu_workaround_flag)) {
        return false;
    }
...
static int kbase_jm_enter_protected_mode(struct kbase_device *kbdev, struct kbase_jd_atom **katom, int idx, int js)
{
    int err = 0;
    ...
    switch (katom[idx]->protected_state.enter) {
        case KBASE_ATOM_ENTER_PROTECTED_CHECK:
            ...
            katom[idx]->protected_state.enter = KBASE_ATOM_ENTER_PROTECTED_VINSTR;
        ...
        case KBASE_ATOM_ENTER_PROTECTED_FINISHED:
            ...
            KBASE_DEBUG_ASSERT_MSG(kbase_jd_katom_is_protected(katom[idx]) == kbase_gpu_in_protected_mode(kbdev),
...
static int kbase_jm_exit_protected_mode(struct kbase_device *kbdev, struct kbase_jd_atom **katom, int idx, int js)
{
    int err = 0;
...
void kbase_backend_slot_update(struct kbase_device *kbdev)
{
        ...
        for (idx = 0; idx < SLOT_RB_SIZE; idx++) {
            ...
            switch (katom[idx]->gpu_rb_state) {
                ...
                case KBASE_ATOM_GPU_RB_WAITING_BLOCKED:
                    ...
                    katom[idx]->gpu_rb_state = KBASE_ATOM_GPU_RB_WAITING_PROTECTED_MODE_PREV;

                    /* ***TRANSITION TO HIGHER STATE*** */
                    /* fallthrough */
                case KBASE_ATOM_GPU_RB_WAITING_PROTECTED_MODE_PREV:
                    if (kbase_gpu_check_secure_atoms(kbdev, !kbase_jd_katom_is_protected(katom[idx]))) {
                        break;
                    }

                    if ((idx == 1) &&
                        (kbase_jd_katom_is_protected(katom[0]) != kbase_jd_katom_is_protected(katom[1]))) {
                        break;
                    }
                    ...
                    katom[idx]->gpu_rb_state = KBASE_ATOM_GPU_RB_WAITING_PROTECTED_MODE_TRANSITION;
                ...
                case KBASE_ATOM_GPU_RB_WAITING_PROTECTED_MODE_TRANSITION:
                    ...
                    } else if (kbase_gpu_in_protected_mode(kbdev) && !kbase_jd_katom_is_protected(katom[idx])) {
                    ...
                    katom[idx]->gpu_rb_state = KBASE_ATOM_GPU_RB_WAITING_FOR_CORE_AVAILABLE;
                ...
                case KBASE_ATOM_GPU_RB_WAITING_FOR_CORE_AVAILABLE:
                    ...
                    cores_ready = kbasep_js_job_check_ref_cores(kbdev, js, katom[idx]);

                    if (katom[idx]->event_code == BASE_JD_EVENT_PM_EVENT) {
                        katom[idx]->gpu_rb_state = KBASE_ATOM_GPU_RB_RETURN_TO_JS;
                        break;
                    }
                    ...
                    kbase_js_affinity_retain_slot_cores(kbdev, js, katom[idx]->affinity);
                    katom[idx]->gpu_rb_state = KBASE_ATOM_GPU_RB_WAITING_AFFINITY;
                ...
                case KBASE_ATOM_GPU_RB_READY:
                    ...
                    if (idx == 1) {
                        ...
                        if ((katom[0]->gpu_rb_state != KBASE_ATOM_GPU_RB_SUBMITTED &&
                    ...
                    if ((kbdev->serialize_jobs & KBASE_SERIALIZE_INTER_SLOT) && other_slots_busy(kbdev, js)) {
                        break;
                    }

                    if ((kbdev->serialize_jobs & KBASE_SERIALIZE_RESET) && kbase_reset_gpu_active(kbdev)) {
                        break;
                    }
                     ...
                     * metrics. */
                    kbase_pm_metrics_update(kbdev, &katom[idx]->start_timestamp);
    ...
    if (kbase_hw_has_issue(kbdev, BASE_HW_ISSUE_8987)) {
        WARN_ON((kbase_gpu_atoms_submitted(kbdev, 0) || kbase_gpu_atoms_submitted(kbdev, 1)) &&
...
#define HAS_DEP(katom)                                                                                                 \
    ((katom)->pre_dep || (katom)->atom_flags & (KBASE_KATOM_FLAG_X_DEP_BLOCKED | KBASE_KATOM_FLAG_FAIL_BLOCKER))
...
bool kbase_gpu_irq_evict(struct kbase_device *kbdev, int js)
{
    ...
    if (next_katom && katom->kctx == next_katom->kctx && next_katom->gpu_rb_state == KBASE_ATOM_GPU_RB_SUBMITTED &&
        HAS_DEP(next_katom) &&
        (kbase_reg_read(kbdev, JOB_SLOT_REG(js, JS_HEAD_NEXT_LO), NULL) != 0 ||
         kbase_reg_read(kbdev, JOB_SLOT_REG(js, JS_HEAD_NEXT_HI), NULL) != 0)) {
        kbase_reg_write(kbdev, JOB_SLOT_REG(js, JS_COMMAND_NEXT), JS_COMMAND_NOP, NULL);
        next_katom->gpu_rb_state = KBASE_ATOM_GPU_RB_READY;

        KBASE_TLSTREAM_TL_NRET_ATOM_LPU(katom, &kbdev->gpu_props.props.raw_props.js_features[katom->slot_nr]);
        KBASE_TLSTREAM_TL_NRET_ATOM_AS(katom, &kbdev->as[katom->kctx->as_nr]);
        KBASE_TLSTREAM_TL_NRET_CTX_LPU(katom->kctx, &kbdev->gpu_props.props.raw_props.js_features[katom->slot_nr]);
                           ...
                           ktime_t *end_timestamp)
{
    ...
    if (kbase_hw_has_issue(kbdev, BASE_HW_ISSUE_TMIX_8438)) {
        if (completion_code == BASE_JD_EVENT_STOPPED && (katom->atom_flags & KBASE_KATOM_FLAG_BEEN_HARD_STOPPED)) {
            completion_code = BASE_JD_EVENT_TERMINATED;
        }
    }

    if ((kbase_hw_has_issue(kbdev, BASE_HW_ISSUE_6787) || (katom->core_req & BASE_JD_REQ_SKIP_CACHE_END)) &&
    ...
    if (completion_code == BASE_JD_EVENT_STOPPED) {
         ...
         */
        if (next_katom && katom->kctx == next_katom->kctx && next_katom->sched_priority == katom->sched_priority) {
        ...
        for (i = 0; i < kbdev->gpu_props.num_job_slots; i++) {
            ...
            if (katom_idx0 && katom_idx0->kctx == katom->kctx && HAS_DEP(katom_idx0) &&
                ...
                if (katom_idx1 && katom_idx1->kctx == katom->kctx && HAS_DEP(katom_idx1) &&
                    katom_idx0->gpu_rb_state != KBASE_ATOM_GPU_RB_SUBMITTED) {
            ...
            } else if (katom_idx1 && katom_idx1->kctx == katom->kctx && HAS_DEP(katom_idx1) &&
    ...
    KBASE_TRACE_ADD_SLOT_INFO(kbdev, JM_JOB_DONE, kctx, katom, katom->jc, js, completion_code);
    ...
    if (job_tail != 0 && job_tail != katom->jc) {
        ...
        katom->jc = job_tail;
        if (was_updated) {
            KBASE_TRACE_ADD_SLOT(kbdev, JM_UPDATE_HEAD, katom->kctx, katom, job_tail, js);
        }
    }
    ...
    kbase_device_trace_register_access(kctx, REG_WRITE, JOB_CONTROL_REG(JOB_IRQ_CLEAR), 1 << js);
...
#ifdef CONFIG_GPU_TRACEPOINTS
    {
        ...
        if (next_katom && next_katom->gpu_rb_state == KBASE_ATOM_GPU_RB_SUBMITTED) {
            ...
            trace_gpu_sched_switch(kbasep_make_job_slot_string(js, js_string, sizeof(js_string)),
                                   ktime_to_ns(*end_timestamp), (u32)next_katom->kctx->id, 0, next_katom->work_id);
            kbdev->hwaccess.backend.slot_rb[js].last_context = next_katom->kctx;
        } else {
            char js_string[16];

            trace_gpu_sched_switch(kbasep_make_job_slot_string(js, js_string, sizeof(js_string)),
                                   ktime_to_ns(ktime_get()), 0, 0, 0);
    ...
    for (js = 0; js < kbdev->gpu_props.num_job_slots; js++) {
        ...
        for (idx = 0; idx < SLOT_RB_SIZE; idx++) {
            ...
            if (katom->protected_state.exit == KBASE_ATOM_EXIT_PROTECTED_RESET_WAIT) {
                ...
                /* protected mode sanity checks */
                KBASE_DEBUG_ASSERT_MSG(kbase_jd_katom_is_protected(katom) == kbase_gpu_in_protected_mode(kbdev),
                                       "Protected mode of atom (%d) doesn't match protected mode of GPU (%d)",
                                       kbase_jd_katom_is_protected(katom), kbase_gpu_in_protected_mode(kbdev));
                KBASE_DEBUG_ASSERT_MSG((kbase_jd_katom_is_protected(katom) && js == 0) ||
...
static inline void kbase_gpu_stop_atom(struct kbase_device *kbdev, int js, struct kbase_jd_atom *katom, u32 action)
{
...
static inline void kbase_gpu_remove_atom(struct kbase_device *kbdev, struct kbase_jd_atom *katom, u32 action,
                                         bool disjoint)
{
...
{
    kbase_timeline_job_slot_done(katom->kctx->kbdev, katom->kctx, katom, katom->slot_nr,
                                       ...
                                       struct kbase_jd_atom *katom, u32 action)
{
    ...
    if (katom) {
        ...
        if (katom_idx1) {
            katom_idx1_valid = (katom_idx1 == katom) || (katom_idx0_valid && (katom_idx0->kctx == katom_idx1->kctx));
        } else {
            katom_idx1_valid = false;
        }
    } else {
        katom_idx0_valid = (katom_idx0 && (!kctx || katom_idx0->kctx == kctx));
        katom_idx1_valid = (katom_idx1 && (!kctx || katom_idx1->kctx == kctx) && prio_idx0 == prio_idx1);
    }
    ...
    if (katom_idx0_valid) {
        if (katom_idx0->gpu_rb_state != KBASE_ATOM_GPU_RB_SUBMITTED) {
            ...
            if (katom_idx1_valid) {
                ...
                kbase_jm_return_atom_to_js(kbdev, katom_idx1);
                katom_idx1->kctx->blocked_js[js][prio_idx1] = true;
            }
            ...
            if (katom_idx1 && katom_idx1->gpu_rb_state == KBASE_ATOM_GPU_RB_SUBMITTED) {
                ...
                if (kbase_reg_read(kbdev, JOB_SLOT_REG(js, JS_COMMAND_NEXT), NULL) == 0) {
                    ...
                    if (kbase_reg_read(kbdev, JOB_SLOT_REG(js, JS_HEAD_NEXT_LO), NULL) != 0 ||
                        kbase_reg_read(kbdev, JOB_SLOT_REG(js, JS_HEAD_NEXT_HI), NULL) != 0) {
                        /* idx1 removed successfully,
                         * will be handled in IRQ */
                        kbase_job_evicted(katom_idx1);
                        kbase_gpu_remove_atom(kbdev, katom_idx1, action, true);
                        stop_x_dep_idx1 = should_stop_x_dep_slot(katom_idx1);
...
void kbase_gpu_cacheclean(struct kbase_device *kbdev)
{
    ...
    KBASE_DEBUG_ASSERT_MSG(kbdev->hwcnt.backend.state != KBASE_INSTR_STATE_CLEANING,
                           "Instrumentation code was cleaning caches, but Job Management code cleared their IRQ - "
                           "Instrumentation code will now hang.");
...
void kbase_backend_cacheclean(struct kbase_device *kbdev, struct kbase_jd_atom *katom)
{
    if (katom->need_cache_flush_cores_retained) {
        ...
        kbase_pm_unrequest_cores(kbdev, false, katom->need_cache_flush_cores_retained);
...
void kbase_backend_complete_wq(struct kbase_device *kbdev, struct kbase_jd_atom *katom)
{
    ...
    if (kbase_hw_has_issue(kbdev, BASE_HW_ISSUE_10969) && (katom->core_req & BASE_JD_REQ_FS) &&
        katom->event_code == BASE_JD_EVENT_TILE_RANGE_FAULT &&
        (katom->atom_flags & KBASE_KATOM_FLAG_BEEN_SOFT_STOPPPED) && !(katom->atom_flags & KBASE_KATOM_FLAGS_RERUN)) {
        dev_dbg(kbdev->dev,
                "Soft-stopped fragment shader job got a TILE_RANGE_FAULT. Possible HW issue, trying SW workaround\n");
                                          ...
                                          enum kbase_atom_coreref_state coreref_state)
{
    ...
    spin_lock_irqsave(&kbdev->hwaccess_lock, flags);
    kbasep_js_job_check_deref_cores_nokatom(kbdev, core_req, affinity, coreref_state);
    spin_unlock_irqrestore(&kbdev->hwaccess_lock, flags);
...
void kbase_gpu_dump_slots(struct kbase_device *kbdev)
{
    ...
    for (js = 0; js < kbdev->gpu_props.num_job_slots; js++) {
        ...
        for (idx = 0; idx < SLOT_RB_SIZE; idx++) {
            ...
            if (katom) {
                dev_info(kbdev->dev, "  js%d idx%d : katom=%p gpu_rb_state=%d\n", js, idx, katom, katom->gpu_rb_state);
            } else {
