...
static int kbase_job_fault_event_wait(struct kbase_device *kbdev, struct base_job_fault_event *event)
{
    ...
    if (list_empty(event_list)) {
        spin_unlock_irqrestore(&kbdev->job_fault_event_lock, flags);
        if (wait_event_interruptible(kbdev->job_fault_wq, kbase_is_job_fault_event_pending(kbdev))) {
            return -ERESTARTSYS;
        }
        spin_lock_irqsave(&kbdev->job_fault_event_lock, flags);
    }
...
{
    debugfs_create_file("job_fault", S_IRUGO, kbdev->mali_debugfs_directory, kbdev, &kbasep_debug_job_fault_fops);
}
