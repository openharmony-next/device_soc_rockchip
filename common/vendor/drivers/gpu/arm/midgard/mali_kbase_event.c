...
{
    if (event->core_req & BASE_JD_REQ_EVENT_ONLY_ON_FAILURE) {
        if (event->event_code == BASE_JD_EVENT_DONE) {
            /* Don't report the event */
            kbase_event_process_noreport(ctx, event);
            return;
        }
    }

    if (event->core_req & BASEP_JD_REQ_EVENT_NEVER) {
        /* Don't report the event */
        kbase_event_process_noreport(ctx, event);
        return;
    }
    KBASE_TLSTREAM_TL_ATTRIB_ATOM_STATE(event, TL_ATOM_STATE_POSTED);
    if (event->core_req & BASE_JD_REQ_EVENT_COALESCE) {
        /* Don't report the event until other event(s) have completed */
        mutex_lock(&ctx->event_mutex);
        list_add_tail(&event->dep_item[0], &ctx->event_coalesce_list);
        ...
        list_add_tail(&event->dep_item[0], &ctx->event_list);
