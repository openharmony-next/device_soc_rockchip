...
The MMU reads in 8 page table entries from memory at a time, if we have more than one page fault within the same 8 pages
and page tables are updated accordingly, the MMU does not re-read the page table entries from memory for the subsequent
page table updates and generates duplicate page faults as the page table information used by the MMU is not valid.   */
...
#define KBASE_REG_ZONE_MCU_SHARED_SIZE (((0x08000000ULL) >> PAGE_SHIFT) - KBASE_REG_ZONE_MCU_SHARED_BASE)
#endif
...
static inline struct kbase_mem_phy_alloc *kbase_alloc_create(struct kbase_context *kctx, size_t nr_pages,
                                                             enum kbase_memory_type type, int group_id)
{
    ...
    if (type == KBASE_MEM_TYPE_NATIVE) {
        alloc->imported.native.nr_struct_pages = (alloc_size + (PAGE_SIZE - 1)) >> PAGE_SHIFT;
...
static inline int kbase_reg_prepare_native(struct kbase_va_region *reg, struct kbase_context *kctx, int group_id)
{
    KBASE_DEBUG_ASSERT(reg);
    KBASE_DEBUG_ASSERT(!reg->cpu_alloc);
    KBASE_DEBUG_ASSERT(!reg->gpu_alloc);
    ...
    if (kbase_ctx_flag(kctx, KCTX_INFINITE_CACHE) && (reg->flags & KBASE_REG_CPU_CACHED)) {
        reg->gpu_alloc = kbase_alloc_create(kctx, reg->nr_pages, KBASE_MEM_TYPE_NATIVE, group_id);
 ...
 *            @config can hold
 */
static inline void kbase_mem_pool_config_set_max_size(struct kbase_mem_pool_config *const config, size_t const max_size)
{
 ...
 *         can hold
 */
static inline size_t kbase_mem_pool_config_get_max_size(const struct kbase_mem_pool_config *const config)
{
 ...
 */
int kbase_mem_pool_init(struct kbase_mem_pool *pool, const struct kbase_mem_pool_config *config, unsigned int order,
                        int group_id, struct kbase_device *kbdev, struct kbase_mem_pool *next_pool);
 ...
 */
int kbase_mem_pool_alloc_pages(struct kbase_mem_pool *pool, size_t nr_4k_pages, struct tagged_addr *pages,
 ...
 * Note : Caller must hold the pool lock.
 */
int kbase_mem_pool_alloc_pages_locked(struct kbase_mem_pool *pool, size_t nr_4k_pages, struct tagged_addr *pages);
 ...
 */
void kbase_mem_pool_free_pages(struct kbase_mem_pool *pool, size_t nr_pages, struct tagged_addr *pages, bool dirty,
                               bool reclaimed);
 ...
 * Like kbase_mem_pool_free() but optimized for freeing many pages.
 */
void kbase_mem_pool_free_pages_locked(struct kbase_mem_pool *pool, size_t nr_pages, struct tagged_addr *pages,
                                      bool dirty, bool reclaimed);
...
void kbase_sync_single(struct kbase_context *kctx, struct tagged_addr cpu_pa, struct tagged_addr gpu_pa, off_t offset,
                       size_t size, enum kbase_sync_type sync_fn);
 ...
 */
static inline void kbase_jit_request_phys_increase_locked(struct kbase_context *kctx, size_t needed_pages)
{
#if !MALI_USE_CSF
 ...
 */
static inline void kbase_link_event_mem_page(struct kbase_context *kctx, struct kbase_va_region *reg)
{
    lockdep_assert_held(&kctx->reg_lock);
 ...
 * have KBASE_REG_CSF_EVENT flag set for it.
 */
static inline void kbase_unlink_event_mem_page(struct kbase_context *kctx, struct kbase_va_region *reg)
{
    lockdep_assert_held(&kctx->reg_lock);
