...
#if !defined(TRACE_MALI_H) || defined(TRACE_HEADER_MULTI_READ)
#define TRACE_MALI_H
...
TRACE_EVENT(mali_job_slots_event, TP_PROTO(u32 gpu_id, u32 event_id, u32 tgid, u32 pid, u8 job_id),
            ...
            TP_STRUCT__entry(__field(u32, gpu_id) __field(u32, event_id) __field(u32, tgid) __field(u32, pid)
                                 __field(u8, job_id)),
            TP_fast_assign(__entry->gpu_id = gpu_id; __entry->event_id = event_id; __entry->tgid = tgid;
                           __entry->pid = pid; __entry->job_id = job_id;),
            TP_printk("gpu=%u event=%u tgid=%u pid=%u job_id=%u", __entry->gpu_id, __entry->event_id, __entry->tgid,
                      __entry->pid, __entry->job_id));
 ...
 */
TRACE_EVENT(mali_pm_status, TP_PROTO(u32 gpu_id, u32 event_id, u64 value), TP_ARGS(gpu_id, event_id, value),
            TP_STRUCT__entry(__field(u32, gpu_id) __field(u32, event_id) __field(u64, value)),
            TP_fast_assign(__entry->gpu_id = gpu_id; __entry->event_id = event_id; __entry->value = value;),
            TP_printk("gpu=%u event %u = %llu", __entry->gpu_id, __entry->event_id, __entry->value));
...
TRACE_EVENT(mali_page_fault_insert_pages, TP_PROTO(u32 gpu_id, s32 event_id, u64 value),
            ...
            TP_fast_assign(__entry->gpu_id = gpu_id; __entry->event_id = event_id; __entry->value = value;),
            TP_printk("gpu=%u event %d = %llu", __entry->gpu_id, __entry->event_id, __entry->value));
 ...
 */
TRACE_EVENT(mali_total_alloc_pages_change, TP_PROTO(u32 gpu_id, s64 event_id), TP_ARGS(gpu_id, event_id),
...
#define KBASE_MMU_FAULT_STATUS_DECODED_STRING(status) (((status) & (1u << 10)) ? "DECODER_FAULT" : "SLAVE_FAULT")
...
#define KBASE_MMU_FAULT_STATUS_LEVEL(status) KBASE_MMU_FAULT_CODE_LEVEL(KBASE_MMU_FAULT_STATUS_CODE(status))
 ...
 */
TRACE_EVENT(
    mali_mmu_page_fault_grow, TP_PROTO(struct kbase_va_region *reg, struct kbase_fault *fault, size_t new_pages),
    TP_ARGS(reg, fault, new_pages),
    TP_STRUCT__entry(__field(u64, start_addr) __field(u64, fault_addr) __field(u64, fault_extra_addr)
                         __field(size_t, new_pages) __field(u32, status)),
    TP_fast_assign(__entry->start_addr = ((u64)reg->start_pfn) << PAGE_SHIFT; __entry->fault_addr = fault->addr;
                   __entry->fault_extra_addr = fault->extra_addr; __entry->new_pages = new_pages;
                   __entry->status = fault->status;),
    TP_printk("start=0x%llx fault_addr=0x%llx fault_extra_addr=0x%llx new_pages=%zu raw_fault_status=0x%x "
              "decoded_faultstatus=%s exception_type=0x%x,%s%u access_type=0x%x,%s source_id=0x%x",
              __entry->start_addr, __entry->fault_addr, __entry->fault_extra_addr, __entry->new_pages, __entry->status,
              KBASE_MMU_FAULT_STATUS_DECODED_STRING(__entry->status), KBASE_MMU_FAULT_STATUS_CODE(__entry->status),
              KBASE_MMU_FAULT_STATUS_EXCEPTION_NAME_PRINT(__entry->status),
              KBASE_MMU_FAULT_STATUS_LEVEL(__entry->status), KBASE_MMU_FAULT_STATUS_ACCESS(__entry->status) >> 8,
 ...
 */
DECLARE_EVENT_CLASS(mali_jit_softjob_template, TP_PROTO(struct kbase_va_region *reg, u8 jit_id), TP_ARGS(reg, jit_id),
                    TP_STRUCT__entry(__field(u64, start_addr) __field(size_t, nr_pages) __field(size_t, backed_pages)
                                         __field(u8, jit_id)),
                    ...
                    TP_printk("jit_id=%u start=0x%llx va_pages=0x%zx backed_size=0x%zx", __entry->jit_id,
 ...
 * allocating memory
 */
DEFINE_EVENT(mali_jit_softjob_template, mali_jit_alloc, TP_PROTO(struct kbase_va_region *reg, u8 jit_id),
             TP_ARGS(reg, jit_id));
 ...
 */
DEFINE_EVENT_PRINT(mali_jit_softjob_template, mali_jit_free, TP_PROTO(struct kbase_va_region *reg, u8 jit_id),
                   TP_ARGS(reg, jit_id),
                   TP_printk("start=0x%llx va_pages=0x%zx backed_size=0x%zx", __entry->start_addr, __entry->nr_pages,
...
TRACE_EVENT(
    mali_jit_report,
    TP_PROTO(struct kbase_jd_atom *katom, struct kbase_va_region *reg, unsigned int id_idx, u64 read_val,
             u64 used_pages),
    TP_ARGS(katom, reg, id_idx, read_val, used_pages),
    TP_STRUCT__entry(__field(u64, start_addr) __field(u64, read_val) __field(u64, used_pages)
                         __field(unsigned long, flags) __field(u8, id_idx) __field(u8, jit_id)),
    TP_fast_assign(__entry->start_addr = ((u64)reg->start_pfn) << PAGE_SHIFT; __entry->read_val = read_val;
                   __entry->used_pages = used_pages; __entry->flags = reg->flags; __entry->id_idx = id_idx;
                   __entry->jit_id = katom->jit_ids[id_idx];),
    TP_printk("start=0x%llx jit_ids[%u]=%u read_type='%s' read_val=0x%llx used_pages=%llu", __entry->start_addr,
              __entry->id_idx, __entry->jit_id,
              __print_symbolic(__entry->flags, {0, "address"}, {KBASE_REG_TILER_ALIGN_TOP, "address with align"},
                               {KBASE_REG_HEAP_INFO_IS_SIZE, "size"},
                               {KBASE_REG_HEAP_INFO_IS_SIZE | KBASE_REG_TILER_ALIGN_TOP, "size with align (invalid)"}),
...
TRACE_EVENT(mali_jit_report_pressure,
            TP_PROTO(struct kbase_va_region *reg, u64 new_used_pages, u64 new_pressure, unsigned int flags),
            TP_ARGS(reg, new_used_pages, new_pressure, flags),
            TP_STRUCT__entry(__field(u64, start_addr) __field(u64, used_pages) __field(u64, new_used_pages)
                                 __field(u64, new_pressure) __field(unsigned int, flags)),
            TP_fast_assign(__entry->start_addr = ((u64)reg->start_pfn) << PAGE_SHIFT;
                           __entry->used_pages = reg->used_pages; __entry->new_used_pages = new_used_pages;
            ...
            TP_printk("start=0x%llx old_used_pages=%llu new_used_pages=%llu new_pressure=%llu report_flags=%s",
                      __entry->start_addr, __entry->used_pages, __entry->new_used_pages, __entry->new_pressure,
...
enum sysgraph_msg { SGR_ARRIVE, SGR_DEP_RES, SGR_SUBMIT, SGR_COMPLETE, SGR_POST, SGR_ACTIVE, SGR_INACTIVE };
 ...
 */
TRACE_EVENT(sysgraph, TP_PROTO(enum sysgraph_msg message, unsigned int proc_id, unsigned int atom_id),
            ...
            TP_fast_assign(__entry->proc_id = proc_id; __entry->message = message; __entry->atom_id = atom_id;),
...
TRACE_EVENT(sysgraph_gpu,
            TP_PROTO(enum sysgraph_msg message, unsigned int proc_id, unsigned int atom_id, unsigned int js),
 ...
 */
TRACE_EVENT(
    mali_jit_report_gpu_mem, TP_PROTO(u64 base_addr, u64 reg_addr, u64 *gpu_mem, unsigned int flags),
    ...
    TP_printk("start=0x%llx read GPU memory base=0x%llx values=%s report_flags=%s", __entry->reg_addr,
              __entry->base_addr, __print_array(__entry->mem_values, ARRAY_SIZE(__entry->mem_values), sizeof(u64)),
              __print_flags(__entry->flags, "|", {KBASE_JIT_REPORT_ON_ALLOC_OR_FREE, "HAPPENED_ON_ALLOC_OR_FREE"})));
...
TRACE_EVENT(mali_jit_trim_from_region,
            TP_PROTO(struct kbase_va_region *reg, size_t freed_pages, size_t old_pages, size_t available_pages,
                     size_t new_pages),
            TP_ARGS(reg, freed_pages, old_pages, available_pages, new_pages),
            TP_STRUCT__entry(__field(u64, start_addr) __field(size_t, freed_pages) __field(size_t, old_pages)
                                 __field(size_t, available_pages) __field(size_t, new_pages)),
            TP_fast_assign(__entry->start_addr = ((u64)reg->start_pfn) << PAGE_SHIFT;
            ...
            TP_printk("start=0x%llx freed_pages=%zu old_pages=%zu available_pages=%zu new_pages=%zu",
                      __entry->start_addr, __entry->freed_pages, __entry->old_pages, __entry->available_pages,
                      __entry->new_pages));
...
TRACE_EVENT(mali_jit_trim, TP_PROTO(size_t freed_pages), TP_ARGS(freed_pages),
            TP_STRUCT__entry(__field(size_t, freed_pages)), TP_fast_assign(__entry->freed_pages = freed_pages;),
