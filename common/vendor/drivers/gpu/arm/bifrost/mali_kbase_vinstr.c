 ...
 */
static int kbasep_vinstr_client_dump(struct kbase_vinstr_client *vcli, enum base_hwcnt_reader_event event_id)
{
    int errcode;
    u64 ts_start_ns;
    u64 ts_end_ns;
    ...
    errcode = kbase_hwcnt_virtualizer_client_dump(vcli->hvcli, &ts_start_ns, &ts_end_ns, tmp_buf);
    if (errcode) {
        return errcode;
    }
     ...
     */
    kbase_hwcnt_dump_buffer_copy_strict(dump_buf, tmp_buf, &vcli->enable_map);
    ...
    meta->cycles.shader_cores = (clk_cnt > 1) ? dump_buf->clk_cnt_buf[1] : 0;
...
static int kbasep_vinstr_client_clear(struct kbase_vinstr_client *vcli)
{
     ...
     * client's buffer.
     */
    return kbase_hwcnt_virtualizer_client_dump(vcli->hvcli, &ts_start_ns, &ts_end_ns, NULL);
}
...
static void kbasep_vinstr_reschedule_worker(struct kbase_vinstr_context *vctx)
{
     ...
     * suspended.
     */
    if ((earliest_next_ns != U64_MAX) && (vctx->suspend_count == 0) && !WARN_ON(earliest_next_ns < cur_ts_ns)) {
        hrtimer_start(&vctx->dump_timer, ns_to_ktime(earliest_next_ns - cur_ts_ns), HRTIMER_MODE_REL);
    }
}
...
{
    struct kbase_vinstr_context *vctx = container_of(work, struct kbase_vinstr_context, dump_work);
    struct kbase_vinstr_client *pos;
    u64 cur_time_ns;
    ...
    list_for_each_entry(pos, &vctx->clients, node)
    {
        if ((pos->next_dump_time_ns != 0) && (pos->next_dump_time_ns < cur_time_ns)) {
            kbasep_vinstr_client_dump(pos, BASE_HWCNT_READER_EVENT_PERIODIC);
        }
    }
...
{
    struct kbase_vinstr_context *vctx = container_of(timer, struct kbase_vinstr_context, dump_timer);
 ...
 * Return: 0 on success, else error code.
 */
static int kbasep_vinstr_client_create(struct kbase_vinstr_context *vctx, struct kbase_ioctl_hwcnt_reader_setup *setup,
                                       struct kbase_vinstr_client **out_vcli)
{
    ...
    errcode = kbase_hwcnt_enable_map_alloc(vctx->metadata, &vcli->enable_map);
    if (errcode) {
        goto error;
    }
    ...
    errcode = kbase_hwcnt_dump_buffer_array_alloc(vctx->metadata, setup->buffer_count, &vcli->dump_bufs);
    if (errcode) {
        goto error;
    }

    errcode = -ENOMEM;
    vcli->dump_bufs_meta = kmalloc_array(setup->buffer_count, sizeof(*vcli->dump_bufs_meta), GFP_KERNEL);
    if (!vcli->dump_bufs_meta) {
        goto error;
    }

    errcode = kbase_hwcnt_virtualizer_client_create(vctx->hvirt, &vcli->enable_map, &vcli->hvcli);
    if (errcode) {
        goto error;
    }
...
int kbase_vinstr_hwcnt_reader_setup(struct kbase_vinstr_context *vctx, struct kbase_ioctl_hwcnt_reader_setup *setup)
{
    ...
    if (!vctx || !setup || (setup->buffer_count == 0) || (setup->buffer_count > MAX_BUFFER_COUNT)) {
        return -EINVAL;
    }
    ...
    errcode = anon_inode_getfd("[mali_vinstr_desc]", &vinstr_client_fops, vcli, O_RDONLY | O_CLOEXEC);
    if (errcode < 0) {
 ...
 * Return: 0 on success, else error code.
 */
static long kbasep_vinstr_hwcnt_reader_ioctl_dump(struct kbase_vinstr_client *cli)
{
    int errcode;

    mutex_lock(&cli->vctx->lock);

    errcode = kbasep_vinstr_client_dump(cli, BASE_HWCNT_READER_EVENT_MANUAL);
 ...
 * Return: 0 on success, else error code.
 */
static long kbasep_vinstr_hwcnt_reader_ioctl_clear(struct kbase_vinstr_client *cli)
{
    int errcode;
...
{
    u32 ver = 5;
    const enum kbase_hwcnt_gpu_group_type type = kbase_hwcnt_metadata_group_type(cli->vctx->metadata, 0);
 ...
 * @return 0 on success, else error code.
 */
static long kbasep_vinstr_hwcnt_reader_ioctl_get_api_version(struct kbase_vinstr_client *cli, unsigned long arg,
                                                             size_t size)
{
    long ret = -EINVAL;
    ...
    if (size == sizeof(u32)) {
        ...
        if (clk_cnt > 0) {
            api_version.features |= KBASE_HWCNT_READER_API_VERSION_FEATURE_CYCLES_TOP;
        }
        if (clk_cnt > 1) {
            api_version.features |= KBASE_HWCNT_READER_API_VERSION_FEATURE_CYCLES_SHADER_CORES;
        }

        ret = copy_to_user((void __user *)arg, &api_version, sizeof(api_version));
    }
    return ret;
}
...
static long kbasep_vinstr_hwcnt_reader_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch (_IOC_NR(cmd)) {
        case _IOC_NR(KBASE_HWCNT_READER_GET_API_VERSION):
            rcode = kbasep_vinstr_hwcnt_reader_ioctl_get_api_version(cli, arg, _IOC_SIZE(cmd));
            break;
        ...
        case _IOC_NR(KBASE_HWCNT_READER_GET_BUFFER_SIZE):
            rcode = put_user((u32)cli->vctx->metadata->dump_buf_bytes, (u32 __user *)arg);
            break;
