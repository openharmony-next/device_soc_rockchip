...
#define KBASE_HWCNT_V5_VALUES_PER_BLOCK (KBASE_HWCNT_V5_HEADERS_PER_BLOCK + KBASE_HWCNT_V5_COUNTERS_PER_BLOCK)
 ...
 */
static int kbasep_hwcnt_backend_gpu_metadata_v5_create(const struct kbase_hwcnt_gpu_v5_info *v5_info,
                                                       bool use_secondary, const struct kbase_hwcnt_metadata **metadata)
{
    ...
    blks[0x0].type = KBASE_HWCNT_GPU_V5_BLOCK_TYPE_PERF_JM;
    blks[0x0].inst_cnt = 1;
    blks[0x0].hdr_cnt = KBASE_HWCNT_V5_HEADERS_PER_BLOCK;
    blks[0x0].ctr_cnt = KBASE_HWCNT_V5_COUNTERS_PER_BLOCK;

    /* One Tiler block */
    blks[0x1].type = KBASE_HWCNT_GPU_V5_BLOCK_TYPE_PERF_TILER;
    blks[0x1].inst_cnt = 1;
    blks[0x1].hdr_cnt = KBASE_HWCNT_V5_HEADERS_PER_BLOCK;
    blks[0x1].ctr_cnt = KBASE_HWCNT_V5_COUNTERS_PER_BLOCK;

    /* l2_count memsys blks */
    blks[0x2].type =
        use_secondary ? KBASE_HWCNT_GPU_V5_BLOCK_TYPE_PERF_MEMSYS2 : KBASE_HWCNT_GPU_V5_BLOCK_TYPE_PERF_MEMSYS;
    blks[0x2].inst_cnt = v5_info->l2_count;
    blks[0x2].hdr_cnt = KBASE_HWCNT_V5_HEADERS_PER_BLOCK;
    blks[0x2].ctr_cnt = KBASE_HWCNT_V5_COUNTERS_PER_BLOCK;
     ...
     */
    blks[0x3].type = use_secondary ? KBASE_HWCNT_GPU_V5_BLOCK_TYPE_PERF_SC2 : KBASE_HWCNT_GPU_V5_BLOCK_TYPE_PERF_SC;
    blks[0x3].inst_cnt = sc_block_count;
    blks[0x3].hdr_cnt = KBASE_HWCNT_V5_HEADERS_PER_BLOCK;
    blks[0x3].ctr_cnt = KBASE_HWCNT_V5_COUNTERS_PER_BLOCK;

    WARN_ON(KBASE_HWCNT_V5_BLOCK_TYPE_COUNT != 0x4);
 ...
 */
static size_t kbasep_hwcnt_backend_gpu_v5_dump_bytes(const struct kbase_hwcnt_gpu_v5_info *v5_info)
{
    WARN_ON(!v5_info);
    return (0x2 + v5_info->l2_count + fls64(v5_info->core_mask)) * KBASE_HWCNT_V5_VALUES_PER_BLOCK *
                                    ...
                                    const struct kbase_hwcnt_metadata **out_metadata, size_t *out_dump_bytes)
{
    ...
    if (info->type == KBASE_HWCNT_GPU_GROUP_TYPE_V5) {
        ...
        errcode = kbasep_hwcnt_backend_gpu_metadata_v5_create(&info->v5, use_secondary, &metadata);
    } else {
        return -EINVAL;
    }
    if (errcode) {
...
int kbase_hwcnt_gpu_dump_get(struct kbase_hwcnt_dump_buffer *dst, void *src,
                             const struct kbase_hwcnt_enable_map *dst_enable_map, u64 pm_core_mask, bool accumulate)
{
    const struct kbase_hwcnt_metadata *metadata;
    const u32 *dump_src;
    ...
    kbase_hwcnt_metadata_for_each_block(metadata, grp, blk, blk_inst)
    {
        ...
        const bool is_shader_core = is_block_type_shader(kbase_hwcnt_metadata_group_type(metadata, grp), blk_type, blk);
        ...
        if (kbase_hwcnt_enable_map_block_enabled(dst_enable_map, grp, blk, blk_inst)) {
            ...
            if (!is_shader_core || (core_mask & 1)) {
                if (accumulate) {
                    ...
                    kbase_hwcnt_dump_buffer_block_copy(dst_blk, src_blk, (hdr_cnt + ctr_cnt));
                }
            } else if (!accumulate) {
 ...
 */
static inline u32 kbasep_hwcnt_backend_gpu_block_map_to_physical(u64 lo, u64 hi)
{
    u32 phys = 0;
    u64 dwords[2] = {lo, hi};
    size_t dword_idx;
    ...
    for (dword_idx = 0; dword_idx < 0x2; dword_idx++) {
        ...
        for (hword_bit = 0; hword_bit < 0x10; hword_bit++) {
            const size_t dword_bit = hword_bit * 0x4;
            const u16 mask = ((dword >> (dword_bit + 0x0)) & 0x1) | ((dword >> (dword_bit + 0x1)) & 0x1) |
                             ((dword >> (dword_bit + 0x2)) & 0x1) | ((dword >> (dword_bit + 0x3)) & 0x1);
            packed |= (mask << hword_bit);
        }
        phys |= ((u32)packed) << (0x10 * dword_idx);
    }
    return phys;
}
...
static inline void kbasep_hwcnt_backend_gpu_block_map_from_physical(u32 phys, u64 *lo, u64 *hi)
{
    ...
    for (dword_idx = 0; dword_idx < 0x2; dword_idx++) {
        ...
        for (hword_bit = 0; hword_bit < 0x10; hword_bit++) {
            ...
            dword |= mask << (dword_bit + 0x0);
            dword |= mask << (dword_bit + 0x1);
            dword |= mask << (dword_bit + 0x2);
            dword |= mask << (dword_bit + 0x3);
        }
                                            ...
                                            const struct kbase_hwcnt_enable_map *src)
{
    ...
    kbase_hwcnt_metadata_for_each_block(metadata, grp, blk, blk_inst)
    {
        ...
        const u64 blk_type = kbase_hwcnt_metadata_block_type(metadata, grp, blk);
        const size_t blk_val_cnt = kbase_hwcnt_metadata_block_values_count(metadata, grp, blk);
    ...
    dst->fe_bm = kbasep_hwcnt_backend_gpu_block_map_to_physical(fe_bm, 0);
    dst->shader_bm = kbasep_hwcnt_backend_gpu_block_map_to_physical(shader_bm, 0);
    dst->tiler_bm = kbasep_hwcnt_backend_gpu_block_map_to_physical(tiler_bm, 0);
    dst->mmu_l2_bm = kbasep_hwcnt_backend_gpu_block_map_to_physical(mmu_l2_bm, 0);
}
                                              ...
                                              const struct kbase_hwcnt_physical_enable_map *src)
{
    ...
    kbasep_hwcnt_backend_gpu_block_map_from_physical(src->fe_bm, &fe_bm, &ignored_hi);
    kbasep_hwcnt_backend_gpu_block_map_from_physical(src->shader_bm, &shader_bm, &ignored_hi);
                                        ...
                                        const struct kbase_hwcnt_enable_map *enable_map)
{
    ...
    kbase_hwcnt_metadata_for_each_block(metadata, grp, blk, blk_inst)
    {
        ...
        const u64 *blk_map = kbase_hwcnt_enable_map_block_instance(enable_map, grp, blk, blk_inst);
        const u32 prfcnt_en = kbasep_hwcnt_backend_gpu_block_map_to_physical(blk_map[0], 0);
