...
static void kbase_mem_pool_sync_page(struct kbase_mem_pool *pool, struct page *p)
{
    ...
    dma_sync_single_for_device(dev, kbase_dma_addr(p), (PAGE_SIZE << pool->order), DMA_BIDIRECTIONAL);
}
...
void kbase_mem_pool_term(struct kbase_mem_pool *pool)
{
    ...
    if (next_pool && nr_to_spill) {
        list_for_each_entry(p, &spill_list, lru) kbase_mem_pool_zero_page(pool, p);
    ...
    while (nr_from_pool--) {
        ...
        if (pool->order) {
            ...
            for (j = 1; j < (1u << pool->order); j++) {
                pages[i++] = as_tagged_tag(page_to_phys(p) + PAGE_SIZE * j, HUGE_PAGE);
            }
        } else {
    ...
    for (i = 0; i < nr_pages_internal; i++) {
        ...
        if (pool->order) {
            *pages++ = as_tagged_tag(page_to_phys(p), HUGE_HEAD | HUGE_PAGE);
            for (j = 1; j < (1u << pool->order); j++) {
                *pages++ = as_tagged_tag(page_to_phys(p) + PAGE_SIZE * j, HUGE_PAGE);
            }
        } else {
...
static void kbase_mem_pool_add_array(struct kbase_mem_pool *pool, size_t nr_pages, struct tagged_addr *pages, bool zero,
                                     bool sync)
{
    struct page *p;
    size_t nr_to_pool = 0;
...
static void kbase_mem_pool_add_array_locked(struct kbase_mem_pool *pool, size_t nr_pages, struct tagged_addr *pages,
                                            bool zero, bool sync)
{
    struct page *p;
    size_t nr_to_pool = 0;
                                      ...
                                      bool dirty, bool reclaimed)
{
    ...
    if (!reclaimed) {
        ...
        kbase_mem_pool_add_array_locked(pool, nr_pages, pages, false, dirty);
