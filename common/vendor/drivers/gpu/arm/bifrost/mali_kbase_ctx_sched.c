        ...
        if (free_as != KBASEP_AS_NR_INVALID) {
            ...
            if (free_as != kctx->as_nr) {
                ...
                if (prev_kctx) {
                    ...
                    KBASE_TLSTREAM_TL_KBASE_CTX_UNASSIGN_AS(kbdev, prev_kctx->id);
...
bool kbase_ctx_sched_inc_refcount_nolock(struct kbase_context *kctx)
{
    ...
    if (atomic_read(&kctx->refcount) > 0) {
        ...
        KBASE_KTRACE_ADD(kctx->kbdev, SCHED_RETAIN_CTX_NOLOCK, kctx, kbase_ktrace_get_ctx_refcnt(kctx));
        result = true;
    }
...
void kbase_ctx_sched_release_ctx_lock(struct kbase_context *kctx)
{
    ...
    if (!WARN_ON(kctx->as_nr == KBASEP_AS_NR_INVALID) && !WARN_ON(atomic_read(&kctx->refcount) <= 0)) {
