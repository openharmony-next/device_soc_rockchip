...
static u64 kbase_job_write_affinity(struct kbase_device *kbdev, base_jd_core_req core_req, int js)
{
    u64 affinity;
    ...
    if ((core_req & (BASE_JD_REQ_FS | BASE_JD_REQ_CS | BASE_JD_REQ_T)) == BASE_JD_REQ_T) {
    ...
    } else if ((core_req & (BASE_JD_REQ_COHERENT_GROUP | BASE_JD_REQ_SPECIFIC_COHERENT_GROUP))) {
        unsigned int num_core_groups = kbdev->gpu_props.num_core_groups;
        struct mali_base_gpu_coherent_group_info *coherency_info = &kbdev->gpu_props.props.coherency_info;
         ...
         */
        if (js == BASE_MEM_PROT_CPU_WR && num_core_groups > 1) {
    ...
    kbase_reg_write(kbdev, JOB_SLOT_REG(js, JS_AFFINITY_NEXT_LO), affinity & BASE_MEM_FLAGS_MAX);
    kbase_reg_write(kbdev, JOB_SLOT_REG(js, JS_AFFINITY_NEXT_HI), affinity >> BASE_MEM_FLAGS_NR_HI_BITS);
...
static u64 select_job_chain(struct kbase_jd_atom *katom)
{
    ...
    compiletime_assert((1ull << (sizeof(katom->renderpass_id) * BASE_MEM_PROT_GPU_WR)) <=
    ...
    dev_dbg(kctx->kbdev->dev, "Selected job chain 0x%llx for end atom %p in state %d\n", jc, (void *)katom,
            (int)rp->state);
...
void kbase_job_hw_submit(struct kbase_device *kbdev, struct kbase_jd_atom *katom, int js)
{
    ...
    kbase_reg_write(kbdev, JOB_SLOT_REG(js, JS_HEAD_NEXT_LO), jc_head & BASE_MEM_FLAGS_MAX);
    kbase_reg_write(kbdev, JOB_SLOT_REG(js, JS_HEAD_NEXT_HI), jc_head >> BASE_MEM_FLAGS_NR_HI_BITS);
    ...
    cfg |= JS_CONFIG_THREAD_PRI(BASE_MEM_PROT_GPU_WR);

    if ((katom->atom_flags & KBASE_KATOM_FLAG_PROTECTED) || (katom->core_req & BASE_JD_REQ_END_RENDERPASS)) {
    ...
    if (kbase_hw_has_feature(kbdev, BASE_HW_FEATURE_FLUSH_REDUCTION)) {
        kbase_reg_write(kbdev, JOB_SLOT_REG(js, JS_FLUSH_ID_NEXT), katom->flush_id);
    }
    ...
    /* GO ! */
    dev_dbg(kbdev->dev, "JS: Submitting atom %p from ctx %p to js[%d] with head=0x%llx", katom, kctx, js, jc_head);

    KBASE_KTRACE_ADD_JM_SLOT_INFO(kbdev, JM_SUBMIT, kctx, katom, jc_head, js, (u32)affinity);

    KBASE_TLSTREAM_AUX_EVENT_JOB_SLOT(kbdev, kctx, js, kbase_jd_atom_id(kctx, katom), TL_JS_EVENT_START);

    KBASE_TLSTREAM_TL_ATTRIB_ATOM_CONFIG(kbdev, katom, jc_head, affinity, cfg);
    KBASE_TLSTREAM_TL_RET_CTX_LPU(kbdev, kctx, &kbdev->gpu_props.props.raw_props.js_features[katom->slot_nr]);
    KBASE_TLSTREAM_TL_RET_ATOM_AS(kbdev, katom, &kbdev->as[kctx->as_nr]);
    KBASE_TLSTREAM_TL_RET_ATOM_LPU(kbdev, katom, &kbdev->gpu_props.props.raw_props.js_features[js], "ctx_nr,atom_nr");
    ...
    if (!kbase_backend_nr_atoms_submitted(kbdev, js)) {
        ...
        trace_gpu_sched_switch(kbasep_make_job_slot_string(js, js_string, sizeof(js_string)),
                               ktime_to_ns(katom->start_timestamp), (u32)katom->kctx->id, 0, katom->work_id);
    ...
    kbase_reg_write(kbdev, JOB_SLOT_REG(js, JS_COMMAND_NEXT), JS_COMMAND_START);
}
 ...
 */
static void kbasep_job_slot_update_head_start_timestamp(struct kbase_device *kbdev, int js, ktime_t end_timestamp)
{
 ...
 */
static void kbasep_trace_tl_event_lpu_softstop(struct kbase_device *kbdev, int js)
{
    KBASE_TLSTREAM_TL_EVENT_LPU_SOFTSTOP(kbdev, &kbdev->gpu_props.props.raw_props.js_features[js]);
}
...
void kbase_job_done(struct kbase_device *kbdev, u32 done)
{
    ...
    while (done) {
        ...
        do {
            ...
            if (failed & (1u << i)) {
                ...
                if (completion_code == BASE_JD_EVENT_STOPPED) {
                     ...
                     * be resumed */
                    job_tail = (u64)kbase_reg_read(kbdev, JOB_SLOT_REG(i, JS_TAIL_LO)) |
                               ((u64)kbase_reg_read(kbdev, JOB_SLOT_REG(i, JS_TAIL_HI)) << BASE_MEM_FLAGS_NR_HI_BITS);
            ...
            nr_done -= (active >> i) & 1;
            nr_done -= (active >> (i + KBASE_BASE_MEM_PROTECTED)) & KBASE_RESET_GPU_PREPARED;
            ...
            while (nr_done) {
                if (nr_done == 1) {
                    kbase_gpu_complete_hw(kbdev, i, completion_code, job_tail, &end_timestamp);
        ...
        spurious:
            done = kbase_reg_read(kbdev, JOB_CONTROL_REG(JOB_IRQ_RAWSTAT));

            failed = done >> KBASE_BASE_MEM_PROTECTED;
                                                 ...
                                                 base_jd_core_req core_reqs, struct kbase_jd_atom *target_katom)
{
    ...
    job_in_head_before = ((u64)kbase_reg_read(kbdev, JOB_SLOT_REG(js, JS_HEAD_LO))) |
                         (((u64)kbase_reg_read(kbdev, JOB_SLOT_REG(js, JS_HEAD_HI))) << BASE_MEM_FLAGS_NR_HI_BITS);
    ...
    if (action == JS_COMMAND_SOFT_STOP) {
        ...
        if (kbase_hw_has_feature(kbdev, BASE_HW_FEATURE_JOBCHAIN_DISAMBIGUATION)) {
            action = (target_katom->atom_flags & KBASE_KATOM_FLAGS_JOBCHAIN) ? JS_COMMAND_SOFT_STOP_1
        ...
        if (kbase_hw_has_feature(kbdev, BASE_HW_FEATURE_JOBCHAIN_DISAMBIGUATION)) {
            action = (target_katom->atom_flags & KBASE_KATOM_FLAGS_JOBCHAIN) ? JS_COMMAND_HARD_STOP_1
...
void kbase_job_slot_ctx_priority_check_locked(struct kbase_context *kctx, struct kbase_jd_atom *target_katom)
{
    ...
    for (i = 0; i < kbase_backend_nr_atoms_on_slot(kbdev, js); i++) {
        ...
        if ((kbdev->js_ctx_scheduling_mode == KBASE_JS_PROCESS_LOCAL_PRIORITY_MODE) && (katom->kctx != kctx)) {
            continue;
        }
...
static int softstop_start_rp_nolock(struct kbase_context *kctx, struct kbase_va_region *reg)
{
    ...
    if (!(katom->core_req & BASE_JD_REQ_START_RENDERPASS)) {
        dev_dbg(kctx->kbdev->dev, "Atom %p on job slot is not start RP\n", (void *)katom);
        return -EPERM;
    }
    ...
    rp->state = (rp->state == KBASE_JD_RP_START ? KBASE_JD_RP_PEND_OOM : KBASE_JD_RP_RETRY_PEND_OOM);
...
int kbase_job_slot_softstop_start_rp(struct kbase_context *const kctx, struct kbase_va_region *const reg)
{
    struct kbase_device *const kbdev = kctx->kbdev;
    int err;
    unsigned long flags;
...
void kbase_jm_wait_for_zero_jobs(struct kbase_context *kctx)
{
    ...
    if (kbase_prepare_to_reset_gpu(kbdev)) {
        dev_err(kbdev->dev,
                "Issueing GPU soft-reset because jobs failed to be killed (within %d ms) as part of context "
                "termination (e.g. process exit)\n",
                ZAP_TIMEOUT);
...
u32 kbase_backend_get_current_flush_id(struct kbase_device *kbdev)
{
    ...
    if (kbase_hw_has_feature(kbdev, BASE_HW_FEATURE_FLUSH_REDUCTION)) {
        ...
        if (kbdev->pm.backend.gpu_powered) {
            flush_id = kbase_reg_read(kbdev, GPU_CONTROL_REG(LATEST_FLUSH));
        }
                                     ...
                                     u32 sw_flags)
{
    ...
    kbase_backend_soft_hard_stop_slot(kbdev, NULL, js, target_katom, JS_COMMAND_SOFT_STOP | sw_flags);
}
...
void kbase_job_slot_hardstop(struct kbase_context *kctx, int js, struct kbase_jd_atom *target_katom)
{
    ...
    stopped = kbase_backend_soft_hard_stop_slot(kbdev, kctx, js, target_katom, JS_COMMAND_HARD_STOP);
}
...
static void kbase_debug_dump_registers(struct kbase_device *kbdev)
{
    ...
    dev_err(kbdev->dev, "  GPU_IRQ_MASK=0x%08x    JOB_IRQ_MASK=0x%08x     MMU_IRQ_MASK=0x%08x",
            kbase_reg_read(kbdev, GPU_CONTROL_REG(GPU_IRQ_MASK)), kbase_reg_read(kbdev, JOB_CONTROL_REG(JOB_IRQ_MASK)),
    ...
    dev_err(kbdev->dev, "  TILER_CONFIG=0x%08x    JM_CONFIG=0x%08x",
            kbase_reg_read(kbdev, GPU_CONTROL_REG(TILER_CONFIG)), kbase_reg_read(kbdev, GPU_CONTROL_REG(JM_CONFIG)));
}
...
{
    struct kbase_device *kbdev = container_of(timer, struct kbase_device, hwaccess.backend.reset_timer);

    KBASE_DEBUG_ASSERT(kbdev);

    /* Reset still pending? */
    if (atomic_cmpxchg(&kbdev->hwaccess.backend.reset_gpu, KBASE_RESET_GPU_COMMITTED, KBASE_RESET_GPU_HAPPENING) ==
...
bool kbase_prepare_to_reset_gpu_locked(struct kbase_device *kbdev)
{
    ...
    if (atomic_cmpxchg(&kbdev->hwaccess.backend.reset_gpu, KBASE_RESET_GPU_NOT_PENDING, KBASE_RESET_GPU_PREPARED) !=
...
void kbase_reset_gpu(struct kbase_device *kbdev)
{
    ...
    KBASE_DEBUG_ASSERT(atomic_read(&kbdev->hwaccess.backend.reset_gpu) == KBASE_RESET_GPU_PREPARED);
    ...
    hrtimer_start(&kbdev->hwaccess.backend.reset_timer, HR_TIMER_DELAY_MSEC(kbdev->reset_timeout_ms), HRTIMER_MODE_REL);
...
{
    if (atomic_cmpxchg(&kbdev->hwaccess.backend.reset_gpu, KBASE_RESET_GPU_NOT_PENDING, KBASE_RESET_GPU_SILENT) !=
        KBASE_RESET_GPU_NOT_PENDING) {
        /* Some other thread is already resetting the GPU */
        return -EAGAIN;
    }
