 ...
 */
static void opp_translate(struct kbase_device *kbdev, unsigned long freq, unsigned long volt, u64 *core_mask,
...
static int kbase_devfreq_target(struct device *dev, unsigned long *target_freq, u32 flags)
{
    ...
    opp_translate(kbdev, nominal_freq, nominal_volt, &core_mask, freqs, volts);
    ...
    for (i = 0; i < kbdev->nr_clocks; i++) {
        if (kbdev->regulators[i] && kbdev->current_voltages[i] != volts[i] && old_freqs[i] < freqs[i]) {
            int err;
...
static int kbase_devfreq_init_core_mask_table(struct kbase_device *kbdev)
{
    ...
    return 0;
#else
    struct device_node *opp_node = of_parse_phandle(kbdev->dev->of_node, "operating-points-v2", 0);
...
#ifdef CONFIG_REGULATOR
        err = of_property_read_u32_array(node, "opp-microvolt", opp_volts, kbdev->nr_regulators);
        if (err < 0) {
...
{
    struct kbase_devfreq_queue_info *info = container_of(work, struct kbase_devfreq_queue_info, work);
...
static unsigned long kbase_devfreq_get_static_power(struct devfreq *devfreq, unsigned long voltage)
{
...
int kbase_devfreq_init(struct kbase_device *kbdev)
{
    ...
    if (dp->max_state > 0) {
        ...
        kbdev->gpu_props.props.core_props.gpu_freq_khz_max = dp->freq_table[0] / 0x3e8;
    };
    ...
    if (err) {
        dev_err(kbdev->dev, "Failed to register OPP notifier (%d)\n", err);
...
#ifdef CONFIG_DEVFREQ_THERMAL
    if (of_find_compatible_node(kbdev->dev->of_node, NULL, "simple-power-model")) {
        of_property_read_u32(kbdev->dev->of_node, "dynamic-power-coefficient", (u32 *)&kbase_dcp->dyn_power_coeff);
        kbdev->model_data = rockchip_ipa_power_model_init(kbdev->dev, "gpu_leakage");
        if (IS_ERR_OR_NULL(kbdev->model_data)) {
            ...
            kbase_dcp->dyn_power_coeff = kbdev->model_data->dynamic_coefficient;
        }
        ...
        kbdev->devfreq_cooling = of_devfreq_cooling_register_power(kbdev->dev->of_node, kbdev->devfreq, kbase_dcp);
        ...
        kbdev->devfreq_cooling =
            of_devfreq_cooling_register_power(kbdev->dev->of_node, kbdev->devfreq, &kbase_ipa_power_model_ops);
        if (IS_ERR(kbdev->devfreq_cooling)) {
            err = PTR_ERR(kbdev->devfreq_cooling);
            dev_err(kbdev->dev, "Failed to register cooling device (%d)\n", err);
            goto cooling_failed;
        }
    }
#endif
