...
static void kbase_gpu_release_atom(struct kbase_device *kbdev, struct kbase_jd_atom *katom, ktime_t *end_timestamp)
{
    ...
    switch (katom->gpu_rb_state) {
        ...
        case KBASE_ATOM_GPU_RB_SUBMITTED:
            ...
            KBASE_TLSTREAM_TL_NRET_CTX_LPU(kbdev, kctx, &kbdev->gpu_props.props.raw_props.js_features[katom->slot_nr]);
...
static int kbase_jm_protected_entry(struct kbase_device *kbdev, struct kbase_jd_atom **katom, int idx, int js)
{
    int err = 0;
    ...
    switch (katom[idx]->protected_state.enter) {
        ...
        case KBASE_ATOM_ENTER_PROTECTED_HWCNT:
            ...
            if (!kbdev->protected_mode_hwcnt_disabled) {
                ...
                queue_work(system_wq, &kbdev->protected_mode_hwcnt_disable_work);
#else
                queue_work(system_highpri_wq, &kbdev->protected_mode_hwcnt_disable_work);
#endif
                return -EAGAIN;
            }
        ...
        case KBASE_ATOM_ENTER_PROTECTED_IDLE_L2:
            ...
            if (kbdev->system_coherency == COHERENCY_ACE) {
                ...
                if (kbase_pm_get_ready_cores(kbdev, KBASE_PM_CORE_L2) ||
                    kbase_pm_get_trans_cores(kbdev, KBASE_PM_CORE_L2) || kbase_is_gpu_removed(kbdev)) {
                    /*
        ...
        for (idx = 0; idx < SLOT_RB_SIZE; idx++) {
            ...
            switch (katom[idx]->gpu_rb_state) {
                ...
                case KBASE_ATOM_GPU_RB_READY:
                    ...
                    if (!kbasep_jm_is_js_free(kbdev, js, katom[idx]->kctx)) {
                        break;
                    }
#endif
...
bool kbase_gpu_irq_evict(struct kbase_device *kbdev, int js, u32 completion_code)
{
    struct kbase_jd_atom *katom;
    ...
    if (next_katom && katom->kctx == next_katom->kctx && next_katom->gpu_rb_state == KBASE_ATOM_GPU_RB_SUBMITTED &&
        (HAS_DEP(next_katom) || next_katom->sched_priority == katom->sched_priority) &&
        (kbase_reg_read(kbdev, JOB_SLOT_REG(js, JS_HEAD_NEXT_LO)) != 0 ||
         kbase_reg_read(kbdev, JOB_SLOT_REG(js, JS_HEAD_NEXT_HI)) != 0)) {
        kbase_reg_write(kbdev, JOB_SLOT_REG(js, JS_COMMAND_NEXT), JS_COMMAND_NOP);
        ...
        if (completion_code == BASE_JD_EVENT_STOPPED) {
            ...
            KBASE_TLSTREAM_TL_NRET_ATOM_AS(kbdev, next_katom, &kbdev->as[next_katom->kctx->as_nr]);
                           ...
                           ktime_t *end_timestamp)
{
    ...
    dev_dbg(kbdev->dev, "Atom %p completed on hw with code 0x%x and job_tail 0x%llx (s:%d)\n", (void *)katom,
            completion_code, job_tail, js);
    ...
    if ((katom->core_req & BASE_JD_REQ_SKIP_CACHE_END) && completion_code != BASE_JD_EVENT_DONE &&
    ...
    if (completion_code == BASE_JD_EVENT_STOPPED) {
        ...
        if (!kbase_ctx_flag(katom->kctx, KCTX_DYING)) {
            dev_warn(kbdev->dev, "error detected from slot %d, job status 0x%08x (%s)", js, completion_code,
                     kbase_gpu_exception_name(completion_code));
        }
    ...
    if (job_tail != 0 && job_tail != katom->jc) {
        ...
        dev_dbg(kbdev->dev, "Update job chain address of atom %p to resume from 0x%llx\n", (void *)katom, job_tail);
    ...
    if (katom) {
        dev_dbg(kbdev->dev, "Cross-slot dependency %p has become runnable.\n", (void *)katom);
...
void kbase_backend_reset(struct kbase_device *kbdev, ktime_t *end_timestamp)
{
    ...
    for (js = 0; js < kbdev->gpu_props.num_job_slots; js++) {
        ...
        for (idx = 0; idx < SLOT_RB_SIZE; idx++) {
            ...
            if ((katom->gpu_rb_state < KBASE_ATOM_GPU_RB_SUBMITTED) && !kbase_ctx_flag(katom->kctx, KCTX_DYING)) {
                keep_in_jm_rb = true;
            }
                                       ...
                                       struct kbase_jd_atom *katom, u32 action)
{
    ...
    if (katom_idx0_valid) {
        if (katom_idx0->gpu_rb_state != KBASE_ATOM_GPU_RB_SUBMITTED) {
            ...
            if (katom_idx1_valid && katom_idx1->gpu_rb_state == KBASE_ATOM_GPU_RB_SUBMITTED) {
                /* katom_idx0 and katom_idx1 are on GPU */

                if (kbase_reg_read(kbdev, JOB_SLOT_REG(js, JS_COMMAND_NEXT)) == 0) {
                    ...
                    if (kbase_reg_read(kbdev, JOB_SLOT_REG(js, JS_HEAD_NEXT_LO)) != 0 ||
                        kbase_reg_read(kbdev, JOB_SLOT_REG(js, JS_HEAD_NEXT_HI)) != 0) {
