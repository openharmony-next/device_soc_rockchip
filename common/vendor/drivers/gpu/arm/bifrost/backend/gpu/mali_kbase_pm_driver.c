...
MODULE_PARM_DESC(corestack_driver_control, "Let the driver power on/off the GPU core stack independently "
...
bool kbase_pm_is_mcu_desired(struct kbase_device *kbdev)
{
     ...
     * power policy.
     */
    return (kbdev->pm.backend.mcu_desired && (kbdev->pm.backend.pm_current_policy == &kbase_pm_always_on_policy_ops));
}
#endif
...
bool kbase_pm_is_l2_desired(struct kbase_device *kbdev)
{
    ...
    if (kbdev->pm.backend.protected_transition_override && kbdev->pm.backend.protected_l2_override) {
        return true;
    }
...
static void mali_cci_flush_l2(struct kbase_device *kbdev)
{
    ...
    raw = kbase_reg_read(kbdev, GPU_CONTROL_REG(GPU_IRQ_RAWSTAT));
                            ...
                            enum kbasep_pm_action action)
{
    ...
    if (kbase_dummy_job_wa_enabled(kbdev) && action == ACTION_PWRON && core_type == KBASE_PM_CORE_SHADER &&
...
static void kbase_pm_trigger_hwcnt_disable(struct kbase_device *kbdev)
{
    ...
    if (kbase_hwcnt_context_disable_atomic(kbdev->hwcnt_gpu_ctx)) {
        ...
        queue_work(system_wq, &backend->hwcnt_disable_work);
#else
        queue_work(system_highpri_wq, &backend->hwcnt_disable_work);
#endif
    }
}
...
static int kbase_pm_l2_update_state(struct kbase_device *kbdev)
{
        ...
        switch (backend->l2_state) {
            case KBASE_L2_OFF:
                if (kbase_pm_is_l2_desired(kbdev)) {
                     ...
                     */
                    kbase_pm_invoke(kbdev, KBASE_PM_CORE_TILER, tiler_present, ACTION_PWRON);

                    /* If we have more than one L2 cache then we
                     * must power them on explicitly.
                     */
                    if (l2_present != 1) {
                        kbase_pm_invoke(kbdev, KBASE_PM_CORE_L2, l2_present & ~1, ACTION_PWRON);
                    }
            ...
            case KBASE_L2_PEND_ON:
                if (!l2_trans && l2_ready == l2_present && !tiler_trans && tiler_ready == tiler_present) {
            ...
            case KBASE_L2_POWER_DOWN:
                if (!backend->l2_always_on) {
                     ...
                     * tiler.
                     */
                    kbase_pm_invoke(kbdev, KBASE_PM_CORE_L2, l2_present, ACTION_PWROFF);
                } else {
    ...
    if (kbdev->pm.backend.invoke_poweroff_wait_wq_when_l2_off && backend->l2_state == KBASE_L2_OFF) {
...
{
    unsigned long flags;
    struct kbasep_pm_tick_timer_state *stt = container_of(data, struct kbasep_pm_tick_timer_state, work);
    struct kbase_device *kbdev = container_of(stt, struct kbase_device, pm.backend.shader_tick_timer);
        ...
        switch (backend->shaders_state) {
            case KBASE_SHADERS_OFF_CORESTACK_OFF:
                ...
                if (backend->shaders_desired && backend->l2_state == KBASE_L2_ON) {
                    ...
                    if (backend->hwcnt_disabled) {
                        if (corestack_driver_control) {
                            kbase_pm_invoke(kbdev, KBASE_PM_CORE_STACK, stacks_avail, ACTION_PWRON);
                        }
            ...
            case KBASE_SHADERS_OFF_CORESTACK_PEND_ON:
                if (!stacks_trans && stacks_ready == stacks_avail) {
                    ...
                    kbase_pm_invoke(kbdev, KBASE_PM_CORE_SHADER, backend->shaders_avail, ACTION_PWRON);
            ...
            case KBASE_SHADERS_ON_CORESTACK_ON:
                ...
                if (!backend->shaders_desired || (backend->shaders_desired_mask != shaders_ready)) {
                    backend->hwcnt_desired = false;
                    if (!backend->hwcnt_disabled) {
                        kbase_pm_trigger_hwcnt_disable(kbdev);
                    }
                ...
                if (!backend->hwcnt_disabled) {
                    ...
                    if (kbdev->pm.backend.protected_transition_override ||
                        ...
                        if (!hrtimer_active(&stt->timer)) {
                            hrtimer_start(&stt->timer, stt->configured_interval, HRTIMER_MODE_REL);
                        }
                     ...
                     * check pass
                     */
                    backend->shaders_avail = (backend->shaders_desired_mask | shaders_ready);

                    kbase_pm_invoke(kbdev, KBASE_PM_CORE_SHADER, backend->shaders_avail & ~shaders_ready, ACTION_PWRON);
                    backend->shaders_state = KBASE_SHADERS_PEND_ON_CORESTACK_ON;
            ...
            case KBASE_SHADERS_PEND_OFF_CORESTACK_ON:
                if (!shaders_trans && !shaders_ready) {
                    if (corestack_driver_control) {
                        kbase_pm_invoke(kbdev, KBASE_PM_CORE_STACK, stacks_avail, ACTION_PWROFF);
                    }
...
static bool kbase_pm_is_in_desired_state_nolock(struct kbase_device *kbdev)
{
...
#if !MALI_USE_CSF
    if (kbdev->pm.backend.shaders_desired && kbdev->pm.backend.shaders_state != KBASE_SHADERS_ON_CORESTACK_ON) {
        in_desired_state = false;
    ...
    if (kbase_pm_is_mcu_desired(kbdev) && kbdev->pm.backend.mcu_state != KBASE_MCU_ON) {
        in_desired_state = false;
    } else if (!kbase_pm_is_mcu_desired(kbdev) && kbdev->pm.backend.mcu_state != KBASE_MCU_OFF) {
        in_desired_state = false;
    }
#endif
...
static void kbase_pm_trace_power_state(struct kbase_device *kbdev)
{
    ...
    KBASE_TLSTREAM_AUX_PM_STATE(kbdev, KBASE_PM_CORE_L2, kbase_pm_get_ready_cores(kbdev, KBASE_PM_CORE_L2));
    KBASE_TLSTREAM_AUX_PM_STATE(kbdev, KBASE_PM_CORE_SHADER, kbase_pm_get_ready_cores(kbdev, KBASE_PM_CORE_SHADER));
    KBASE_TLSTREAM_AUX_PM_STATE(kbdev, KBASE_PM_CORE_TILER, kbase_pm_get_ready_cores(kbdev, KBASE_PM_CORE_TILER));

    if (corestack_driver_control) {
        KBASE_TLSTREAM_AUX_PM_STATE(kbdev, KBASE_PM_CORE_STACK, kbase_pm_get_ready_cores(kbdev, KBASE_PM_CORE_STACK));
    }
}
...
void kbase_pm_update_state(struct kbase_device *kbdev)
{
    ...
    if (prev_mcu_state != KBASE_MCU_OFF && kbdev->pm.backend.mcu_state == KBASE_MCU_OFF) {
        if (kbase_pm_l2_update_state(kbdev)) {
            return;
        }
    }
...
{
    struct kbasep_pm_tick_timer_state *stt = container_of(timer, struct kbasep_pm_tick_timer_state, timer);
    struct kbase_device *kbdev = container_of(stt, struct kbase_device, pm.backend.shader_tick_timer);
    struct kbase_pm_backend_data *backend = &kbdev->pm.backend;
    unsigned long flags;
...
int kbase_pm_wait_for_desired_state(struct kbase_device *kbdev)
{
    ...
    if (!remaining) {
        ...
        dev_info(kbdev->dev, "Wait for desired PM state got interrupted");
        err = (int)remaining;
    }
...
void kbase_pm_clock_on(struct kbase_device *kbdev, bool is_resume)
{
    ...
    if (kbdev->dummy_job_wa.flags & KBASE_DUMMY_JOB_WA_FLAG_LOGICAL_SHADER_POWER) {
        spin_lock_irqsave(&kbdev->hwaccess_lock, flags);
        kbase_dummy_job_wa_execute(kbdev, kbase_pm_get_present_cores(kbdev, KBASE_PM_CORE_SHADER));
        spin_unlock_irqrestore(&kbdev->hwaccess_lock, flags);
    }
...
{
#if MALI_USE_CSF
    kbdev->hw_quirks_jm = kbase_reg_read(kbdev, GPU_CONTROL_REG(CSF_CONFIG));
#else
    u32 hw_quirks_jm = kbase_reg_read(kbdev, GPU_CONTROL_REG(JM_CONFIG));
    ...
    if (kbase_hw_has_feature(kbdev, BASE_HW_FEATURE_IDVS_GROUP_SIZE)) {
        ...
        if (of_property_read_u32(kbdev->dev->of_node, "idvs-group-size", &tmp)) {
            tmp = default_idvs_group_size;
        }

        if (tmp > IDVS_GROUP_MAX_SIZE) {
            dev_err(kbdev->dev, "idvs-group-size of %d is too large. Maximum value is %d", tmp, IDVS_GROUP_MAX_SIZE);
            tmp = default_idvs_group_size;
        }
...
{
    u32 hw_quirks_sc = kbase_reg_read(kbdev, GPU_CONTROL_REG(SHADER_CONFIG));
...
{
    u32 hw_quirks_tiler = kbase_reg_read(kbdev, GPU_CONTROL_REG(TILER_CONFIG));
...
static int kbase_pm_hw_issues_detect(struct kbase_device *kbdev)
{
    ...
    if (!of_property_read_u32(np, "quirks_jm", &kbdev->hw_quirks_jm)) {
        dev_info(kbdev->dev, "Found quirks_jm = [0x%x] in Devicetree\n", kbdev->hw_quirks_jm);
    } else {
    ...
    if (!of_property_read_u32(np, "quirks_sc", &kbdev->hw_quirks_sc)) {
    ...
    if (!of_property_read_u32(np, "quirks_tiler", &kbdev->hw_quirks_tiler)) {
    ...
    if (!of_property_read_u32(np, "quirks_mmu", &kbdev->hw_quirks_mmu)) {
...
static void kbase_pm_hw_issues_apply(struct kbase_device *kbdev)
{
    ...
    kbase_reg_write(kbdev, GPU_CONTROL_REG(TILER_CONFIG), kbdev->hw_quirks_tiler);

    kbase_reg_write(kbdev, GPU_CONTROL_REG(L2_MMU_CONFIG), kbdev->hw_quirks_mmu);
#if MALI_USE_CSF
    kbase_reg_write(kbdev, GPU_CONTROL_REG(CSF_CONFIG), kbdev->hw_quirks_jm);
#else
    kbase_reg_write(kbdev, GPU_CONTROL_REG(JM_CONFIG), kbdev->hw_quirks_jm);
#endif
}
...
static int kbase_pm_do_reset(struct kbase_device *kbdev)
{
    ...
    if (kbdev->pm.backend.callback_soft_reset) {
        ...
        kbase_reg_write(kbdev, GPU_CONTROL_REG(GPU_COMMAND), GPU_COMMAND_SOFT_RESET);
    }
...
{
    kbase_reg_write(kbdev, GPU_CONTROL_REG(GPU_COMMAND), GPU_COMMAND_SET_PROTECTED_MODE);
    return 0;
}
...
int kbase_pm_init_hw(struct kbase_device *kbdev, unsigned int flags)
{
    ...
    WARN_ON(kbase_reg_read(kbdev, GPU_CONTROL_REG(GPU_STATUS)) & GPU_STATUS_PROTECTED_MODE_ACTIVE);
