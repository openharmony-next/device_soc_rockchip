...
static int current_power_get(void *data, u64 *val)
{
     ...
     */
    kbase_get_real_power(df, &power, kbdev->current_nominal_freq, (kbdev->current_voltages[0] / 1000));
...
void kbase_ipa_debugfs_init(struct kbase_device *kbdev)
{
    ...
    debugfs_create_file("ipa_current_power", 0444, kbdev->mali_debugfs_directory, kbdev, &current_power);
    debugfs_create_file("ipa_force_fallback_model", 0644, kbdev->mali_debugfs_directory, kbdev, &force_fallback_model);
