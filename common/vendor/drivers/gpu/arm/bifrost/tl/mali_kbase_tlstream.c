 ...
 */
static void kbasep_packet_header_setup(char *buffer, enum tl_packet_family pkt_family, enum tl_packet_class pkt_class,
                                       enum tl_packet_type pkt_type, unsigned int stream_id, int numbered)
{
    u32 words[2] = {
...
void kbase_tlstream_init(struct kbase_tlstream *stream, enum tl_stream_type stream_type, wait_queue_head_t *ready_read)
{
    ...
    for (i = 0; i < PACKET_COUNT; i++) {
        kbasep_packet_header_setup(stream->buffer[i].data, tl_stream_cfg[stream_type].pkt_family,
                                            ...
                                            unsigned int wb_size)
{
    ...
    kbasep_packet_header_update(stream->buffer[wb_idx].data, wb_size - PACKET_HEADER_SIZE, stream->numbered);
