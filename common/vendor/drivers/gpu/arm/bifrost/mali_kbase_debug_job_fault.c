...
{
#if KERNEL_VERSION(4, 7, 0) <= LINUX_VERSION_CODE && KERNEL_VERSION(4, 15, 0) > LINUX_VERSION_CODE
    int ret = wait_event_interruptible_timeout(kbdev->job_fault_wq, kbase_is_job_fault_event_pending(kbdev),
...
#else
    return wait_event_interruptible(kbdev->job_fault_wq, kbase_is_job_fault_event_pending(kbdev));
#endif
}

/* wait until the fault happen and copy the event */
static int kbase_job_fault_event_wait(struct kbase_device *kbdev, struct base_job_fault_event *event)
{
    struct list_head *event_list = &kbdev->job_fault_event_list;
...
static void kbase_job_fault_resume_event_cleanup(struct kbase_context *kctx)
{
    ...
    while (!list_empty(event_list)) {
        ...
        event = kbase_job_fault_event_dequeue(kctx->kbdev, &kctx->job_fault_resume_event_list);
...
{
    struct base_job_fault_event *event = container_of(data, struct base_job_fault_event, job_fault_work);
...
static struct base_job_fault_event *kbase_job_fault_event_queue(struct list_head *event_list,
                                                                struct kbase_jd_atom *atom, u32 completion_code)
{
    struct base_job_fault_event *event;
...
static void kbase_job_fault_event_post(struct kbase_device *kbdev, struct kbase_jd_atom *katom, u32 completion_code)
{
    struct base_job_fault_event *event;
    unsigned long flags;
    ...
    dev_info(katom->kctx->kbdev->dev, "Job fault happen, start dump: %d_%d", katom->kctx->tgid, katom->kctx->id);
}
...
bool kbase_debug_job_fault_process(struct kbase_jd_atom *katom, u32 completion_code)
{
    ...
    if (atomic_read(&kctx->job_fault_count) > 0) {
        kbase_job_fault_event_queue(&kctx->job_fault_resume_event_list, katom, completion_code);
...
static int debug_job_fault_show(struct seq_file *m, void *v)
{
    ...
    dev_info(kbdev->dev, "debug job fault seq show:%d_%d, %d", kctx->tgid, kctx->id, event->reg_offset);
    ...
    for (i = 0; i < REGISTER_OFFSET_FLAG; i++) {
        if (kctx->reg_dump[event->reg_offset] == REGISTER_DUMP_TERMINATION_FLAG) {
            break;
        }
        seq_printf(m, "%08x: %08x\n", kctx->reg_dump[event->reg_offset], kctx->reg_dump[1 + event->reg_offset]);
        event->reg_offset += REGISTER_OFFSET_UNIT;
    }
...
{
    debugfs_create_file("job_fault", 0x100, kbdev->mali_debugfs_directory, kbdev, &kbasep_debug_job_fault_fops);
}
...
int kbase_debug_job_fault_dev_init(struct kbase_device *kbdev)
{
    ...
    kbdev->job_fault_resume_workq = alloc_workqueue("kbase_job_fault_resume_work_queue", WQ_MEM_RECLAIM, 1);
...
void kbase_debug_job_fault_context_init(struct kbase_context *kctx)
{
     ...
     */
    kctx->reg_dump = vmalloc(REGISTER_ADDRESS_NEED * REGISTER_ADDRESS_DOUBLE);
    if (kctx->reg_dump == NULL) {
        return;
    }

    if (kbase_debug_job_fault_reg_snapshot_init(kctx, REGISTER_ADDRESS_NEED) == false) {
        vfree(kctx->reg_dump);
        kctx->reg_dump = NULL;
    }
