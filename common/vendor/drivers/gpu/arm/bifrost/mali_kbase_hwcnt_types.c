 ...
 */
#define KBASE_HWCNT_ALIGN_UPWARDS(value, alignment) ((value) + (((alignment) - ((value) % (alignment))) % (alignment)))
                                ...
                                const struct kbase_hwcnt_metadata **out_metadata)
{
    ...
    for (grp = 0; grp < desc->grp_cnt; grp++) {
        ...
        struct kbase_hwcnt_block_metadata *blk_mds = (struct kbase_hwcnt_block_metadata *)(buf + offset);
        ...
        for (blk = 0; blk < grp_desc->blk_cnt; blk++) {
            ...
            blk_md->dump_buf_stride =
                KBASE_HWCNT_ALIGN_UPWARDS(n_values, (KBASE_HWCNT_BLOCK_BYTE_ALIGNMENT / KBASE_HWCNT_VALUE_BYTES));
     ...
     */
    WARN_ON(metadata->dump_buf_bytes != (metadata->enable_map_bytes * BITS_PER_BYTE * KBASE_HWCNT_VALUE_BYTES));
...
int kbase_hwcnt_enable_map_alloc(const struct kbase_hwcnt_metadata *metadata, struct kbase_hwcnt_enable_map *enable_map)
{
    u64 *enable_map_buf;
...
int kbase_hwcnt_dump_buffer_alloc(const struct kbase_hwcnt_metadata *metadata, struct kbase_hwcnt_dump_buffer *dump_buf)
{
                                        ...
                                        struct kbase_hwcnt_dump_buffer_array *dump_bufs)
{
    ...
    for (buf_idx = 0; buf_idx < n; buf_idx++) {
        ...
        const size_t clk_cnt_buf_offset = (dump_buf_bytes * n) + (clk_cnt_buf_bytes * buf_idx);
                                  ...
                                  const struct kbase_hwcnt_enable_map *dst_enable_map)
{
    ...
    if (WARN_ON(!dst) || WARN_ON(!dst_enable_map) || WARN_ON(dst->metadata != dst_enable_map->metadata)) {
        return;
    }
                                              ...
                                              const struct kbase_hwcnt_enable_map *dst_enable_map)
{
    ...
    kbase_hwcnt_metadata_for_each_block(metadata, grp, blk, blk_inst)
    {
        ...
        val_cnt = KBASE_HWCNT_ALIGN_UPWARDS(val_cnt, (KBASE_HWCNT_BLOCK_BYTE_ALIGNMENT / KBASE_HWCNT_VALUE_BYTES));
                                  ...
                                  const struct kbase_hwcnt_enable_map *dst_enable_map)
{
    ...
    if (WARN_ON(!dst) || WARN_ON(!src) || WARN_ON(!dst_enable_map) || WARN_ON(dst == src) ||
        WARN_ON(dst->metadata != src->metadata) || WARN_ON(dst->metadata != dst_enable_map->metadata)) {
        return;
    }
                                               ...
                                               const struct kbase_hwcnt_enable_map *dst_enable_map)
{
    ...
    kbase_hwcnt_metadata_for_each_block(metadata, grp, blk, blk_inst)
    {
        ...
        kbase_hwcnt_dump_buffer_block_accumulate_strict(dst_blk, src_blk, blk_em, hdr_cnt, ctr_cnt);
    }
