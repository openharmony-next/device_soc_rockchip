...
#define PRFCNT_BASE_LO                                                                                                 \
    0x060 /* (RW) Performance counter memory                                                                           \
           * region base address, low word                                                                             \
           */
#define PRFCNT_BASE_HI                                                                                                 \
    0x064 /* (RW) Performance counter memory                                                                           \
           * region base address, high word                                                                            \
           */
#define PRFCNT_CONFIG                                                                                                  \
    0x068 /* (RW) Performance counter                                                                                  \
           * configuration                                                                                             \
           */

#define PRFCNT_CSHW_EN                                                                                                 \
    0x06C /* (RW) Performance counter                                                                                  \
           * enable for Command Stream Hardware                                                                        \
           */

#define PRFCNT_SHADER_EN                                                                                               \
    0x070 /* (RW) Performance counter enable                                                                           \
           * flags for shader cores                                                                                    \
           */
#define PRFCNT_TILER_EN                                                                                                \
    0x074 /* (RW) Performance counter enable                                                                           \
           * flags for tiler                                                                                           \
           */
#define PRFCNT_MMU_L2_EN                                                                                               \
    0x07C /* (RW) Performance counter enable                                                                           \
...
#define GPU_COMMAND_CODE_PAYLOAD(opcode, payload) ((u32)(opcode) | ((u32)(payload) << 8))
...
#define GPU_COMMAND_SOFT_RESET GPU_COMMAND_CODE_PAYLOAD(GPU_COMMAND_CODE_RESET, GPU_COMMAND_RESET_PAYLOAD_SOFT_RESET)

/* Immediately reset the entire GPU. */
#define GPU_COMMAND_HARD_RESET GPU_COMMAND_CODE_PAYLOAD(GPU_COMMAND_CODE_RESET, GPU_COMMAND_RESET_PAYLOAD_HARD_RESET)

/* Clear all performance counters, setting them all to zero. */
#define GPU_COMMAND_PRFCNT_CLEAR GPU_COMMAND_CODE_PAYLOAD(GPU_COMMAND_CODE_PRFCNT, GPU_COMMAND_PRFCNT_PAYLOAD_CLEAR)

/* Sample all performance counters, writing them out to memory */
#define GPU_COMMAND_PRFCNT_SAMPLE GPU_COMMAND_CODE_PAYLOAD(GPU_COMMAND_CODE_PRFCNT, GPU_COMMAND_PRFCNT_PAYLOAD_SAMPLE)

/* Starts the cycle counter, and system timestamp propagation */
#define GPU_COMMAND_CYCLE_COUNT_START GPU_COMMAND_CODE_PAYLOAD(GPU_COMMAND_CODE_TIME, GPU_COMMAND_TIME_ENABLE)

/* Stops the cycle counter, and system timestamp propagation */
#define GPU_COMMAND_CYCLE_COUNT_STOP GPU_COMMAND_CODE_PAYLOAD(GPU_COMMAND_CODE_TIME, GPU_COMMAND_TIME_DISABLE)
...
#define GPU_COMMAND_SET_PROTECTED_MODE GPU_COMMAND_CODE_PAYLOAD(GPU_COMMAND_CODE_SET_PROTECTED_MODE, 0)
...
#define GPU_FAULTSTATUS_ADDR_VALID_FLAG (1ul << GPU_FAULTSTATUS_ADDR_VALID_SHIFT)

#define GPU_FAULTSTATUS_JASID_VALID_SHIFT 11
#define GPU_FAULTSTATUS_JASID_VALID_FLAG (1ul << GPU_FAULTSTATUS_JASID_VALID_SHIFT)

#define GPU_FAULTSTATUS_JASID_SHIFT 12
#define GPU_FAULTSTATUS_JASID_MASK (0xF << GPU_FAULTSTATUS_JASID_SHIFT)
#define GPU_FAULTSTATUS_JASID_GET(reg_val) (((reg_val)&GPU_FAULTSTATUS_JASID_MASK) >> GPU_FAULTSTATUS_JASID_SHIFT)
 ...
 */
#define GPU_IRQ_REG_COMMON (GPU_FAULT | GPU_PROTECTED_FAULT | RESET_COMPLETED | POWER_CHANGED_ALL | MCU_STATUS_GPU_IRQ)
