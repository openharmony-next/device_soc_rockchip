        ...
        struct kbase_mmu_setup * const setup)
{
    ...
    setup->memattr =
        (AS_MEMATTR_IMPL_DEF_CACHE_POLICY <<
            (AS_MEMATTR_INDEX_IMPL_DEF_CACHE_POLICY * GPU_FAULTSTATUS_ACCESS_TYPE_SHIFT)) |
        (AS_MEMATTR_FORCE_TO_CACHE_ALL    <<
            (AS_MEMATTR_INDEX_FORCE_TO_CACHE_ALL * GPU_FAULTSTATUS_ACCESS_TYPE_SHIFT)) |
        (AS_MEMATTR_WRITE_ALLOC           <<
            (AS_MEMATTR_INDEX_WRITE_ALLOC * GPU_FAULTSTATUS_ACCESS_TYPE_SHIFT)) |
        (AS_MEMATTR_AARCH64_OUTER_IMPL_DEF   <<
            (AS_MEMATTR_INDEX_OUTER_IMPL_DEF * GPU_FAULTSTATUS_ACCESS_TYPE_SHIFT)) |
        (AS_MEMATTR_AARCH64_OUTER_WA         <<
            (AS_MEMATTR_INDEX_OUTER_WA * GPU_FAULTSTATUS_ACCESS_TYPE_SHIFT)) |
        (AS_MEMATTR_AARCH64_NON_CACHEABLE    <<
            (AS_MEMATTR_INDEX_NON_CACHEABLE * GPU_FAULTSTATUS_ACCESS_TYPE_SHIFT));
        ...
        struct kbase_as *as, struct kbase_fault *fault)
{
    ...
    u32 const exception_type = (status & GPU_FAULTSTATUS_EXCEPTION_TYPE_MASK);
    u32 const exception_data = (status >> GPU_FAULTSTATUS_ACCESS_TYPE_SHIFT) & 0xFFFFFF;
        ...
        struct kbase_fault *fault)
{
    ...
    access_type = (fault->status >> GPU_FAULTSTATUS_ACCESS_TYPE_SHIFT) & 0x3;
    source_id = (fault->status >> GPU_FAULTSTATUS_SOURCE_ID_SHIFT);
...
{
    const int num_as = GPU_FAULTSTATUS_SOURCE_ID_SHIFT;
    ...
    while (bf_bits | pf_bits) {
        ...
        if (kbase_hw_has_feature(kbdev, BASE_HW_FEATURE_AARCH64_MMU)) {
            ...
            fault->extra_addr <<= AS_FAULTADDRESS_LO;
            fault->extra_addr |= kbase_reg_read(kbdev,
