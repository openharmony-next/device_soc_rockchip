 ...
 */
static void kbase_mmu_flush_invalidate(struct kbase_context *kctx, u64 vpfn, size_t nr, bool sync);
 ...
 */
static void kbase_mmu_flush_invalidate_no_ctx(struct kbase_device *kbdev, u64 vpfn, size_t nr, bool sync, int as_nr);
...
static void kbase_mmu_sync_pgd(struct kbase_device *kbdev, dma_addr_t handle, size_t size)
{
...
static int kbase_mmu_update_pages_no_flush(struct kbase_context *kctx, u64 vpfn, struct tagged_addr *phys, size_t nr,
 ...
 */
static size_t reg_grow_calc_extra_pages(struct kbase_device *kbdev, struct kbase_va_region *reg, size_t fault_rel_pfn)
{
...
static void kbase_gpu_mmu_handle_write_faulting_as(struct kbase_device *kbdev, struct kbase_as *faulting_as,
                                                   u64 start_pfn, size_t nr, u32 op)
{
    mutex_lock(&kbdev->mmu_hw_mutex);

    kbase_mmu_hw_clear_fault(kbdev, faulting_as, KBASE_MMU_FAULT_TYPE_PAGE);
    kbase_mmu_hw_do_operation(kbdev, faulting_as, start_pfn, nr, op, 1);
...
static void kbase_gpu_mmu_handle_write_fault(struct kbase_context *kctx, struct kbase_as *faulting_as)
{
    ...
    if (kbase_is_region_invalid_or_free(region)) {
        ...
        kbase_mmu_report_fault_and_kill(kctx, faulting_as, "Memory is not mapped on the GPU", &faulting_as->pf_data);
        return;
    }

    if (!(region->flags & KBASE_REG_GPU_WR)) {
        kbase_gpu_vm_unlock(kctx);
        kbase_mmu_report_fault_and_kill(kctx, faulting_as, "Region does not have write permissions",
                                        &faulting_as->pf_data);
        return;
    }
    ...
    ret = kbase_mmu_update_pages_no_flush(kctx, fault_pfn, &kbase_get_gpu_phy_pages(region)[pfn_offset], 1,
                                          region->flags, region->gpu_alloc->group_id);

    /* flush L2 and unlock the VA (resumes the MMU) */
    op = AS_COMMAND_FLUSH_PT;

    kbase_gpu_mmu_handle_write_faulting_as(kbdev, faulting_as, fault_pfn, 1, op);

    kbase_gpu_vm_unlock(kctx);
}

static void kbase_gpu_mmu_handle_permission_fault(struct kbase_context *kctx, struct kbase_as *faulting_as)
{
    ...
    switch (AS_FAULTSTATUS_ACCESS_TYPE_GET(fault->status)) {
        ...
        case AS_FAULTSTATUS_ACCESS_TYPE_EX:
            kbase_mmu_report_fault_and_kill(kctx, faulting_as, "Execute Permission fault", fault);
            break;
        case AS_FAULTSTATUS_ACCESS_TYPE_READ:
            kbase_mmu_report_fault_and_kill(kctx, faulting_as, "Read Permission fault", fault);
            break;
        default:
            kbase_mmu_report_fault_and_kill(kctx, faulting_as, "Unknown Permission fault", fault);
            break;
    }
}
#endif
 ...
 */
static bool page_fault_try_alloc(struct kbase_context *kctx, struct kbase_va_region *region, size_t new_pages,
                                 int *pages_to_grow, bool *grow_2mb_pool, struct kbase_sub_alloc **prealloc_sas)
{
    ...
    for (pool_level = 0; pool_level < MAX_POOL_LEVEL; pool_level++) {
        ...
        if (pages_to_alloc_4k) {
            gpu_pages[pool_level] = kbase_alloc_phy_pages_helper_locked(region->gpu_alloc, pool,
                                                                        pages_to_alloc_4k_per_alloc, &prealloc_sas[0]);

            if (!gpu_pages[pool_level]) {
                alloc_failed = true;
            } else if (region->gpu_alloc != region->cpu_alloc) {
                cpu_pages[pool_level] = kbase_alloc_phy_pages_helper_locked(
                    region->cpu_alloc, pool, pages_to_alloc_4k_per_alloc, &prealloc_sas[1]);
    ...
    if (new_pages) {
        ...
        for (pool_level = 0; pool_level <= max_pool_level; pool_level++) {
            ...
            if (region->gpu_alloc != region->cpu_alloc) {
                if (pages_alloced[pool_level] && cpu_pages[pool_level]) {
                    kbase_free_phy_pages_helper_locked(region->cpu_alloc, pool, cpu_pages[pool_level],
                                                       pages_alloced[pool_level]);
                }
            }

            if (pages_alloced[pool_level] && gpu_pages[pool_level]) {
                kbase_free_phy_pages_helper_locked(region->gpu_alloc, pool, gpu_pages[pool_level],
                                                   pages_alloced[pool_level]);
            }
...
void kbase_mmu_page_fault_worker(struct work_struct *data)
{
    ...
    kbdev = container_of(faulting_as, struct kbase_device, as[as_no]);
    dev_dbg(kbdev->dev, "Entering %s %p, fault_pfn %lld, as_no %d\n", __func__, (void *)data, fault_pfn, as_no);
    ...
    switch (fault_status & AS_FAULTSTATUS_EXCEPTION_CODE_MASK) {
        ...
        case AS_FAULTSTATUS_EXCEPTION_CODE_TRANSTAB_BUS_FAULT:
            kbase_mmu_report_fault_and_kill(kctx, faulting_as, "Translation table bus fault", fault);
            goto fault_done;
        ...
        case AS_FAULTSTATUS_EXCEPTION_CODE_MEMORY_ATTRIBUTES_FAULT:
            if (kbase_hw_has_feature(kbdev, BASE_HW_FEATURE_AARCH64_MMU)) {
                kbase_mmu_report_fault_and_kill(kctx, faulting_as, "Memory attributes fault", fault);
            } else {
                kbase_mmu_report_fault_and_kill(kctx, faulting_as, "Unknown fault code", fault);
            }
            goto fault_done;
    ...
    for (i = 0; i != ARRAY_SIZE(prealloc_sas); ++i) {
        ...
        if (!prealloc_sas[i]) {
            kbase_mmu_report_fault_and_kill(kctx, faulting_as,
                                            "Failed pre-allocating memory for sub-allocations' metadata", fault);
            goto fault_done;
        }
    }
    ...
    while (1) {
        ...
        if (kbase_is_region_invalid_or_free(region)) {
            kbase_gpu_vm_unlock(kctx);
            kbase_mmu_report_fault_and_kill(kctx, faulting_as, "Memory is not mapped on the GPU", fault);
            goto fault_done;
        }

        if (region->gpu_alloc->type == KBASE_MEM_TYPE_IMPORTED_UMM) {
            kbase_gpu_vm_unlock(kctx);
            kbase_mmu_report_fault_and_kill(kctx, faulting_as, "DMA-BUF is not mapped on the GPU", fault);
            goto fault_done;
        }

        if (region->gpu_alloc->group_id >= MEMORY_GROUP_MANAGER_NR_GROUPS) {
            kbase_gpu_vm_unlock(kctx);
            kbase_mmu_report_fault_and_kill(kctx, faulting_as, "Bad physical memory group ID", fault);
            goto fault_done;
        }
        ...
        if ((region->flags & KBASE_REG_DONT_NEED)) {
            kbase_gpu_vm_unlock(kctx);
            kbase_mmu_report_fault_and_kill(kctx, faulting_as, "Don't need memory can't be grown", fault);
            goto fault_done;
        }
        ...
        if (fault_rel_pfn < current_backed_size) {
             ...
             * raised again).
             */
            kbase_mmu_hw_do_operation(kbdev, faulting_as, 0, 0, AS_COMMAND_UNLOCK, 1);
        ...
        grown = page_fault_try_alloc(kctx, region, new_pages, &pages_to_grow, &grow_2mb_pool, prealloc_sas);
        ...
        if (grown) {
             ...
             */
            err = kbase_mmu_insert_pages_no_flush(kbdev, &kctx->mmu, region->start_pfn + pfn_offset,
            ...
            if (err) {
                 ...
                 * invalidated in here
                 */
                kbase_mmu_report_fault_and_kill(kctx, faulting_as, "Page table update failure", fault);
                goto fault_done;
            }
            ...
            if (region->threshold_pages && kbase_reg_current_backed_size(region) > region->threshold_pages) {
                ...
                if (kbase_mmu_switch_to_ir(kctx, region) >= 0) {
                    dev_dbg(kctx->kbdev->dev, "Get region %p for IR\n", (void *)region);
            ...
            kbase_mmu_hw_do_operation(kbdev, faulting_as, fault->addr >> PAGE_SHIFT, new_pages, op, 1);
            ...
            if (pages_to_grow > 0) {
                ...
                if (grow_2mb_pool) {
                    ...
                    struct kbase_mem_pool *const lp_mem_pool = &kctx->mem_pools.large[region->gpu_alloc->group_id];

                    pages_to_grow = (pages_to_grow + ((1 << lp_mem_pool->order) - 1)) >> lp_mem_pool->order;

                    ret = kbase_mem_pool_grow(lp_mem_pool, pages_to_grow);
                } else {
#endif
                    struct kbase_mem_pool *const mem_pool = &kctx->mem_pools.small[region->gpu_alloc->group_id];
            ...
            if (ret < 0) {
                ...
                kbase_mmu_report_fault_and_kill(kctx, faulting_as, "Page allocation failure", fault);
            } else {
 ...
 */
static int mmu_get_pgd_at_level(struct kbase_device *kbdev, struct kbase_mmu_table *mmut, u64 vpfn, int level,
                                phys_addr_t *out_pgd)
{
    phys_addr_t pgd;
    int l;
    ...
    for (l = MIDGARD_MMU_TOPLEVEL; l < level; l++) {
        ...
        if (err) {
            dev_dbg(kbdev->dev, "%s: mmu_get_next_pgd failure at level %d\n", __func__, l);
            return err;
        }
    }
...
static int mmu_get_bottom_pgd(struct kbase_device *kbdev, struct kbase_mmu_table *mmut, u64 vpfn, phys_addr_t *out_pgd)
{
    return mmu_get_pgd_at_level(kbdev, mmut, vpfn, MIDGARD_MMU_BOTTOMLEVEL, out_pgd);
}

static void mmu_insert_pages_failure_recovery(struct kbase_device *kbdev, struct kbase_mmu_table *mmut, u64 from_vpfn,
                                              u64 to_vpfn)
{
    phys_addr_t pgd;
    u64 vpfn = from_vpfn;
    ...
    while (vpfn < to_vpfn) {
        ...
        kbase_mmu_sync_pgd(kbdev, kbase_dma_addr(phys_to_page(pgd)) + 0x8 * idx, 0x8 * pcount);
        kunmap(phys_to_page(pgd));
 ...
 */
int kbase_mmu_insert_single_page(struct kbase_context *kctx, u64 vpfn, struct tagged_addr phys, size_t nr,
                                 unsigned long flags, int const group_id)
{
    ...
    while (remain) {
        ...
        do {
            ...
            err = kbase_mem_pool_grow(&kbdev->mem_pools.small[kctx->mmu.group_id], MIDGARD_MMU_BOTTOMLEVEL);
            mutex_lock(&kctx->mmu.mmu_lock);
        } while (!err);
        if (err) {
            ...
            if (recover_required) {
                 ...
                 * completed
                 */
                mmu_insert_pages_failure_recovery(kbdev, &kctx->mmu, start_vpfn, start_vpfn + recover_count);
            }
            goto fail_unlock;
        }
        ...
        kbase_mmu_sync_pgd(kbdev, kbase_dma_addr(p) + (index * sizeof(u64)), count * sizeof(u64));
...
static inline void cleanup_empty_pte(struct kbase_device *kbdev, struct kbase_mmu_table *mmut, u64 *pte)
{
    phys_addr_t tmp_pgd;
    struct page *tmp_p;
    ...
    kbase_mem_pool_free(&kbdev->mem_pools.small[mmut->group_id], tmp_p, false);
...
u64 kbase_mmu_create_ate(struct kbase_device *const kbdev, struct tagged_addr const phy, unsigned long const flags,
...
int kbase_mmu_insert_pages_no_flush(struct kbase_device *kbdev, struct kbase_mmu_table *mmut, const u64 start_vpfn,
                                    struct tagged_addr *phys, size_t nr, unsigned long flags, int const group_id)
{
    phys_addr_t pgd;
    u64 *pgd_page;
    ...
    while (remain) {
        ...
        if (!vindex && is_huge_head(*phys)) {
            cur_level = MIDGARD_MMU_LEVEL(0x2);
        } else {
        ...
        do {
            ...
            err = kbase_mem_pool_grow(&kbdev->mem_pools.small[mmut->group_id], cur_level);
            mutex_lock(&mmut->mmu_lock);
        } while (!err);
        ...
        if (err) {
            ...
            if (insert_vpfn != start_vpfn) {
                 ...
                 * completed
                 */
                mmu_insert_pages_failure_recovery(kbdev, mmut, start_vpfn, insert_vpfn);
            }
            goto fail_unlock;
        }
        ...
        if (cur_level == MIDGARD_MMU_LEVEL(0x2)) {
            int level_index = (insert_vpfn >> 0x9) & 0x1FF;
            u64 *target = &pgd_page[level_index];

            if (mmu_mode->pte_is_valid(*target, cur_level)) {
                cleanup_empty_pte(kbdev, mmut, target);
            }
            *target = kbase_mmu_create_ate(kbdev, *phys, flags, cur_level, group_id);
        } else {
            for (i = 0; i < count; i++) {
                ...
                *target = kbase_mmu_create_ate(kbdev, phys[i], flags, cur_level, group_id);
            }
        }
        ...
        kbase_mmu_sync_pgd(kbdev, kbase_dma_addr(p) + (vindex * sizeof(u64)), count * sizeof(u64));
 ...
 * number 'as_nr'.
 */
int kbase_mmu_insert_pages(struct kbase_device *kbdev, struct kbase_mmu_table *mmut, u64 vpfn, struct tagged_addr *phys,
                           size_t nr, unsigned long flags, int as_nr, int const group_id)
{
    int err;
 ...
 * other locking.
 */
static void kbase_mmu_flush_invalidate_noretain(struct kbase_context *kctx, u64 vpfn, size_t nr, bool sync)
{
    ...
    err = kbase_mmu_hw_do_operation(kbdev, &kbdev->as[kctx->as_nr], vpfn, nr, op, 0);
    if (err) {
 ...
 */
static void kbase_mmu_flush_invalidate_as(struct kbase_device *kbdev, struct kbase_as *as, u64 vpfn, size_t nr,
                                          bool sync)
{
    int err;
    u32 op;
...
static void kbase_mmu_flush_invalidate_no_ctx(struct kbase_device *kbdev, u64 vpfn, size_t nr, bool sync, int as_nr)
{
    ...
    if (nr) {
        kbase_mmu_flush_invalidate_as(kbdev, &kbdev->as[as_nr], vpfn, nr, sync);
    }
}
...
static void kbase_mmu_flush_invalidate(struct kbase_context *kctx, u64 vpfn, size_t nr, bool sync)
{
    ...
    if (ctx_is_in_runpool) {
        ...
        kbase_mmu_flush_invalidate_as(kbdev, &kbdev->as[kctx->as_nr], vpfn, nr, sync);
 ...
 * information.
 */
int kbase_mmu_teardown_pages(struct kbase_device *kbdev, struct kbase_mmu_table *mmut, u64 vpfn, size_t nr, int as_nr)
{
    phys_addr_t pgd;
    u64 start_vpfn = vpfn;
    ...
    while (nr) {
        ...
        for (level = MIDGARD_MMU_TOPLEVEL; level <= MIDGARD_MMU_BOTTOMLEVEL; level++) {
            ...
            index = (vpfn >> ((0x3 - level) * 0x9)) & 0x1FF;
            page = kmap(phys_to_page(pgd));
            if (mmu_mode->ate_is_valid(page[index], level)) {
                ...
                switch (level) {
                    case MIDGARD_MMU_LEVEL(0x0):
                        count = 0x8000000;
                        break;
                    case MIDGARD_MMU_LEVEL(0x1):
                        count = 0x40000;
                        break;
                    case MIDGARD_MMU_LEVEL(0x2):
                        count = 0x200;
                        break;
                    case MIDGARD_MMU_LEVEL(0x3):
                        count = 0x1;
                        break;
                }
                if (count > nr) {
        ...
        switch (level) {
            ...
            case MIDGARD_MMU_LEVEL(1):
                dev_warn(kbdev->dev, "%s: No support for ATEs at level %d\n", __func__, level);
                kunmap(phys_to_page(pgd));
                goto out;
            case MIDGARD_MMU_LEVEL(0x2):
                ...
                if (count >= 0x200) {
                    ...
                    dev_warn(kbdev->dev,
                             "%s: limiting teardown as it tries to do a partial 2MB teardown, need 512, but have %d to "
                             "tear down\n",
                             __func__, count);
                    pcount = 0;
                }
            ...
            case MIDGARD_MMU_BOTTOMLEVEL:
                ...
                break;
            default:
                dev_err(kbdev->dev, "%s: found non-mapped memory, early out\n", __func__);
                vpfn += count;
                nr -= count;
                continue;
        }
        ...
        kbase_mmu_sync_pgd(kbdev, kbase_dma_addr(phys_to_page(pgd)) + 0x8 * index, 0x8 * pcount);
                                           ...
                                           unsigned long flags, int const group_id)
{
    ...
    while (nr) {
        ...
        if (err) {
            dev_warn(kbdev->dev, "mmu_get_bottom_pgd failure\n");
            goto fail_unlock;
        }
        ...
        for (i = 0; i < count; i++) {
            pgd_page[index + i] = kbase_mmu_create_ate(kbdev, phys[i], flags, MIDGARD_MMU_BOTTOMLEVEL, group_id);
        }
...
int kbase_mmu_update_pages(struct kbase_context *kctx, u64 vpfn, struct tagged_addr *phys, size_t nr,
                           unsigned long flags, int const group_id)
{
    int err;

    err = kbase_mmu_update_pages_no_flush(kctx, vpfn, phys, nr, flags, group_id);
    kbase_mmu_flush_invalidate(kctx, vpfn, nr, true);
    return err;
}

static void mmu_teardown_level(struct kbase_device *kbdev, struct kbase_mmu_table *mmut, phys_addr_t pgd, int level,
                               u64 *pgd_page_buffer)
{
    phys_addr_t target_pgd;
    struct page *p;
                   ...
                   struct kbase_context *const kctx, int const group_id)
{
    ...
    mmut->mmu_teardown_pages = kmalloc(PAGE_SIZE * 0x4, GFP_KERNEL);
    ...
    while (!mmut->pgd) {
        ...
        err = kbase_mem_pool_grow(&kbdev->mem_pools.small[mmut->group_id], MIDGARD_MMU_BOTTOMLEVEL);
        if (err) {
...
void kbase_mmu_term(struct kbase_device *kbdev, struct kbase_mmu_table *mmut)
{
    if (mmut->pgd) {
        mutex_lock(&mmut->mmu_lock);
        mmu_teardown_level(kbdev, mmut, mmut->pgd, MIDGARD_MMU_TOPLEVEL, mmut->mmu_teardown_pages);
        mutex_unlock(&mmut->mmu_lock);
...
static size_t kbasep_mmu_dump_level(struct kbase_context *kctx, phys_addr_t pgd, int level, char **const buffer,
                                    size_t *size_left)
{
    phys_addr_t target_pgd;
    u64 *pgd_page;
...
void *kbase_mmu_dump(struct kbase_context *kctx, int nr_pages)
{
    ...
    if (kaddr) {
        ...
        config[0x0] = as_setup.transtab;
        config[0x1] = as_setup.memattr;
        config[0x2] = as_setup.transcfg;
