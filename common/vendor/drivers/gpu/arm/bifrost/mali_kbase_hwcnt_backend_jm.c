 ...
 */
static void kbasep_hwcnt_backend_jm_on_freq_change(struct kbase_clk_rate_listener *rate_listener, u32 clk_index,
                                                   u32 clk_rate_hz)
{
...
static void kbasep_hwcnt_backend_jm_cc_enable(struct kbase_hwcnt_backend_jm *backend_jm,
                                              const struct kbase_hwcnt_enable_map *enable_map, u64 timestamp_ns)
{
...
static int kbasep_hwcnt_backend_jm_dump_enable_nolock(struct kbase_hwcnt_backend *backend,
                                                      const struct kbase_hwcnt_enable_map *enable_map)
{
    ...
    if (!backend_jm || !enable_map || backend_jm->enabled || (enable_map->metadata != backend_jm->info->metadata)) {
        return -EINVAL;
    }
...
static int kbasep_hwcnt_backend_jm_dump_request(struct kbase_hwcnt_backend *backend, u64 *dump_time_ns)
{
    struct kbase_hwcnt_backend_jm *backend_jm = (struct kbase_hwcnt_backend_jm *)backend;
    struct kbase_device *kbdev;
    const struct kbase_hwcnt_metadata *metadata;
        ...
        *dump_time_ns = kbasep_hwcnt_backend_jm_timestamp_ns(backend);
        ...
        kbase_hwcnt_metadata_for_each_clock(metadata, clk)
        {
            ...
            if (clk == KBASE_CLOCK_DOMAIN_TOP) {
                /* Read cycle count for top clock domain. */
                kbase_backend_get_gpu_time_norequest(kbdev, &current_cycle_count, NULL, NULL);
            } else {
                /*
                 * Estimate cycle count for non-top clock
                 * domain.
                 */
                current_cycle_count = kbase_ccswe_cycle_at(&backend_jm->ccswe_shader_cores, *dump_time_ns);
            }
            backend_jm->cycle_count_elapsed[clk] = current_cycle_count - backend_jm->prev_cycle_count[clk];
...
static int kbasep_hwcnt_backend_jm_dump_get(struct kbase_hwcnt_backend *backend, struct kbase_hwcnt_dump_buffer *dst,
                                            const struct kbase_hwcnt_enable_map *dst_enable_map, bool accumulate)
{
    struct kbase_hwcnt_backend_jm *backend_jm = (struct kbase_hwcnt_backend_jm *)backend;
    size_t clk;

    if (!backend_jm || !dst || !dst_enable_map || (backend_jm->info->metadata != dst->metadata) ||
    ...
    return kbase_hwcnt_gpu_dump_get(dst, backend_jm->cpu_dump_va, dst_enable_map, backend_jm->pm_core_mask, accumulate);
}
                                          ...
                                          struct kbase_hwcnt_backend_jm **out_backend)
{
    ...
    backend->kctx = kbase_create_context(kbdev, true, BASE_CONTEXT_SYSTEM_MONITOR_SUBMIT_DISABLED, 0, NULL);
    if (!backend->kctx) {
        goto alloc_error;
    }
    ...
    backend->cpu_dump_va = kbase_phy_alloc_mapping_get(backend->kctx, backend->gpu_dump_va, &backend->vmap);
    if (!backend->cpu_dump_va) {
        goto alloc_error;
    }
                                        ...
                                        struct kbase_hwcnt_backend **out_backend)
{
    ...
    errcode = kbasep_hwcnt_backend_jm_create((const struct kbase_hwcnt_backend_jm_info *)info, &backend);
    if (errcode) {
        return errcode;
    }
                                               ...
                                               const struct kbase_hwcnt_backend_jm_info **out_info)
{
    ...
    errcode = kbase_hwcnt_gpu_metadata_create(&hwcnt_gpu_info, info->use_secondary, &info->metadata, &info->dump_bytes);
    if (errcode) {
        goto error;
    }
...
void kbase_hwcnt_backend_jm_destroy(struct kbase_hwcnt_backend_interface *iface)
{
    ...
    kbasep_hwcnt_backend_jm_info_destroy((const struct kbase_hwcnt_backend_jm_info *)iface->info);
