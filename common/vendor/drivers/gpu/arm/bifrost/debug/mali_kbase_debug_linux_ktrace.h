...
DECLARE_EVENT_CLASS(mali_add_template, TP_PROTO(struct kbase_context *kctx, u64 info_val), TP_ARGS(kctx, info_val),
                    TP_STRUCT__entry(__field(pid_t, kctx_tgid) __field(u32, kctx_id) __field(u64, info_val)),
                    TP_fast_assign(__entry->kctx_id = (kctx) ? kctx->id : 0u;
                                   __entry->kctx_tgid = (kctx) ? kctx->tgid : 0; __entry->info_val = info_val;),
                    TP_printk("kctx=%d_%u info=0x%llx", __entry->kctx_tgid, __entry->kctx_id, __entry->info_val));

/* DEFINE_MALI_ADD_EVENT is available also to backends for backend-specific
 * simple trace codes
 */
#define DEFINE_MALI_ADD_EVENT(name)                                                                                    \
    DEFINE_EVENT(mali_add_template, mali_##name, TP_PROTO(struct kbase_context *kctx, u64 info_val),                   \
