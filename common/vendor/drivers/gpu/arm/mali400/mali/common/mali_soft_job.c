...
void mali_soft_job_destroy(struct mali_soft_job *job)
{
    ...
    if (NULL != job) {
        if (0 < mali_osk_atomic_dec_return(&job->refcount)) return;
...
struct mali_soft_job *mali_soft_job_create(struct mali_soft_job_system *system, mali_soft_job_type type, u64 user_job)
{
    ...
    notification = _mali_osk_notification_create(MALI_NOTIFICATION_SOFT_ACTIVATED, sizeof(mali_uk_soft_job_activated_s));
