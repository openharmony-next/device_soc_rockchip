...
{
    mali_osk_atomic_init(&pp_counter_per_sub_job_count, 0);
}
                       ...
                       mali_uk_pp_start_job_s __user *uargs, u32 id)
{
    ...
    if (NULL != job) {
        ...
        if (0 != _mali_osk_copy_from_user(&job->uargs, uargs, sizeof(mali_uk_pp_start_job_s))) {
            goto fail;
        }

        if (job->uargs.num_cores > MALI_PP_MAX_SUB_JOBS) {
        ...
        if (!((perf_counter_flag & MALI_PERFORMANCE_COUNTER_FLAG_SRC0_ENABLE) ||
              (perf_counter_flag & MALI_PERFORMANCE_COUNTER_FLAG_SRC1_ENABLE))) {
            u32 sub_job_count = mali_osk_atomic_read(&pp_counter_per_sub_job_count);
        ...
        job->tid = mali_osk_get_tid();

        mali_osk_atomic_init(&job->sub_jobs_completed, 0);
        mali_osk_atomic_init(&job->sub_job_errors, 0);
        ...
        if (job->uargs.num_memory_cookies > 0) {
            ...
            job->memory_cookies = mali_osk_malloc(size);
        ...
        if (MALI_OSK_ERR_OK != mali_pp_job_check(job)) {
            /* Not a valid job. */
            goto fail;
        }
...
void mali_pp_job_delete(struct mali_pp_job *job)
{
    ...
    mali_osk_atomic_term(&job->sub_jobs_completed);
    mali_osk_atomic_term(&job->sub_job_errors);
    mali_osk_atomic_dec(&session->number_of_pp_jobs);
    _mali_osk_free(job);
