...
struct mali_pm_domain *mali_pm_domain_create(u32 pmu_mask)
{
    ...
    if (NULL != domain) {
        ...
        domain->use_count = 0;
        mali_osk_list_init(&domain->group_list);
        mali_osk_list_init(&domain->l2_cache_list);
...
void mali_pm_domain_delete(struct mali_pm_domain *domain)
{
    ...
    mali_osk_list_delinit(&domain->group_list);
    mali_osk_list_delinit(&domain->l2_cache_list);
...
u32 mali_pm_domain_ref_get(struct mali_pm_domain *domain)
{
    ...
    ++domain->use_count;
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_CODE, ("PM domain %p: ref_get, use_count => %u\n", domain, domain->use_count));
