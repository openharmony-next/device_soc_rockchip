...
static u32 calculate_window_render_fps(u64 time_period)
{
     ...
     * equally first.
     */
    leading_zeroes = _mali_osk_clz((u32)(max >> MALI_OSK_CLZ_START));
    shift_val = MALI_OSK_CLZ_START - leading_zeroes;
...
void mali_dvfs_policy_realize(struct mali_gpu_utilization_data *data, u64 time_period)
{
    ...
    if ((mali_desired_fps <= current_fps) && (current_fps < mali_max_system_fps)) {
        under_perform_boundary_value = MALI_PERCENTAGE_TO_UTILIZATION_FRACTION(MALI_PERCENTAGE_NINETY);
        over_perform_boundary_value = MALI_PERCENTAGE_TO_UTILIZATION_FRACTION(MALI_PERCENTAGE_SEVENTY);
    } else if ((mali_fps_step1 <= current_fps) && (current_fps < mali_desired_fps)) {
        under_perform_boundary_value = MALI_PERCENTAGE_TO_UTILIZATION_FRACTION(MALI_PERCENTAGE_FIVETY_FIVE);
        over_perform_boundary_value = MALI_PERCENTAGE_TO_UTILIZATION_FRACTION(MALI_PERCENTAGE_THIRTY_FIVE);
    } else if ((mali_fps_step2 <= current_fps) && (current_fps < mali_fps_step1)) {
        under_perform_boundary_value = MALI_PERCENTAGE_TO_UTILIZATION_FRACTION(MALI_PERCENTAGE_SEVENTY);
        over_perform_boundary_value = MALI_PERCENTAGE_TO_UTILIZATION_FRACTION(MALI_PERCENTAGE_FIVETY);
    } else {
        under_perform_boundary_value = MALI_PERCENTAGE_TO_UTILIZATION_FRACTION(MALI_PERCENTAGE_FIVETY_FIVE);
        over_perform_boundary_value = MALI_PERCENTAGE_TO_UTILIZATION_FRACTION(MALI_PERCENTAGE_THIRTY_FIVE);
    }
...
real_setting:
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_INFORMATOIN, ("Using ARM power policy:  eclapse time = %d\n", elapse_time));
#endif
}
...
mali_osk_errcode_t mali_dvfs_policy_init(void)
{
    ...
    if (MALI_OSK_ERR_OK == mali_osk_device_data_get(&data)) {
        ...
        MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_INFORMATOIN, ("Mali DVFS init: get platform data error .\n"));
    }
