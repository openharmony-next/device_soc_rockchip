...
static mali_osk_errcode_t mali_pm_create_pm_domains(void);
...
mali_osk_errcode_t mali_pm_initialize(void)
{
    ...
    pm_work = mali_osk_wq_create_work(mali_pm_update_work, NULL);
    if (NULL == pm_work) {
        mali_pm_terminate();
        return MALI_OSK_ERR_FAULT;
    }
                  ...
                  u32 num_domains)
{
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_MESSAGE, ("PM: wanted domain mask = 0x%08X (get refs)\n", pd_mask_wanted));
                  ...
                  u32 num_domains)
{
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_MESSAGE, ("PM: wanted domain mask = 0x%08X (put refs)\n", pd_mask_wanted));
...
{
    mali_osk_wq_schedule_work(pm_work);
}
...
void mali_pm_os_suspend(mali_bool os_suspend)
{
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_MESSAGE, ("Mali PM: OS suspend\n"));
...
void mali_pm_os_resume(void)
{
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_MESSAGE, ("Mali PM: OS resume\n"));
    ...
    if (MALI_TRUE == mali_pm_runtime_active) {
        ...
        if (NULL != pmu) {
            ...
            MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_MESSAGE, ("Mali PM: OS resume 0x%x \n", pmu_mask_current));
        }
...
mali_bool mali_pm_runtime_suspend(void)
{
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_MESSAGE, ("Mali PM: Runtime suspend\n"));
...
void mali_pm_runtime_resume(void)
{
    ...
    if (NULL != pmu) {
        mali_pmu_reset(pmu);
        pmu_mask_current = mali_pmu_get_mask(pmu);
        MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_MESSAGE, ("Mali PM: Runtime resume 0x%x \n", pmu_mask_current));
    }
...
static void mali_pm_state_lock(void)
{
    mali_osk_spinlock_irq_lock(pm_lock_state);
}

static void mali_pm_state_unlock(void)
{
    mali_osk_spinlock_irq_unlock(pm_lock_state);
}

void mali_pm_exec_lock(void)
{
    mali_osk_mutex_wait(pm_lock_exec);
}
...
static mali_bool mali_pm_common_suspend(void)
{
    ...
    if (0 != pd_mask_wanted) {
        MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_DATA, ("PM: Aborting suspend operation\n\n\n"));
        mali_pm_state_unlock();
        return MALI_FALSE;
    }
...
static void mali_pm_set_default_pm_domain_config(void)
{
    ...
    if (MALI_OSK_ERR_OK == mali_osk_resource_find(
        ...
        if (mali_is_mali400()) {
            ...
            domain_config[MALI_DOMAIN_INDEX_PP0] = 0x01 << MALI_DOMAIN_INDEX_PP0;
        } else if (mali_is_mali470()) {
            domain_config[MALI_DOMAIN_INDEX_PP0] = 0x01 << MALI_DOMAIN_INDEX_GP;
        }
    }
    ...
    if (MALI_OSK_ERR_OK == mali_osk_resource_find(
        ...
        if (mali_is_mali400()) {
            ...
            domain_config[MALI_DOMAIN_INDEX_PP1] = 0x01 << MALI_DOMAIN_INDEX_PP1;
        } else if (mali_is_mali470()) {
            domain_config[MALI_DOMAIN_INDEX_PP1] = 0x01 << MALI_DOMAIN_INDEX_PP0;
        }
    }
    ...
    if (MALI_OSK_ERR_OK == mali_osk_resource_find(
        ...
        if (mali_is_mali400()) {
            ...
            domain_config[MALI_DOMAIN_INDEX_PP2] = 0x01 << MALI_DOMAIN_INDEX_PP1;
        } else if (mali_is_mali470()) {
            domain_config[MALI_DOMAIN_INDEX_PP2] = 0x01 << MALI_DOMAIN_INDEX_PP0;
        }
    }
    ...
    if (MALI_OSK_ERR_OK == mali_osk_resource_find(
        ...
        if (mali_is_mali400()) {
            ...
            domain_config[MALI_DOMAIN_INDEX_PP3] = 0x01 << MALI_DOMAIN_INDEX_PP1;
        } else if (mali_is_mali470()) {
            domain_config[MALI_DOMAIN_INDEX_PP3] = 0x01 << MALI_DOMAIN_INDEX_PP0;
        }
    }
...
static void mali_pm_set_pmu_domain_config(void)
{
    ...
    mali_osk_device_data_pmu_config_get(domain_config, MALI_MAX_NUMBER_OF_DOMAINS - 1);

    for (i = 0; i < MALI_MAX_NUMBER_OF_DOMAINS - 1; i++) {
        if (0 != domain_config[i]) {
            MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_INFORMATOIN, ("Using customer pmu config:\n"));
            break;
        }
    }

    if (MALI_MAX_NUMBER_OF_DOMAINS - 1 == i) {
        MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_INFORMATOIN, ("Using hw detect pmu config:\n"));
        mali_pm_set_default_pm_domain_config();
    }

    for (i = 0; i < MALI_MAX_NUMBER_OF_DOMAINS - 1; i++) {
        if (domain_config[i]) {
            MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_INFORMATOIN, ("domain_config[%d] = 0x%x \n", i, domain_config[i]));
        }
    }
...
void mali_pm_get_best_power_cost_mask(int num_requested, int *dst)
{
    ...
    mali_osk_memcpy(dst, mali_pm_domain_power_cost_result[num_requested], MALI_MAX_NUMBER_OF_DOMAINS * sizeof(int));
}
