...
static mali_osk_errcode_t fill_page(mali_io_address mapping, u32 data);
...
u32 mali_allocate_empty_page(mali_io_address *virt_addr)
{
    ...
    if (MALI_OSK_ERR_OK != mali_mmu_get_table_page(&address, &mapping)) {
        /* Allocation failed */
        MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_INFORMATOIN, ("Mali MMU: Failed to get table page for empty pgdir\n"));
        return 0;
    }
    ...
    if (MALI_OSK_ERR_OK != err) {
        ...
        MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_INFORMATOIN, ("Mali MMU: Failed to zero page\n"));
        return 0;
    }
...
mali_osk_errcode_t mali_mmu_pagedir_unmap(struct mali_page_directory *pagedir, u32 mali_address, u32 size)
{
    ...
    for (i = first_pde; i <= last_pde; i++) {
        ...
        if (0 == pagedir->page_entries_usage_count[i]) {
            u32 page_phys;
            void *page_virt;
            MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_CODE, ("Releasing page table as this is the last reference\n"));
...
struct mali_page_directory *mali_mmu_pagedir_alloc(void)
{
    ...
    pagedir = mali_osk_calloc(1, sizeof(struct mali_page_directory));
    if (NULL == pagedir) {
        return NULL;
    }
...
static mali_osk_errcode_t mali_mmu_dump_page(mali_io_address page, u32 phys_addr, struct dump_info *info)
{
    if (NULL != info) {
        ...
        if (NULL != info->buffer) {
            ...
            *info->buffer = phys_addr;
            ...
            mali_osk_memcpy(info->buffer, page, page_size_in_bytes);
...
static mali_osk_errcode_t dump_mmu_page_table(struct mali_page_directory *pagedir, struct dump_info *info)
{
