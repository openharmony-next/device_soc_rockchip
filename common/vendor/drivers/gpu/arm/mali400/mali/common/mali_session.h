...
struct mali_session_data {
    ...
    MALI_OSK_LIST_HEAD(link);        /**< Link for list of all sessions */
    MALI_OSK_LIST_HEAD(pp_job_list); /**< List of all PP jobs on this session */
    ...
    _mali_osk_list_t
        pp_job_fb_lookup_list[MALI_PP_JOB_FB_LOOKUP_LIST_SIZE]; /**< List of PP job lists per frame builder id.  Used to
                                                                   link jobs from same frame builder. */
    ...
    mali_bool use_high_priority_job_queue; /**< If MALI_TRUE, jobs added from this session will use the high priority
                                              job queues. */
    u32 pid;
    char *comm;
...
mali_osk_errcode_t mali_session_initialize(void);
...
{
    mali_osk_spinlock_irq_lock(mali_sessions_lock);
}

MALI_STATIC_INLINE void mali_session_unlock(void)
{
    mali_osk_spinlock_irq_unlock(mali_sessions_lock);
}
...
#define MALI_SESSION_FOREACH(session, tmp, link)                                                                       \
    MALI_OSK_LIST_FOREACHENTRY(session, tmp, &mali_sessions, struct mali_session_data, link)
...
MALI_STATIC_INLINE void mali_session_send_notification(struct mali_session_data *session,
                                                       _mali_osk_notification_t *object)
{
    mali_osk_notification_queue_send(session->ioctl_queue, object);
}
...
{
    MALI_DEBUG_ASSERT_POINTER(session);
    mali_osk_atomic_inc(&session->number_of_window_jobs);
}
