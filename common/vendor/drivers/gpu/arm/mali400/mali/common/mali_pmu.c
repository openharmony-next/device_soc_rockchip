...
static mali_osk_errcode_t mali_pmu_wait_for_command_finish(
...
struct mali_pmu_core *mali_pmu_create(_mali_osk_resource_t *resource)
{
    ...
    pmu = (struct mali_pmu_core *)mali_osk_malloc(
              sizeof(struct mali_pmu_core));
    if (NULL != pmu) {
        pmu->registered_cores_mask = 0; /* to be set later */

        if (MALI_OSK_ERR_OK == mali_hw_core_create(&pmu->hw_core,
...
mali_osk_errcode_t mali_pmu_power_down(struct mali_pmu_core *pmu, u32 mask)
{
    ...
    if (0 == mask || 0 == ((~stat) & mask)) return MALI_OSK_ERR_OK;
