...
static mali_osk_errcode_t mali_pmu_wait_for_command_finish(struct mali_pmu_core *pmu);
...
void mali_pmu_delete(struct mali_pmu_core *pmu)
{
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_INFORMATOIN, ("Mali PMU: Deleting Mali PMU core\n"));
...
mali_osk_errcode_t mali_pmu_power_down(struct mali_pmu_core *pmu, u32 mask)
{
    ...
    MALI_DEBUG_ASSERT(0 ==
                      (mali_hw_core_register_read(&pmu->hw_core, PMU_REG_ADDR_MGMT_INT_RAWSTAT) & PMU_REG_VAL_IRQ));
