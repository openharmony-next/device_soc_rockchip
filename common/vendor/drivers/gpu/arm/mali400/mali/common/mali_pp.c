...
struct mali_pp_core *mali_pp_create(const _mali_osk_resource_t *resource, struct mali_group *group,
                                    mali_bool is_virtual, u32 bcast_id)
{
...
mali_osk_errcode_t mali_pp_stop_bus_wait(struct mali_pp_core *core)
{
    ...
    for (i = 0; i < MALI_REG_POLL_COUNT_FAST; i++) {
        if (mali_hw_core_register_read(&core->hw_core, MALI200_REG_ADDR_MGMT_STATUS) &
...
mali_osk_errcode_t mali_pp_hard_reset(struct mali_pp_core *core)
{
    ...
    mali_hw_core_register_write(&core->hw_core, reset_wait_target_register,
                                0x00000000); /* set it back to the default */
...
void mali_pp_reset_async(struct mali_pp_core *core)
{
    ...
    mali_hw_core_register_write(&core->hw_core, MALI200_REG_ADDR_MGMT_CTRL_MGMT,
                                MALI400PP_REG_VAL_CTRL_MGMT_SOFT_RESET);
}
...
void mali_pp_job_start(struct mali_pp_core *core, struct mali_pp_job *job, u32 sub_job, mali_bool restart_virtual)
{
    ...
    mali_hw_core_register_write_relaxed_conditional(
        &core->hw_core, MALI200_REG_ADDR_FRAME, mali_pp_job_get_addr_frame(job, sub_job),
        mali_frame_registers_reset_values[MALI200_REG_ADDR_FRAME / sizeof(u32)]);
    ...
    if (!mali_pp_job_is_virtual(job) || restart_virtual) {
        mali_hw_core_register_write_relaxed_conditional(
            &core->hw_core, MALI200_REG_ADDR_STACK, mali_pp_job_get_addr_stack(job, sub_job),
            mali_frame_registers_reset_values[MALI200_REG_ADDR_STACK / sizeof(u32)]);
    }
    ...
    mali_hw_core_register_write_array_relaxed_conditional(&core->hw_core, relative_address,
                                                          &frame_registers[start_index], nr_of_regs,
    ...
    mali_hw_core_register_write_relaxed_conditional(&core->hw_core, relative_address, frame_registers[start_index],
                                                    mali_frame_registers_reset_values[start_index]);
    ...
    if (wb0_registers[0]) { /* M200_WB0_REG_SOURCE_SELECT register */
        mali_hw_core_register_write_array_relaxed_conditional(&core->hw_core, MALI200_REG_ADDR_WB0, wb0_registers,
                                                              MALI_PP_MAX_WB_REGISTERS, mali_wb_registers_reset_values);
    }

    if (wb1_registers[0]) { /* M200_WB1_REG_SOURCE_SELECT register */
        mali_hw_core_register_write_array_relaxed_conditional(&core->hw_core, MALI200_REG_ADDR_WB1, wb1_registers,
                                                              MALI_PP_MAX_WB_REGISTERS, mali_wb_registers_reset_values);
    }

    if (wb2_registers[0]) { /* M200_WB2_REG_SOURCE_SELECT register */
        mali_hw_core_register_write_array_relaxed_conditional(&core->hw_core, MALI200_REG_ADDR_WB2, wb2_registers,
                                                              MALI_PP_MAX_WB_REGISTERS, mali_wb_registers_reset_values);
    }
    ...
    if (MALI_HW_CORE_NO_COUNTER != counter_src0) {
        ...
        mali_hw_core_register_write_relaxed_conditional(&core->hw_core, MALI200_REG_ADDR_MGMT_PERF_CNT_0_ENABLE,
                                                        MALI200_REG_VAL_PERF_CNT_ENABLE,
                                                        mali_perf_cnt_enable_reset_value);
    }
    if (MALI_HW_CORE_NO_COUNTER != counter_src1) {
        ...
        mali_hw_core_register_write_relaxed_conditional(&core->hw_core, MALI200_REG_ADDR_MGMT_PERF_CNT_1_ENABLE,
                                                        MALI200_REG_VAL_PERF_CNT_ENABLE,
                                                        mali_perf_cnt_enable_reset_value);
    }
    ...
    if (job->uargs.perf_counter_flag & MALI_PERFORMANCE_COUNTER_FLAG_HEATMAP_ENABLE) {
        mali_hw_core_register_write_relaxed(&core->hw_core, MALI200_REG_ADDR_MGMT_PERFMON_CONTR,
                                            ((job->uargs.tilesx & 0x3FF) << MALI400PP_REG_VAL_CTRL_SHIFT) | 1);
        mali_hw_core_register_write_relaxed(&core->hw_core, MALI200_REG_ADDR_MGMT_PERFMON_BASE,
                                            job->uargs.heatmap_mem & MALI400PP_REG_VAL_CTRL_MASK);
    }
#endif /* CONFIG_MALI400_HEATMAPS_ENABLED */

    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_MESSAGE,
                     ("Mali PP: Starting job 0x%08X part %u/%u on PP core %s\n", job, sub_job + 1,
                      mali_pp_job_get_sub_job_count(job), core->hw_core.description));
    ...
    mali_hw_core_register_write_relaxed(&core->hw_core, MALI200_REG_ADDR_MGMT_CTRL_MGMT,
                                        MALI200_REG_VAL_CTRL_MGMT_START_RENDERING);
#else
    mali_hw_core_register_write_relaxed(&core->hw_core, MALI200_REG_ADDR_MGMT_INT_RAWSTAT,
...
void mali_pp_update_performance_counters(struct mali_pp_core *parent, struct mali_pp_core *child,
