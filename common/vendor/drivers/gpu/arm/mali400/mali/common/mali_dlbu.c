...
typedef enum mali_dlbu_register {
    ...
    MALI_DLBU_REGISTER_TLLIST_VBASEADDR = 0x0008,        /**< Tile list virtual base address;
                                                                31:12 Virtual address to the tile list. This address is used
                                                            when        calculating the call address sent to PP.*/
...
{
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_INFORMATOIN, ("Mali DLBU: Initializing\n"));

    if (MALI_OSK_ERR_OK == mali_mmu_get_table_page(&mali_dlbu_phys_addr, &mali_dlbu_cpu_addr)) {
...
struct mali_dlbu_core *mali_dlbu_create(const _mali_osk_resource_t *resource)
{
    ...
    if (NULL != core) {
        if (MALI_OSK_ERR_OK == mali_hw_core_create(&core->hw_core, resource, MALI_DLBU_SIZE)) {
...
mali_osk_errcode_t mali_dlbu_reset(struct mali_dlbu_core *dlbu)
{
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_CODE, ("Mali DLBU: mali_dlbu_reset: %s\n", dlbu->hw_core.description));

    dlbu_registers[0x0] = mali_dlbu_phys_addr | 1; /* bit 0 enables the whole core */
    ...
    mali_hw_core_register_write_array_relaxed(&dlbu->hw_core, MALI_DLBU_REGISTER_MASTER_TLLIST_PHYS_ADDR,
                                              dlbu_registers, 7);
