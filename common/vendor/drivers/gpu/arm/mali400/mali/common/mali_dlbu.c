...
typedef enum mali_dlbu_register {
    MALI_DLBU_REGISTER_MASTER_TLLIST_PHYS_ADDR =
        0x0000, /**< Master tile list physical base address;
    ...
    MALI_DLBU_REGISTER_MASTER_TLLIST_VADDR =
        0x0004, /**< Master tile list virtual base address;
    ...
    MALI_DLBU_REGISTER_TLLIST_CONF =
        0x0010, /**< Tile list configuration;
                          29:28 select the size of each allocated block: 0=128
                   bytes, 1=256, 2=512, 3=1024 21:16 2^n number of tiles to be
                   binned to one tile list in Y direction 5:0 2^n number of
                   tiles to be binned to one tile list in X direction */
    MALI_DLBU_REGISTER_START_TILE_POS =
        0x0014, /**< Start tile positions;
    ...
    MALI_DLBU_REGISTER_PP_ENABLE_MASK =
        0x0018, /**< PP enable mask;
...
struct mali_dlbu_core {
    ...
    u32 pp_cores_mask; /**< This is a mask for the PP cores whose operation will
                          be controlled by LBU see
                          MALI_DLBU_REGISTER_PP_ENABLE_MASK register */
};

mali_osk_errcode_t mali_dlbu_initialize(void)
{
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_INFORMATOIN,
                     ("Mali DLBU: Initializing\n"));

    if (MALI_OSK_ERR_OK ==
        mali_mmu_get_table_page(&mali_dlbu_phys_addr, &mali_dlbu_cpu_addr)) {
...
struct mali_dlbu_core *mali_dlbu_create(const _mali_osk_resource_t *resource)
{
    ...
    MALI_DEBUG_PRINT(
        ...
        ("Mali DLBU: Creating Mali dynamic load balancing unit: %s\n",
    ...
    if (NULL != core) {
        if (MALI_OSK_ERR_OK ==
            ...
            if (MALI_OSK_ERR_OK == mali_dlbu_reset(core)) {
                return core;
            }
            MALI_PRINT_ERROR(
...
mali_osk_errcode_t mali_dlbu_reset(struct mali_dlbu_core *dlbu)
{
    ...
    MALI_DEBUG_PRINT(
        ...
        ("Mali DLBU: mali_dlbu_reset: %s\n", dlbu->hw_core.description));

    dlbu_registers[0x0] =
        mali_dlbu_phys_addr | 1; /* bit 0 enables the whole core */
    dlbu_registers[0x1] = MALI_DLBU_VIRT_ADDR;
    dlbu_registers[0x2] = 0;
    dlbu_registers[0x3] = 0;
    dlbu_registers[0x4] = 0;
    dlbu_registers[0x5] = 0;
    dlbu_registers[0x6] = dlbu->pp_cores_mask;
...
void mali_dlbu_update_mask(struct mali_dlbu_core *dlbu)
{
    ...
    mali_hw_core_register_write(
        &dlbu->hw_core, MALI_DLBU_REGISTER_PP_ENABLE_MASK, dlbu->pp_cores_mask);
}
...
void mali_dlbu_add_group(struct mali_dlbu_core *dlbu, struct mali_group *group)
{
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_MESSAGE,
                     ("Mali DLBU: Adding core[%d] New mask= 0x%02x\n", bcast_id,
                      dlbu->pp_cores_mask));
}

/* Remove a group from the DLBU */
void mali_dlbu_remove_group(struct mali_dlbu_core *dlbu,
                            struct mali_group *group)
{
    ...
    dlbu->pp_cores_mask &= ~bcast_id;
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_MESSAGE,
                     ("Mali DLBU: Removing core[%d] New mask= 0x%02x\n",
                      bcast_id, dlbu->pp_cores_mask));
}

/* Configure the DLBU for \a job. This needs to be done before the job is
 * started on the groups in the DLBU. */
void mali_dlbu_config_job(struct mali_dlbu_core *dlbu, struct mali_pp_job *job)
{
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_CODE, ("Mali DLBU: Starting job\n"));

    /* Writing 4 registers:
     * DLBU registers except the first two (written once at DLBU initialisation
