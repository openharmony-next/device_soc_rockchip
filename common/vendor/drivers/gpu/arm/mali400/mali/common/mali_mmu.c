...
static void mali_mmu_disable_stall(struct mali_mmu_core *mmu)
{
    ...
    if (mmu_status & MALI_MMU_STATUS_BIT_PAGE_FAULT_ACTIVE) {
        MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_INFORMATOIN,
                         ("Aborting MMU disable stall request since it is in pagefault state.\n"));
        return;
    }
    ...
    if (MALI_REG_POLL_COUNT_FAST == i) {
        MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_WRANING,
                         ("Disable stall request failed, MMU status is 0x%08X\n",
                          mali_hw_core_register_read(&mmu->hw_core, MALI_MMU_REGISTER_STATUS)));
    }
}
...
void mali_mmu_activate_page_directory(struct mali_mmu_core *mmu, struct mali_page_directory *pagedir)
{
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_DATA,
                     ("Asked to activate page directory 0x%x on MMU %s\n", pagedir, mmu->hw_core.description));
...
void mali_mmu_activate_empty_page_directory(struct mali_mmu_core *mmu)
{
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_MESSAGE,
                     ("Activating the empty page directory on MMU %s\n", mmu->hw_core.description));
...
void mali_mmu_activate_fault_flush_page_directory(struct mali_mmu_core *mmu)
{
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_MESSAGE,
                     ("Activating the page fault flush page directory on MMU %s\n", mmu->hw_core.description));
    stall_success = mali_mmu_enable_stall(mmu);
...
static void mali_mmu_probe_trigger(void *data)
{
    ...
    mali_hw_core_register_write(&mmu->hw_core, MALI_MMU_REGISTER_INT_RAWSTAT,
                                MALI_MMU_INTERRUPT_PAGE_FAULT | MALI_MMU_INTERRUPT_READ_BUS_ERROR);
}
