...
static mali_osk_errcode_t mali_mmu_probe_ack(void *data);

MALI_STATIC_INLINE mali_osk_errcode_t mali_mmu_raw_reset(struct mali_mmu_core *mmu);
...
mali_osk_errcode_t mali_mmu_initialize(void)
{
...
{
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_MESSAGE, ("Mali MMU: terminating\n"));
    ...
    if (NULL != mmu) {
        if (MALI_OSK_ERR_OK == mali_hw_core_create(&mmu->hw_core, resource, MALI_MMU_REGISTERS_SIZE)) {
            if (MALI_OSK_ERR_OK == mali_group_add_mmu_core(group, mmu)) {
                ...
                if (MALI_OSK_ERR_OK == mali_mmu_reset(mmu)) {
...
static mali_bool mali_mmu_enable_stall(struct mali_mmu_core *mmu)
{
    ...
    if (mmu_status & MALI_MMU_STATUS_BIT_PAGE_FAULT_ACTIVE) {
        MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_MESSAGE, ("Aborting MMU stall request since it is in pagefault state.\n"));
        return MALI_FALSE;
    }
    ...
    if (MALI_REG_POLL_COUNT_FAST == i) {
        MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_INFORMATOIN, ("Enable stall request failed, MMU status is 0x%08X\n", mali_hw_core_register_read(&mmu->hw_core, MALI_MMU_REGISTER_STATUS)));
        return MALI_FALSE;
    }

    if (mmu_status & MALI_MMU_STATUS_BIT_PAGE_FAULT_ACTIVE) {
        MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_INFORMATOIN, ("Aborting MMU stall request since it has a pagefault.\n"));
        return MALI_FALSE;
    }
...
static void mali_mmu_disable_stall(struct mali_mmu_core *mmu)
{
    ...
    if (mmu_status & MALI_MMU_STATUS_BIT_PAGE_FAULT_ACTIVE) {
        MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_INFORMATOIN, ("Aborting MMU disable stall request since it is in pagefault state.\n"));
        return;
    }
    ...
    if (MALI_REG_POLL_COUNT_FAST == i) MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_WRANING, ("Disable stall request failed, MMU status is 0x%08X\n", mali_hw_core_register_read(&mmu->hw_core, MALI_MMU_REGISTER_STATUS)));
}

void mali_mmu_page_fault_done(struct mali_mmu_core *mmu)
{
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_CODE, ("Mali MMU: %s: Leaving page fault mode\n", mmu->hw_core.description));
...
mali_osk_errcode_t mali_mmu_reset(struct mali_mmu_core *mmu)
{
    mali_osk_errcode_t err = MALI_OSK_ERR_FAULT;
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_MESSAGE, ("Mali MMU: mali_kernel_mmu_reset: %s\n", mmu->hw_core.description));

    if (MALI_OSK_ERR_OK == mali_mmu_raw_reset(mmu)) {
...
void mali_mmu_activate_page_directory(struct mali_mmu_core *mmu, struct mali_page_directory *pagedir)
{
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_DATA, ("Asked to activate page directory 0x%x on MMU %s\n", pagedir, mmu->hw_core.description));
...
void mali_mmu_activate_empty_page_directory(struct mali_mmu_core *mmu)
{
    ...
    MALI_DEBUG_ASSERT_POINTER(mmu);
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_MESSAGE, ("Activating the empty page directory on MMU %s\n", mmu->hw_core.description));
...
void mali_mmu_activate_fault_flush_page_directory(struct mali_mmu_core *mmu)
{
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_MESSAGE, ("Activating the page fault flush page directory on MMU %s\n", mmu->hw_core.description));
    stall_success = mali_mmu_enable_stall(mmu);
...
static mali_osk_errcode_t mali_mmu_probe_ack(void *data)
{
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_INFORMATOIN, ("mali_mmu_probe_irq_acknowledge: intstat 0x%x\n", int_stat));
    if (int_stat & MALI_MMU_INTERRUPT_PAGE_FAULT) {
        MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_INFORMATOIN, ("Probe: Page fault detect: PASSED\n"));
        mali_hw_core_register_write(&mmu->hw_core, MALI_MMU_REGISTER_INT_CLEAR, MALI_MMU_INTERRUPT_PAGE_FAULT);
    } else {
        MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_WRANING, ("Probe: Page fault detect: FAILED\n"));
    }

    if (int_stat & MALI_MMU_INTERRUPT_READ_BUS_ERROR) {
        MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_INFORMATOIN, ("Probe: Bus read error detect: PASSED\n"));
        mali_hw_core_register_write(&mmu->hw_core, MALI_MMU_REGISTER_INT_CLEAR, MALI_MMU_INTERRUPT_READ_BUS_ERROR);
    } else {
        MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_WRANING, ("Probe: Bus read error detect: FAILED\n"));
    }
