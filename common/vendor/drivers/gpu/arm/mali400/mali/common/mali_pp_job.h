...
struct mali_pp_job *mali_pp_job_create(struct mali_session_data *session, mali_uk_pp_start_job_s *uargs, u32 id);
...
MALI_STATIC_INLINE void mali_pp_job_fb_lookup_remove(struct mali_pp_job *job)
{
    MALI_DEBUG_ASSERT_POINTER(job);
    MALI_DEBUG_ASSERT_SCHEDULER_LOCK_HELD();
    mali_osk_list_delinit(&job->session_fb_lookup_list);
}
...
MALI_STATIC_INLINE void mali_pp_job_mark_unstarted_failed(struct mali_pp_job *job)
{
    ...
    for (i = 0; i < jobs_remaining; i++) {
        mali_osk_atomic_inc(&job->sub_jobs_completed);
        mali_osk_atomic_inc(&job->sub_job_errors);
    }
}
...
MALI_STATIC_INLINE mali_osk_errcode_t mali_pp_job_check(struct mali_pp_job *job)
{
    MALI_DEBUG_ASSERT_POINTER(job);
