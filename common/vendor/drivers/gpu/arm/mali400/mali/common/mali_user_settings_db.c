...
static u32 mali_user_settings[MALI_UK_USER_SETTING_MAX];
const char *mali_uk_user_setting_descriptions[] = MALI_UK_USER_SETTING_DESCRIPTIONS;

static void mali_user_settings_notify(mali_uk_user_setting_t setting, u32 value)
{
...
void mali_set_user_setting(mali_uk_user_setting_t setting, u32 value)
{
    mali_bool notify = MALI_FALSE;

    if (setting >= MALI_UK_USER_SETTING_MAX) {
...
u32 mali_get_user_setting(mali_uk_user_setting_t setting)
{
    if (setting >= MALI_UK_USER_SETTING_MAX) {
        return 0;
    }
...
mali_osk_errcode_t _mali_ukk_get_user_setting(mali_uk_get_user_setting_s *args)
{
    ...
    if (MALI_UK_USER_SETTING_MAX > setting) {
...
mali_osk_errcode_t _mali_ukk_get_user_settings(mali_uk_get_user_settings_s *args)
{
    ...
    mali_osk_memcpy(args->settings, mali_user_settings, sizeof(mali_user_settings));
