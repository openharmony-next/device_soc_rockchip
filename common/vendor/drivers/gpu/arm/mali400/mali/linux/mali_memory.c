...
static void mali_mem_vma_open(struct vm_area_struct *vma)
{
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_CODE, ("Open called on vma %p\n", vma));
...
static vm_fault_t mali_mem_vma_fault(struct vm_fault *vmf)
{
    ...
    if ((mem_bkend->type == MALI_MEM_COW &&
        ...
        ret = mali_mem_cow_allocate_on_demand(mem_bkend, (address - vma->vm_start) / PAGE_SIZE);
        mutex_unlock(&mem_bkend->mutex);

        if (ret != MALI_OSK_ERR_OK) {
            return VM_FAULT_OOM;
        }
        prefetch_num = 1;
        ...
        int ret2 = MALI_OSK_ERR_OK;

        mutex_lock(&mem_bkend->mutex);
        if (mem_bkend->flags & MALI_MEM_BACKEND_FLAG_COW_CPU_NO_WRITE) {
            ret2 = mali_mem_swap_cow_page_on_demand(mem_bkend, offset_in_bkend, &vmf->page);
        } else {
            ret2 = mali_mem_swap_allocate_page_on_demand(mem_bkend, offset_in_bkend, &vmf->page);
        }
        mutex_unlock(&mem_bkend->mutex);

        if (ret2 != MALI_OSK_ERR_OK) {
            MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_INFORMATOIN,
                             ("Mali swap memory page fault process failed, address=0x%x\n", address));
            return VM_FAULT_OOM;
        } else {
...
int mali_mmap(struct file *filp, struct vm_area_struct *vma)
{
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_CODE,
                     ("MMap() handler: start=0x%08X, phys=0x%08X, size=0x%08X vma->flags 0x%08x\n",
    ...
    if (mali_alloc->flags & MALI_MEMORY_ALLOCATE_DEFER_BIND) {
    ...
    if (!(vma->vm_flags & VM_WRITE)) {
        MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_CODE, ("mmap allocation with read only !\n"));
...
mali_osk_errcode_t mali_mem_mali_map_resize(mali_mem_allocation *descriptor, u32 new_size)
{
    ...
    if (new_size > old_size) {
        ...
        return mali_mmu_pagedir_map(session->page_directory, descriptor->mali_vma_node.vm_node.start + old_size,
                                    new_size - old_size);
    }
...
mali_osk_errcode_t mali_memory_session_begin(struct mali_session_data *session_data)
{
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_DATA, ("Memory session begin\n"));
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_DATA, ("MMU session begin: success\n"));
    MALI_SUCCESS;
}
