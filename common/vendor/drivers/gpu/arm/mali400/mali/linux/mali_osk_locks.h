...
#else
#define _mali_osk_locks_debug_init(x, y, z)                                                                            \
    do {                                                                                                               \
    } while (0)
#define _mali_osk_locks_debug_add(x)                                                                                   \
    do {                                                                                                               \
    } while (0)
#define _mali_osk_locks_debug_remove(x)                                                                                \
    do {                                                                                                               \
    } while (0)
#endif
...
/** @brief Before mali_osk_spinlock_irq_lock/unlock/term() is called, init function should be
 * called to initial spinlock and flags in struct _mali_osk_spinlock_irq_t. */
static inline _mali_osk_spinlock_irq_t *mali_osk_spinlock_irq_init(_mali_osk_lock_flags_t flags,
                                                                   _mali_osk_lock_order_t order)
{
...
static inline void mali_osk_spinlock_irq_lock(_mali_osk_spinlock_irq_t *lock)
{
...
static inline mali_osk_errcode_t mali_osk_mutex_wait_interruptible(_mali_osk_mutex_t *lock)
{
    ...
    if (mutex_lock_interruptible(&lock->mutex)) {
        ...
        err = MALI_OSK_ERR_RESTARTSYSCALL;
    }
