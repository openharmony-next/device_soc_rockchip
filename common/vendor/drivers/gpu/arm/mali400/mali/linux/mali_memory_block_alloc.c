...
static mali_block_allocator *mali_mem_block_allocator_create(u32 base_address, u32 size)
{
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_MESSAGE,
                     ("Mali block allocator create for region starting at 0x%08X length 0x%08X\n", base_address, size));
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_CODE, ("%d usable bytes\n", usable_size));
    num_blocks = usable_size / MALI_BLOCK_SIZE;
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_CODE, ("which becomes %d blocks\n", num_blocks));
    ...
    info = mali_osk_calloc(1, sizeof(mali_block_allocator));
    if (NULL != info) {
...
u32 mali_mem_block_free(mali_mem_block_mem *block_mem)
{
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_CODE,
                     ("BLOCK Mem free : allocated size = 0x%x, free size = 0x%x\n",
                      block_mem->count * MALI_OSK_MALI_PAGE_SIZE, free_pages_nr * MALI_OSK_MALI_PAGE_SIZE));
    block_mem->count = 0;
...
mali_osk_errcode_t mali_mem_block_unref_node(struct mali_page_node *node)
{
    mali_block_allocator *info = mali_mem_block_gobal_allocator;
...
mali_osk_errcode_t mali_memory_core_resource_dedicated_memory(u32 start, u32 size)
{
    ...
    if (MALI_OSK_ERR_OK != _mali_osk_mem_reqregion(start, size, "Dedicated Mali GPU memory")) {
        MALI_DEBUG_PRINT(
            1, ("Failed to request memory region for frame buffer (0x%08X - 0x%08X)\n", start, start + size - 1));
        return MALI_OSK_ERR_FAULT;
    }
...
u32 mali_mem_block_allocator_stat(void)
{
    ...
    return (allocator->total_num - atomic_read(&allocator->free_num)) * MALI_OSK_MALI_PAGE_SIZE;
}
