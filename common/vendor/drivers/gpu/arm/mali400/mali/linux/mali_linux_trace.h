 ...
 */
#ifndef COMMON_VENDOR_DRIVERS_GPU_ARM_MALI400_MALI_LINUX_MALI_LINUX_TRACE_H
#define COMMON_VENDOR_DRIVERS_GPU_ARM_MALI400_MALI_LINUX_MALI_LINUX_TRACE_H
...
TRACE_EVENT(mali_timeline_event,
            ...
            TP_PROTO(unsigned int event_id, unsigned int d0, unsigned int d1, unsigned int d2, unsigned int d3,
                     unsigned int d4),

            TP_ARGS(event_id, d0, d1, d2, d3, d4),

            TP_STRUCT__entry(__field(unsigned int, event_id) __field(unsigned int, d0) __field(unsigned int, d1)
                                 __field(unsigned int, d2) __field(unsigned int, d3) __field(unsigned int, d4)),

            TP_fast_assign(__entry->event_id = event_id; __entry->d0 = d0; __entry->d1 = d1; __entry->d2 = d2;
                           __entry->d3 = d3; __entry->d4 = d4;),
...
TRACE_EVENT(mali_hw_counter,
            ...
            TP_STRUCT__entry(__field(unsigned int, counter_id) __field(unsigned int, value)),

            TP_fast_assign(__entry->counter_id = counter_id;),
...
TRACE_EVENT(mali_sw_counters,
            ...
            TP_STRUCT__entry(__field(pid_t, pid) __field(pid_t, tid) __field(void *, surface_id)
                                 __field(unsigned int *, counters)),

            TP_fast_assign(__entry->pid = pid; __entry->tid = tid; __entry->surface_id = surface_id;
...
TRACE_EVENT(mali_core_active,
            ...
            TP_PROTO(pid_t pid, unsigned int active, unsigned int core_type, unsigned int core_id,
                     unsigned int frame_builder_id, unsigned int flush_id),
            ...
            TP_printk("%s|%d|%s%i:%x|%d", __entry->active ? "S" : "F", __entry->pid, __entry->core_type ? "GP" : "PP",
                      __entry->core_id, __entry->flush_id, __entry->frame_builder_id));
