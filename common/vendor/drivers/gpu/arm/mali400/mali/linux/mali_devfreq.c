...
mali_devfreq_cur_freq(struct device *dev, unsigned long *freq)
{
    ...
    *freq = mdev->current_freq;
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_INFORMATOIN, ("mali_devfreq_cur_freq: freq = %d \n", *freq));
    return 0;
}
...
int mali_devfreq_init(struct mali_device *mdev)
{
    ...
    if (callbacks) {
        ...
        if (IS_ERR_OR_NULL(mdev->devfreq_cooling)) {
            ...
            MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_INFORMATOIN, ("Mali GPU Thermal Cooling installed \n"));
        }
    }
#endif
...
void mali_devfreq_term(struct mali_device *mdev)
{
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_INFORMATOIN, ("Term Mali devfreq\n"));
