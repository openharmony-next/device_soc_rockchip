...
{
    mali_uk_profiling_add_event_s kargs;
    mali_osk_errcode_t err;

    MALI_CHECK_NON_NULL(uargs, -EINVAL);

    if (0 != copy_from_user(&kargs, uargs, sizeof(mali_uk_profiling_add_event_s))) {
        return -EFAULT;
    }
...
int profiling_report_sw_counters_wrapper(struct mali_session_data *session_data, mali_uk_sw_counters_report_s __user *uargs)
{
    ...
    if (0 != copy_from_user(&kargs, uargs, sizeof(mali_uk_sw_counters_report_s))) {
        return -EFAULT;
    }
...
int profiling_get_stream_fd_wrapper(struct mali_session_data *session_data, mali_uk_profiling_stream_fd_get_s __user *uargs)
{
    ...
    if (0 != copy_from_user(&kargs, uargs, sizeof(mali_uk_profiling_stream_fd_get_s))) {
        return -EFAULT;
    }
    ...
    if (0 != copy_to_user(uargs, &kargs, sizeof(mali_uk_profiling_stream_fd_get_s))) {
        return -EFAULT;
    }
...
{
    mali_uk_profiling_control_set_s kargs;
    mali_osk_errcode_t err;
    ...
    if (0 !=  kargs.control_packet_size) {
        ...
        kernel_response_data = mali_osk_calloc(1, kargs.response_packet_size);
