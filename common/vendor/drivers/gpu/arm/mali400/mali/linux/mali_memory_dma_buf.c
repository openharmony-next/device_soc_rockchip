...
int mali_dma_buf_get_size(struct mali_session_data *session, mali_uk_dma_buf_get_size_s __user *user_arg)
{
    mali_uk_dma_buf_get_size_s args;
    int fd;
    struct dma_buf *buf;

    /* get call arguments from user space. copy_from_user returns how many bytes which where NOT copied */
    if (0 != copy_from_user(&args, (void __user *)user_arg, sizeof(mali_uk_dma_buf_get_size_s))) {
        return -EFAULT;
    }
...
mali_osk_errcode_t mali_mem_bind_dma_buf(mali_mem_allocation *alloc, mali_mem_backend *mem_backend, int fd, u32 flags)
{
    ...
    dma_mem = mali_osk_calloc(1, sizeof(struct mali_dma_buf_attachment));
    if (NULL == dma_mem) {
        goto failed_alloc_mem;
    }
