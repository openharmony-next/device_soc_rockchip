...
mali_osk_errcode_t mali_mem_bind_ext_buf(mali_mem_allocation *alloc,
        ...
        u32 flag)
{
    ...
    if (!size) MALI_ERROR(MALI_OSK_ERR_INVALID_ARGS);

    /* size must be a multiple of the system page size */
    if (size % MALI_OSK_MALI_PAGE_SIZE) MALI_ERROR(MALI_OSK_ERR_INVALID_ARGS);

    /* Validate the mali physical range */
    if (MALI_OSK_ERR_OK != mali_mem_validation_check(phys_addr, size)) {
        return MALI_OSK_ERR_FAULT;
    }

    if (flag & MALI_MAP_EXTERNAL_MAP_GUARD_PAGE) {
    ...
    if (alloc->flags & MALI_MEM_FLAG_MALI_GUARD_PAGE) {
        mali_mmu_pagedir_update(session->page_directory, virt + size, phys, MALI_OSK_MALI_PAGE_SIZE, MALI_MMU_FLAGS_DEFAULT);
    }
    MALI_DEBUG_PRINT(3,
