...
static mali_osk_errcode_t _mali_profiling_global_stream_list_dequeue(struct list_head *stream_list, mali_profiling_stream **new_mali_profiling_stream)
{
    unsigned long irq_flags;
            ...
            if (global_mali_profiling_counters[counter_index].enabled) {
                ...
                if (global_mali_profiling_counters[counter_index].counter_id >= COUNTER_L2_0_C0 &&
                    ...
                    if (global_mali_profiling_counters[counter_index].counter_id == COUNTER_TOTAL_ALLOC_PAGES) {
                        ...
                        global_mali_profiling_counters[counter_index].current_counter_value = total_alloc_mem / MALI_OSK_MALI_PAGE_SIZE;
                    }
                    _mali_profiling_stream_add_counter(mali_counter_stream, current_time, global_mali_profiling_counters[counter_index].key,
...
static mali_bool _mali_profiling_global_counters_init(void)
{
    ...
    global_mali_profiling_counters = mali_osk_calloc(num_global_mali_profiling_counters, sizeof(mali_profiling_counter));
...
mali_osk_errcode_t _mali_ukk_profiling_add_event(mali_uk_profiling_add_event_s *args)
{
    ...
    _mali_osk_profiling_add_event(args->event_id, _mali_osk_get_pid(), mali_osk_get_tid(), args->data[2], args->data[3], args->data[4]);
...
void mali_profiling_control(u32 action, u32 value)
{
    ...
    case FBDUMP_CONTROL_ENABLE:
        mali_set_user_setting(MALI_UK_USER_SETTING_COLORBUFFER_CAPTURE_ENABLED, (value == 0 ? MALI_FALSE : MALI_TRUE));
        break;
