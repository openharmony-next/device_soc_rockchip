...
const char *_mali_mem_counter_descriptions[] = MALI_MEM_COUTNER_DESCRIPTIONS;
const char *_mali_special_counter_descriptions[] = MALI_SPCIAL_COUNTER_DESCRIPTIONS;
...
static mali_osk_errcode_t _mali_profiling_global_stream_list_dequeue(struct list_head *stream_list,
                                                                     mali_profiling_stream **new_mali_profiling_stream)
{
    unsigned long irq_flags;
...
static void _mali_profiling_global_stream_list_queue(struct list_head *stream_list,
...
static ssize_t _mali_profiling_stream_read(struct file *filp, char __user *buffer, size_t size, loff_t *f_pos);
...
static ssize_t _mali_profiling_stream_read(struct file *filp, char __user *buffer, size_t size, loff_t *f_pos)
{
    ...
    while (!_mali_profiling_global_stream_queue_list_if_empty()) {
        ...
        if (used_size <= ((u32)size - copy_len)) {
            ...
            if (copy_to_user(&buffer[copy_len], current_mali_profiling_stream->data,
                             current_mali_profiling_stream->used_size)) {
                current_mali_profiling_stream->used_size = 0;
                _mali_profiling_global_stream_list_queue(&global_mali_stream_list->free_list,
                                                         current_mali_profiling_stream);
                return -EFAULT;
            }
...
static int _mali_profiling_stream_release(struct inode *inode, struct file *filp)
{
    mali_osk_atomic_init(&stream_fd_if_used, 0);
    return 0;
}
...
static u32 _mali_profiling_read_packet_int(unsigned char *const buf, u32 *const pos, u32 const packet_size)
{
    ...
    if (shift < 0x8 * sizeof(int_value) && (byte_value & 0x40) != 0) {
        int_value |= -(1 << shift);
    }
...
static void _mali_profiling_stream_add_counter(mali_profiling_stream *profiling_stream, s64 current_time, u32 key,
                                               u32 counter_value)
{
...
static enum hrtimer_restart _mali_profiling_sampling_counters(struct hrtimer *timer)
{
    ...
    if (MALI_OSK_ERR_OK ==
        _mali_profiling_global_stream_list_dequeue(&global_mali_stream_list->free_list, &mali_counter_stream)) {
        ...
        if (MALI_TRUE == l2_cache_counter_if_enabled) {
            int i, j = 0;
            mali_profiling_l2_counter_values l2_counters_values;
            mali_profiling_get_l2_counters(&l2_counters_values);

            for (i = COUNTER_L2_0_C0; i <= COUNTER_L2_2_C1; i++) {
                if ((j % 0x02) == 0) {
                    _mali_osk_profiling_record_global_counters(i, l2_counters_values.cores[j / 0x02].value0);
                } else {
                    _mali_osk_profiling_record_global_counters(i, l2_counters_values.cores[j / 0x02].value1);
                }
                j++;
            }
        }
            ...
            if (global_mali_profiling_counters[counter_index].enabled) {
                ...
                if (global_mali_profiling_counters[counter_index].counter_id >= COUNTER_L2_0_C0 &&
                    ...
                    _mali_profiling_stream_add_counter(
                        mali_counter_stream, current_time, global_mali_profiling_counters[counter_index].key,
                    ...
                    if (global_mali_profiling_counters[counter_index].counter_id == COUNTER_TOTAL_ALLOC_PAGES) {
                        ...
                        global_mali_profiling_counters[counter_index].current_counter_value =
                            total_alloc_mem / MALI_OSK_MALI_PAGE_SIZE;
                    }
                    _mali_profiling_stream_add_counter(
                        mali_counter_stream, current_time, global_mali_profiling_counters[counter_index].key,
                        global_mali_profiling_counters[counter_index].current_counter_value);
    ...
    if (num_counters_enabled != 0 && profiling_sample_rate != 0) {
...
static void _mali_profiling_sampling_core_activity_switch(int counter_id, int core, u32 activity, u32 pid)
{
    ...
    if (NULL != global_mali_profiling_counters) {
        ...
        for (i = 0; i < num_global_mali_profiling_counters; i++) {
            if (counter_id == global_mali_profiling_counters[i].counter_id &&
                global_mali_profiling_counters[i].enabled) {
                ...
                if (NULL != mali_core_activity_stream) {
                    if ((mali_core_activity_stream_dequeue_time + MALI_PROFILING_STREAM_HOLD_TIME < current_time) ||
                        (MALI_PROFILING_STREAM_DATA_DEFAULT_SIZE >
                         MALI_PROFILING_STREAM_BUFFER_SIZE - mali_core_activity_stream->used_size)) {
                        _mali_profiling_global_stream_list_queue(&global_mali_stream_list->queue_list,
                ...
                add_size +=
                    _mali_profiling_pack_long(mali_core_activity_stream->data, MALI_PROFILING_STREAM_BUFFER_SIZE,
                                              mali_core_activity_stream->used_size + add_size, (s64)current_time);
                add_size += _mali_profiling_pack_int(mali_core_activity_stream->data, MALI_PROFILING_STREAM_BUFFER_SIZE,
                                                     mali_core_activity_stream->used_size + add_size, core);
                add_size += _mali_profiling_pack_int(mali_core_activity_stream->data, MALI_PROFILING_STREAM_BUFFER_SIZE,
                                                     mali_core_activity_stream->used_size + add_size,
                                                     (s32)global_mali_profiling_counters[i].key);
                add_size += _mali_profiling_pack_int(mali_core_activity_stream->data, MALI_PROFILING_STREAM_BUFFER_SIZE,
                                                     mali_core_activity_stream->used_size + add_size, activity);
                add_size += _mali_profiling_pack_int(mali_core_activity_stream->data, MALI_PROFILING_STREAM_BUFFER_SIZE,
                                                     mali_core_activity_stream->used_size + add_size, pid);

                _mali_profiling_set_packet_size(mali_core_activity_stream->data + mali_core_activity_stream->used_size +
                                                    1,
                                                add_size - STREAM_HEADER_SIZE);
...
static mali_bool _mali_profiling_global_counters_init(void)
{
    ...
    num_global_mali_profiling_counters = 0x03 * (num_gp_cores + num_pp_cores) + 0x02 * num_l2_cache_cores +
                                         MALI_PROFILING_SW_COUNTERS_NUM + MALI_PROFILING_SPECIAL_COUNTERS_NUM +
                                         MALI_PROFILING_MEM_COUNTERS_NUM;
    global_mali_profiling_counters =
        mali_osk_calloc(num_global_mali_profiling_counters, sizeof(mali_profiling_counter));
    ...
    for (core_id = 0; core_id < num_gp_cores; core_id++) {
        ...
        mali_osk_snprintf(global_mali_profiling_counters[counter_index].counter_name,
                          ...
                          "ARM_Mali-%s_VP_%d_active", mali_name, core_id);
    ...
    for (core_id = 0; core_id < num_pp_cores; core_id++) {
        ...
        mali_osk_snprintf(global_mali_profiling_counters[counter_index].counter_name,
                          sizeof(global_mali_profiling_counters[counter_index].counter_name),
                          "ARM_Mali-%s_FP_%d_active", mali_name, core_id);
    ...
    for (counter_id = FIRST_SW_COUNTER; counter_id <= LAST_SW_COUNTER; counter_id++) {
        ...
        mali_osk_snprintf(global_mali_profiling_counters[counter_index].counter_name,
                          sizeof(global_mali_profiling_counters[counter_index].counter_name), "ARM_Mali-%s_SW_%d",
                          mali_name, counter_id - FIRST_SW_COUNTER);
    }
...
void _mali_profiling_notification_mem_counter(struct mali_session_data *session, u32 counter_id, u32 key, int enable)
{
    ...
    if (session != NULL) {
        ...
        notification = _mali_osk_notification_create(MALI_NOTIFICATION_ANNOTATE_PROFILING_MEM_COUNTER,
                                                     sizeof(mali_uk_annotate_profiling_mem_counter_s));
        if (notification != NULL) {
            mali_uk_annotate_profiling_mem_counter_s *data = notification->result_buffer;
...
void _mali_profiling_notification_enable(struct mali_session_data *session, u32 sampling_rate, int enable)
{
    ...
    if (session != NULL) {
        ...
        notification = _mali_osk_notification_create(MALI_NOTIFICATION_ANNOTATE_PROFILING_ENABLE,
                                                     sizeof(mali_uk_annotate_profiling_enable_s));
        if (notification != NULL) {
            mali_uk_annotate_profiling_enable_s *data = notification->result_buffer;
...
mali_osk_errcode_t _mali_osk_profiling_init(mali_bool auto_start)
{
    ...
    new_mali_profiling_stream_list =
        (mali_profiling_stream_list *)kmalloc(sizeof(mali_profiling_stream_list), GFP_KERNEL);
...
void _mali_osk_profiling_term(void)
{
    ...
    mali_osk_atomic_term(&stream_fd_if_used);
...
void _mali_osk_profiling_stop_sampling(u32 pid)
{
    if (pid == current_profiling_pid) {
        ...
        for (i = 0; i < num_global_mali_profiling_counters; ++i) {
            mali_profiling_set_event(global_mali_profiling_counters[i].counter_id, MALI_HW_CORE_NO_COUNTER);
        ...
        mali_profiling_control(FBDUMP_CONTROL_ENABLE, 0);
        mali_profiling_control(SW_COUNTER_ENABLE, 0);
...
void _mali_osk_profiling_add_event(u32 event_id, u32 data0, u32 data1, u32 data2, u32 data3, u32 data4)
{
    ...
    if (0 != profiling_sample_rate) {
        u32 channel;
        u32 state;
        channel = (event_id >> 0x10) & 0xFF;
        state = ((event_id >> 0x18) & 0xF) << 0x18;

        switch (state) {
            case MALI_PROFILING_EVENT_TYPE_SINGLE:
                if ((MALI_PROFILING_EVENT_CHANNEL_GPU >> 0x10) == channel) {
            ...
            case MALI_PROFILING_EVENT_TYPE_START:
                if ((MALI_PROFILING_EVENT_CHANNEL_GP0 >> 0x10) == channel) {
                    _mali_profiling_sampling_core_activity_switch(COUNTER_VP_ACTIVITY, 0, 1, data1);
                } else if (channel >= (MALI_PROFILING_EVENT_CHANNEL_PP0 >> 0x10) &&
                           (MALI_PROFILING_EVENT_CHANNEL_PP7 >> 0x10) >= channel) {
...
{
    trace_mali_sw_counters(_mali_osk_get_pid(), mali_osk_get_tid(), NULL, counters);
}
...
mali_osk_errcode_t _mali_ukk_profiling_stream_fd_get(mali_uk_profiling_stream_fd_get_s *args)
{
    ...
    if (mali_osk_atomic_inc_return(&stream_fd_if_used) == 1) {
        s32 fd =
            anon_inode_getfd("[mali_profiling_stream]", &mali_profiling_stream_fops, session, O_RDONLY | O_CLOEXEC);

        args->stream_fd = fd;
        if (fd < 0) {
            mali_osk_atomic_dec(&stream_fd_if_used);
...
mali_osk_errcode_t _mali_ukk_profiling_control_set(mali_uk_profiling_control_set_s *args)
{
    ...
    if (global_mali_profiling_counters == NULL && MALI_FALSE == _mali_profiling_global_counters_init()) {
        ...
        switch (control_type) {
                ...
                *response_packet_data = PACKET_HEADER_COUNTERS_ACK;
                ...
                for (i = 0; i < num_global_mali_profiling_counters; ++i) {
                    ...
                    if (global_mali_profiling_counters[i].counter_id == COUNTER_VP_ACTIVITY) {
                        args->response_packet_size += _mali_profiling_pack_int(response_packet_data, output_buffer_size,
                                                                               args->response_packet_size, (s32)1);
                    } else if (global_mali_profiling_counters[i].counter_id == COUNTER_FP_ACTIVITY) {
                        args->response_packet_size +=
                            _mali_profiling_pack_int(response_packet_data, output_buffer_size,
                                                     args->response_packet_size, (s32)mali_pp_get_glob_num_pp_cores());
                    } else {
                        args->response_packet_size += _mali_profiling_pack_int(response_packet_data, output_buffer_size,
                                                                               args->response_packet_size, (s32)-1);
                    }
            ...
            case PACKET_HEADER_COUNTERS_ENABLE: {
                ...
                while (request_pos < control_packet_size) {
                    ...
                    for (i = 0; i < num_global_mali_profiling_counters; ++i) {
                        ...
                        if (strncmp(global_mali_profiling_counters[i].counter_name,
                                    (char *)(control_packet_data + begin), name_size) == 0) {
                            if (!sw_counter_if_enabled &&
                                (FIRST_SW_COUNTER <= global_mali_profiling_counters[i].counter_id &&
                                 global_mali_profiling_counters[i].counter_id <= LAST_SW_COUNTER)) {
                                sw_counter_if_enabled = MALI_TRUE;
                                mali_profiling_control(SW_COUNTER_ENABLE, 1);
                            }

                            if (COUNTER_FILMSTRIP == global_mali_profiling_counters[i].counter_id) {
                                mali_profiling_control(FBDUMP_CONTROL_ENABLE, 1);
                                mali_profiling_control(FBDUMP_CONTROL_RATE, event & 0xff);
                                mali_profiling_control(FBDUMP_CONTROL_RESIZE_FACTOR, (event >> 0x08) & 0xff);
                            }

                            if (global_mali_profiling_counters[i].counter_id >= FIRST_MEM_COUNTER &&
                                global_mali_profiling_counters[i].counter_id <= LAST_MEM_COUNTER) {
                                _mali_profiling_notification_mem_counter(
                                    session, global_mali_profiling_counters[i].counter_id, key, 1);
                            ...
                            mali_profiling_set_event(global_mali_profiling_counters[i].counter_id,
            ...
            case PACKET_HEADER_START_CAPTURE_VALUE: {
                ...
                profiling_sample_rate =
                    _mali_profiling_read_packet_int(control_packet_data, &request_pos, control_packet_size);
                ...
                if (num_counters_enabled != 0 && profiling_sample_rate != 0) {
                    ...
                    hrtimer_start(&profiling_sampling_timer,
                                  ktime_set(profiling_sample_rate / 0x3B9ACA00, profiling_sample_rate % 0x3B9ACA00),
...
int mali_profiling_set_event(u32 counter_id, s32 event_id)
{
    if (COUNTER_VP_0_C0 == counter_id) {
         ...
         *    the predictability of the returned data (as you would be guaranteed data for all the selected HW
         * counters).
         *
        ...
        if (l2_cache_core != NULL) {
            ...
            mali_l2_cache_core_set_counter_src(l2_cache_core, counter_src, event_id);
 ...
 *
 * @param values Pointer to a mali_profiling_l2_counter_values structure where
...
u32 mali_profiling_get_l2_counters(mali_profiling_l2_counter_values *values)
{
    ...
    MALI_DEBUG_ASSERT(l2_cores_num <= 0x03);
    ...
    for (i = 0; i < l2_cores_num; i++) {
        ...
        mali_l2_cache_core_get_counter_values(l2_cache, &values->cores[i].source0, &values->cores[i].value0,
                                              &values->cores[i].source1, &values->cores[i].value1);
    }
...
void mali_profiling_control(u32 action, u32 value)
{
    switch (action) {
        case FBDUMP_CONTROL_ENABLE:
            mali_set_user_setting(MALI_UK_USER_SETTING_COLORBUFFER_CAPTURE_ENABLED,
                                  (value == 0 ? MALI_FALSE : MALI_TRUE));
            break;
        case FBDUMP_CONTROL_RATE:
            mali_set_user_setting(MALI_UK_USER_SETTING_BUFFER_CAPTURE_N_FRAMES, value);
            break;
        case SW_COUNTER_ENABLE:
            mali_set_user_setting(MALI_UK_USER_SETTING_SW_COUNTER_ENABLED, value);
            break;
        case FBDUMP_CONTROL_RESIZE_FACTOR:
            mali_set_user_setting(MALI_UK_USER_SETTING_BUFFER_CAPTURE_RESIZE_FACTOR, value);
            break;
        default:
...
EXPORT_SYMBOL(mali_profiling_set_event);
EXPORT_SYMBOL(mali_profiling_get_l2_counters);
EXPORT_SYMBOL(mali_profiling_control);
EXPORT_SYMBOL(mali_profiling_get_api_version);
EXPORT_SYMBOL(mali_profiling_get_mali_version);
