...
mali_osk_errcode_t mali_mem_secure_mali_map(mali_mem_secure *secure_mem, struct mali_session_data *session, u32 vaddr, u32 props)
{
    ...
    for_each_sg(secure_mem->sgt->sgl, sg, secure_mem->sgt->nents, i) {
        ...
        MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_MESSAGE, ("The secure mem physical address: 0x%x gpu virtual address: 0x%x! \n", phys, virt));
        virt += size;
    }
...
int mali_mem_secure_cpu_map(mali_mem_backend *mem_bkend, struct vm_area_struct *vma)
{
    ...
    for_each_sg(secure_mem->sgt->sgl, sg, secure_mem->sgt->nents, i) {
        ...
        for (j = 0; j < size / MALI_OSK_MALI_PAGE_SIZE; j++) {
            ...
            MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_MESSAGE, ("The secure mem physical address: 0x%x , cpu virtual address: 0x%x! \n", phys, addr));
        }
    }
    return ret;
}
