...
{
    return gp_job->required_varying_memsize / MALI_OSK_MALI_PAGE_SIZE;
}

mali_osk_errcode_t mali_mem_defer_bind_manager_init(void)
{
    mali_dmem_man = mali_osk_calloc(1, sizeof(struct mali_defer_bind_manager));
    if (!mali_dmem_man) {
...
mali_osk_errcode_t mali_mem_prepare_mem_for_job(struct mali_gp_job *next_gp_job, mali_defer_mem_block *dblock)
{
    u32 require_page;
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_CODE,
                     ("mali_mem_defer_prepare_mem_work, require alloc page 0x%x\n", require_page));
    /* allocate more pages from OS */
    if (MALI_OSK_ERR_OK != mali_mem_defer_alloc_mem(require_page, next_gp_job->session, dblock)) {
...
mali_osk_errcode_t mali_mem_defer_bind_allocation_prepare(mali_mem_allocation *alloc, struct list_head *list,
                                                          u32 *required_varying_memsize)
{
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_CODE, ("bind_allocation_prepare:: allocation =%x vaddr=0x%x!\n", alloc,
    ...
    bk_list->page_num = mem_bkend->size / MALI_OSK_MALI_PAGE_SIZE;
                                                         ...
                                                         struct list_head *pages)
{
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_CODE,
                     ("mali_mem_defer_bind_allocation, bind bkend = %x page num=0x%x vaddr=%x session=%x\n", mem_bkend,
                      bk_node->page_num, bk_node->vaddr, session));
    ...
    if (mem_bkend->type == MALI_MEM_OS) {
        mali_mem_os_mali_map(&mem_bkend->os_mem, session, bk_node->vaddr, 0, mem_bkend->os_mem.count,
...
static struct list_head *mali_mem_defer_get_free_page_list(u32 count, struct list_head *pages,
                                                           mali_defer_mem_block *dblock)
{
    int i = 0;
...
*/
mali_osk_errcode_t mali_mem_defer_bind(struct mali_gp_job *gp, struct mali_defer_mem_block *dmem_block)
{
    struct mali_defer_mem *dmem = NULL;
    struct mali_backend_bind_list *bkn, *bkn_tmp;
    LIST_HEAD(pages);

    if (gp->required_varying_memsize != (atomic_read(&dmem_block->num_free_pages) * MALI_OSK_MALI_PAGE_SIZE)) {
        MALI_DEBUG_PRINT_ERROR(
            ("#BIND:  The memsize of varying buffer not match to the pagesize of the dmem_block!!## \n"));
        return MALI_OSK_ERR_FAULT;
    }

    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_CODE, ("#BIND: GP job=%x## \n", gp));
    dmem = (mali_defer_mem *)mali_osk_calloc(1, sizeof(struct mali_defer_mem));
    if (dmem) {
