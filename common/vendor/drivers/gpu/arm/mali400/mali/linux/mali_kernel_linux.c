...
static void mali_init_cpu_clock_timer_func(unsigned long data)
{
    ...
    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_MESSAGE, ("Init Cpu %d cycle counter- First two samples: %08x %08x \n", current_cpu, sample0, sample1));
}
...
void mali_init_cpu_time_counters_on_all_cpus(int print_only)
{
    ...
    if (print_only) {
        if ((0 == mali_cpu_clock_last_value[2]) && (0 == mali_cpu_clock_last_value[3])) {
            ...
            MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_INFORMATOIN, ("CPU cycle counters readout all: %08x %08x\n", mali_cpu_clock_last_value[0], mali_cpu_clock_last_value[1]));
        } else {
            MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_INFORMATOIN, ("CPU cycle counters readout all: %08x %08x %08x %08x\n", mali_cpu_clock_last_value[0], mali_cpu_clock_last_value[1], mali_cpu_clock_last_value[2], mali_cpu_clock_last_value[3]));
        }
    }
}
#endif
    ...
    if (MALI_OSK_ERR_OK == mali_osk_wq_init()) {
        ...
        if (MALI_OSK_ERR_OK == mali_initialize_subsystems()) {
            ...
            if (0 == err) {
                ...
                if (0 == err) {
                    MALI_DEBUG_PRINT(MALI_KERNEL_LEVEL_INFORMATOIN, ("mali_probe(): Successfully initialized driver for platform device %s\n", pdev->name));
...
static long mali_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case MALI_IOC_MEM_QUERY_MMU_PAGE_TABLE_DUMP_SIZE:
        ...
        err = mem_query_mmu_page_table_dump_size_wrapper(session_data, (mali_uk_query_mmu_page_table_dump_size_s __user *)arg);
        break;
