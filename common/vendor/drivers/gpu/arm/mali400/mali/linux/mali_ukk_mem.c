...
{
    _ali_uk_alloc_mem_s kargs;
    mali_osk_errcode_t err;

    MALI_CHECK_NON_NULL(uargs, -EINVAL);
    MALI_CHECK_NON_NULL(session_data, -EINVAL);

    if (0 != copy_from_user(&kargs, uargs, sizeof(_ali_uk_alloc_mem_s))) {
        return -EFAULT;
    }
...
{
    mali_uk_free_mem_s kargs;
    mali_osk_errcode_t err;

    MALI_CHECK_NON_NULL(uargs, -EINVAL);
    MALI_CHECK_NON_NULL(session_data, -EINVAL);

    if (0 != copy_from_user(&kargs, uargs, sizeof(mali_uk_free_mem_s))) {
        return -EFAULT;
    }
    kargs.ctx = (uintptr_t)session_data;
...
{
    mali_uk_bind_mem_s kargs;
    mali_osk_errcode_t err;

    MALI_CHECK_NON_NULL(uargs, -EINVAL);
    MALI_CHECK_NON_NULL(session_data, -EINVAL);

    if (0 != copy_from_user(&kargs, uargs, sizeof(mali_uk_bind_mem_s))) {
        return -EFAULT;
    }
    kargs.ctx = (uintptr_t)session_data;
...
{
    mali_uk_unbind_mem_s kargs;
    mali_osk_errcode_t err;

    MALI_CHECK_NON_NULL(uargs, -EINVAL);
    MALI_CHECK_NON_NULL(session_data, -EINVAL);

    if (0 != copy_from_user(&kargs, uargs, sizeof(mali_uk_unbind_mem_s))) {
        return -EFAULT;
    }
    kargs.ctx = (uintptr_t)session_data;
...
int mem_cow_wrapper(struct mali_session_data *session_data, mali_uk_cow_mem_s __user *uargs)
{
    ...
    if (0 != copy_from_user(&kargs, uargs, sizeof(mali_uk_cow_mem_s))) {
        return -EFAULT;
    }
    kargs.ctx = (uintptr_t)session_data;
...
{
    mali_uk_cow_modify_range_s kargs;
    mali_osk_errcode_t err;

    MALI_CHECK_NON_NULL(uargs, -EINVAL);
    MALI_CHECK_NON_NULL(session_data, -EINVAL);

    if (0 != copy_from_user(&kargs, uargs, sizeof(mali_uk_cow_modify_range_s))) {
        return -EFAULT;
    }
    kargs.ctx = (uintptr_t)session_data;
...
int mem_resize_mem_wrapper(struct mali_session_data *session_data, mali_uk_mem_resize_s __user *uargs)
{
    ...
    if (0 != copy_from_user(&kargs, uargs, sizeof(mali_uk_mem_resize_s))) {
        return -EFAULT;
    }
    kargs.ctx = (uintptr_t)session_data;
...
{
    mali_uk_mem_write_safe_s kargs;
    mali_osk_errcode_t err;

    MALI_CHECK_NON_NULL(uargs, -EINVAL);
    MALI_CHECK_NON_NULL(session_data, -EINVAL);

    if (0 != copy_from_user(&kargs, uargs, sizeof(mali_uk_mem_write_safe_s))) {
        return -EFAULT;
    }
...
{
    mali_uk_dump_mmu_page_table_s kargs;
    mali_osk_errcode_t err;
...
{
    mali_osk_errcode_t err;
    mali_uk_profiling_memory_usage_get_s kargs;

    MALI_CHECK_NON_NULL(uargs, -EINVAL);
    MALI_CHECK_NON_NULL(session_data, -EINVAL);

    if (0 != copy_from_user(&kargs, uargs, sizeof(mali_uk_profiling_memory_usage_get_s))) {
        return -EFAULT;
    }
    ...
    if (0 != copy_to_user(uargs, &kargs, sizeof(mali_uk_profiling_memory_usage_get_s))) {
        return -EFAULT;
    }
