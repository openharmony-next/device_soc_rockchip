...
static int rockchip_pm_add_one_domain(struct rockchip_pmu *pmu, struct device_node *node)
{
    ...
    if (pd->num_qos > 0) {
        ...
        pd->qos_save_regs[0] = (u32 *)devm_kmalloc(pmu->dev, sizeof(u32) * MAX_QOS_REGS_NUM * pd->num_qos, GFP_KERNEL);
        if (!pd->qos_save_regs[0]) {
            error = -ENOMEM;
            goto err_unprepare_clocks;
        }
        qos_is_need_init[0] = kzalloc(sizeof(bool) * MAX_QOS_REGS_NUM * pd->num_qos, GFP_KERNEL);
