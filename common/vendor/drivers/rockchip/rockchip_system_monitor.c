...
void rockchip_set_system_status(unsigned long rockchip_system_status)
{
    ...
    while (rockchip_system_status) {
        single_status_offset = fls(rockchip_system_status) - 1;
        rockchip_system_status &= ~(1 << single_status_offset);
...
void rockchip_clear_system_status(unsigned long rockchip_system_status)
{
    unsigned long old_system_status;
    unsigned int single_status_offset;
...
int rockchip_add_system_status_interface(struct device *dev)
{
    ...
    return compat_only_sysfs_link_entry_to_kobj(&dev->kobj, system_monitor->kobj, "system_status", NULL);
}
...
static struct video_info *rockchip_parse_video_info(const char *buf)
{
    ...
    pr_debug("%c,width=%d,height=%d,ishevc=%d,videoFramerate=%d,streamBitrate=%d\n", buf[0], video_info->width,
             video_info->height, video_info->ishevc, video_info->videoFramerate, video_info->streamBitrate);
    kfree(str);
...
static struct video_info *rockchip_find_video_info(const char *buf)
{
    ...
    {
        if (info->width == video_info->width && info->height == video_info->height &&
            info->ishevc == video_info->ishevc && info->videoFramerate == video_info->videoFramerate &&
...
static ssize_t status_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t n)
{
    if (!n) {
        return -EINVAL;
    }
...
static struct system_monitor_attr status = __ATTR(system_status, 0644, status_show, status_store);

static int rockchip_get_temp_freq_table(struct device_node *np, char *porp_name, struct temp_freq_table **freq_table)
{
...
static int rockchip_get_adjust_volt_table(struct device_node *np, char *porp_name, struct volt_adjust_table **table)
{
...
static int rockchip_get_low_temp_volt(struct monitor_dev_info *info, unsigned long rate, int *delta_volt)
{
    ...
    for (i = 0; info->low_temp_adjust_table[i].volt != INT_MAX; i++) {
        if (_rate >= info->low_temp_adjust_table[i].min && _rate <= info->low_temp_adjust_table[i].max) {
...
static int rockchip_init_temp_opp_table(struct monitor_dev_info *info)
{
    ...
    info->opp_table = kzalloc(sizeof(*info->opp_table) * max_count, GFP_KERNEL);
    if (!info->opp_table) {
        return -ENOMEM;
    }
    ...
    list_for_each_entry(opp, &opp_table->opp_list, node)
    {
        ...
        if ((opp->supplies[0].u_volt + delta_volt) <= info->max_volt) {
            info->opp_table[i].low_temp_volt = opp->supplies[0].u_volt + delta_volt;
            if (info->opp_table[i].low_temp_volt < info->low_temp_min_volt) {
                info->opp_table[i].low_temp_volt = info->low_temp_min_volt;
            }
            if (!reach_max_volt) {
                low_limit = opp->rate;
            }
...
static int monitor_device_parse_wide_temp_config(struct device_node *np, struct monitor_dev_info *info)
{
    struct device *dev = info->dev;
    ...
    of_property_read_u32(np, "rockchip,temp-hysteresis", &info->temp_hysteresis);
    if (of_property_read_u32(np, "rockchip,low-temp", &info->low_temp)) {
        info->low_temp = INT_MIN;
    }
    rockchip_get_adjust_volt_table(np, "rockchip,low-temp-adjust-volt", &info->low_temp_adjust_table);
    ...
    if (of_property_read_u32(np, "rockchip,high-temp-max-volt", &value)) {
        info->high_temp_max_volt = ULONG_MAX;
    } else {
        info->high_temp_max_volt = value;
    }
    rockchip_init_temp_opp_table(info);
    rockchip_get_temp_freq_table(np, "rockchip,temp-freq-table", &info->high_limit_table);
    if (!info->high_limit_table) {
        rockchip_get_temp_freq_table(np, "rockchip,high-temp-limit-table", &info->high_limit_table);
    }
    if (!info->high_limit_table) {
    ...
    dev_info(dev, "l=%d h=%d hyst=%d l_limit=%lu h_limit=%lu h_table=%d\n", info->low_temp, info->high_temp,
             info->temp_hysteresis, info->low_limit, info->high_limit, info->high_limit_table ? true : false);
    ...
    if (!info->low_temp_adjust_table && !info->low_temp_min_volt && !info->low_limit && !info->high_limit &&
...
static int monitor_device_parse_status_config(struct device_node *np, struct monitor_dev_info *info)
{
    int ret;

    ret = of_property_read_u32(np, "rockchip,video-4k-freq", &info->video_4k_freq);
    ret &= of_property_read_u32(np, "rockchip,reboot-freq", &info->reboot_freq);
...
static int monitor_device_parse_early_min_volt(struct device_node *np, struct monitor_dev_info *info)
{
    return of_property_read_u32(np, "rockchip,early-min-microvolt", &info->early_min_volt);
}

static int monitor_device_parse_read_margin(struct device_node *np, struct monitor_dev_info *info)
{
...
static int monitor_device_parse_scmi_clk(struct device_node *np, struct monitor_dev_info *info)
{
    struct clk *clk;
...
static int monitor_device_parse_dt(struct device *dev, struct monitor_dev_info *info)
{
...
static int rockchip_adjust_low_temp_opp_volt(struct monitor_dev_info *info, bool is_low_temp)
{
    struct device *dev = info->dev;
    struct opp_table *opp_table;
    struct dev_pm_opp *opp;
    ...
    {
        if (is_low_temp) {
            if (opp->supplies[0].u_volt_max < info->opp_table[i].low_temp_volt) {
                opp->supplies[0].u_volt_max = info->opp_table[i].low_temp_volt;
            }
            opp->supplies[0].u_volt = info->opp_table[i].low_temp_volt;
            ...
            opp->supplies[0].u_volt_max = info->opp_table[i].max_volt;
        }
        i++;
    }
    mutex_unlock(&opp_table->lock);
...
static void rockchip_low_temp_adjust(struct monitor_dev_info *info, bool is_low)
{
...
static void rockchip_high_temp_adjust(struct monitor_dev_info *info, bool is_high)
{
    struct monitor_dev_profile *devp = info->devp;
    int ret = 0;
...
static int rockchip_system_monitor_wide_temp_adjust(struct monitor_dev_info *info, int temp)
{
...
static void rockchip_system_monitor_wide_temp_init(struct monitor_dev_info *info)
{
    int ret, temp;
...
static void rockchip_system_monitor_early_regulator_init(struct monitor_dev_info *info)
{
...
static int rockchip_system_monitor_freq_qos_requset(struct monitor_dev_info *info)
{
    ...
    if (info->devp->type == MONITOR_TPYE_CPU) {
        ...
        ret = freq_qos_add_request(&policy->constraints, &info->max_temp_freq_req, FREQ_QOS_MAX, max_default_value);
        if (ret < 0) {
            dev_info(info->dev, "failed to add temp freq constraint\n");
            return ret;
        }
        ret = freq_qos_add_request(&policy->constraints, &info->min_sta_freq_req, FREQ_QOS_MIN,
                                   FREQ_QOS_MIN_DEFAULT_VALUE);
        if (ret < 0) {
            dev_info(info->dev, "failed to add sta freq constraint\n");
            freq_qos_remove_request(&info->max_temp_freq_req);
            return ret;
        }
        ret = freq_qos_add_request(&policy->constraints, &info->max_sta_freq_req, FREQ_QOS_MAX,
        ...
        ret = dev_pm_qos_add_request(devfreq->dev.parent, &info->dev_max_freq_req, DEV_PM_QOS_MAX_FREQUENCY,
                                     max_default_value);
        if (ret < 0) {
...
static int rockchip_system_monitor_parse_supplies(struct device *dev, struct monitor_dev_info *info)
{
    struct opp_table *opp_table;
...
int rockchip_monitor_check_rate_volt(struct monitor_dev_info *info, bool is_set_clk)
{
    ...
    if (old_rate == new_rate) {
        if (info->regulator_count > 1) {
            if (old_volt == new_volt && new_mem_volt == old_mem_volt) {
                goto unlock;
            }
    ...
    if (new_rate >= old_rate) {
        if (info->regulator_count > 1) {
            ret = regulator_set_voltage(mem_reg, new_mem_volt, INT_MAX);
            if (ret) {
...
static int rockchip_system_monitor_parse_dt(struct system_monitor *monitor)
{
    ...
    if (of_property_read_u32(np, "rockchip,polling-delay", &monitor->delay)) {
        monitor->delay = THERMAL_POLLING_DELAY;
    }

    if (of_property_read_string(np, "rockchip,temp-offline-cpus", &buf)) {
        cpumask_clear(&system_monitor->temp_offline_cpus);
    } else {
        cpulist_parse(buf, &system_monitor->temp_offline_cpus);
    }

    if (of_property_read_u32(np, "rockchip,offline-cpu-temp", &system_monitor->offline_cpus_temp)) {
...
static void rockchip_system_monitor_cpu_on_off(void)
{
    ...
    if (system_monitor->is_temp_offline) {
        cpumask_or(&offline_cpus, &system_monitor->status_offline_cpus, &system_monitor->temp_offline_cpus);
    } else {
        cpumask_copy(&offline_cpus, &system_monitor->status_offline_cpus);
    }
    ...
    cpumask_andnot(&online_cpus, cpu_possible_mask, &system_monitor->offline_cpus);
    cpumask_xor(&online_cpus, cpu_online_mask, &online_cpus);
    if (cpumask_empty(&online_cpus)) {
        goto out;
    }
    for_each_cpu(cpu, &online_cpus) add_cpu(cpu);
...
static void rockchip_system_monitor_temp_cpu_on_off(int temp)
{
    ...
    } else if (temp < system_monitor->offline_cpus_temp - system_monitor->temp_hysteresis) {
...
static void rockchip_system_monitor_thermal_update(void)
{
    ...
    list_for_each_entry(info, &monitor_dev_list, node) rockchip_system_monitor_wide_temp_adjust(info, temp);
    up_read(&mdev_list_sem);

    rockchip_system_monitor_temp_cpu_on_off(temp);

out:
    mod_delayed_work(system_freezable_wq, &system_monitor->thermal_work, msecs_to_jiffies(system_monitor->delay));
}
...
static void rockchip_system_status_cpu_limit_freq(struct monitor_dev_info *info, unsigned long rockchip_system_status)
{
    unsigned int target_freq = 0;

    if (rockchip_system_status & SYS_STATUS_REBOOT) {
        freq_qos_update_request(&info->max_sta_freq_req, info->reboot_freq);
        freq_qos_update_request(&info->min_sta_freq_req, info->reboot_freq);
        return;
    }

    if (info->video_4k_freq && (rockchip_system_status & SYS_STATUS_VIDEO_4K)) {
    ...
    if (info->status_max_limit) {
        freq_qos_update_request(&info->max_sta_freq_req, info->status_max_limit);
    } else {
        freq_qos_update_request(&info->max_sta_freq_req, FREQ_QOS_MAX_DEFAULT_VALUE);
    }
}

static void rockchip_system_status_limit_freq(unsigned long rockchip_system_status)
{
    struct monitor_dev_info *info;
    ...
    {
        if (info->devp->type == MONITOR_TPYE_CPU) {
            rockchip_system_status_cpu_limit_freq(info, rockchip_system_status);
        }
    }
    up_read(&mdev_list_sem);
}

static void rockchip_system_status_cpu_on_off(unsigned long rockchip_system_status)
{
    struct cpumask offline_cpus;
    ...
    cpumask_clear(&offline_cpus);
    if (rockchip_system_status & SYS_STATUS_VIDEO_4K) {
        cpumask_copy(&offline_cpus, &system_monitor->video_4k_offline_cpus);
    }
...
static int rockchip_system_status_notifier(struct notifier_block *nb, unsigned long rockchip_system_status, void *ptr)
{
    rockchip_system_status_limit_freq(rockchip_system_status);

    rockchip_system_status_cpu_on_off(rockchip_system_status);

    return NOTIFY_OK;
}

static int monitor_pm_notify(struct notifier_block *nb, unsigned long mode, void *_unused)
{
    switch (mode) {
...
static int rockchip_monitor_reboot_notifier(struct notifier_block *nb, unsigned long action, void *ptr)
{
...
static int rockchip_monitor_fb_notifier(struct notifier_block *nb, unsigned long action, void *ptr)
{
...
static int rockchip_eink_devfs_notifier(struct notifier_block *nb, unsigned long action, void *ptr)
{
    switch (action) {
        case EBC_ON:
            rockchip_clear_system_status(SYS_STATUS_LOW_POWER);
...
static void system_monitor_early_min_volt_function(struct work_struct *work)
{
    ...
    list_for_each_entry(info, &monitor_dev_list, node)
    {
        ...
        if (ret) {
            dev_err(&rdev->dev, "%s: failed to set volt\n", __func__);
        }
...
static DECLARE_DELAYED_WORK(system_monitor_early_min_volt_work, system_monitor_early_min_volt_function);

static int rockchip_system_monitor_probe(struct platform_device *pdev)
{
    struct device *dev = &pdev->dev;

    system_monitor = devm_kzalloc(dev, sizeof(struct system_monitor), GFP_KERNEL);
    if (!system_monitor) {
        return -ENOMEM;
    }
    ...
    if (system_monitor->tz) {
        INIT_DELAYED_WORK(&system_monitor->thermal_work, rockchip_system_monitor_thermal_check);
        mod_delayed_work(system_freezable_wq, &system_monitor->thermal_work, msecs_to_jiffies(system_monitor->delay));
    }

    system_monitor->status_nb.notifier_call = rockchip_system_status_notifier;
