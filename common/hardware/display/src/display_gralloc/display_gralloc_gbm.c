...
static uint32_t ConvertFormatToDrm(PixelFormat fmtIn)
{
    ...
    DISPLAY_DEBUGLOG("fmtIn %{public}d : %{public}s, outFmt %{public}d : %{public}s", fmtIn, GetPixelFmtStr(fmtIn),
...
int32_t GbmAllocMem(const AllocInfo *info, BufferHandle **buffer)
{
    ...
    DISPLAY_DEBUGLOG("requeset width %{public}d, heigt %{public}d, format %{public}d", info->width, info->height,
                     drmFmt);
