    ...
    namespace HDI {
        namespace DISPLAY {
            ...
            int32_t DrmPlane::Init(DrmDevice &drmDevice)
            {
                ...
                DISPLAY_CHK_RETURN((ret != DISPLAY_SUCCESS), DISPLAY_FAILURE,
                                   DISPLAY_LOGE("cat not get plane in fence prop id"));
                ...
                for (int i = 0; i < static_cast<int> ARRAY_SIZE(planeTypeNames); i++) {
                    ...
                    if (find_name) {
                        DISPLAY_LOGI("find plane id %{public}d, type %{public}x %{public}s", GetId(),
                                     planeTypeNames[i].type, planeTypeNames[i].name);
