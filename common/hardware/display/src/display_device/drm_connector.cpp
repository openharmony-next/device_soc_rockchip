    ...
    namespace HDI {
        namespace DISPLAY {
            ...
            DrmConnector::DrmConnector(drmModeConnector c, FdPtr &fd)
                : mId(c.connector_id), mPhyWidth(c.mmWidth), mPhyHeight(c.mmHeight), mEncoderId(c.encoder_id),
                  mConnectState(c.connection), mDrmFdPtr(fd)
            {
            ...
            void DrmConnector::InitModes(drmModeConnector c)
            {
                ...
                for (int i = 0; i < c.count_modes; i++) {
                    ...
                    DISPLAY_DEBUGLOG(
                        "mode: hdisplay %{public}d, vdisplay %{public}d vrefresh %{public}d type %{public}d",
            ...
            int32_t DrmConnector::Init(DrmDevice &drmDevice)
            {
                ...
                DISPLAY_CHK_RETURN((ret != DISPLAY_SUCCESS), DISPLAY_FAILURE,
                                   DISPLAY_LOGE("cat not get out fence prop id"));
                ...
                if (ret == DISPLAY_SUCCESS) {
                    ...
                    DISPLAY_DEBUGLOG("prop brightness is %{public}d, level is %{public}d", mPropBrightnessId,
                                     mBrightnessLevel);
                } else {
            ...
            int32_t DrmConnector::TryPickEncoder(IdMapPtr<DrmEncoder> &encoders, uint32_t encoderId,
                                                 IdMapPtr<DrmCrtc> &crtcs, uint32_t &crtcId)
            {
                int ret;
                ...
                DISPLAY_CHK_RETURN(
                    (ret == DISPLAY_SUCCESS), DISPLAY_SUCCESS,
                    DISPLAY_DEBUGLOG("connector : %{public}d pick encoder : %{public}d", mId, encoder->GetId()));
                return DISPLAY_FAILURE;
            }

            int32_t DrmConnector::PickIdleCrtcId(IdMapPtr<DrmEncoder> &encoders, IdMapPtr<DrmCrtc> &crtcs,
                                                 uint32_t &crtcId)
            {
                DISPLAY_DEBUGLOG();
                DISPLAY_DEBUGLOG("encoder_id %{public}d", mEncoderId);
                ...
                DISPLAY_CHK_RETURN(
                    (ret == DISPLAY_SUCCESS), DISPLAY_SUCCESS,
                    DISPLAY_DEBUGLOG("connector : %{public}d pick endcoder : %{public}d crtcId : %{public}d", mId,
                                     mEncoderId, crtcId));
            ...
            std::shared_ptr<DrmCrtc> DrmConnector::UpdateCrtcId(IdMapPtr<DrmEncoder> &encoders,
                                                                ...
                                                                drmModeConnectorPtr c, int *crtc_id)
            {
                ...
                for (auto crtcIter = crtcs.begin(); crtcIter != crtcs.end(); ++crtcIter) {
                    ...
                    if (possibleCrtcs == (1 << posCrts->GetPipe())) {
                        DISPLAY_DEBUGLOG("find crtc id %{public}d, pipe %{public}d", posCrts->GetId(),
                                         posCrts->GetPipe());
                        crtc = posCrts;
            ...
            bool DrmConnector::HandleHotplug(IdMapPtr<DrmEncoder> &encoders, IdMapPtr<DrmCrtc> &crtcs, bool plugIn)
            {
                ...
                if (mConnectState == c->connection) {
                    ...
                    drmModeCreatePropertyBlob(drmFd, &c->modes[0], sizeof(c->modes[0]), &blob_id);
