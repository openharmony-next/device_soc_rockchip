...
#define DW9763_RING_PD_CONTROL_REG		0x02
#define DW9763_DATAM_REG 0X03
#define DW9763_DATAL_REG 0X04

#define DW9763_STATUS_ADDR			0x05
#define DW9763_SAC_PRESC_REG			0x06
#define DW9763_SAC_TIME_REG	0x07
#define DW9763_FLAG_REG 0X10
...
#define DW9763_DEFAULT_STEP_MODE	0xd
...
static int dw9763_write_reg(struct i2c_client *client,
	u8 addr, u32 val, u8 len)
{
	u32 buf_i, val_i;
	u8 buf[5];
	...
	unsigned int *cur_pos)
{
	...
	ret = dw9763_read_reg(client, DW9763_DATAM_REG, &val, 2);
	if (ret != 0)
		goto err;
	...
	unsigned int dest_pos)
{
	...
	ret = dw9763_write_reg(client, DW9763_DATAM_REG, position & 0x3ff, 2);
	if (ret != 0)
		goto err;
...
static int dw9763t_init(struct dw9763_device *dev)
{
	int ret = 0;
	...
	ret = dw9763_write_reg(client, DW9763_RING_PD_CONTROL_REG, 0x01, 1);
	if (ret < 0)
		goto err;

	ret = dw9763_write_reg(client, DW9763_RING_PD_CONTROL_REG, 0x00, 1);
	if (ret < 0)
		goto err;
	...
	ret = dw9763_write_reg(client, DW9763_RING_PD_CONTROL_REG, 0x02, 1);
	if (ret < 0)
		goto err;


	ret = dw9763_write_reg(client, DW9763_SAC_PRESC_REG, 0x61, 2);
	if (ret < 0)
		goto err;

	ret = dw9763_write_reg(client, DW9763_SAC_TIME_REG, 0x39, 2);
	if (ret < 0)
		goto err;
	...
	ret = dw9763_write_reg(client, DW9763_SACT_REG, step_mode, 1);
	if (ret < 0)
		goto err;
	control_mode = 0x31;
	control_mode |= (dev->control_mode & 0x07) << 1;
	ret = dw9763_write_reg(client, DW9763_CONTROL, control_mode, 1);
	if (ret < 0)
		goto err;
*/
	dev_info(&client->dev, "dw9763t_init OK!!!\n");
	return 0;
err:
...
static long dw9763_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	struct dw9763_device *dw9763_dev = sd_to_dw9763_vcm(sd);

//	if (cmd == RK_VIDIOC_VCM_TIMEINFO) {
		vcm_tim = (struct rk_cam_vcm_tim *)arg;

		vcm_tim->vcm_start_t.tv_sec = dw9763_dev->start_move_tv.tv_sec;
		vcm_tim->vcm_start_t.tv_usec =
				dw9763_dev->start_move_tv.tv_usec;
		vcm_tim->vcm_end_t.tv_sec = dw9763_dev->end_move_tv.tv_sec;
		vcm_tim->vcm_end_t.tv_usec = dw9763_dev->end_move_tv.tv_usec;
			...
			const struct i2c_device_id *id)
{
	...
	printk("XXXXXXXXXXXXXXX dw9763_probe\n");

	dev_info(&client->dev, "probing...\n");
	if (of_property_read_u32(np,
		OF_CAMERA_VCMDRV_START_CURRENT,
		(unsigned int *)&start_current)) {
		start_current = DW9763_DEFAULT_START_CURRENT;
		dev_info(&client->dev,
	...
	if (of_property_read_u32(np,
		...
		(unsigned int *)&rated_current)) {
		rated_current = DW9763_DEFAULT_RATED_CURRENT;
		dev_info(&client->dev,
			"could not get module %s from dts!\n",
			OF_CAMERA_VCMDRV_RATED_CURRENT);
	}
	...
	current_distance = current_distance * DW9763_MAX_REG /
						DW9763_MAX_CURRENT;
	dw9763_dev->step = (current_distance + (VCMDRV_MAX_LOG - 1)) /
						VCMDRV_MAX_LOG;
	dw9763_dev->start_current = start_current * DW9763_MAX_REG /
						DW9763_MAX_CURRENT;
	dw9763_dev->rated_current = dw9763_dev->start_current +
						VCMDRV_MAX_LOG *
						dw9763_dev->step;
	dw9763_dev->step_mode     = step_mode;
	...
	case 0:
		...
		return -EINVAL;
	case 1:
		dw9763_dev->mv_time_per_pos = (126 + step_mode * 2) * dw9763_dev->step;
	...
	case 6:
	case 7:
		dw9763_dev->mv_time_per_pos = (6300 + step_mode * 100);
		dev_dbg(&client->dev, "control_mode is LSC mode\n");
		break;
	default:
