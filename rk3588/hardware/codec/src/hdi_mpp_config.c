...
int32_t GetDefaultConfig(RKHdiBaseComponent *pBaseComponent)
{
    ...
    MPP_RET ret = pBaseComponent->mpi->control(ctx, MPP_DEC_GET_CFG, pBaseComponent->cfg);
    if (ret != MPP_OK) {
...
{
    RKMppApi *mppApi = pBaseComponent->mppApi;
    MPP_RET ret = mppApi->HdiMppBufferGroupGet(&pBaseComponent->frmGrp,
...
int32_t SetParamPixleFmt(RKHdiBaseComponent *pBaseComponent, Param *param)
{
    ...
    MPP_RET ret = mppApi->HdiMppEncCfgSetS32(pBaseComponent->cfg, "prep:format", rkPixFmt);
    if (ret != MPP_OK) {
...
int32_t SetParamGop(RKHdiBaseComponent *pBaseComponent, Param *param)
{
    ...
    MPP_RET ret = mppApi->HdiMppEncCfgSetS32(pBaseComponent->cfg, "rc:gop", gopSet->gop);
    if (ret != MPP_OK) {
